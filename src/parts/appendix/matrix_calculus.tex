...
\subsection{高斯分布的 Fisher 信息阵：多参数化族（选读）}
\label{sec:matderiv-gauss-fim}

\cref{thm:matderiv-fim-reparam} 给的是 FIM \emph{随参数化变换}的一般律；本小节把它落到最常用的具体分布——多元高斯——上，给出\emph{同一个}高斯在五种参数化下的显式 FIM。它们既是 \cref{ch:slam_est} 的 CRLB（\cref{thm:crlb}）在高斯情形的显式形式，也是变分推断 / 自然梯度下降（NGD）把“KL 散度的曲率”当度量张量时所需的那个矩阵\cite{barfoot2024state,amari1998natural}。这套结果把本附录的三件零件——vec/克罗内克（\cref{prop:matderiv-vec-kron}）、对称复制矩阵 $\mathbf{D}$（\cref{def:matderiv-vech}）、迹技巧（\cref{eq:matderiv-trace-id}）——一次性用全。

\paragraph{FIM 即 KL 散度的曲率。}
设高斯 $q(\mathbf{x})=\mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma})$，负对数似然
\begin{equation}
  -\ln q(\mathbf{x})=\tfrac12(\mathbf{x}-\boldsymbol{\mu})^\top\boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})+\tfrac12\ln|\boldsymbol{\Sigma}|+\text{const}.
  \label{eq:matderiv-gauss-nll}
\end{equation}
取参数 $\boldsymbol{\theta}$，两个无穷接近的高斯 $q,q'$ 间的 KL 散度展开到二阶恰为 $\mathrm{KL}(q\|q')\approx\tfrac12\,\delta\boldsymbol{\theta}^\top\boldsymbol{\mathcal{I}}_{\boldsymbol{\theta}}\,\delta\boldsymbol{\theta}$，其中
\begin{equation}
  \boldsymbol{\mathcal{I}}_{\boldsymbol{\theta}}=\mathbb{E}_q\Big[\frac{\partial^2\,(-\ln q(\mathbf{x}))}{\partial\boldsymbol{\theta}^\top\partial\boldsymbol{\theta}}\Big]
  \label{eq:matderiv-gauss-fim-def}
\end{equation}
就是 FIM——它是 KL 散度在参数空间的\emph{黎曼度量张量}（一阶项的期望为零，故 KL 的最低阶为这条二次型）\cite{barfoot2024state}。下面对几种参数化算出 \cref{eq:matderiv-gauss-fim-def}。

\begin{proposition}[多元高斯 FIM 的五种参数化]\label{prop:matderiv-gauss-fim}
对 $\mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma})$（$\boldsymbol{\mu}\in\mathbb{R}^N$、$\boldsymbol{\Sigma}\in\mathbb{R}^{N\times N}$），其 FIM 随参数化为\cite{barfoot2024state,magnus2019matrix}：
\begin{enumerate}
  \item \textbf{正则参数} $\boldsymbol{\theta}=[\boldsymbol{\mu};\operatorname{vec}(\boldsymbol{\Sigma})]$（均值 $+$ 拉直协方差）：
  \begin{equation}
    \boldsymbol{\mathcal{I}}_{\boldsymbol{\theta}}=\begin{bmatrix}\boldsymbol{\Sigma}^{-1}&\mathbf{0}\\\mathbf{0}&\tfrac12\big(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1}\big)\end{bmatrix},
    \qquad
    \boldsymbol{\mathcal{I}}_{\boldsymbol{\theta}}^{-1}=\begin{bmatrix}\boldsymbol{\Sigma}&\mathbf{0}\\\mathbf{0}&2\big(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma}\big)\end{bmatrix}.
    \label{eq:matderiv-gauss-fim-canon}
  \end{equation}
  \item \textbf{正则参数 $+$ 对称性} $\boldsymbol{\gamma}=[\boldsymbol{\mu};\operatorname{vech}(\boldsymbol{\Sigma})]$（用半向量化扣除冗余，\cref{def:matderiv-vech}）：协方差块经复制矩阵 $\mathbf{D}$ 夹心，
  \begin{equation}
    \boldsymbol{\mathcal{I}}_{\boldsymbol{\gamma}}=\begin{bmatrix}\boldsymbol{\Sigma}^{-1}&\mathbf{0}\\\mathbf{0}&\tfrac12\,\mathbf{D}^\top\big(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1}\big)\mathbf{D}\end{bmatrix},
    \qquad
    \boldsymbol{\mathcal{I}}_{\boldsymbol{\gamma}}^{-1}=\begin{bmatrix}\boldsymbol{\Sigma}&\mathbf{0}\\\mathbf{0}&2\,\mathbf{D}^{+}\big(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma}\big)\mathbf{D}^{+\top}\end{bmatrix}.
    \label{eq:matderiv-gauss-fim-sym}
  \end{equation}
  \item \textbf{混合参数} $\boldsymbol{\alpha}=[\boldsymbol{\mu};\operatorname{vec}(\boldsymbol{\Sigma}^{-1})]$（均值 $+$ \emph{信息阵}，大规模稀疏问题常用）：
  \begin{equation}
    \boldsymbol{\mathcal{I}}_{\boldsymbol{\alpha}}=\begin{bmatrix}\boldsymbol{\Sigma}^{-1}&\mathbf{0}\\\mathbf{0}&\tfrac12\big(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma}\big)\end{bmatrix}.
    \label{eq:matderiv-gauss-fim-hybrid}
  \end{equation}
  \item \textbf{自然参数} $\boldsymbol{\eta}=[\boldsymbol{\Sigma}^{-1}\boldsymbol{\mu};\operatorname{vec}(\boldsymbol{\Sigma}^{-1})]$（信息向量 $+$ 信息阵）：FIM \emph{不再块对角}，
  \begin{equation}
    \boldsymbol{\mathcal{I}}_{\boldsymbol{\eta}}=\begin{bmatrix}\boldsymbol{\Sigma}&-\boldsymbol{\Sigma}(\boldsymbol{\mu}^\top\otimes\mathbf{1})\\-(\boldsymbol{\mu}\otimes\mathbf{1})\boldsymbol{\Sigma}&\tfrac12(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma})+(\boldsymbol{\mu}\otimes\mathbf{1})\boldsymbol{\Sigma}(\boldsymbol{\mu}^\top\otimes\mathbf{1})\end{bmatrix}.
    \label{eq:matderiv-gauss-fim-natural}
  \end{equation}
\end{enumerate}
对每种，把 $\boldsymbol{\Sigma}^{-1}$ 与 $\boldsymbol{\Sigma}$ 互换即得逆 FIM（混合/自然类似）。
\end{proposition}
\begin{proof}[正则参数的推导（其余靠重参数化）]
对 \cref{eq:matderiv-gauss-nll} 取二阶微分并对 $q$ 求期望，用 $\mathbb{E}[\mathbf{x}-\boldsymbol{\mu}]=\mathbf{0}$、$\mathbb{E}[(\mathbf{x}-\boldsymbol{\mu})(\mathbf{x}-\boldsymbol{\mu})^\top]=\boldsymbol{\Sigma}$ 消去一次项与交叉项，得到干净的二阶型
\begin{equation*}
  -\mathbb{E}_q[\mathrm{d}^2\ln q]
  =\mathrm{d}\boldsymbol{\mu}^\top\boldsymbol{\Sigma}^{-1}\mathrm{d}\boldsymbol{\mu}
  +\tfrac12\operatorname{tr}\!\big(\boldsymbol{\Sigma}^{-1}\mathrm{d}\boldsymbol{\Sigma}\,\boldsymbol{\Sigma}^{-1}\mathrm{d}\boldsymbol{\Sigma}\big).
\end{equation*}
均值项直接读出 $\boldsymbol{\Sigma}^{-1}$。协方差项用迹--vec 桥 $\operatorname{tr}(\mathbf{A}^\top\mathbf{B})=\operatorname{vec}(\mathbf{A})^\top\operatorname{vec}(\mathbf{B})$（\cref{eq:matderiv-vec-tr}）与核心式 $\operatorname{vec}(\boldsymbol{\Sigma}^{-1}\mathrm{d}\boldsymbol{\Sigma}\,\boldsymbol{\Sigma}^{-1})=(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1})\operatorname{vec}(\mathrm{d}\boldsymbol{\Sigma})$（\cref{eq:matderiv-vec-AXB}）改写为 $\tfrac12\operatorname{vec}(\mathrm{d}\boldsymbol{\Sigma})^\top(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1})\operatorname{vec}(\mathrm{d}\boldsymbol{\Sigma})$，读出协方差块 $\tfrac12(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1})$，即 \cref{eq:matderiv-gauss-fim-canon}。其余四式都由\emph{重参数化}（\cref{thm:matderiv-fim-reparam}）从它得到：对称版用 $\operatorname{vec}(\mathrm{d}\boldsymbol{\Sigma})=\mathbf{D}\,\mathrm{d}\operatorname{vech}(\boldsymbol{\Sigma})$（\cref{eq:matderiv-dup}）夹一个 $\mathbf{D}$；混合版用雅可比 $\operatorname{vec}(\mathrm{d}\boldsymbol{\Sigma})=-(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma})\operatorname{vec}(\mathrm{d}\boldsymbol{\Sigma}^{-1})$（即 $\mathrm{d}\boldsymbol{\Sigma}^{-1}=-\boldsymbol{\Sigma}^{-1}\mathrm{d}\boldsymbol{\Sigma}\,\boldsymbol{\Sigma}^{-1}$ 拉直），夹心后 $\tfrac12(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1})$ 被两侧 $(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma})$ 抵成 $\tfrac12(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma})$；自然版的均值分量 $\boldsymbol{\Sigma}^{-1}\boldsymbol{\mu}$ 同时含 $\boldsymbol{\mu}$ 与 $\boldsymbol{\Sigma}^{-1}$，其雅可比的非对角块把均值与协方差\emph{耦合}进来，故 $\boldsymbol{\mathcal{I}}_{\boldsymbol{\eta}}$ 不再块对角。\hfill$\square$
\end{proof}

\begin{note}
\textbf{这族 FIM 在本书何处用.}\;(1) \textbf{协方差 vs 信息对偶}：正则（\cref{eq:matderiv-gauss-fim-canon}）与混合（\cref{eq:matderiv-gauss-fim-hybrid}）的协方差块互为 $\boldsymbol{\Sigma}\!\leftrightarrow\!\boldsymbol{\Sigma}^{-1}$ 的克罗内克对偶，正是 \cref{ch:slam_est} 协方差形式 $\leftrightarrow$ 信息形式两套滤波的几何根源。(2) \textbf{对称性不可省}：若忽略 $\boldsymbol{\Sigma}$ 对称、直接用 $\operatorname{vec}(\boldsymbol{\Sigma})$ 的 $N^2$ 维参数，FIM 把对角外元重复计两次（\cref{eq:matderiv-gauss-fim-canon} 实为\emph{过参数化}的奇异度量）；正确做法是 \cref{eq:matderiv-gauss-fim-sym} 用 $\mathbf{D}$ 降到 $N(N{+}1)/2$ 个独立参数——与 \cref{prop:matderiv-sym} 对协方差求导取对称化导数是同一回事。(3) \textbf{自然梯度}：把 \cref{eq:matderiv-gauss-fim-hybrid} 当度量、用 $\boldsymbol{\mathcal{I}}^{-1}\nabla$ 代替 $\nabla$，即得高斯变分推断的自然梯度下降（NGD），其更新对参数化\emph{不变}——这把本书的高斯估计与现代变分 SLAM / ESGVI 接上口\cite{barfoot2024state}。$\tfrac12(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1})$ 这一协方差块就是 NGD 在协方差方向上的步长缩放因子。
\end{note}
...
\begin{itemize}
  ...
  \item \textbf{统计变换（选读）}：FIM 在重参数化 $\boldsymbol{\theta}=\mathbf{g}(\boldsymbol{\phi})$ 下作雅可比夹心 $\boldsymbol{\mathcal{I}}_{\boldsymbol{\phi}}=\mathbf{J}^\top\boldsymbol{\mathcal{I}}_{\boldsymbol{\theta}}\mathbf{J}$（\cref{thm:matderiv-fim-reparam}），CRLB 随之 $\mathbf{J}^{-1}(\cdot)\mathbf{J}^{-\top}$，达界性不变。具体到多元高斯，五种参数化（正则/对称/混合/自然）的显式 FIM 见 \cref{prop:matderiv-gauss-fim}：协方差块 $\tfrac12(\boldsymbol{\Sigma}^{-1}\otimes\boldsymbol{\Sigma}^{-1})$（混合形为 $\tfrac12(\boldsymbol{\Sigma}\otimes\boldsymbol{\Sigma})$），是协方差/信息对偶与自然梯度下降的度量。
\end{itemize}
