...
\begin{enumerate}
  ...
  \item 一个控制器类只定义了 \texttt{init/\allowbreak update/\allowbreak starting/\allowbreak stopping} 四个回调函数，它自己\emph{从不}主动循环。那么“每 1\,ms 跑一次 \texttt{update}”这件事是谁在驱动？“先读传感器、再算、再发指令”的顺序又是谁保证的？
...
设想你要把控制器先在仿真里调通，再搬到真机。最直觉、也最糟的写法是：仿真版写一套 \texttt{readSim()/\allowbreak writeSim()}、真机版写一套 \texttt{readReal()/\allowbreak writeReal()}，控制算法里到处穿插 \texttt{if (是仿真) \dots else \dots}。等到要移植，你得把散落在各处的读写调用\emph{逐一}替换\cite{unitree2023quadruped}。这不仅工作量大、易错，更糟的是\emph{控制算法与硬件细节被焊死在了一起}——任何一方改动都牵连另一方。
...
\begin{itemize}
  ...
  \item \textbf{\texttt{A1-QP-MPC-Controller}}\cite{dicarlo2018cheetah}：\emph{入门级 MPC 范例}。是 MIT Cheetah 3 凸 MPC（\cref{ch:quad_mpc} 的主角范式）的极简复刻，主程序 \texttt{MainGazebo.\allowbreak cpp} 用\emph{双线程}（力控线程 + 主更新流程）把“凸 MPC 怎么跑起来”剥到最小可读。适合在读懂理论后，第一次看“MPC 工程长什么样”。
...
仿真/实物切换 & OOP 动态绑定：\texttt{IOInterface*} 指基类、指向 \texttt{IOROS}/\allowbreak\texttt{IOSDK} & 硬件抽象插件：\texttt{LeggedHWSim} 假扮硬件、控制器不知情 & 单一 Gazebo 主程序、双线程 \\
架构骨架 & FSM（Passive/FixedStand/Trotting\dots），每态四函数 & 控制器生命周期 init/update/starting/stopping + 管理器 & 双线程（力控 + 主更新）\\
关节命令 & \texttt{MotorCmd\{mode,q,dq,tau,Kp,Kd\}}，式 \cref{eq:qp-hybrid} & \texttt{HybridJointInterface.\allowbreak setCommand($\cdot$)}，底层 \texttt{EffortJointInterface} & \texttt{update\_foot\_forces\_grf}$\to$\texttt{send\_cmd} \\
...
\subsection{单腿层：静力学 \texorpdfstring{$\mathbf{J}^\top$}{J^T} 映射与摆动腿修正力}
\label{subsec:qp-leg}
...
\cref{ch:quad_mpc} 范本章的工程落地，把单刚体动力学沿预测时域堆叠成凸 QP。\texttt{A1-QP-MPC} 的 \texttt{MainGazebo.\allowbreak cpp} 把它剥到最小可读（见 \cref{sec:qp-fsm} 双线程结构）。
...
\texttt{unitree\_guide} 的 FSM 与 \texttt{legged\_control} 的生命周期回调（\cref{ins:qp-passive-active}）是两种不同粒度的“骨架”。FSM 管的是\emph{行为级}状态切换（站立$\leftrightarrow$小跑$\leftrightarrow$急停），每态四函数 \texttt{enter/\allowbreak run/\allowbreak exit/\allowbreak checkChange}；生命周期回调管的是\emph{控制器级}的加载/启用/运行/停用，四回调 \texttt{init/\allowbreak update/\allowbreak starting/\allowbreak stopping}。二者并不矛盾：一个工业级系统完全可以在“一个被生命周期管理的控制器”内部，再用一个 FSM 组织行为。教学框架把 FSM 摆在最外层、最显眼，正是为了让初学者一眼看清“机器人现在在干嘛、怎么切换”。
\end{insight}
...
\begin{tikzpicture}[>=Stealth, node distance=10mm and 14mm,
  ...
  \node[font=\scriptsize, gray!55!black, align=center, below=3mm of stand] {每态四函数：\texttt{enter}/\allowbreak\texttt{run}/\allowbreak\texttt{exit}/\allowbreak\texttt{checkChange}};
\end{tikzpicture}
...
A1QP 的主程序 \texttt{MainGazebo.\allowbreak cpp} 把“凸 MPC 怎么跑起来”剥到最小：分成两个线程\cite{dicarlo2018cheetah}。\emph{力控线程}跑 \texttt{update\_foot\_forces\_grf}——计算期望地面支撑力（即凸 MPC 的核心求解）；\emph{主更新流程}跑 \texttt{main\_update}$\to$\texttt{send\_cmd}——处理急停、期望状态、模式切换、步态、状态估计，并把力折算成关节命令下发。双线程的意义与 \cref{ins:qp-freq-decouple} 一致：MPC 求解慢、单独占一个线程，主流程高频跑、读 MPC 最新解。这是理解级联 MPC 架构的最小工程模型。
...
% --------------------------------------------------------------
%  以下 5 条陷阱来自工业级 OCS2（legged_control）移植\emph{自定义}四足的实战根因，
%  与上面 4 条（关节名/碰撞体/构型/零点）并列。它们的共同特征是“静默”——
%  不报错、不丢维度、RViz 看着正常，却让机器人穿地 / 翻车 / 估计错，最难排查。
% --------------------------------------------------------------

\begin{pitfall}[无名 \texttt{<material>} $\to$ URDF$\to$SDF 解析期静默丢碰撞 $\to$ 穿地]\label{pit:qp-urdf-sdf}
这条与 \cref{ins:qp-collision} 同源——都是\textbf{URDF 在转成 Gazebo 内部格式（SDF）时静默丢东西}，只是这里丢的不是“算得慢”，而是\emph{碰撞体本身}。根因极隐蔽：SolidWorks 导出的 URDF 里若有 \texttt{<material>} 标签\emph{缺少 \texttt{name} 属性}（这是非法 URDF），\texttt{urdfdom}/\allowbreak\texttt{sdformat} 在 URDF$\to$SDF 解析时会\emph{静默丢弃}与之相关的 \texttt{<collision>} 标签——\textbf{零 warning}。而 \texttt{<visual>} 标签照常保留，于是\textbf{RViz 里看得到、Gazebo 物理却穿模}：机器人几乎没有地面碰撞、缓缓下沉穿地。实战中一份 URDF 仅 $14$ 个无名 \texttt{<material>}，就让 $20$ 个 \texttt{<collision>} 中的 $17$ 个被悄悄丢掉\cite{sleiman2021unified}。

\noindent\textbf{金标准排查（碰撞数守恒核对）}：用
\begin{equation*}
  \texttt{gz sdf -p robot.\allowbreak urdf | grep -c collision}
\end{equation*}
数出 SDF 里实际生成的碰撞体个数，与 URDF 里手写的 \texttt{<collision>} 个数\emph{逐一核对}。\textbf{二者必须相等}；若 SDF 数偏少（如 URDF $20\to$ SDF $3$），就是解析器静默丢了碰撞——首要嫌犯即无名 \texttt{<material>} 或其他非法 URDF 写法。给所有 \texttt{<material>} 补上 \texttt{name} 后，SDF 碰撞数应当恢复（$3\to17$）。这条“数不变量守恒”比盲目调初值/阻尼高效得多。
\end{pitfall}

\begin{figure}[ht]
\centering
\begin{tikzpicture}[>=Stealth, node distance=4mm and 12mm,
  urdfbox/.style={draw, rounded corners, align=center, font=\small, inner sep=4pt, minimum height=0.95cm, text width=2.4cm},
  flow/.style={->, thick, gray!55!black}]
  \node[urdfbox, fill=gray!8] (urdf) {URDF\\\scriptsize 20 个 \texttt{<collision>}\\\scriptsize（含无名 material）};
  \node[urdfbox, right=of urdf, fill=second!10, draw=second!60!black] (parse) {\texttt{urdfdom}\\\texttt{sdformat}\\\scriptsize 解析\\\scriptsize（零 warning）};
  \node[urdfbox, above right=of parse, fill=third!10, draw=third!60!black] (rviz) {RViz\\\scriptsize visual 全在\\\scriptsize（看着正常）};
  \node[urdfbox, below right=of parse, fill=main!10, draw=main!60!black] (gz) {Gazebo (SDF)\\\scriptsize 仅 3 个 collision\\\scriptsize → 穿地下沉};
  \draw[flow] (urdf)--(parse);
  \draw[flow] (parse.east)--node[above,font=\scriptsize]{visual 保留}(rviz.west);
  \draw[flow] (parse.east)--node[below,font=\scriptsize]{\textcolor{red!70!black}{collision 静默丢 17}}(gz.west);
  \node[font=\scriptsize, gray!60!black, align=center, below=2mm of gz] {核对：\texttt{gz sdf -p}$\,|\,$\texttt{grep -c collision} 应 $=20$};
\end{tikzpicture}
\caption{无名 \texttt{<material>} 致 URDF$\to$SDF 静默丢碰撞（\cref{pit:qp-urdf-sdf}）：\texttt{<visual>} 全部保留（RViz 看着正常），\texttt{<collision>} 被悄悄丢弃（Gazebo 物理穿模）。碰撞数守恒核对（URDF 数 $=$ SDF 数）是定位此坑的金标准。}
\label{fig:qp-urdf-sdf}
\end{figure}

\begin{pitfall}[fixed-joint lumping：足端与 IMU 两处必须显式禁合并]\label{pit:qp-fixed-lumping}
Gazebo 默认把 \emph{固定关节}（fixed joint）连同其子 link \textbf{合并进父 link}（lumping），合并后子 link 不再是独立的 Gazebo link。而控制框架\emph{靠 link 名查找}对象，于是有两处必须用 \texttt{<disableFixedJointLumping>}（或 \texttt{dont\_collapse}）显式禁合并，否则\emph{静默退化}\cite{sleiman2021unified}：
\begin{itemize}
  \item \textbf{\texttt{*\_foot\_fixed}（足端固定关节）必加}：否则 \texttt{*\_FOOT} 被合并、不再是真 Gazebo link $\Rightarrow$ 接触管理器（ContactManager）报不出足端接触 $\Rightarrow$ 卡尔曼滤波收不到接触相 $\Rightarrow$ 状态估计出错（\cref{ch:quad_est}）。URDF 明明改对了，Gazebo 行为却悄悄变样。
  \item \textbf{\texttt{base\_imu\_joint}（机身$\to$IMU 固定关节）必加}：否则 \texttt{base\_imu} 被合并 $\Rightarrow$ 硬件层解析 IMU 时 \texttt{GetLink("base\_imu")} 返空指针 $\Rightarrow$ 触发 \texttt{ROS\_ASSERT} 直接崩溃。
\end{itemize}
两坑一“静默退化”（估计错）、一“硬崩溃”（断言失败），都源自同一机制——\textbf{框架按 link 名查的对象被 lumping 吃掉了}。
\end{pitfall}

\begin{pitfall}[关节阻尼/摩擦被 CAD 导出污染 $\to$ 裕度悄悄侵蚀 $\to$ 翻]\label{pit:qp-cad-damping}
这是最隐蔽的一类 sim-to-real 坑：\textbf{不报错、不丢碰撞、维度也全对}，只让稳定裕度悄悄变薄直至翻车。SolidWorks 等 CAD 导出 URDF 时，常把每个关节的 \texttt{joint\_damping} / \texttt{joint\_friction} 填成\emph{离谱的大值}（实战见 \texttt{damping} 被填成正常值的 $200$ 倍、\texttt{friction} 的 $5$ 倍）。其致命之处在于一个\emph{模型$-$仿真失配}：\textbf{上层 MPC 的质心动力学模型\emph{完全忽略}关节摩擦/阻尼，而 Gazebo 物理却真实施加它}。于是相对 MPC“心目中无摩擦的指令力矩”，仿真里的机器人响应变\emph{迟钝}（sluggish）——同样的前馈力矩产生不了预期加速度，稳定裕度被持续侵蚀（margin erosion），最终翻倒\cite{sleiman2021unified}。

\noindent\textbf{对策}：移植任何\emph{自带 URDF} 的机器人，先核对 \texttt{joint\_damping} / \texttt{joint\_friction} 是否被 CAD 导出污染成大值，回退到合理小值。这条与 \cref{pit:qp-urdf-sdf}（无名 material）、\cref{pit:qp-jointname}（关节名）、\cref{ins:qp-zero}（零点）并列，是第四类 sim-to-real 根因；完整的 sim-to-real 失配机理见 \cref{ch:quad_advanced}。
\end{pitfall}

\begin{pitfall}[CppAD 缓存陈旧：改了 URDF 必清缓存]\label{pit:qp-cppad-cache}
OCS2 的动力学用 CppAD 做自动微分，并\emph{把编译产物缓存}起来复用（避免每次启动都重新生成、编译微分库，那很慢）。代价是：\textbf{改了 URDF（连杆树/惯量/限位）后，陈旧缓存会让 MPC 仍用\emph{旧}模型/旧线性化}，表现为“改了没生效”或求解巨慢\cite{sleiman2021unified}。对策：首跑时置 \texttt{recompileLibrariesCppAd=true} 强制重生成、再关回，或直接清掉缓存目录（\texttt{/\allowbreak tmp/\allowbreak legged\_control/\allowbreak <robot>}）。这与 \cref{pit:qp-urdf-sdf} 的“数守恒核对”互补——后者查 URDF\emph{有没有进对} Gazebo，本条查 URDF\emph{有没有进对} MPC。
\end{pitfall}

\begin{pitfall}[URDF 与 OCS2 \texttt{*.info} 各管一摊：改一份漏另一份]\label{pit:qp-task-vs-urdf}
工业级框架的机器人配置\emph{分散在两份}文件里，各管一摊：\textbf{URDF} 管连杆树、惯量、关节限位、碰撞体、关节零位；\textbf{OCS2 的 \texttt{*.info}}（如 \texttt{task.info}/\allowbreak\texttt{gait.info}）管代价权重 $Q/R$、约束、力矩上界、步态序列。移植时只改其中\emph{一}份、漏改另一份，是典型陷阱——而且往往\emph{不报错}，只让行为悄悄不对。一个代表性例子：\emph{关节力矩上界}同时出现在 URDF 的 \texttt{effort} 与 \texttt{task.info} 的力矩约束里，而 WBC 实际读的是后者；只改了 URDF 的 \texttt{effort}、漏改 \texttt{task.info}，会让某关节的力矩仍被旧上界“饿死”（该例的完整故事见 \cref{ch:quad_wbc}）。\textbf{排查口诀：凡改一份配置，先问“另一份要不要同步”}。
\end{pitfall}
...
RViz 看着正常但机器人穿地下沉 & 无名 \texttt{<material>}$\to$解析期静默丢 collision & \texttt{gz sdf -p}$\,|\,$\texttt{grep -c collision} 核对碰撞数守恒（\cref{pit:qp-urdf-sdf}）\\
足端接触报不出 / IMU 解析崩溃 & fixed joint 被 lumping 合并掉 & \texttt{*\_foot\_fixed}/\allowbreak\texttt{base\_imu\_joint} 加 \texttt{disableFixedJointLumping}（\cref{pit:qp-fixed-lumping}）\\
翻车却\emph{无任何报错}、维度/碰撞都对 & 关节阻尼/摩擦被 CAD 导出污染成大值 & 核对 \texttt{joint\_damping}/\allowbreak\texttt{joint\_friction} 回退合理值（\cref{pit:qp-cad-damping}）\\
改了 URDF 却“没生效”/求解巨慢 & CppAD 微分库缓存陈旧 & \texttt{recompileLibrariesCppAd=true} 或清缓存目录（\cref{pit:qp-cppad-cache}）\\
某关节力矩/限位行为不对 & URDF 与 \texttt{task.info} 只改了一份 & 改一份即同步核对另一份（\cref{pit:qp-task-vs-urdf}）\\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  \item \textbf{写 URDF/xacro}：用 xacro 宏描述机器人结构；资源路径用 \texttt{package://}；视觉用精细网格、\emph{碰撞用简单几何体}（\cref{ins:qp-collision}）；采用标准垂直构型（\cref{ins:qp-standard-config}）；关节名与控制器硬编码名一致（\cref{pit:qp-jointname}）。\emph{写完即做三项守恒核对}：用 \texttt{gz sdf -p}$\,|\,$\texttt{grep -c collision} 核对碰撞数守恒（URDF 数 $=$ SDF 数，\cref{pit:qp-urdf-sdf}）；核对 \texttt{joint\_damping}/\allowbreak\texttt{joint\_friction} 是\emph{合理小值}而非 CAD 导出的污染大值（\cref{pit:qp-cad-damping}）；为全部 \texttt{<material>} 命名（防静默丢碰撞）。
  ...
  \item \textbf{选状态估计}：仿真里可用 \emph{cheater}（直接取 Gazebo 真值）或卡尔曼滤波二选一；真机只能用滤波（\cref{ch:quad_est}）\cite{sleiman2021unified}。\emph{bring-up 顺序：先用 cheater 跑通}（零噪真值排除估计误差，先确认控制器/步态/模型本身没问题），\emph{再切到 KF}（引入真实漂移）——这才是部署的真门槛（\cref{ch:quad_est}）。
...
以下纯工程项凝练登记、细节见 \cref{ch:engineering}：前馈力矩报文幅度上限 $|\tau_{\mathrm{ff}}|<2^{15}/256=128$（$2$ 字节有符号、$\times256$ 倍描述、符号位占 $1$ 位）\cite{unitree2023quadruped}；\texttt{/\allowbreak tmp/\allowbreak legged\_control/\allowbreak <type>.\allowbreak urdf} 必须由 xacro 生成；ROS melodic / Gazebo9 版本依赖；电机 FOC / 减速比换算。这些不影响理论理解，故不在本章推导。
\end{note}
...
\begin{enumerate}
  ...
  \item \textbf{症状}：RViz 看着正常，机器人却\emph{缓缓穿地下沉}、WBC 求解刷屏报错。\textbf{原因}：无名 \texttt{<material>} 致 URDF$\to$SDF 解析期静默丢碰撞（\cref{pit:qp-urdf-sdf}）。\textbf{对策}：用 \texttt{gz sdf -p}$\,|\,$\texttt{grep -c collision} 核对碰撞数守恒（URDF 数 $=$ SDF 数），给全部 material 命名。
  \item \textbf{症状}：足端\emph{接触报不出}、卡尔曼滤波收不到接触相而估计漂移；或 IMU 解析时崩溃。\textbf{原因}：fixed joint 被 Gazebo lumping 合并掉（\cref{pit:qp-fixed-lumping}）。\textbf{对策}：\texttt{*\_foot\_fixed} 与 \texttt{base\_imu\_joint} 加 \texttt{<disableFixedJointLumping>}。
  \item \textbf{症状}：机器人\emph{翻车}，但日志\emph{无任何报错}、碰撞与维度核对都正常。\textbf{原因}：关节阻尼/摩擦被 CAD 导出污染成大值，MPC 模型忽略而 Gazebo 真实施加，裕度被侵蚀（\cref{pit:qp-cad-damping}）；或 URDF 的 \texttt{effort} 与 \texttt{task.info} 力矩上界不一致（\cref{pit:qp-task-vs-urdf}）。\textbf{对策}：核对 \texttt{joint\_damping}/\allowbreak\texttt{joint\_friction} 回退合理值，并核对两份配置一致。
  \item \textbf{症状}：改了 URDF 却\emph{像没生效}、或 MPC 求解突然巨慢。\textbf{原因}：CppAD 微分库缓存陈旧、仍用旧模型（\cref{pit:qp-cppad-cache}）。\textbf{对策}：首跑置 \texttt{recompileLibrariesCppAd=true} 再关，或清 \texttt{/\allowbreak tmp/\allowbreak legged\_control/\allowbreak <robot>} 缓存目录。
\end{enumerate}
