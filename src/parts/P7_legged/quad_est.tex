...
\subsection{整合 \texorpdfstring{$28$}{28} 维观测方程}
...
\paragraph{支撑腿内部也该可变：窗口函数渐变协方差。}
上面的二值膨胀只把腿分成“摆动（大协方差）/ 支撑（小协方差）”两档，对支撑相\emph{一视同仁}。但更精细地看，\emph{支撑相内部}也并非处处一样可信：足端\emph{刚开始触地}与\emph{即将结束触地、准备腾空}的瞬间，足端位置都会有明显抖动（落地冲击、卸载回弹），此时即便已判触地也仍需\emph{较大}协方差；唯有支撑相\emph{中段}稳定踩实时，足端位置抖动才小、协方差才该小。于是应给支撑腿一个\emph{随触地进度连续变化}的协方差\cite{unitree2023quadruped}。

为此引入\emph{窗口函数}（window function）$\sigma(x)$ 作“可信度”，其中 $x\in[0,1]$ 是触地相位（$0$ 刚触地、$1$ 即将离地，由步态调度器或触地检测给出）。$\sigma$ 在定义域\emph{两端小、中间大}，恰好编码“信任触地中段、不信任始末”。用分段（梯形）函数表示\cite{unitree2023quadruped}：
\begin{equation}
  \sigma(x)=\begin{cases}
    \dfrac{x}{w_r}, & 0\le x< w_r,\\[6pt]
    1, & w_r\le x\le 1-w_r,\\[4pt]
    \dfrac{1-x}{w_r}, & 1-w_r< x\le 1,
  \end{cases}
  \label{eq:qe-window}
\end{equation}
其中 $w_r\in(0,\tfrac12)$ 是斜升/斜降段的相位宽度（爬坡半宽）。设足端\emph{稳定触地}时的噪声协方差为 $\mathbf{C}_{\mathrm{init}}$，则整个触地周期内该足端的协方差取\cite{unitree2023quadruped}：
\begin{equation}
  \mathbf{C}_{\mathrm{stance}}=\big[\,1+(1-\sigma)\,L\,\big]\,\mathbf{C}_{\mathrm{init}},
  \label{eq:qe-cstance}
\end{equation}
$L$ 为代替 $+\infty$ 的极大数（与 \cref{eq:qe-cov-inflate} 的 $n_{big}$ 同类）。读两端：$\sigma=1$（中段，最信任）$\Rightarrow\mathbf{C}_{\mathrm{stance}}=\mathbf{C}_{\mathrm{init}}$；$\sigma=0$（触地始末，不信任）$\Rightarrow\mathbf{C}_{\mathrm{stance}}=(1+L)\mathbf{C}_{\mathrm{init}}$ 极大——即支撑腿的协方差在触地周期内\emph{由大到小再到大}地连续变化（\cref{fig:qe-window}），而非二值跳变。

\begin{figure}[ht]
\centering
\begin{tikzpicture}[>=Stealth,scale=1.0]
  \draw[->] (0,0)--(5.8,0) node[right,font=\scriptsize]{触地相位 $x$};
  \draw[->] (0,0)--(0,2.35) node[above,font=\scriptsize]{可信度 $\sigma(x)$};
  \draw (0,0.05)--(0,-0.05) node[below,font=\scriptsize]{$0$};
  \draw (5,0.05)--(5,-0.05) node[below,font=\scriptsize]{$1$};
  \draw[dashed,gray!55] (0,1.8)--(5,1.8); \node[font=\scriptsize] at (-0.25,1.8){$1$};
  % trapezoid window: w_r=0.25 -> ramp 0..1.25, plateau 1.25..3.75, ramp 3.75..5
  \draw[main,very thick] (0,0)--(1.25,1.8)--(3.75,1.8)--(5,0);
  \draw[dashed,gray!55] (1.25,0)--(1.25,1.8); \node[font=\scriptsize] at (1.25,-0.32){$w_r$};
  \draw[dashed,gray!55] (3.75,0)--(3.75,1.8); \node[font=\scriptsize] at (3.75,-0.32){$1-w_r$};
  \node[main,font=\scriptsize] at (2.5,2.08){平台：稳定支撑，$\mathbf{C}_{\mathrm{stance}}=\mathbf{C}_{\mathrm{init}}$};
  \node[third,font=\scriptsize,align=center] at (0.62,0.95){刚触地\\抖动大};
  \node[third,font=\scriptsize,align=center] at (4.4,0.95){将离地\\抖动大};
\end{tikzpicture}
\caption{支撑腿协方差的渐变窗口函数 $\sigma(x)$（\cref{eq:qe-window}）。触地中段 $\sigma=1$、协方差取最小 $\mathbf{C}_{\mathrm{init}}$（最信任）；触地始末 $\sigma\to0$、协方差经 \cref{eq:qe-cstance} 膨胀到 $(1+L)\mathbf{C}_{\mathrm{init}}$（不信任落地冲击/卸载回弹的抖动）。这把“摆动 / 支撑”二值开关细化为“摆动 / 触地始末 / 稳定支撑”三档连续。}
\label{fig:qe-window}
\end{figure}

\begin{insight}[二值膨胀 $\subset$ 渐变窗口：合并 OCS2 与宇树两源所长]\label{ins:qe-cov-graded}
\cref{eq:qe-cov-inflate} 的二值膨胀与 \cref{eq:qe-cstance} 的渐变窗口\emph{不是两套对立方案，而是同一思想的粗细两档}，正可合并：把可信度 $\sigma$ 看作连续旋钮，则\textbf{二值膨胀是 $\sigma$ 只取 $\{0,1\}$ 两端的硬量化}——摆动腿对应 $\sigma\to0$（$\mathbf{C}_{\mathrm{stance}}\to(1+L)\mathbf{C}_{\mathrm{init}}$，正是 $n_{big}$ 膨胀），稳定支撑对应 $\sigma=1$（$\mathbf{C}_{\mathrm{init}}$）；而\textbf{渐变窗口把这个开关在触地始末用 $0<\sigma<1$ 平滑过渡}，于是“摆动/支撑”二档被细化成“摆动 / 触地始末 / 稳定支撑”三档连续。两源的取舍因此清晰：\textbf{OCS2/论文的二值膨胀}实现最简、在\emph{平地 $+$ 固定步态}下足够（落足时刻可预测、相位切换抖动可忽略，呼应 \cref{ins:qe-prob-boundary}）；\textbf{宇树的渐变窗口}在\emph{快步态 / 非结构化地形}下额外平滑了相位切换处的估计跳变（落地冲击不被当作可信观测猛拉机身速度），代价是要多标定爬坡半宽 $w_r$ 与触地相位 $x$。工程上可\emph{先用二值膨胀跑通、再在相位切换抖动明显时升级为渐变窗口}——二者共享同一个 $\mathbf{C}_{\mathrm{init}}$ 与 $L$，升级只是把 $\sigma$ 从两级量化放开为连续。
\end{insight}
...
\end{equation}
\cref{eq:qe-cov-recur} 只需保存 $(\bar{\boldsymbol z}_{n-1},\mathbf{C}_{n-1},n)$ 即可逐样本更新，内存恒定、适合上机标定。物理读法：第一项是新样本对当前均值的“离群度”外积（带 $1/n^2$ 量级衰减，样本越多单次扰动越弱），第二项是旧协方差的继承（带 $\tfrac{n-1}{n}\!\to\!1$ 的遗忘因子）。过程噪声 $\mathbf{Q}$ 则\emph{部分可测、部分只能整定}。可测的那部分来自\textbf{输入量噪声的传播}：离散 KF（\cref{eq:qe-kf-system}）把一切过程噪声并入 $\boldsymbol w\sim(0,\mathbf{Q})$，而输入 $\boldsymbol u$（由 IMU 读数算出，\cref{eq:qe-world-acc}）本身带噪声 $\boldsymbol o$。把它显式写进状态递推：
\begin{equation}
  \boldsymbol x_k=\mathbf{A}\boldsymbol x_{k-1}+\mathbf{B}(\boldsymbol u_{k-1}+\boldsymbol o_{k-1})
  =\mathbf{A}\boldsymbol x_{k-1}+\mathbf{B}\boldsymbol u_{k-1}+\underbrace{\mathbf{B}\,\boldsymbol o_{k-1}}_{\boldsymbol w_u},
  \label{eq:qe-Qu-input}
\end{equation}
可见过程噪声中由输入产生的分量为 $\boldsymbol w_u=\mathbf{B}\boldsymbol o_{k-1}$。$\boldsymbol o$ 的协方差 $\mathbf{C}_u$ 与 $\mathbf{R}$ 一样可经 \cref{eq:qe-cov-recur} 直接递推测量（因 $\boldsymbol u$ 由 IMU 给出），再经输入矩阵 $\mathbf{B}$ 传播为 $\boldsymbol w_u$ 的协方差：
\begin{equation}
  \boxed{\;\mathbf{Q}_u=\frac1n\sum_{i=1}^{n}\mathbf{B}(\boldsymbol o_i-\bar{\boldsymbol o}_n)(\boldsymbol o_i-\bar{\boldsymbol o}_n)^\top\mathbf{B}^\top=\mathbf{B}\,\mathbf{C}_u\,\mathbf{B}^\top.\;}
  \label{eq:qe-Qu}
\end{equation}
故 $\mathbf{Q}$ 的输入噪声分量 $\mathbf{Q}_u=\mathbf{B}\mathbf{C}_u\mathbf{B}^\top$ \emph{可直测}；剩下的（单刚体建模简化、前向欧拉离散化误差等）才\emph{无}真值残差可采、只能按模型可信度\emph{整定}（\cref{note:qe-imu-bridge} 把 IMU 零偏并入 $\mathbf{Q}$）。这就把笼统的“$\mathbf{R}$ 易测、$\mathbf{Q}$ 难定”精确化为：$\mathbf{R}$ 与 $\mathbf{Q}$ 的输入分量 $\mathbf{Q}_u$ 皆可直测，唯 $\mathbf{Q}$ 的建模/离散化分量才是真正只能调的部分。
\end{derivation}

\begin{theorem}[协方差等比例缩放不变性]\label{thm:qe-cov-scale}
若把过程噪声 $\mathbf{Q}$、观测噪声 $\mathbf{R}$ 与初始后验协方差 $\mathbf{P}_0^+$ \emph{同乘}一个正系数 $a>0$（即 $\mathbf{Q}'=a\mathbf{Q}$、$\mathbf{R}'=a\mathbf{R}$、$\mathbf{P}_0^{+\prime}=a\mathbf{P}_0^+$），则离散卡尔曼滤波在任意时刻 $k$ 给出的最优状态估计 $\hat{\boldsymbol x}_k^+$ \emph{完全不变}。因此协方差矩阵可\emph{任意等比例缩放}到便于计算的量级，KF 的输出只取决于 $\mathbf{Q}$ 与 $\mathbf{R}$ 的\emph{比值}。
\end{theorem}
\begin{proof}
归纳。$k=1$ 的先验估计 $\hat{\boldsymbol x}_1^-=\mathbf{A}\hat{\boldsymbol x}_0^++\mathbf{B}\boldsymbol u_0$ 不含协方差，故不变。先验协方差（\cref{eq:qe-kf-predict}）随 $a$ 整体放大：
\begin{equation}
  \mathbf{P}_1^{-\prime}=\mathbf{A}(a\mathbf{P}_0^+)\mathbf{A}^\top+a\mathbf{Q}=a\big(\mathbf{A}\mathbf{P}_0^+\mathbf{A}^\top+\mathbf{Q}\big)=a\,\mathbf{P}_1^-.
  \label{eq:qe-scale-P}
\end{equation}
代入增益（\cref{eq:qe-kf-gain}），分子的 $a$ 与分母括号内整体提出的 $a$ 相消：
\begin{equation}
  \mathbf{K}_1'=a\mathbf{P}_1^-\mathbf{H}^\top\big(a\mathbf{R}+\mathbf{H}\,a\mathbf{P}_1^-\mathbf{H}^\top\big)^{-1}
  =a\mathbf{P}_1^-\mathbf{H}^\top\cdot\tfrac1a\big(\mathbf{R}+\mathbf{H}\mathbf{P}_1^-\mathbf{H}^\top\big)^{-1}=\mathbf{K}_1.
  \label{eq:qe-scale-K}
\end{equation}
增益不变 $\Rightarrow$ 后验估计 $\hat{\boldsymbol x}_1^+=\hat{\boldsymbol x}_1^-+\mathbf{K}_1(\boldsymbol z_1-\mathbf{H}\hat{\boldsymbol x}_1^-)$ 不变；而后验协方差（\cref{eq:qe-kf-cov}）因 $\mathbf{K}_1$ 不变、$\mathbf{P}_1^-,\mathbf{R}$ 各带因子 $a$，亦满足 $\mathbf{P}_1^{+\prime}=a\mathbf{P}_1^+$。于是 $k=2$ 的初始条件 $(\hat{\boldsymbol x}_1^+,a\mathbf{P}_1^+)$ 与 $k=1$ 同构，归纳得任意 $k$ 的 $\hat{\boldsymbol x}_k^+$ 不变、$\mathbf{P}_k^{+\prime}=a\mathbf{P}_k^+$。\qed
\end{proof}

\begin{note}[只整定比值：过大 $\mathbf{Q}$ $\equiv$ 缩小 $\mathbf{R}$]\label{note:qe-cov-ratio}
\cref{thm:qe-cov-scale} 有两条直接用处。其一，整定时\emph{只需关心 $\mathbf{Q}/\mathbf{R}$ 的相对大小}，可把它们缩放到数值最舒服的量级而不改变估计——这也解释了 \cref{eq:qe-cov-inflate} 的膨胀本质上是一次\emph{相对}缩放（把摆动腿块相对其余块放大 $n_{big}$ 倍），其绝对量级无所谓、唯比值要紧。其二，由不变性，\textbf{把 $\mathbf{Q}$ 调得过大，等价于变相缩小 $\mathbf{R}$}：滤波器会过度信任测量值，若测量本身噪声大，最优估计便继承大噪声，表现为状态向量的\emph{高频抖振}。故 $\mathbf{Q}$ 的常用调法是\emph{先给大、再逐步减小}：过大则模型几乎不被信任（抖振），过小则过度信任可能不准的模型（发散），在二者间找平衡（\cref{sec:qe-pitfall} 的发散/抖振两类陷阱正是这两端）。
\end{note}
...
\begin{itemize}
  \item \textbf{仿真 cheater}（如 \texttt{FromTopicStateEstimate}）：直接订阅仿真器发布的\emph{地面真值}（Gazebo \texttt{p3d} 插件挂在 \texttt{base} 上给出的 \texttt{/\allowbreak ground\_truth/\allowbreak state}），\emph{透传不滤波}——位姿、速度都是仿真内核的精确值。对应一个 \texttt{legged\_cheater\_controller}。它\emph{不是}本章任何一条估计线，而是“假装估计已经完美”的占位。
  \item \textbf{生产 KF}（如 \texttt{LinearKalmanFilter}）：正是本章的 $18$ 维线性卡尔曼滤波——\textbf{IMU 加速度做预测}（\cref{eq:qe-kf-predict}，输入由 \cref{eq:qe-world-acc} 给出）、\textbf{足端正运动学做更新}（\cref{eq:qe-obs-z}），\textbf{按接触相调噪声}（支撑足信任运动学、摆动足协方差 $\times n_{big}$，\cref{eq:qe-cov-inflate}）。可选再融合 \texttt{/\allowbreak tracking\_camera/\allowbreak odom} 等外部里程计。
\end{itemize}
...
\begin{enumerate}
  \item \textbf{IMU 传感器噪声}：在仿真模型（如 \texttt{imu.xacro}/\allowbreak\texttt{gazebo.\allowbreak xacro}）的 \texttt{gaussianNoise} 项——这是\emph{注入到读数里}的噪声，决定 $\boldsymbol a_{out},\boldsymbol\omega_{out}$ 有多脏（\cref{eq:qe-aout-3d}）；
...
\end{enumerate}
关键提醒：第(1)类（传感器噪声）与第(2)类（滤波器协方差）是\emph{两件事}——前者是世界注入的真实噪声，后者是滤波器\emph{以为}有多少噪声；二者不匹配（如传感器无噪却把 $\mathbf{R}$ 设很大）会让 KF 过度不信观测、估计滞后。
...
具体地，一套“左右对称性检查 $+$ 站姿 FK 核对”脚本（\texttt{check\_lr\_symmetry.\allowbreak py} $+$ \texttt{verify\_pose}）会在机器人\emph{静止站立}时，最先从\textbf{左右足端世界系 $z$ 的劈叉量}（z-spread $\approx3\,\mathrm{mm}$）里揪出这个头号不对称——远早于你在行走轨迹里看到左偏。这把左右不对称从“行走故障”重定位成\textbf{站立标定阶段的可观测症状}：\emph{站立时若左右髋劈叉方向相反/不对称、或左右足高对不齐，第一怀疑点不是 KF 协方差，而是几何不对称（连杆参数左右不镜像）或关节序映射错配}。
\end{insight}
...
渐变窗口协方差 \cref{eq:qe-cstance} & $\mathbf{C}_{\mathrm{stance}}=[1+(1-\sigma)L]\mathbf{C}_{\mathrm{init}}$ & \cref{ins:qe-cov-graded} \\
...
输入噪声传播 \cref{eq:qe-Qu} & $\mathbf{Q}_u=\mathbf{B}\mathbf{C}_u\mathbf{B}^\top$ & \cref{deriv:qe-cov-meas} \\
协方差缩放不变 \cref{thm:qe-cov-scale} & 同乘 $a$ 不改估计，只整定比值 & \cref{note:qe-cov-ratio} \\
\bottomrule
\end{tabular}
\end{table}
