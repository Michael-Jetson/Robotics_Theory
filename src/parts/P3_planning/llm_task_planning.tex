...
\rebuilt{本章重渲染自一份 Tutorial 笔记（任务运动规划线“大模型任务规划”），源稿由 AI 撰写。已经一轮联网复核：论文归属、年份、会议绝大多数核实无误（详见 claims 台账）；\textbf{PlanBench 的 $\approx 3\%$ 经核实为伪数值（应为常识域约 $34\%$、全域约 $12\%$），已就地删除并 \texttt{\textbackslash rebuilt} 标注}；Guan 的“$40$ 余动作 / $48$ 任务”经核实\emph{属实}（与论文摘要一致）。其余存疑/待核处已就地 \texttt{\textbackslash pz}/\allowbreak\texttt{\textbackslash rebuilt} 标注。}
...
\begin{itemize}
  \item \textbf{PDDL 与经典规划器}（经典规划部）：PDDL 用 \texttt{domain}（动作模型）+ \texttt{problem}（初末状态）描述规划问题；经典规划器在合法 PDDL 上能\emph{完备}地搜出满足约束的计划、并可配置求\emph{最优}。\rebuilt{此句把“完备 + 可配置最优”一揽子归给 FF 与 Fast Downward \emph{两者}，经裁决判为混淆（soundness 两者皆真，但另两项需分开讲）：\textbf{FF} 是\emph{满足型}（satisficing）规划器，其 $h_{\text{FF}}$ 启发式不可采纳、\emph{无最优配置}；且核心搜索（enforced hill-climbing + helpful actions 剪枝）\emph{不完备}，仅靠遇死端时回退到无剪枝的最佳优先搜索这一“安全网”才取得完备性（Hoffmann \& Nebel, JAIR 2001）。\textbf{Fast Downward} 才\emph{可配置最优}（如 \texttt{seq-opt-lmcut} $=$ A$^\ast$ $+$ 可采纳 LM-cut），亦可配满足型（\texttt{seq-sat-lama}）；其完备性同样依配置而定。正确表述：\emph{Fast Downward 可配置完备/最优，FF 是满足型、完备性靠回退兜底}。}本章的 LLM-as-Modeler（\cref{sec:llm-modeler}）正是把 LLM 当作“PDDL 的写手”，把搜索与验证仍交给这些经典规划器——\textbf{不懂 PDDL，连 LLM 生成的 PDDL 对不对都判断不了}。
...
\noindent 这些不是“模型不够大”能根治的偶发 bug，而是“续写而非推理”这个本质带来的\emph{系统性风险}。Valmeekam 等人 2023 年在 NeurIPS 用一个专门的规划基准（PlanBench）做了批判性测量：GPT 系列模型\emph{自主}生成可执行计划的成功率\emph{显著偏低}\cite{valmeekam2023planbench}。这个结果，是对“LLM 是个好规划器”这一幻觉最直接的证伪。\rebuilt{原句的“常识规划任务上平均只有约 $3\%$”系裁决判错（数值不可复现、与基准不符）：PlanBench 实测——GPT-4 在常识 Blocksworld 一次性提示约 $34\%$（论文原文“approximately $34\%$”），跨全部域（含 Mystery 混淆域）GPT-4 约 $12\%$；$3\%$ 量级仅出现在刻意混淆的非常识 Mystery 域，与此处“常识规划任务”的框定矛盾。原始 $3\%$ 断言已删，正确方向：常识域 GPT-4 约 $34\%$、全域约 $12\%$（详见 claims 台账，待写定稿引用）。}%
% 〔已注释·裁决判错·跳过不重建〕原句："在常识规划任务上，GPT 系列模型自主生成可执行计划的成功率极低，平均只有约 3%。3% 这个数字，是对……最直接的证伪。"——3% 为伪数值。
...
\begin{itemize}
  \item \textbf{以为“模型够大就能直接规划”。} 最普遍的误区。PlanBench 的实证\cite{valmeekam2023planbench}说明自主规划的失败是\emph{机制性}的（续写不等于推理），不是规模能根治的。后续模型会让数字涨一些，但“不验证、会幻觉”的本质不变。\pz{此处原作“$3\%$ 的实证”，该数值经裁决判为伪（PlanBench 实测 GPT-4 常识域约 $34\%$、全域约 $12\%$），已抹去具体数字、仅保留定性结论。}
...
\begin{enumerate}
  ...
  \item “LLM 自主规划成功率\emph{显著偏低}”——这个结论在什么任务上测的？能否推广到“LLM 在所有规划任务上都同样低”？为什么不能（提示：as-Modeler 与 as-Planner 的区别；又如域的难度，常识 Blocksworld vs 混淆 Mystery 域差异极大）？\pz{此题原以具体数值“约 $3\%$”设问，该数值经裁决判为伪，已改为定性表述。}
...
主动场景查询（active scene query）\pz{存疑：原论文该类官方名为 “Active Scene Description”（主动场景\emph{描述}），非 “active scene query”；“query” 仅出现在其定义“feedback in response to active queries by the LLM planner”中，故此标签是可辩护的意译但与原名不符。前两类名（success detection / passive scene description）则与原文一致} & 针对某疑问，环境的回答是？ & LLM \emph{主动}提问、感知/人回答 & LLM 问“杯子什么颜色？”$\to$“蓝色” \\
\bottomrule
\end{tabular}
\end{table}
...
LLM-as-Modeler 把规划拆成“建模”和“求解”两步，让 LLM 只干第一步：LLM 根据自然语言指令 $+$ 领域背景，生成 PDDL domain（动作模型）和/或 problem；再交 Fast Downward / FF 在合法 PDDL 上搜索。这条路的巨大好处源自前置自测 Q3：\textbf{经典规划器在合法 PDDL 上是完备、可验证、可最优的。}\pz{严格说“可最优”只对可配置最优的规划器成立（如 Fast Downward 的 A$^\ast$+可采纳启发），\emph{FF 是满足型、不保证最优}；完备性也依配置/回退而定——详见前置桥接处 \texttt{\textbackslash rebuilt} 更正} 一旦 LLM 写出的 PDDL 经检验确认无误，剩下的搜索就由可信的经典规划器全权接管——它找到的计划\emph{必然合法}（满足所有前提）、可配置\emph{最优}，绝不会幻觉出不存在的动作。LLM 的“不严谨”被彻底隔离在“建模”之外、碰不到最终计划。

LLM+P（Liu 等，2023）是这条路的开创性框架：LLM+P 接收一个规划问题的自然语言描述，先把它转成 PDDL，再用经典规划器快速求解，最后把解翻译回自然语言；实验显示 LLM+P 能为大多数问题给出最优解，而 LLM 自己对多数问题连可行解都给不出\cite{liu2023llmp}。注意最后半句——这正是 LLM 自主规划之弱的另一面：\textbf{同一个 LLM，让它直接规划近乎失败，让它只翻译成 PDDL 再交规划器，却能给出最优解。}\pz{原文此处写作“那个 $3\%$ 的另一面”，回指 \cref{sec:llm-not-planner} 已删的伪数值 $3\%$，故改为“LLM 自主规划之弱”。}

\begin{insight}[LLM+P 是对核心边界最有力的量化背书]
LLM+P 的实验结果，是对本章核心边界最有力的一次量化背书。同一个 LLM、同一批问题，区别只在于\textbf{让它干哪一步}——直接规划（弱项，多数问题给不出可行解\pz{原作“$\approx 3\%$”，该 PlanBench 数值经裁决判为伪、已抹去；此处改用 LLM+P 论文自身的定性结论“LLM 对多数问题连可行解都给不出”，与上句一致}）还是翻译建模（强项，接近最优）。这把“增强而非取代”从一句口号变成可复现的实验事实：\textbf{LLM 的价值不在于替规划器做搜索，而在于替人做那件繁琐的“把自然语言翻译成形式化模型”的活儿。} 它把人类规划专家从“手写 PDDL”里解放出来，同时一丝不让地保住了经典规划器的所有理论保证——这就是钟摆被拉回的极致。
\end{insight}
...
\node[llm, below=of in] (front) {\textbf{LLM 前端}（本章）——补语言与常识，产出“待验证的候选”\\ (1) 意图理解 $+$ 常识补全；(2) 感知接地（VLM）；\\ (3) 二选一范式产出候选（as-Planner / as-Modeler）；(4) 三层校验，失败则自纠错};
...
\textbf{边界（要清醒的地方）}：别信“纯 LLM 端到端、无任何校验”的方案（安全攸关部署必然出事，PlanBench 测得的自主规划低成功率是警钟\pz{原文此处作“$\approx 3\%$”，该数值经裁决判为伪，已抹去}）；别把本章方法和端到端 VLA 混为一谈（本章是符号/语言层的任务规划，VLA 是端到端动作生成）；别迷信 benchmark（很多“LLM 会规划”的乐观结论来自评测宽松或任务泄漏）；别忘了地基（这个方向跑得越快，越要经典 TAMP 的功底兜底）。
...
\begin{itemize}
  \item \textbf{为什么需要 LLM}（\cref{sec:llm-why}）：经典规划器有两个软肋——听不懂自然语言、缺常识；这正是 LLM 的两个强项。但 LLM 本质是\emph{续写引擎而非推理引擎}——它生成“统计上最像好计划的文字”，不保证可行、合法、最优（PlanBench 测得其自主规划成功率显著偏低\pz{原作“$\approx 3\%$”，数值经裁决判为伪、已抹去}）。这道“像 vs 是”的鸿沟，是全章一切设计的起点。
...
\begin{itemize}
  ...
  \item \textbf{批判与边界}：Valmeekam 等\cite{valmeekam2023planbench}（PlanBench，用严格基准戳破“LLM 会规划”的乐观\pz{原作“本章 $\approx 3\%$ 的来源”，该数值经裁决判为伪、已从正文删除}）；Mendez-Mendez 等\cite{mendezmendez2025llmtamp}（LLM 替代 TAMP 求解器组件仍更差）。
