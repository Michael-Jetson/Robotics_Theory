...
%  + Solovey 2020（RRT* 最优性勘误）+ Gammell 2014/2020（Informed-RRT*/BIT*）
%  + Harabor-Grastien 2011（JPS）+ Dubins 1957 / Reeds-Shepp 1990（非完整曲线）
%  + Mellinger-Kumar 2011（minimum-snap）。自包含独立记录，读者无需翻原书。
...
\begin{enumerate}
  ...
  \item \textbf{判别}各采样算法的概率完备性与渐近最优性，掌握临界连接半径 $r(n)\sim(\log n/n)^{1/d}$ 的几何来由，并说明 Informed-RRT$^*$/BIT$^*$ 如何在保持最优的前提下加速收敛；
  \item \textbf{处理}非完整约束：写出 Dubins 最短路的六种词型与 Reeds--Shepp 的扩展，区分状态格、kinodynamic 采样如何重新定义图/树的“边”；
  \item \textbf{写出}轨迹优化的一般泛函形式，推导 CHOMP 的协变梯度更新与 minimum-snap 的等式约束 QP，并说明“全局采样/搜索 + 安全走廊 + 局部优化”的级联流水线。
\end{enumerate}
...
\node[grp, right=of disc] (search) {搜索式\\ Dijkstra, A$^*$, 加权A$^*$, JPS, D$^*$};
\node[grp, below=7mm of search] (sample) {采样式\\ PRM, RRT, RRT$^*$, Informed, BIT$^*$};
\node[grp, below=7mm of sample] (kino) {运动学约束\\ Dubins/RS, 状态格, kinodynamic};
\node[opt, below=7mm of kino] (traj) {优化式\\ CHOMP, minimum-snap, TrajOpt};
...
\noindent\textbf{推荐路径}：第 \ref{sec:plan-cspace} 节（构型空间）是一切的地基；第 \ref{sec:plan-search}--\ref{sec:plan-astar} 节（图搜索 + A$^*$，含最优性证明）是\emph{搜索式}主干，也是采样式查询阶段复用的工具；第 \ref{sec:plan-grid} 节（栅格启发、加权 A$^*$、JPS）面向工程；第 \ref{sec:plan-sampling}--\ref{sec:plan-optimal} 节（采样式 + 渐近最优理论，含 Informed/FMT$^*$/BIT$^*$）是本章\emph{篇幅最大、最核心}的部分，VIO/导航/操作臂规划都依赖它；第 \ref{sec:plan-kinodynamic} 节（运动学约束：Dubins/Reeds--Shepp、状态格、kinodynamic）面向真实车/无人机；第 \ref{sec:plan-traj} 节（轨迹优化，含 CHOMP 与 minimum-snap）是收尾，衔接后续控制章。带（选读）标记的 D$^*$、JPS、渐近最优完整证明为进阶/选读。
...
\subsection{典型构型空间：\texorpdfstring{$\SO(2),\SE(2),\SO(3),\SE(3)$}{(2), (2), (3), (3)}}
\label{sec:plan-so3}
...
当状态 $\mathbf{x}$ 经 \texttt{Q.\allowbreak extract\_min()} 出队（变死态）时，$C(\mathbf{x})=C^*(\mathbf{x})$——即不可能再以更低代价到达 $\mathbf{x}$。
\end{theorem}
...
% ════════════════════════════════════════════════════════════════
\section{A\texorpdfstring{$^*$}{^*} 搜索与启发函数}
...
\subsection{加权 A\texorpdfstring{$^*$}{^*}：以可控次优换速度}
\label{sec:plan-weighted}

A$^*$ 的最优性以扩展大量节点为代价。许多工程场景（实时导航、博弈寻路）宁可要一条\emph{近似}最短、但快得多的路。\textbf{加权 A$^*$}（Weighted A$^*$，Pohl 1970）把评价函数改为
\begin{equation}
  f_\varepsilon(\mathbf{x})=g(\mathbf{x})+\varepsilon\,h(\mathbf{x}),\qquad\varepsilon\ge1,
  \label{eq:plan-weighted-astar}
\end{equation}
即把启发\emph{放大} $\varepsilon$ 倍，使搜索更激进地扑向目标。$\varepsilon=1$ 退化为 A$^*$；$\varepsilon\to\infty$ 退化为贪心最佳优先。其代价是丧失最优性，但损失\emph{有界}。

\begin{theorem}[加权 A$^*$ 的 $\varepsilon$-次优界]\label{thm:plan-weighted}
设 $h$ 可采纳。则加权 A$^*$（\cref{eq:plan-weighted-astar}）返回的解代价 $C$ 满足
\begin{equation}
  C\le\varepsilon\,C^*,
  \label{eq:plan-weighted-bound}
\end{equation}
其中 $C^*$ 为最优代价。即解至多比最优差 $\varepsilon$ 倍（\textbf{有界次优}，bounded-suboptimal）。
\end{theorem}
\begin{proof}（同 \cref{thm:plan-astar-opt} 的结构）
在算法选中某目标 $G$ 时，最优路径上必有节点 $\mathbf{n}$ 仍在 open 中，对它 $f_\varepsilon(\mathbf{n})=g(\mathbf{n})+\varepsilon h(\mathbf{n})\le g(\mathbf{n})+\varepsilon h^*(\mathbf{n})\le\varepsilon\,(g(\mathbf{n})+h^*(\mathbf{n}))=\varepsilon C^*$（首不等号用可采纳，次不等号因 $\varepsilon\ge1$ 且 $g\ge0$）。因 $G$ 被先选中，$g(G)=f_\varepsilon(G)\le f_\varepsilon(\mathbf{n})\le\varepsilon C^*$。
\end{proof}

\noindent 实践上 $\varepsilon\in[1.5,3]$ 常能把扩展节点数降一两个数量级而路径仅略长。\textbf{ARA$^*$}（Anytime Repairing A$^*$，Likhachev 等 2003\cite{likhachev2004ara}）据此构造\emph{anytime} 搜索：先用大 $\varepsilon$ 飞快得一条 $\varepsilon$-次优解，再逐步减小 $\varepsilon\to1$ 并\emph{复用}上一轮的搜索努力，时间允许时收敛到最优——这是把\cref{eq:plan-weighted-bound} 的可控次优变成“随时可中断、越跑越优”的关键工程技巧。

\subsection{\texorpdfstring{$\dagger$}{x} Jump Point Search：栅格上的对称性消除}
\label{sec:plan-jps}

\paragraph{动机：均匀栅格充满对称路径。}
在无障碍的均匀代价栅格上，从 $\mathbf{x}_I$ 到 $\mathbf{x}_G$ 有指数多条\emph{等长}最短路（先右后上、先上后右……只是同一矩形对角的不同走法）。A$^*$ 会把这些对称路径上的中间节点全部入队、逐个展开，做大量无谓功。\textbf{Jump Point Search}（JPS，Harabor \& Grastien 2011\cite{harabor2011jps}）通过\emph{在线图剪枝}消除这种对称性：它在每个方向上“跳过”那些可被对称地推迟展开的节点，只把少数\textbf{跳点}（jump point）放入 open 表，中间节点永不展开，却\emph{保证返回与 A$^*$ 同样的最优路径}。

\paragraph{两条剪枝规则。}
JPS 对一个从父节点 $\mathbf{p}$ 沿方向 $\vec{d}$ 到达的节点 $\mathbf{x}$ 定义\emph{自然邻居}（natural neighbors，沿 $\vec{d}$ 继续走可达、且不绕路的邻居）与\emph{被迫邻居}（forced neighbors）：
\begin{itemize}
  \item \textbf{直线移动}：$\mathbf{x}$ 的邻居 $\mathbf{n}$ 是被迫邻居，当且仅当 $\mathbf{n}$ \emph{不能}在不经过 $\mathbf{x}$ 的前提下被某条不劣于经 $\mathbf{x}$ 的路径到达——直观上即 $\mathbf{x}$ 旁有障碍“挡住”了本可绕开 $\mathbf{x}$ 的捷径。
  \item \textbf{对角移动}：先递归检查两个分量直线方向，再处理对角自身的被迫邻居。
\end{itemize}
\textbf{跳点}是满足下述任一条件的节点：(1) 即目标 $\mathbf{x}_G$；(2) 至少有一个被迫邻居；(3) 沿对角方向移动时，其某个直线分量方向上能（递归地）找到跳点。\texttt{Jump} 过程沿固定方向连续推进，遇到跳点才返回、入 open 表；其余格子被“跳过”。

\begin{insight}
JPS 的妙处在于：它\emph{不改变} A$^*$ 的最优性与可采纳性框架——跳点之间的直线段代价由 Chebyshev/octile 距离精确给出，跳点构成的图与原栅格图\emph{保持同一最优解集}。它只是把“展开哪些节点”这一\emph{对称冗余}在线消除，故在开阔栅格上常把 A$^*$ 加速一个数量级以上（Harabor--Grastien 报告）。代价：仅适用于\emph{均匀代价、无障碍代价梯度}的栅格（带权地形、非栅格图不适用）。其后继 JPS+、有界 JPS 进一步预处理跳点以提速。
\end{insight}
...
\begin{codebox}[text]{PRM 预处理（Karaman-Frazzoli Alg.1 / LaValle BUILD\_ROADMAP）}
PRM (preprocessing)
 1  V <- {};  E <- {};
 2  for i = 0, ..., n do
 3      x_rand <- SampleFree_i;
...
% ════════════════════════════════════════════════════════════════
\section{渐近最优采样规划：RRT\texorpdfstring{$^*$}{^*}、RRG、PRM\texorpdfstring{$^*$}{^*}}
...
\subsection{RRT\texorpdfstring{$^*$}{^*}：选最优父 + 重接线}
...
\subsection{加速收敛：Informed-RRT\texorpdfstring{$^*$}{^*}、FMT\texorpdfstring{$^*$}{^*}、BIT\texorpdfstring{$^*$}{^*}}
\label{sec:plan-informed}

RRT$^*$ 渐近最优，但\emph{收敛慢}：一旦找到首解代价 $c_{best}$，绝大多数后续采样落在“不可能改善当前解”的区域而被浪费。三类现代算法据此大幅加速，仍保持渐近最优。

\paragraph{Informed-RRT$^*$：只在能改善解的椭球内采样。}
Gammell 等 2014\cite{gammell2014informed} 的关键观察：在最小化路径长度时，能\emph{改善}当前解 $c_{best}$ 的构型，必落在以起点 $\mathbf{x}_I$、目标 $\mathbf{x}_G$ 为\textbf{两焦点}的 $n$ 维椭球（\emph{长椭球}，prolate hyperspheroid）内——因为经过该椭球外一点的任何路径，其 $\|\mathbf{x}-\mathbf{x}_I\|+\|\mathbf{x}-\mathbf{x}_G\|>c_{best}$，不可能更短。该椭球的横轴（transverse，最长轴）长 $c_{best}$，其余 $n-1$ 条共轭轴长
\begin{equation}
  c_{\min}=\|\mathbf{x}_G-\mathbf{x}_I\|,\qquad
  \text{横轴}=c_{best},\quad\text{共轭轴}=\sqrt{c_{best}^2-c_{\min}^2}.
  \label{eq:plan-informed-axes}
\end{equation}
直接\emph{在此椭球内均匀采样}（而非整个 $\mathcal{C}$）：取单位球内均匀点 $\mathbf{x}_{ball}$，经 $\mathbf{x}_{rand}=\mathbf{C}\mathbf{L}\mathbf{x}_{ball}+\mathbf{x}_{center}$ 映入椭球，其中 $\mathbf{x}_{center}=(\mathbf{x}_I+\mathbf{x}_G)/2$、$\mathbf{L}=\mathrm{diag}(c_{best}/2,\sqrt{c_{best}^2-c_{\min}^2}/2,\dots)$、旋转 $\mathbf{C}$ 由 $(\mathbf{x}_G-\mathbf{x}_I)/c_{\min}$ 经一次 SVD 对齐世界系得到。其余结构与 RRT$^*$ 完全相同。

\begin{insight}
Informed 采样\textbf{不改变渐近最优性}（椭球随 $c_{best}$ 下降而收缩，但始终覆盖最优路径所在的同伦类），却使“改善解的概率”不再随空间增大或维数升高而趋零——这是 RRT$^*$ 在大场景/高维下收敛极慢的根因。一旦尚\emph{未}找到首解（$c_{best}=\infty$），椭球退化为整个空间，Informed-RRT$^*$ 与 RRT$^*$ 一致；找到首解后立即聚焦。直觉上它把“盲目铺满全空间”换成“只精修起点—目标之间那条细长走廊”。
\end{insight}

\paragraph{FMT$^*$：动态规划式的单批前向推进。}
\textbf{FMT$^*$}（Fast Marching Tree，Janson 等 2015\cite{janson2015fmt}）先一次性采 $n$ 个样本，再用一种\emph{懒惰}（lazy）的动态规划在样本构成的隐式 $r$-disc 图上\emph{单调前向推进}建树：始终从代价最小的前沿节点扩展，且\emph{推迟碰撞检测}到真正需要连边时——它在“样本图连通假设”下少做碰撞检测、不做重接线，故比 RRT$^*$/PRM$^*$ 更快收敛到与之相同的渐近最优解。其前向推进的“波前”思想源自求解 Eikonal 方程的 Fast Marching Method。

\paragraph{BIT$^*$：把图搜索与采样统一。}
\textbf{BIT$^*$}（Batch Informed Trees，Gammell 等 2020\cite{gammell2020bit}）是当前最有影响力的最优规划器之一，它\emph{统一}了图搜索（A$^*$ 的有序性）与采样式（anytime 可扩展性）：把一批样本视为一张\textbf{隐式随机几何图}（RGG），用启发式（lazy）的 \emph{增量 A$^*$/LPA$^*$} 在该图上按 $g+h$ 有序搜索；解到手或样本用尽后，再\emph{加一批新样本}（在 Informed 椭球内采）加密 RGG，并\emph{复用}上一批的搜索努力。于是 BIT$^*$ 同时拥有：A$^*$ 的“按代价有序、扩展最少节点”、Informed 采样的“聚焦椭球”、与采样式的“anytime + 渐近最优”。实验中其收敛速度与最终解质量普遍优于 RRT$^*$ 与 FMT$^*$（Gammell 等 2020）。其谱系可总结为
\[
  \text{RRT}^*\ \xrightarrow{\text{聚焦采样}}\ \text{Informed-RRT}^*\ \xrightarrow{\text{批 + 有序图搜索}}\ \text{BIT}^*,
\]
而 FMT$^*$ 是“单批 + 动态规划前向”的并列分支。

\begin{pitfall}[选型陷阱：以为“渐近最优”就等于“实际跑得快收敛”]
\cref{thm:plan-ao-pos} 只保证 $n\to\infty$ 时解趋最优，\emph{不}保证有限时间内收敛快。朴素 RRT$^*$ 在大地图/高维下可能跑很久解仍明显次优——因为采样仍铺满全空间。\textbf{对策}：若已能拿到首解，用 \emph{Informed} 采样（\cref{eq:plan-informed-axes}）把后续样本限在椭球内；多查询且要极致收敛用 BIT$^*$。误把“渐近最优”当“快速最优”是工程选型的常见误区。
\end{pitfall}

% ════════════════════════════════════════════════════════════════
\section{运动学约束与曲线规划：状态格、Dubins/Reeds--Shepp}
\label{sec:plan-kinodynamic}

\paragraph{动机：前面的“直线段”对真实车/机器人不可行。}
至此所有方法的 \texttt{Steer}/边都默认两构型间可走\emph{任意方向的直线段}。但汽车、差速机器人、固定翼无人机受\textbf{非完整约束}（nonholonomic constraint）：汽车不能横向平移、有最小转弯半径，固定翼不能悬停。此时 $\mathcal{C}_{free}$ 中两点的“可行连接”不再是直线，规划须把\emph{运动学/动力学约束}纳入边的定义。本节给出两条主线：用\emph{解析最短曲线}做 \texttt{Steer}（Dubins/Reeds--Shepp），与用\emph{运动基元}离散控制（状态格、kinodynamic 采样）。

\subsection{Dubins 与 Reeds--Shepp 最短曲线}
\label{sec:plan-dubins}

\paragraph{Dubins 车模型。}
Dubins 车（Dubins 1957\cite{dubins1957curves}）以恒定前进速度行驶、转向受最小转弯半径 $\rho$ 限制：
\begin{equation}
  \dot x=v\cos\theta,\quad\dot y=v\sin\theta,\quad\dot\theta=\frac{v}{\rho}\,u,\quad u\in[-1,1],
  \label{eq:plan-dubins-model}
\end{equation}
状态 $(x,y,\theta)\in\SE(2)$、控制 $u$ 为归一化转向。求两个\emph{带朝向}构型间的最短可行路径。

\begin{theorem}[Dubins 最短路的六种词型]\label{thm:plan-dubins}
半径 $\rho$ 约束下，任意两构型间的最短路必是\emph{最大曲率圆弧}（左转 L、右转 R，半径 $\rho$）与\emph{直线段}（S）的拼接，且必属于以下\textbf{六种词型}之一：
\begin{equation}
  \underbrace{\{\,\mathrm{RSR},\ \mathrm{RSL},\ \mathrm{LSR},\ \mathrm{LSL}\,\}}_{\text{CSC 型}}\ \cup\ \underbrace{\{\,\mathrm{RLR},\ \mathrm{LRL}\,\}}_{\text{CCC 型}}.
  \label{eq:plan-dubins-words}
\end{equation}
（C = 圆弧 circle，S = 直线 straight。）
\end{theorem}
\noindent Dubins 1957 用几何论证、Johnson 1974 用 Pontryagin 极大值原理证明该结论：因控制 $u$ 线性进入哈密顿量，最优控制是\emph{bang-bang}（$u=\pm1$ 全力转或 $u=0$ 直行），故路径只由最大曲率弧与直线拼成。求最短路只需对六种词型各算一次闭式长度、取最小——$O(1)$ 解析，无需搜索。这使 Dubins 曲线成为固定翼/前进式车辆采样规划里理想的 \texttt{Steer}：把 RRT 的“直线 \texttt{Steer}”换成“Dubins \texttt{Steer}”、距离度量换成 Dubins 路径长，即得尊重转弯半径的可行树。

\begin{figure}[ht]
\centering
\small
\begin{tikzpicture}[>=Stealth, scale=0.95]
  % LSL example: left arc, straight, left arc
  \coordinate (s) at (0,0);
  \coordinate (g) at (5.4,1.1);
  % start config arrow
  \draw[->, main!70!black, thick] (s) -- ++(0.7,0) node[right, font=\scriptsize]{};
  \fill[main!70!black] (s) circle (1.4pt) node[below left, font=\scriptsize]{$\mathbf{q}_I$};
  % left turning circle at start (center above)
  \draw[second!50, dashed] (0,0.9) circle (0.9);
  % arc 1 (L): from start curving up-left then to tangent
  \draw[main!70!black, very thick] (0,0) arc (-90:-20:0.9);
  \coordinate (t1) at ($(0,0.9)+(-20:0.9)$);
  % straight segment
  \coordinate (t2) at (4.5,1.7);
  \draw[main!70!black, very thick] (t1) -- (t2);
  \node[font=\scriptsize, main!60!black] at (2.6,1.5) {S};
  % arc 2 (L) into goal
  \draw[third!60!black, dashed] (5.4,2.0) circle (0.9);
  \draw[main!70!black, very thick] (t2) arc (200:270:0.9);
  \draw[->, main!70!black, thick] (g) -- ++(0.6,-0.25) node[right, font=\scriptsize]{};
  \fill[main!70!black] (g) circle (1.4pt) node[below right, font=\scriptsize]{$\mathbf{q}_G$};
  \node[font=\scriptsize, second!50!black] at (-0.9,0.9) {L};
  \node[font=\scriptsize, third!50!black] at (6.3,2.0) {L};
  \node[font=\scriptsize, gray] at (2.7,-0.4) {Dubins LSL：左弧 $\to$ 直线 $\to$ 左弧（半径 $\rho$）};
\end{tikzpicture}
\caption{Dubins 最短曲线（LSL 词型示意）：两段半径 $\rho$ 的圆弧由一条公切线段相连。六种词型 \cref{eq:plan-dubins-words} 各对应一种“起/终圆 + 公切线（CSC）或中间桥接圆（CCC）”的几何，取长度最小者即最短路。Reeds--Shepp 在此基础上允许倒车（弧/线可负向），词型扩为 48 种、含尖点（cusp）。}
\label{fig:plan-dubins}
\end{figure}

\paragraph{Reeds--Shepp 曲线：允许倒车。}
Dubins 车只能前进。\textbf{Reeds--Shepp 车}（Reeds \& Shepp 1990\cite{reeds1990optimal}）允许\emph{前进与倒车}（速度可正可负），更贴合能倒车的汽车。代价是词型大增：最短路在 $\{\mathrm{L},\mathrm{R},\mathrm{S}\}$ 段 + 正/负向组合下共 \textbf{48 种}候选词型（去重后实际需枚举的更少），含\textbf{尖点}（cusp，方向反转处速度过零）。同样可对全部词型解析求长、取最小，仍是 $O(1)$。Reeds--Shepp 距离是带倒车泊车、Hybrid-A$^*$ 等自动驾驶规划器的标准度量。

\begin{pitfall}[概念误区：用欧氏直线段给非完整系统做 Steer]
在汽车/固定翼上，若沿用普通 RRT 的\emph{欧氏直线} \texttt{Steer} 与 $\|\cdot\|$ 距离，生成的边\emph{物理不可行}（要求车横移或瞬时改向）。\textbf{后果}：规划出的“路径”车根本走不了，或跟踪时严重偏离。\textbf{正确做法}：把 \texttt{Steer} 换成 Dubins（只前进）或 Reeds--Shepp（可倒车）曲线，\emph{距离度量}也相应换成该曲线长度（注意 Dubins 距离\emph{不对称}：$d(\mathbf{a},\mathbf{b})\ne d(\mathbf{b},\mathbf{a})$，因朝向有向）。最近邻查询须用这个非对称度量，不能用欧氏。
\end{pitfall}

\subsection{状态格与 kinodynamic 采样规划}
\label{sec:plan-lattice}

\paragraph{状态格（state lattice）：把控制离散成运动基元。}
另一条路线是\textbf{状态格}（state lattice，Pivtoraiko \& Kelly 2005\cite{pivtoraiko2009lattice}）：预先设计一组满足车辆运动学的\emph{运动基元}（motion primitives，如“直行一格”“左转 30 度弧”），每个基元从一个格点出发、\emph{恰好}落到另一个格点（朝向也对齐）。这些基元把连续控制空间离散成一张\emph{尊重运动学}的有向图——于是又可在其上跑 A$^*$/D$^*$ Lite（\cref{sec:plan-astar}）！状态格把“运动学可行性”烘焙进图的边里，兼得搜索式的最优性保证与运动学可行性，广泛用于自动驾驶（Apollo、Autoware 的格规划器）与火星车（NASA）。

\paragraph{kinodynamic 规划：把动力学纳入采样。}
更一般地，\textbf{kinodynamic 规划}（LaValle--Kuffner 2001\cite{lavalle2001kinodynamic}）在\emph{状态空间} $\mathbf{x}=(\mathbf{q},\dot{\mathbf{q}})$（含速度）而非纯构型上规划，边须满足微分约束 $\dot{\mathbf{x}}=f(\mathbf{x},\mathbf{u})$。采样式做法（kinodynamic-RRT）把 \texttt{Steer} 换成\emph{前向积分}：从 $\mathbf{x}_{near}$ 采一个控制 $\mathbf{u}$、积分 $\dot{\mathbf{x}}=f(\mathbf{x},\mathbf{u})$ 一小段时间得 $\mathbf{x}_{new}$（保证动力学可行），其余 RRT 框架不变。难点是\emph{两点边值问题}（精确连到指定 $\mathbf{x}_{rand}$ 通常无闭式解，故 kinodynamic-RRT 常只能“朝 $\mathbf{x}_{rand}$ 积分逼近”而非精确到达）。其渐近最优版本（如 SST、Li 等 2016）需对“最优控制连接”做近似。

\begin{insight}
三条运动学路线的分工：\textbf{Dubins/Reeds--Shepp} 给\emph{解析}的两点最短曲线（适合做 \texttt{Steer} 与距离，但仅限特定车模型）；\textbf{状态格}把运动学\emph{离散}进图、复用 A$^*$ 的最优性（适合结构化道路）；\textbf{kinodynamic 采样}最通用（任意 $\dot{\mathbf{x}}=f(\mathbf{x},\mathbf{u})$），代价是丧失解析连接、最优性更难保证。它们都在回答同一问题：当“两点直线”不再可行时，如何重新定义图/树的\emph{边}。这与 \cref{sec:plan-cspace} 末“移植到流形”的接口是同一精神——把欧氏假设换成问题真正的可行连接。
\end{insight}

\begin{practice}
\begin{enumerate}
  \item 论证 Dubins 距离\emph{不对称}：举一对构型 $\mathbf{a},\mathbf{b}\in\SE(2)$ 使 $d(\mathbf{a},\mathbf{b})\ne d(\mathbf{b},\mathbf{a})$（提示：朝向相反时起/终圆切换）。这对 RRT 的最近邻查询有何影响？
  \item 用 \cref{eq:plan-informed-axes} 推导：当 $c_{best}\to c_{\min}$（解趋近直线距离）时 Informed 椭球退化成什么？当 $c_{best}\to\infty$（尚无解）呢？
  \item （开放思考）为什么状态格能直接跑 A$^*$ 而 kinodynamic-RRT 一般不能直接跑 A$^*$？（提示：A$^*$ 需要离散、对齐的图顶点；前向积分一般不落在固定格点上。）
\end{enumerate}
\end{practice}
...
\subsection{minimum-snap：多项式轨迹与安全飞行走廊}
\label{sec:plan-minsnap}

\paragraph{动机：四旋翼要的不是“光滑曲线”，而是“可执行的高阶光滑”。}
CHOMP/TrajOpt 在一般 $\mathcal{C}$ 上做局部优化。但对四旋翼这类\emph{微分平坦}（differentially flat）系统，存在一条更直接、更优雅的路线。Mellinger \& Kumar 2011\cite{mellinger2011minimum} 指出：四旋翼的全部状态与输入都可写成 4 个\textbf{平坦输出} $\sigma=(x,y,z,\psi)$（三维位置 + 偏航）及其有限阶导数的函数。其姿态由\emph{加速度}决定、角速度由\emph{加加速度}（jerk）决定、而控制输入（电机推力变化率）正比于\emph{四阶导}——\textbf{snap}。因此“控制输入平滑、不饱和”等价于让位置的\emph{四阶导（snap）尽量小}。

\begin{definition}[minimum-snap 轨迹优化]\label{def:plan-minsnap}
把轨迹按时间分 $M$ 段，每段 $i$ 的每个平坦输出分量取 $N$ 次多项式
\begin{equation}
  P_i(t)=\sum_{j=0}^{N}p_{i,j}\,(t-t_{i-1})^{j},\qquad t\in[t_{i-1},t_i].
  \label{eq:plan-poly-seg}
\end{equation}
最小化全程\emph{第 $r$ 阶导平方积分}（$r=4$ 即 snap，$r=3$ 即 jerk）：
\begin{equation}
  \min_{\mathbf{p}}\ J=\sum_{i=1}^{M}\int_{t_{i-1}}^{t_i}\Big(\frac{d^{r}P_i(t)}{dt^{r}}\Big)^2dt
  \;=\;\mathbf{p}^\top\mathbf{Q}\,\mathbf{p},
  \label{eq:plan-minsnap-cost}
\end{equation}
其中 $\mathbf{p}$ 堆叠全部多项式系数，$\mathbf{Q}=\mathrm{blkdiag}(\mathbf{Q}_1,\dots,\mathbf{Q}_M)$ 是\emph{对称半正定}的块对角阵，$\mathbf{Q}_i$ 仅由段时长 $t_i-t_{i-1}$ 决定（对多项式逐项积分得 $(\mathbf{Q}_i)_{kl}\propto$ 已知系数 $\times(t_i-t_{i-1})^{k+l-2r+1}$）。约束为线性：端点位置/导数固定、中间 waypoint 经过、相邻段在 waypoint 处导数\emph{连续}（直到 $r-1$ 阶）：
\begin{equation}
  \mathbf{A}\mathbf{p}=\mathbf{b}\quad(\text{端点 + waypoint + 连续性}).
  \label{eq:plan-minsnap-con}
\end{equation}
\cref{eq:plan-minsnap-cost}--\cref{eq:plan-minsnap-con} 构成一个\textbf{等式约束二次规划（QP）}：$\min_{\mathbf{p}}\mathbf{p}^\top\mathbf{Q}\mathbf{p}$ s.t. $\mathbf{A}\mathbf{p}=\mathbf{b}$。
\end{definition}

\begin{derivation}[minimum-snap QP 的解与数值稳健形式]\label{der:plan-minsnap}
等式约束 QP 用 Lagrange 乘子 $\boldsymbol\lambda$：$\mathcal{L}=\mathbf{p}^\top\mathbf{Q}\mathbf{p}+\boldsymbol\lambda^\top(\mathbf{A}\mathbf{p}-\mathbf{b})$，KKT 条件
\begin{equation}
  \begin{bmatrix}2\mathbf{Q}&\mathbf{A}^\top\\\mathbf{A}&\mathbf{0}\end{bmatrix}
  \begin{bmatrix}\mathbf{p}\\\boldsymbol\lambda\end{bmatrix}
  =\begin{bmatrix}\mathbf{0}\\\mathbf{b}\end{bmatrix}
  \label{eq:plan-minsnap-kkt}
\end{equation}
一次线性求解即得全局最优 $\mathbf{p}$（$\mathbf{Q}\succeq0$、约束相容时凸）。但高次幂的 $\mathbf{Q}$ 条件数极大、数值病态。Richter--Bry--Roy 2013\cite{richter2016polynomial} 给出稳健重构：把决策变量从\emph{系数} $\mathbf{p}$ 换成各 waypoint 的\emph{导数值} $\mathbf{d}$（端点导数已知、中间导数自由），经 $\mathbf{p}=\mathbf{M}^{-1}\mathbf{d}$ 的映射把问题化为\emph{无约束} QP $\min_{\mathbf{d}_{free}}\mathbf{d}^\top\mathbf{R}\mathbf{d}$（$\mathbf{R}=\mathbf{M}^{-\top}\mathbf{Q}\mathbf{M}^{-1}$），对自由导数闭式求解，远比 \cref{eq:plan-minsnap-kkt} 稳定，且天然支持单段无约束闭式。
\end{derivation}

\paragraph{安全飞行走廊（safe flight corridor）。}
minimum-snap 本身不含避障——它只保证光滑。工程上把避障与平滑\emph{解耦}：先用搜索/采样（A$^*$/RRT$^*$/JPS）得粗略无碰路径，再沿路径\emph{膨胀}出一串相互重叠的\textbf{凸自由区}（多面体或球，称\emph{安全飞行走廊}，Liu 等 2017\cite{liu2017corridor}），最后把“每段轨迹必须落在对应凸区内”作为\emph{凸（线性/二阶锥）约束}加进 \cref{def:plan-minsnap} 的 QP。因走廊凸、多项式系数线性，整体仍是凸 QP/SOCP，可实时求解。这把“全局找路（非凸、离散）”与“局部光滑（凸、连续）”\emph{干净分离}，是当今四旋翼敏捷飞行（Fast-Planner、EGO-Planner 等）的主流架构。

\begin{insight}
\textbf{minimum-snap 与 CHOMP 的分工}：CHOMP 在代价场上做\emph{无约束、可处理任意障碍场}的梯度下降，但需迭代、可能陷局部极小；minimum-snap 把问题压成\emph{一个凸 QP}、一次解出全局最优，但避障靠外部走廊喂凸约束、且专用于平坦系统（四旋翼/全向车）。前者通用而启发式，后者高效而结构化。\textbf{为何是 snap 而非速度/加速度？}——因为四旋翼控制输入恰好出现在位置的四阶导（微分平坦），最小化 snap 直接对应“最省力、不饱和电机”的物理量；地面车/机械臂常用 \emph{minimum-jerk}（$r=3$，对应平滑力矩、乘员舒适）。$r$ 的选择由“哪一阶导对应实际控制输入/约束”决定。
\end{insight}

\paragraph{代表方法与级联流水线。}\rebuilt
\textbf{CHOMP} 用协变梯度下降，障碍项基于工作空间距离场；无完备性（可能以仍有碰撞的轨迹终止），故常配 Hamiltonian Monte Carlo 缓解局部极小并获概率完备。\textbf{STOMP}（Kalakrishnan 等 2011\cite{kalakrishnan2011stomp}）\emph{无梯度}，采样一批噪声轨迹按代价加权更新，适合不可微代价/硬约束。\textbf{TrajOpt}（Schulman 等 2013\cite{schulman2014trajopt}）把轨迹优化写成序列凸规划（SQP），碰撞约束用连续碰撞凸近似，处理硬约束更稳健。\textbf{minimum-snap}（\cref{sec:plan-minsnap}）把平坦系统的轨迹压成凸 QP、配安全走廊避障。\textbf{KOMO / 因子图轨迹优化}把轨迹优化表为非线性最小二乘 $\min\sum_k\|f_k(\xi)\|^2$，用 GN/LM 求解，与本书 SLAM 后端天然衔接。实际系统常\textbf{级联}：A$^*$/RRT$^*$/JPS 给全局初值 $\to$ minimum-snap/CHOMP/TrajOpt 局部平滑成可执行轨迹。
...
“渐近最优”就是“实际收敛快” & 仅 $n\to\infty$ 保证；有限时间须 Informed/BIT$^*$ 聚焦（\cref{sec:plan-informed}）\\
汽车/固定翼可用欧氏直线 Steer & 非完整约束须用 Dubins/Reeds--Shepp 曲线 + 非对称距离（\cref{sec:plan-dubins}）\\
最小化速度/加速度即“最平滑” & 平坦系统看哪阶导对应控制输入：四旋翼用 snap（\cref{sec:plan-minsnap}）\\
minimum-snap 自带避障 & 仅保证光滑；避障靠外部安全走廊喂凸约束（\cref{sec:plan-minsnap}）\\
...
$\varepsilon$ & 加权 A$^*$ 启发放大系数（$\ge1$） & \cref{eq:plan-weighted-astar} \\
$c_{best},c_{\min}$ & Informed 当前解代价 / 起终直线距离 & \cref{eq:plan-informed-axes} \\
$\rho$ & 最小转弯半径（Dubins/Reeds--Shepp） & \cref{eq:plan-dubins-model} \\
$\mathbf{p},\mathbf{Q}$ & 多项式系数 / minimum-snap 代价阵 & \cref{eq:plan-minsnap-cost} \\
\bottomrule
\end{tabular}
\end{table}
...
加权 A$^*$ $\varepsilon$-次优 & $f=g+\varepsilon h$，解 $\le\varepsilon C^*$ & \cref{thm:plan-weighted} \\
...
Informed 椭球 & 改善解的样本落在起终为焦点的椭球 & \cref{eq:plan-informed-axes} \\
Dubins 六词型 & CSC + CCC，bang-bang 解析最短路 & \cref{thm:plan-dubins} \\
minimum-snap QP & $\min\mathbf{p}^\top\mathbf{Q}\mathbf{p}$ s.t. $\mathbf{A}\mathbf{p}=\mathbf{b}$ & \cref{def:plan-minsnap} \\
...
\# & 知识点 & 核心要点 & 难度 \\
\midrule
1 & 构型空间 & 刚体避障 $\to$ 点找路；$\SO(3)\cong\mathbb{RP}^3$ & \(\bigstar\bigstar\) \\
2 & 钢琴搬运工问题 & PSPACE-hard，显式 $\mathcal{C}_{obs}$ 不可行 & \(\bigstar\bigstar\) \\
3 & 图搜索框架 & 仅 $Q$ 排序不同；BFS/DFS/Dijkstra/A$^*$ & \(\bigstar\bigstar\) \\
4 & 动态规划 & Bellman 递推、导航函数 & \(\bigstar\bigstar\bigstar\) \\
5 & A$^*$ 与启发 & 可采纳 $\Rightarrow$ 最优、一致 $\Rightarrow$ 无重复展开 & \(\bigstar\bigstar\bigstar\) \\
6 & 栅格启发与加速 & 曼哈顿/对角/欧氏、平方陷阱、加权 A$^*$、JPS & \(\bigstar\bigstar\) \\
7 & RRT / RRT-Connect & Voronoi 偏置、双树贪心连接 & \(\bigstar\bigstar\bigstar\) \\
8 & PRM & 多查询路标、$r$-disc 图、union-find & \(\bigstar\bigstar\bigstar\) \\
9 & RRT$^*$/RRG/PRM$^*$ & 选最优父 + 重接线、临界半径 & \(\bigstar\bigstar\bigstar\bigstar\) \\
10 & 渐近最优理论 & 概率完备/渐近最优、RRT 非最优证明 & \(\bigstar\bigstar\bigstar\bigstar\bigstar\) \\
11 & 加速最优规划 & Informed 椭球、FMT$^*$、BIT$^*$ & \(\bigstar\bigstar\bigstar\bigstar\) \\
12 & 运动学约束规划 & Dubins/Reeds--Shepp、状态格、kinodynamic & \(\bigstar\bigstar\bigstar\) \\
13 & 轨迹优化 & CHOMP 协变梯度、minimum-snap QP、安全走廊、级联 & \(\bigstar\bigstar\bigstar\bigstar\) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{动态重规划与栅格加速}：Stentz《D$^*$》\cite{stentz1994optimal}（ICRA 1994）——部分已知/动态环境的增量最优搜索，现代用其后继 D$^*$ Lite；Harabor \& Grastien《JPS》\cite{harabor2011jps}（AAAI 2011）——均匀栅格的对称性消除；Likhachev 等《ARA$^*$》\cite{likhachev2004ara}（NeurIPS 2003）——anytime 有界次优搜索。
  ...
  \item \textbf{加速最优采样}：Gammell 等《Informed RRT$^*$》\cite{gammell2014informed}（IROS 2014）——长椭球聚焦采样；《BIT$^*$》\cite{gammell2020bit}（IJRR 2020）——图搜索 $\times$ 采样的统一；Janson 等《FMT$^*$》\cite{janson2015fmt}（IJRR 2015）——单批动态规划前向。
  \item \textbf{非完整最短曲线}：Dubins《On Curves of Minimal Length\dots》\cite{dubins1957curves}（1957）——前进式车的六词型；Reeds \& Shepp\cite{reeds1990optimal}（1990）——允许倒车的扩展；状态格见 Pivtoraiko 等\cite{pivtoraiko2009lattice}、kinodynamic 见 LaValle \& Kuffner\cite{lavalle2001kinodynamic}。
  \item \textbf{轨迹优化}：Zucker 等《CHOMP》\cite{zucker2013chomp}（IJRR 2013）——协变梯度；STOMP\cite{kalakrishnan2011stomp}、TrajOpt\cite{schulman2014trajopt} 为同类代表；Mellinger \& Kumar《minimum-snap》\cite{mellinger2011minimum}（ICRA 2011）——四旋翼微分平坦 + QP，配 Richter 等\cite{richter2016polynomial} 的稳健无约束重构与 Liu 等\cite{liu2017corridor} 的安全飞行走廊。
\end{itemize}
...
后续章节 & 关系 & 本章铺垫 \\
\midrule
\cref{ch:control} 控制导论 & 轨迹跟踪 / 微分平坦 / 动力学可行轨迹的执行 & minimum-snap、kinodynamic、级联流水线 \\
\cref{ch:lie} 李群（前置） & $\SE(3)$ 上规划的距离 / 插值 / 梯度 & C-space 流形、右扰动雅可比 \\
\cref{ch:nlopt} 非线性优化（前置） & 轨迹优化 = 最小二乘/QP + GN/LM & CHOMP 的 $\mathbf{A}^{-1}\nabla$、minimum-snap QP、因子图轨迹优化 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{症状}：RRT$^*$ 在大地图/高维下跑很久解仍明显次优。\textbf{原因}：采样仍铺满全空间，落在“能改善解”椭球外的样本被浪费（\cref{sec:plan-informed}）。\textbf{对策}：拿到首解后改用 Informed-RRT$^*$ 把样本限在椭球内（\cref{eq:plan-informed-axes}）；要极致收敛用 BIT$^*$。
  \item \textbf{症状}：规划出的路径汽车/固定翼根本走不了（要求横移或瞬时改向）。\textbf{原因}：对非完整系统沿用了欧氏直线 \texttt{Steer} 与 $\|\cdot\|$ 距离。\textbf{对策}：\texttt{Steer} 换 Dubins（只前进）/Reeds--Shepp（可倒车）曲线，距离换该曲线长度（注意非对称，\cref{sec:plan-dubins}）；结构化道路可改用状态格 + A$^*$。
  \item \textbf{症状}：JPS 给出的路径比 A$^*$ 长或行为异常。\textbf{原因}：把 JPS 用在\emph{带权地形/有障碍代价梯度}的栅格上——JPS 只对均匀代价栅格保最优（\cref{sec:plan-jps}）。\textbf{对策}：非均匀代价改回标准 A$^*$/加权 A$^*$。
  \item \textbf{症状}：minimum-snap 解出的多项式系数数值爆炸/轨迹乱跳。\textbf{原因}：高次幂 $\mathbf{Q}$ 病态（\cref{eq:plan-minsnap-kkt} 直接求解）。\textbf{对策}：改用 Richter 等的“以 waypoint 导数为变量”的无约束重构（\cref{der:plan-minsnap}）；段时长分配要合理（按距离/速度归一），避免极短段。
  \item \textbf{症状}：minimum-snap 轨迹穿过障碍。\textbf{原因}：minimum-snap 只保证光滑、不含避障。\textbf{对策}：先用 A$^*$/JPS/RRT$^*$ 找无碰路径，再沿其生成安全飞行走廊、把“落在凸走廊内”作为凸约束加进 QP（\cref{sec:plan-minsnap}）。
