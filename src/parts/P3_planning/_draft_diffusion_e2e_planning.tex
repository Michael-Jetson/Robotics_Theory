...
\begin{note}
\textbf{PLUTO 复现命脉（部分已核实）.}\;\rebuilt{标 PARTIAL 者待逐行核对：}负样本如何生成（六个增强函数、正负两类，PARTIAL）——负样本质量直接决定 CIL 有没有用；数据增强扰动幅度分布（PARTIAL）——太小没效果，太大引入不现实状态；对比 loss 权重 $\lambda$ 与温度 $\tau$（码中具体值需核实）；\textbf{CIL 启用开关}（\emph{已核实}）——\texttt{model.\allowbreak use\_hidden\_proj=true} $+$ \texttt{custom\_trainer.\allowbreak use\_contrast\_loss=true}，复现 CIL 必须打开这两个开关否则退化成普通模仿；nuPlan 闭环的 reactive/non-reactive 设置（评估脚本）——背景车是否对你反应，闭环难度差异大。
\end{note}
...
碰撞后优化是可有可无的后处理 & 它是安全指标达标的关键，\texttt{N\_ITER}/\allowbreak\texttt{LR}/代价形式 UNSPECIFIED 须从码提取（\cref{sec:de-collision}）\\
...
\begin{enumerate}
  ...
  \item \textbf{症状}：UniAD 碰撞率\emph{显著高于论文}、轨迹在障碍附近抖动。\textbf{原因}：碰撞后优化被跳过或参数乱设（\texttt{N\_ITER}/\allowbreak\texttt{LR} UNSPECIFIED，\cref{sec:de-collision}）。\textbf{对策}：从代码提取超参，把碰撞优化当一等公民复现并做敏感性实验。
