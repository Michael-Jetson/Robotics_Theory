...
\begin{table}[H]\centering\small
\caption{多机运动协调 MARL 化的代表工作（年份/会议以原文为准）}
...
% ════════════════════════════════════════════════════════════════
\section{从零搭建：MQE 风格环境 \texorpdfstring{$+$}{+} MAPPO 参数共享训练循环}
\label{sec:mc-train}
...
\subsection{关键设计二：为什么训练时要随机化智能体数量 \texorpdfstring{$N$}{N}}
...
\subsection{理论-工程桥接：可迁移观测表征——本体感知 \texorpdfstring{$+$}{+} 相对观测}
...
\begin{center}
\small\textbf{问题分类决策树}\\[3pt]
\begin{forest} dtree
[{任务是否安全关键？\\（载人/贵重/密集人群）}
  [{是 → 必须有硬安全保证\\纯 MARL 不可单独用\\→ 传统规控，或 MARL+安全层混合}]
  [{否 → 交互是否复杂到\\难以显式建模？\\（非凸接触、高维耦合、难建模动力学）}
    [{是 → 传统规控难写方程\\MARL 数据驱动优势凸显\\→ MARL（或 MARL 主导+传统兜底）}]
    [{否（交互可清晰建模）→\\环境/参数是否高度多变、\\需强适应性？}
      [{是 → MARL 的泛化优势\\→ 倾向 MARL}]
      [{否（环境稳定、模型已知）→ 倾向传统规控\\（更可控、可验证、无需训练）；\\是否需可证明最优/有界次优？}
