...
%            twist 顺序 [v;omega]（线在前、角在后，源文约定，保留并就地声明）；
%            机器人/操作领域自然记号保留。
...
% ════════════════════════════════════════════════════════════════
\section{视角重置：把双臂当成 \texorpdfstring{$N=2$}{N=2} 的多机协同操作}
...
\begin{note}
\textbf{关键前提：统一参考点.}\;
直接拿 \Verb[breaklines]|LOCAL_WORLD_ALIGNED| 雅可比做 $\mathbf{J}_R-\mathbf{J}_L$ 是\emph{错的}——该约定只统一了坐标轴方向，没有把线速度参考点移到同一点。刚体绕物体质心转动时，两末端线速度本来就不同，不统一参考点就做差，会把“正常的绕物体转动”误判成“相对运动”，从而误算出根本不存在的内力。统一参考点的 twist 平移算子（$[\mathbf{v};\boldsymbol{\omega}]$ 顺序）为
\[
...
% ════════════════════════════════════════════════════════════════
\section{闭链约束的两个面孔：\texorpdfstring{$\ker(\mathbf{J}_{\text{rel}})$}{ker(J_rel)} 与 \texorpdfstring{$\ker(\mathbf{G})$}{ker(G)} 的对偶}
...
% \end{equation}
\noindent 内力与"破坏约束的相对运动"在双臂下均为 $6$ 维、互为对偶，这为 \cref{sec:ma-internalforce} 用相对位姿偏差闭环控制内力提供依据。
...
\subsection{把 \texorpdfstring{$\mathbf{J}_{\text{rel}}$}{J_rel} 接进来：关节空间的完整图景}
...
% ════════════════════════════════════════════════════════════════
\section{内力控制：从双臂到 \texorpdfstring{$N$}{N} 臂}
...
\subsection{推广到 \texorpdfstring{$N$}{N} 臂}
\label{sec:ma-narm}
...
\subsection{用 \texorpdfstring{$\alpha$}{alpha} 统一对称与非对称}
...
% ════════════════════════════════════════════════════════════════
\section{多臂负载分配：\texorpdfstring{$N\ge 3$}{N 3} 的优化与分布式}
...
\noindent\textbf{grasp matrix 与摩擦锥的实现要点}（小段伪代码，载体无关）。$\mathbf{G}$ 的每个 $6\times6$ 块按 \cref{eq:ma-grasp} 填（力 $\to$ 力为 $\mathbf{I}_3$、力 $\to$ 力矩为 $[\mathbf{r}_i]_\times$、力矩 $\to$ 力矩为 $\mathbf{I}_3$）；摩擦锥金字塔近似把 $\|\mathbf{f}^{\text{tangent}}\|\le\mu f^{\text{normal}}$ 写成 $|f_x|\le\mu f_z$、$|f_y|\le\mu f_z$（$4$ 条线性不等式，设接触法向为 $+z$）：用 \Verb[breaklines]|build_grasp_matrix(contact_pts, com)| 组装 $\mathbf{G}$、用 \Verb[breaklines]|solve_load_distribution(...)| 解 \cref{eq:ma-qp}，关键参数为按 $1/\tau_{\max,i}$ 设的权重 \verb|w| 与最小法向力 \verb|f_min|；解后用 \Verb[breaklines]|G @ h - F_cmd| $\approx\mathbf{0}$ 验证合成正确。具体求解器调用属工程细节，从略。
