...
%  记号归一：R in SO(3)、T in SE(3)（preamble 宏 \SO \SE \so \se）；分配/搜索/图论自然记号保留。
...
\subsection{为什么“按 bundle 顺序释放”？DMG 与 \texorpdfstring{$50\%$}{50} 界}
...
\subsection{代码验证：CBBA 无冲突 \texorpdfstring{$+$}{+} \texorpdfstring{$50\%$}{50} 界 \texorpdfstring{$+$}{+} 收敛随直径}
...
\subsection{代码验证：优先级规划 \texorpdfstring{$+$}{+} 预留表（看它的快与不完备）}
...
\subsection{子模视角：\texorpdfstring{$50\%$}{50} 与 \texorpdfstring{$1-1/e$}{1-1/e} 背后的同一结构}
...
\subsection{代码验证：子模贪婪的 \texorpdfstring{$1-1/e$}{1-1/e} 界，与违反子模时的失效}
...
\subsection{代码验证一：ECBS-lite——有界次优用 \texorpdfstring{$w$}{w} 换速度}
...
\begin{table}[H]\centering\small
\begin{tabularx}{\linewidth}{p{0.20\linewidth}LL}
\toprule
后续主题 & 关系 & 本章铺垫的知识点 \\
\midrule
...
\begin{table}[H]\centering\small
\begin{tabularx}{\linewidth}{LLLp{0.08\linewidth}}
\toprule
症状 & 可能原因 & 排查步骤 & 相关节 \\
\midrule
