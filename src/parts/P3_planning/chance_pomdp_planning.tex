...
%  POMDP 与信念空间规划(belief/alpha-vector/SARSOP/DESPOT/Active SLAM)、
%  风险敏感规划(一致性风险测度/CVaR/嵌套 CVaR/分布式 RL/CVaR-CBF)。
%  跨部去重：CBF/HJ 可达/鲁棒 MPC 的最深推导留待【控制部】，此处侧重规划/决策用途。
%  记号归一化：旋转 R in SO(3)、位姿 T in SE(3)；规划/控制自然记号保留，
...
[{不确定性下的\\规划与决策}
  [{机会约束\\$\Pr(g\le0)\ge1-\delta$}
    [{ICC/JCC, IRA\\CC-MPC, CC-RRT, DRO}]]
  [{POMDP\\belief 空间决策}
    [{$\alpha$-vector, SARSOP\\DESPOT, Active SLAM}]]
  [{风险敏感\\CVaR 尾部风险}
    [{Artzner 公理, RU 公式\\嵌套 CVaR, CVaR-CBF}]]
]
\end{forest}
\end{center}
...
IRA 迭代几轮后，$\sum\delta_{j,k}$ 悄悄超过 $\delta$（风险转移时若只加不减、或数值误差累积），JCC 保证失效，蒙特卡洛违反率超标。正确写法：每轮迭代后强制 \texttt{deltas *= delta\_total /\allowbreak  deltas.\allowbreak sum()} 重新归一化，确保 $\sum\delta_{j,k}=\delta$ 始终成立。检验：每轮打印 \texttt{deltas.\allowbreak sum()}，应恒等于 $\delta$。
\end{pitfall}
...
IRA 几轮后违反率超标 & 未重新归一化、$\sum\delta_{j,k}$ 漂移 & 每轮强制 \texttt{deltas *= delta/\allowbreak deltas.\allowbreak sum()} \\
