...
这里值得进一步解释 MBGD 为何通常比 SGD 收敛更快。关键在于三种算法所用的搜索方向对真实梯度 $\mathbb{E}[\nabla_w f(w_k, X)]$ 的逼近程度不同。BGD 用全部 $n$ 个样本求平均，其搜索方向 $\frac{1}{n}\sum_{i=1}^{n}\nabla_w f(w_k, x_i)$ 是对真实梯度最精确的估计；SGD 仅用单个样本的梯度 $\nabla_w f(w_k, x_k)$ 来近似真实梯度，由于只用了一个随机样本，这个近似带有较大的随机波动；MBGD 则用一小批样本的平均梯度 $\frac{1}{m}\sum_{j\in\mathcal{I}_k}\nabla_w f(w_k, x_j)$ 来近似真实梯度。由于这一小批样本是独立同分布的，对它们求平均会把单个样本梯度里的随机性“平均掉”：在相同条件下，$m$ 个独立样本平均后的方差约为单样本方差的 $1/m$。因此，$m$ 越大，MBGD 给出的搜索方向就越接近真实梯度，每一步迭代的方向也越可靠，算法自然收敛得更快、更平稳。这也正是\cref{fig:rl-sa-sgd-mean}中批量越大（$m = 50$）收敛越快、而 $m = 1$（此时退化为 SGD）收敛越慢的根本原因。

至于 MBGD 的收敛性，可以仿照 SGD 的情形（方框 6.1 中对\cref{thm:rl-sa-sgd}的证明）来证明。与 SGD 一样，MBGD 也可以被视为一种特殊的 RM 算法：把小批量平均梯度看作真实梯度 $\mathbb{E}[\nabla_w f(w_k, X)]$ 的一个带噪声的观测，其噪声项在条件期望意义下为零且二阶矩有界，于是 RM 定理（\cref{thm:rl-sa-rm}）的三个条件依旧成立，MBGD 的收敛性随之得证。
