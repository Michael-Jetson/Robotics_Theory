...
$1960$–$70$ 年代激光测距系统笨重昂贵，用于大气研究、地形测绘、军事\cite{carlone2026handbook}；$80$ 年代变紧凑但仍以固定式为主；$90$ 年代激光雷达与 GPS、IMU 集成，催生车载/机载移动测绘；$2000$ 年代初二维激光雷达推动了概率建图与 SLAM 的发展——其中 Lu 与 Milios 首创\emph{全局一致}的二维扫描配准，提出用一张\textbf{位姿网络}（network of poses）联合约束各帧位姿，这一思想正是现代\emph{位姿图}（pose-graph，\cref{sec:lidar-loop}）的直接前身，为贯穿全章后端的“图优化”埋下伏笔；其后一批工作把扫描配准纳入\emph{概率}框架，Olson 更以\emph{相关法}（correlation）实现实时配准\cite{carlone2026handbook}。三维激光里程计的方法多由这些二维技术\emph{扩展}而来——靠主动旋转/扫掠或被动平台运动\emph{累积}出稠密点云，再把二维配准的思路抬到三维。$2004$ 年 DARPA Grand Challenge、$2007$ 年 Urban Challenge 把三维激光雷达推上自动驾驶舞台，催生了开创性的 Velodyne HDL-64E。$2014$ 年 LOAM\cite{zhang2014loam} 首次实现\emph{实时}三维扫描配准与建图，成为此后整个激光 SLAM 体系的基础。
...
\paragraph{先问一句：实时激光里程计该提“什么”特征？四条工程判据。}
特征法的头号问题不是“怎么配准”，而是“\emph{提什么}”。点云特征本有许多种（PFH、FPFH 乃至各类深度学习描述子），但\emph{实时} SLAM 对特征另有一套\emph{工程}取舍，高翔\cite{gao2023slamad} 把它凝成四条判据：
\begin{enumerate}
  \item \textbf{反映点云的物理特点}：自动驾驶点云由多线激光雷达扫描而成，不像 RGB-D 那样稠密，而带明显的\emph{线束}（scan-line）结构——每个点都属于某一条扫描线。故不必对整片点云提特征，而可\emph{逐线束}在单条扫描线内做（这正是 \cref{eq:lidar-loam-c} 只取同线邻点的由来）。
  \item \textbf{便于配准}：提出的特征应能直接喂给前面的 ICP / NDT，而非另起一套复杂匹配。
  \item \textbf{省算力、不挑硬件}：特征提取不应吃掉过多 CPU/GPU，更不应依赖特殊硬件。
  \item \textbf{全系统同构}：最好整个里程计/SLAM 系统用\emph{同一种}计算结构，避免一部分跑 CPU、一部分跑 GPU 而徒增数据搬运与资源占用。
\end{enumerate}
正因这四条偏向“简单、可移植”，工业界的激光里程计普遍只用\emph{线束 + 曲率}这类朴素特征，而非繁复的统计/神经描述子。

\begin{insight}[一句认识论提醒：特征提取多是工程经验，缺严格理论]\label{ins:lidar-feat-criteria}
要坦白的是：\textbf{大多数激光特征提取方法并没有明确的理论依据，更多是靠工程实践摸索出来的}\cite{gao2023slamad}。曲率阈值、子区域个数、每区域取几个边/面点，这些数字都来自经验调试而非某条定理；读者大可灵活增删流程。这与本章后面 FAST-LIO 那种“逐行可证”的滤波推导形成鲜明对照——\emph{前端特征是工程艺术，后端融合是数学}。把两者的“可证性”边界看清，调不出好特征时就不会误以为是自己数学没学好。
\end{insight}
...
\subsection{超越低层几何：语义 / 面元 / 强度高层特征}
\label{sec:lidar-highfeat}

LOAM 的边/面是\textbf{低层}几何特征（只看局部曲率或主成分）。在它之上，激光里程计还可借\textbf{高层特征}进一步提精度、增鲁棒——它们比低层特征携带更丰富的环境信息，利于场景理解与更准的建图\cite{carlone2026handbook}。三类最常用：
\begin{itemize}
  \item \textbf{语义特征}：用机器学习/深度学习把点云逐点\emph{分类}为车辆、行人、建筑、植被等类别，\emph{典型用途是区分动/静}——把动态物体（行人、行驶车辆）剔出配准、只用\emph{稳定地标}（建筑、杆状物），里程计的可靠性由此显著提升（代表如语义辅助的 SuMa++\cite{chen2019suma++}）。
  \item \textbf{面元（surfel）特征}：用一个小圆盘（surfel）拟合点云的局部曲面——对邻域点的协方差矩阵做特征分解，椭球盘的主半轴长由特征值定（与 \cref{ch:pointcloud} 的法向/曲率估计同源）。配准时它天然给出\emph{点到面}距离，是稠密/户外激光 SLAM 的常用表示（如 SuMa\cite{behley2018efficient}）。
  \item \textbf{强度特征}：激光回波的\emph{反射率/强度}值，携带材质与表面信息。它给特征匹配多加一维信息，在\emph{无结构}（空旷、几何退化）环境里尤能救场——纯几何配不上时，强度纹理还能提供约束\cite{carlone2026handbook}。
\end{itemize}
这三类与本章前面只把 surfel/强度当作“点云\emph{表示}”（\cref{ch:pointcloud}）不同：此处它们是\emph{增强里程计}的手段。一句话——低层几何决定“配得\emph{上}”，高层语义/强度决定“配得\emph{对}、不被动态物体带偏”。
...
\subsection{把特征法接入该框架：LoamLikeOdom}
\label{sec:lidar-loamlike}

\paragraph{动机：同一套 scan-to-局部地图骨架，只换“源点”。}
上面的直接法 NDT 用\emph{原始点}对体素高斯。但 \cref{sec:lidar-scan-reg} 早已点明：scan-to-局部地图这套骨架（拼局部图 + 关键帧滑窗 + 恒速外推初值）与“配准残差用什么几何”是\emph{正交}的两件事。于是只要把“源”从原始点换成 \cref{sec:lidar-loam} 的\textbf{边/面特征}、把残差换成点到线 \cref{eq:lidar-loam-de} 与点到面 \cref{eq:lidar-loam-dh}，就得到一个\emph{特征法}的 scan-to-局部地图里程计——高翔称之为 \texttt{LoamLikeOdom}\cite{gao2023slamad}。它是“原始 LOAM（scan-to-scan，\cref{sec:lidar-loam}）”与“NDT 直接法（scan-to-局部地图 + 关键帧，本节）”的\textbf{合流}：既留 LOAM 的边/面残差、又套上直接法更稳的局部地图框架，是把前两节零件\emph{拼成第三个系统}的范例。

\paragraph{装配：双 KD 树 + 关键帧滑窗 + 同一外推式。}
与 LOAM 原版（\cref{alg:lidar-loam-odom}）相比的三点差异，恰好都来自“借”了直接法的框架：
\begin{enumerate}
  \item \textbf{边/面各建一棵 KD 树}：局部地图里边特征、面特征\emph{分两棵} KD 树（\texttt{kdtree\_edge\_}、\texttt{kdtree\_surf\_}）；配准时边点只在边树找对应线、面点只在面树找对应面。
  \item \textbf{两类残差合进同一次 GN}：对每个边点取其在边树的近邻拟合直线、算点到线残差（\cref{eq:lidar-loam-de}），对每个面点取近邻拟合平面、算点到面残差（\cref{eq:lidar-loam-dh}），把\emph{所有}边/面残差堆进\emph{同一个}高斯--牛顿问题求当前位姿，迭代数次（实现取 $\le 5$）。
  \item \textbf{关键帧滑窗 + 局部图体素重建}：当位姿相对上一关键帧的平移/转角超阈值（如 $1$\,m / $15^\circ$）即记关键帧，把其边/面点变到世界系压入两个\emph{定长队列}（保留最近 $30$ 个关键帧，超出即 \texttt{pop\_front}）；队列一变就把队内边/面点各自拼起、体素降采样、\emph{重建}两棵 KD 树作下一帧的配准目标。
\end{enumerate}
而\textbf{配准初值用的恰是本节那条恒速外推式}——记最近两帧估计位姿 $\mathbf{T}_1,\mathbf{T}_2$（$\mathbf{T}_1$ 最新），下一帧初值
\begin{equation}
  \check{\mathbf{T}}=\mathbf{T}_1\,(\mathbf{T}_2^{-1}\mathbf{T}_1),
  \label{eq:lidar-loamlike-extrap}
\end{equation}
与直接法 NDT 的 $\check{\mathbf{T}}_k=\mathbf{T}_{k-1}(\mathbf{T}_{k-2}^{-1}\mathbf{T}_{k-1})$（\cref{sec:lidar-direct-ndt}）\emph{同一式}：都把“上一段相对运动”当作“这一段”的猜测。可见特征法与直接法在\emph{框架层面}完全可互换，差别只在残差几何。

\begin{codebox}[C++]{特征法 scan-to-局部地图装配（仿高翔 \texttt{LoamLikeOdom}）}
class LoamLikeOdom {
  struct Options {
    int    min_edge_pts_ = 20;          // 最小边缘点数
    int    min_surf_pts_ = 20;          // 最小平面点数
    double kf_distance_ = 1.0;          // 关键帧距离阈值（米）
    double kf_angle_deg_ = 15;          // 关键帧旋转角度阈值（度）
    int    num_kfs_in_local_map_ = 30;  // 局部地图保留的关键帧数
    int    max_iteration_ = 5;          // ICP 最大迭代次数
    double max_plane_distance_ = 0.05;  // 面点最近邻距离阈值
    double max_line_distance_ = 0.5;    // 边点最近邻距离阈值
    int    min_effective_pts_ = 10;     // 有效近邻点数阈值
  };

  void ProcessPointCloud(FullCloudPtr cloud) {
    // 1) 提特征:边/面分两类（同 LOAM 的曲率判据）
    ...
    if (local_map_edge_ == nullptr || local_map_surf_ == nullptr) {   // 首帧特殊处理
      kdtree_edge_.BuildTree(current_edge);
      kdtree_surf_.BuildTree(current_surf);
      edges_.emplace_back(current_edge); surfs_.emplace_back(current_surf);
      return;
    }
    // 2) 与局部地图配准:边/面残差合进同一个 GN
    SE3 pose = AlignWithLocalMap(current_edge, current_surf);
    if (IsKeyframe(pose)) {                                            // 3) 抽关键帧、重建局部图
      edges_.emplace_back(edge_world); surfs_.emplace_back(surf_world);     // 已变到世界系
      if (edges_.size() > options_.num_kfs_in_local_map_) edges_.pop_front();
      if (surfs_.size() > options_.num_kfs_in_local_map_) surfs_.pop_front();
      local_map_edge_.reset(new PointCloudType); local_map_surf_.reset(new PointCloudType);
      for (auto& s : edges_) *local_map_edge_ += *s;                   // 拼局部图
      for (auto& s : surfs_) *local_map_surf_ += *s;
      local_map_edge_ = VoxelCloud(local_map_edge_, 1.0);             // 体素降采样
      local_map_surf_ = VoxelCloud(local_map_surf_, 1.0);
      kdtree_edge_.BuildTree(local_map_edge_);                        // 重建配准目标
  ...
  SE3 AlignWithLocalMap(CloudPtr edge, CloudPtr surf) {
    SE3 pose;
    if (estimated_poses_.size() >= 2) {            // 恒速外推作初值
      SE3 T1 = estimated_poses_[estimated_poses_.size() - 1];
      SE3 T2 = estimated_poses_[estimated_poses_.size() - 2];
      pose = T1 * (T2.inverse() * T1);             // pose = T1 * (inv(T2) * T1)
    }
    // 迭代 max_iteration_ 次:
    //   边点 -> 边树最近邻拟合直线 -> 点到线残差与雅可比
    //   面点 -> 面树最近邻拟合平面 -> 点到面残差与雅可比
    //   所有残差堆进同一个高斯-牛顿，解 pose
    return pose;
  }

  std::deque<CloudPtr> edges_, surfs_;             // 关键帧滑窗（定长）
  KdTree kdtree_edge_, kdtree_surf_;               // 边/面各一棵 KD 树
};
\end{codebox}

\begin{insight}[特征法 vs 直接法：框架可换，残差是唯一变量]\label{ins:lidar-loamlike}
\texttt{LoamLikeOdom} 把一件事演示得最透：scan-to-局部地图 + 关键帧滑窗 + 恒速外推这套\emph{骨架}，对“边/面残差”还是“体素高斯残差”一视同仁。于是工程上选特征法还是直接法，本质只剩一个问题——你的场景里“边/面特征稳不稳”：结构化室内/城区，特征法省点又稳；非结构自然环境（树丛、灌木、枝叶），边/面特征不可靠，直接法（逐点/体素）反而更好（\cref{sec:lidar-direct-ndt} 的动机段）。\emph{框架不必改，换“源”即可}——这正是把扫描配准内核抽象出来（\cref{sec:lidar-scan-reg}）带来的复用红利。
\end{insight}
...
\subsection{极简变体：两关键帧滑窗的预积分图 LIO}
\label{sec:lidar-preint-graph}

\paragraph{动机：把紧耦合图优化砍到“最小可用”。}
LIO-SAM 是\emph{完整}的因子图 LIO（多关键帧、GPS、回环）。若只想体会“预积分 + 图优化”如何替代滤波，可把它砍到\textbf{最小}：每次只优化\emph{相邻两个}关键帧\cite{gao2023slamad}。高翔在松耦合 LIO（\cref{sec:lidar-loose-lio}）的框架上，仅把“EKF 预测 + 观测”换成“预积分预测 + 图优化观测”——前端去畸变、消息同步原样不动，只改融合内核——即得一个两关键帧滑窗的预积分图 LIO。其因子图（\cref{fig:lidar-preint-graph}）含四类因子：
\begin{itemize}
  \item \textbf{先验因子}：加在\emph{上一个}关键帧上（来自上一轮边缘化，见下）；
  \item \textbf{预积分因子}：连接两关键帧的位姿/速度/零偏（\cref{eq:lidar-preint-v}--\cref{eq:lidar-preint-R}）；
  \item \textbf{零偏随机游走因子}：约束相邻两帧的陀螺/加计零偏缓变；
  \item \textbf{NDT 观测因子}：把当前帧与局部地图配准得到的位姿，加在\emph{下一个}关键帧上作位姿观测。
\end{itemize}
本轮优化完，对\emph{下一个}关键帧位姿做\textbf{边缘化}求其协方差，作为\emph{下一轮}的先验因子——如此把信息从一对关键帧“传”到下一对，是滑窗滤波的图优化对偶（\cref{ch:nlopt} 的边缘化先验）。一个工程提醒：预积分因子关联两帧的高维状态（典型 $15$ 维），转成图优化时把位姿/速度/零偏各设一个顶点（顶点种类多但每条边维度低），比写成一个含大量零块的高维顶点更利于稀疏求解。

\begin{figure}[ht]
\centering
\begin{tikzpicture}[>=Stealth, font=\small,
  var/.style={draw, circle, minimum size=8.5mm, inner sep=0pt, fill=main!7, draw=main!60!black},
  fac/.style={draw, rectangle, minimum size=3.4mm, inner sep=0pt},
  pr/.style={fac, fill=yellow!45, draw=yellow!60!black},
  pi/.style={fac, fill=red!75, draw=red!60!black},
  rw/.style={fac, fill=blue!55, draw=blue!60!black},
  ob/.style={fac, fill=magenta!70, draw=magenta!60!black},
  ln/.style={-, semithick, gray!60!black}]
  \node[var] (Ri) at (0,3) {$\mathbf{R}_i,\mathbf{p}_i$};
  \node[var] (vi) at (0,2) {$\mathbf{v}_i$};
  \node[var] (bgi) at (0,1) {$\mathbf{b}^\omega_i$};
  \node[var] (bai) at (0,0) {$\mathbf{b}^a_i$};
  \node[var] (Rj) at (4,3) {$\mathbf{R}_j,\mathbf{p}_j$};
  \node[var] (vj) at (4,2) {$\mathbf{v}_j$};
  \node[var] (bgj) at (4,1) {$\mathbf{b}^\omega_j$};
  \node[var] (baj) at (4,0) {$\mathbf{b}^a_j$};
  % prior on keyframe i (all states)
  \node[pr] (pr) at (-1.5,3) {};
  \node[font=\scriptsize, above=0pt] at (-1.5,3.18) {先验};
  \draw[ln] (pr)--(Ri); \draw[ln] (pr) to[bend right=12] (vi);
  \draw[ln] (pr) to[bend right=16] (bgi); \draw[ln] (pr) to[bend right=20] (bai);
  % preintegration factor (couples pose/vel/bias of both keyframes)
  \node[pi] (pi) at (2,2.5) {};
  \node[font=\scriptsize, above=0pt] at (2,2.72) {预积分因子};
  \draw[ln] (pi)--(Ri); \draw[ln] (pi)--(vi); \draw[ln] (pi)--(bgi);
  \draw[ln] (pi)--(Rj); \draw[ln] (pi)--(vj);
  % bias random-walk factors
  \node[rw] (rwg) at (2,1) {}; \draw[ln] (rwg)--(bgi); \draw[ln] (rwg)--(bgj);
  \node[rw] (rwa) at (2,0) {}; \draw[ln] (rwa)--(bai); \draw[ln] (rwa)--(baj);
  \node[font=\scriptsize, below=0pt] at (2,-0.28) {零偏随机游走};
  % NDT observation on keyframe j pose
  \node[ob] (ob) at (5.5,3) {};
  \node[font=\scriptsize, above=0pt] at (5.5,3.18) {NDT 位姿观测};
  \draw[ln] (ob)--(Rj);
  \node[font=\scriptsize, main!50!black] at (0,3.75) {上一个关键帧 $i$};
  \node[font=\scriptsize, main!50!black] at (4,3.75) {下一个关键帧 $j$};
\end{tikzpicture}
\caption{预积分图 LIO 的极简模型（两关键帧滑窗）：黄色先验因子钉住上一帧 $i$ 的绝对量，红色预积分因子耦合两帧的位姿/速度/零偏，蓝色零偏随机游走约束零偏缓变，品红 NDT 因子把激光-地图配准位姿作下一帧 $j$ 的观测。本轮解完边缘化 $j$ 得其协方差，作下一轮先验。（仿 \cite{gao2023slamad} 图 8-3 重绘）}
\label{fig:lidar-preint-graph}
\end{figure}

\begin{insight}[为什么“上一个关键帧”必须有先验：预积分只观测状态之差]\label{ins:lidar-preint-zerospace}
预积分因子观测的是两关键帧状态的\emph{差值}而非绝对量\cite{gao2023slamad}：如速度预积分量 $\Delta\mathbf{v}_{ij}$ 量的是 $\mathbf{v}_j-\mathbf{v}_i$（在 $i$ 系，\cref{eq:lidar-preint-v}）。于是若给两帧速度\emph{同时}加一个常量，速度项残差\emph{不变}；再在位移项配上相应调整，位移观测也保持不变——整个状态便能在一个\textbf{零空间}里自由漂移而不增代价。补救正是给\emph{前一个}关键帧加\textbf{先验约束}（钉住绝对量）、给后一个加观测约束，把优化问题锁在有限范围内。这与可观测性分析里“纯相对测量留下规范自由度”是同一回事（\cref{ch:slam_est} 的 gauge freedom）：相对约束定不了绝对基准，必须另有一处绝对信息（先验或绝对观测）来钉。
\end{insight}

\begin{insight}[为什么图 LIO 比 GINS 更难调：观测依赖预测]\label{ins:lidar-preint-debug}
这张图与 \cref{ch:gins} 的 GINS（IMU + RTK 图优化）结构\emph{几乎一样}，却有一处\textbf{本质区别}\cite{gao2023slamad}：GINS 的 RTK 是\emph{绝对}观测，信号好时精度固定、与状态预测无关；而这里的“NDT 观测位姿”是把当前帧\emph{以预测位姿为初值}配到地图得来的——\textbf{观测本身依赖预测}。若某帧预测偏差大，NDT 很可能从差初值配出一个\emph{错误}位姿，这个坏观测再灌回图里就可能让整个 LIO 发散。正因观测与预测\emph{耦合}（而非 RTK 那样彼此独立），基于图优化的 LIO 调试难度明显高于 GINS。实践建议：\emph{先用现成滤波系统把状态跑稳，再上后端图优化}，而非一上来就调两关键帧大图——“从现有系统出发、再做后端优化”是更稳的路径。
\end{insight}
...
\begin{itemize}
  ...
  \item \textbf{全局描述子}：用整帧 scan 的高层模式。两类粗表示：\emph{鸟瞰图（BEV）}——ScanContext++\cite{kim2018scancontext} 用极坐标表示 + 偏航对齐匹配实现朝向不变，RING++\cite{lu2023ringpp} 用 Radon 变换\emph{理论上}证明不变性与觉察性；\emph{距离图像}——可直接借用 CNN/ViT 提特征，OverlapNet\cite{chen2020overlapnet} 用重叠损失实现偏航不变，更近的 FRAME\cite{carlone2026handbook} 则用距离图像在\emph{矿井}等几何退化、GPS 不可用的地下环境实现激光位置识别。
...
\begin{equation}
  ...
  \label{eq:lidar-pgo}
\end{equation}
用 Gauss–Newton/LM 在 $\SE(3)$ 流形上迭代（右扰动更新 $\mathbf{T}_i\leftarrow\mathbf{T}_i\,\mathrm{Exp}(\delta\boldsymbol{\xi}_i)$，雅可比含右雅可比逆，见 \cref{ch:lie}、\cref{ch:nlopt}）。约束集\emph{稀疏}（互连边少），故用\textbf{稀疏矩阵分解}（重排序 + 重线性化）可让 $>1000$ 节点的图在零点几秒内更新\cite{carlone2026handbook}；通用求解器有 g2o、GTSAM\cite{dellaert2017factor}，增量求解用 iSAM2\cite{kaess2012isam2}（Bayes 树，只重解受影响的子图，见 \cref{ch:nlopt} 的 $\sqrt{}$SAM/边缘化），大图上还可用\textbf{层次化}位姿图优化（HOG-Man\cite{carlone2026handbook}：把图按尺度分层、先在粗层求解再逐层细化，进一步压低大规模在线优化的耗时）。
...
\paragraph{第三类范式：学习式激光里程计（与其泛化之忧）。}
除“特征 / 直接”这两类\emph{几何}里程计外，还有一条\textbf{学习式}支线——直接用神经网络从激光测量\emph{回归}自运动\cite{carlone2026handbook}。早期工作用\emph{监督}学习、以真值位姿为标签训练；其后扩展到\emph{无监督}方法（用几何/光度一致性等自监督信号，免真值标注）。这类深度激光里程计在基准上表现可观，但有一个公认隐忧：\textbf{泛化能力存疑}——换一座城市、一种雷达线数、一类天气，模型未必还准。这与 \cref{ch:imu}（惯性里程计）那条“学习式 IO”线\emph{呼应}：学习方法擅长在\emph{见过}的分布里榨取精度，却难担保\emph{没见过}的场景不崩；故当前实战里程计仍以几何法为主、学习法为辅或仅作先验。
...
提取的激光特征都有严格理论支撑 & 多数特征提取是工程经验、无明确理论依据（\cref{ins:lidar-feat-criteria}）\\
图优化 LIO 与 GINS 一样好调 & 雷达里程计观测\emph{依赖预测}、不同于 RTK 绝对观测，图 LIO 更易发散难调（\cref{ins:lidar-preint-debug}）\\
\bottomrule
\end{tabular}
\end{table}
...
李群 / 右扰动 & Sophus \texttt{SO3d::exp/\allowbreak log}、\texttt{SE3d}；更新一律右乘 \texttt{X * Exp(delta)} \\
