...
生成似然场最朴素的办法：对目标点云的每个点，「贴」一个预先算好的\emph{固定大小模板}（如 $20$ 像素边长，每个模板像素存其到中心的距离）。遍历所有目标点、把模板盖上去、取各像素的\emph{最小}距离值，就得到一张距离变换图（实现里常用 $1000\times 1000$、分辨率 $20$ 像素/米的 \texttt{cv::Mat}，对应世界系约 $\pm 25$\,m 的覆盖范围）。\footnote{此处「分辨率 $20$ 像素/米」与下方代码 \texttt{cv::Mat(1000,1000,CV\_32F,30.0)} 中的 $30.0$ 不是同一量：后者是\emph{场的初始值}（即「离任何障碍物都很远」的大距离填充值），前者才是像素/米的空间尺度。}配准时把 ICP 的「残差 + 雅可比」换成似然场的「场值 + 图像梯度」即可。
...
朴素想法：记 $[\mathrm{d}x,\mathrm{d}y]=\mathbf{p}_2-\mathbf{p}_1$，取增量较大的轴为主轴（设为 $x$）；从 $\mathbf{p}_1$ 出发，每当 $x$ 自增 $1$，与真实直线的纵向误差就累加斜率 $\mathrm{d}y/\mathrm{d}x$，误差超过 $0.5$ 就让 $y$ 自增 $1$、误差减 $1$。这里有浮点。Bresenham 的关键技巧是把第 $3$ 步的所有运算\emph{同乘 $2\mathrm{d}x$}：令初始误差 $e=-\mathrm{d}x$，每当 $x$ 自增 $1$，$e\mathrel{+}=2\mathrm{d}y$；若 $e\ge 0$ 则 $y$ 自增 $1$、$e\mathrel{-}=2\mathrm{d}x$。这样\emph{只剩整数加法与乘法}，回避了浮点和除法。若 $y$ 为主轴则交换 $x,y$ 的角色即可。\footnote{边界处 $e=0$（即恰好穿过格点）是一个平局，取 $\ge$ 或 $>$ 都给出合法的栅格直线，二者仅在严格对角穿越的格点上差一格；下文代码与多数实现取 $\ge 0$。}
...
\section*{延伸阅读}
\begin{itemize}
  \item \textbf{吸收主源}：高翔《自动驾驶与机器人中的 SLAM 技术：从理论到实践》\cite{gao2023slamad} 第 6 章（2D SLAM）——本章扫描匹配/似然场/占据栅格/子地图/多分辨率回环的全部出处，代码仓 \texttt{gaoxiang12/\allowbreak slam\_in\_autonomous\_driving}。
