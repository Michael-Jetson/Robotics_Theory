...
\subsection{扩展位姿 \texorpdfstring{$\mathrm{SE}_2(3)$}{SE2(3)} 上的预积分：百川归海}
\label{subsec:imu-se23-preint}

\paragraph{动机：换一个流形，而不是换一种积分。}
\cref{subsec:numint-accuracy,subsec:cont-rot-preint} 的进阶都在“怎么把测量积成增量”上做文章。还有一条\emph{正交}的进阶路线，不动积分方法，而动\textbf{在什么流形上携带状态与不确定度}：把姿态 $\mathbf R$、位置 $\mathbf p$、速度 $\mathbf v$ 三者打包进\emph{一个}李群元素——扩展位姿群 $\mathrm{SE}_2(3)$（\cref{sec:se23} 给出权威定义：群结构 \cref{eq:se23-group}、指数映射 \cref{eq:se23-exp}、伴随 \cref{eq:se23-adjoint}），整段预积分都在这个群上做。本小节沿 Brossard 等\cite{brossard2021associating} 与 Barfoot\cite{barfoot2024state} 的处理，完整给出 $\mathrm{SE}_2(3)$ 上的预积分：群仿射运动模型、$\mathbf N$ 矩阵均值传播、时间机器、协方差传播、批量 MAP 因子；并在三处\emph{洞见}里点明它与 \cref{sec:imu-preint} 的 Forster 分量预积分如何\textbf{殊途同归}——后者正是前者“区间内不转”的退化特例。收益有三：(i) 不确定度建模更一致（群仿射 $\Rightarrow$ 误差动力学自治，接 InEKF，\cref{paper:inekf}）；(ii) 干净纳入标准预积分忽略的地球自转；(iii) 一个把“百家分量”收束成“一群整体”的统一视角。

\begin{note}
\textbf{记号桥接.}\;Barfoot 在惯导一章以 $\mathbf C$ 记旋转、$\mathbf T_{iv}\in\mathrm{SE}_2(3)$ 记群元素，并\emph{唯独}在此处采用\textbf{右扰动}——这与本书全局右扰动主线恰好一致（\cref{sec:se23} 的记号对照）。本小节统一用 $\mathbf R$ 记旋转、$\mathbf T\,\mathrm{Exp}(\delta\boldsymbol\xi)$ 右扰动、李代数坐标 $\boldsymbol\xi=[\boldsymbol\rho;\boldsymbol\nu;\boldsymbol\phi]$（位置、速度、旋转）。本小节出现的 $\mathbf J=\mathbf J_l(\boldsymbol\phi)$ 是 $\SO(3)$ 的\emph{左}雅可比（\cref{eq:Jl}），与 Forster 分量式所用右雅可比 $\mathbf J_r$ 互为转置关系（$\mathbf J_l(\boldsymbol\phi)=\mathbf J_r(-\boldsymbol\phi)=\mathbf J_r(\boldsymbol\phi)^\top$）——这是“左/右两套机器”的接口，下文 \cref{ins:imu-se23-fusion1} 详述。
\end{note}

\paragraph{扩展位姿状态与群仿射运动学。}
把待估状态写成 $\mathrm{SE}_2(3)$ 元素加偏置（速度存于世界系，惯导惯例）：
\begin{equation}
  \mathbf T=\begin{bmatrix}\mathbf R&\mathbf p&\mathbf v\\\mathbf 0^\top&1&0\\\mathbf 0^\top&0&1\end{bmatrix}\in\mathrm{SE}_2(3),
  \qquad \mathbf b=\begin{bmatrix}\mathbf b^g\\\mathbf b^a\end{bmatrix}\in\mathbb R^6.
  \label{eq:imu-se23-state}
\end{equation}
把测量模型 \cref{eq:imu-gyro,eq:imu-accel} 的真值代入连续运动学 \cref{eq:imu-kin-cont}，并把偏置建成随机游走（\cref{eq:bias-brownian}），得\cite{barfoot2024state}
\begin{equation}
  \dot{\mathbf R}=\mathbf R(\tilde{\boldsymbol\omega}-\mathbf b^g-\boldsymbol\eta^g)^\wedge,\quad
  \dot{\mathbf p}=\mathbf v,\quad
  \dot{\mathbf v}=\mathbf R(\tilde{\mathbf a}-\mathbf b^a-\boldsymbol\eta^a)+\mathbf g,\quad
  \dot{\mathbf b}^g=\boldsymbol\eta^{bg},\quad \dot{\mathbf b}^a=\boldsymbol\eta^{ba}.
  \label{eq:imu-se23-groupaffine}
\end{equation}
关键观察：除去重力项 $\mathbf g$ 与偏置，\cref{eq:imu-se23-groupaffine} 的扩展位姿部分在 $\mathrm{SE}_2(3)$ 上恰是 Barrau--Bonnabel 所称的\textbf{群仿射（group-affine）}动力学（\cref{sec:se23} 的核心论断）。重力是一个\emph{无法塞进} $\mathrm{SE}_2(3)$ 同态部分的仿射驱动项——正是它逼出了下文的“时间机器”技巧。把扰动按右扰动 $\mathbf T=\bar{\mathbf T}\,\mathrm{Exp}(\delta\boldsymbol\xi)$、$\mathbf b=\bar{\mathbf b}+\delta\mathbf b$ 分离，可把 \cref{eq:imu-se23-groupaffine} 拆成\emph{标称}模型（传播均值，\cref{eq:imu-se23-meanprop}）与\emph{扰动}模型（传播协方差，\cref{eq:imu-se23-Amat}），与全书“标称走均值、扰动走不确定度”的套路一致。

\paragraph{\texorpdfstring{$\mathbf N$}{N} 矩阵与均值传播。}
沿用 IMU 的两条假设：在小区间 $(\tau_{j-1},\tau_j)$ 内 (i) IMU 读数分段常值、(ii) 偏置常值。于是 $\bar{\boldsymbol\omega}_j=\tilde{\boldsymbol\omega}_j-\bar{\mathbf b}^g$、$\bar{\mathbf a}_j=\tilde{\mathbf a}_j-\bar{\mathbf b}^a$ 在区间内为常量，标称扩展位姿动力学成为\emph{线性定常}（LTI）系统，可\emph{精确}积分。其离散更新为\cite{barfoot2024state}
\begin{equation}
\begin{aligned}
  \bar{\mathbf R}(\tau_j)&=\bar{\mathbf R}(\tau_{j-1})\,\mathrm{Exp}\!\big(\Delta\tau_j\,\bar{\boldsymbol\omega}_j\big),\\
  \bar{\mathbf v}(\tau_j)&=\bar{\mathbf v}(\tau_{j-1})+\Delta\tau_j\Big(\bar{\mathbf R}(\tau_{j-1})\,\mathbf J(\Delta\tau_j\bar{\boldsymbol\omega}_j)\,\bar{\mathbf a}_j+\mathbf g\Big),\\
  \bar{\mathbf p}(\tau_j)&=\bar{\mathbf p}(\tau_{j-1})+\Delta\tau_j\,\bar{\mathbf v}(\tau_{j-1})+\tfrac12\Delta\tau_j^2\Big(\bar{\mathbf R}(\tau_{j-1})\,\mathbf N(\Delta\tau_j\bar{\boldsymbol\omega}_j)\,\bar{\mathbf a}_j+\mathbf g\Big),
\end{aligned}
  \label{eq:imu-se23-meanprop}
\end{equation}
其中 $\mathbf J(\cdot)=\mathbf J_l(\cdot)$ 是 $\SO(3)$ 左雅可比，而 $\mathbf N(\cdot)$ 是一个\textbf{新矩阵函数}——它对速度更新的 $\mathbf J$ 再积一次分，专门刻画“积分区间内旋转也在变”这件事：
\begin{equation}
  \mathbf N(\phi\mathbf a)=2\!\int_0^1\!\alpha\,\mathbf J(\alpha\phi\mathbf a)\,d\alpha
  =2\sum_{n=0}^\infty\frac{1}{(n+2)!}(\phi\mathbf a^\wedge)^n
  =2\frac{1-\cos\phi}{\phi^2}\mathbf I+\Big(1-2\frac{1-\cos\phi}{\phi^2}\Big)\mathbf a\mathbf a^\top+2\frac{\phi-\sin\phi}{\phi^2}\mathbf a^\wedge,
  \label{eq:imu-Nmatrix}
\end{equation}
$\phi=\|\phi\mathbf a\|$ 为转角、$\mathbf a$ 为单位转轴。

\begin{derivation}[$\mathbf N$ 矩阵的闭式来由]
$\mathbf N$ 的积分定义 $\mathbf N=2\int_0^1\alpha\,\mathbf J(\alpha\phi\mathbf a)\,d\alpha$ 直接来自把 LTI 系统的位置分量解析积分：位置是速度的积分，而速度里已含一个 $\mathbf J$，再积一次便对 $\mathbf J$ 加权积分。把 $\SO(3)$ 左雅可比的级数 $\mathbf J(\boldsymbol\psi)=\sum_{n\ge0}\frac{1}{(n+1)!}(\boldsymbol\psi^\wedge)^n$ 代入、令 $\boldsymbol\psi=\alpha\phi\mathbf a$ 并逐项积 $\int_0^1\alpha\cdot\alpha^n\,d\alpha=\frac{1}{n+2}$：
\[
  \mathbf N=2\sum_{n\ge0}\frac{1}{(n+1)!}\frac{1}{n+2}(\phi\mathbf a^\wedge)^n=2\sum_{n\ge0}\frac{1}{(n+2)!}(\phi\mathbf a^\wedge)^n,
\]
即 \cref{eq:imu-Nmatrix} 中间式。再用 $\mathbf a^{\wedge2}=\mathbf a\mathbf a^\top-\mathbf I$（单位轴）把级数按 $\mathbf I,\mathbf a\mathbf a^\top,\mathbf a^\wedge$ 三组分别求和（与 Rodrigues、$\mathbf J$ 闭式同一套技巧，\cref{thm:so3-kinematics} 邻域），即得闭式。两个极限值得记住：$\phi\to0$ 时 $\mathbf N\to\mathbf I$、$\mathbf J\to\mathbf I$，\cref{eq:imu-se23-meanprop} 退回欧拉更新 \cref{eq:imu-euler-meas}。
\end{derivation}

\begin{insight}[殊途同归之一：Forster 分量预积分 $=$ $\mathbf N=\mathbf J=\mathbf I$ 的退化]\label{ins:imu-se23-fusion1}
把 \cref{eq:imu-se23-meanprop} 与 \cref{sec:imu-preint} 的 Forster 欧拉递推 \cref{eq:imu-euler-meas} 并排：Forster 用\emph{区间起点}的姿态 $\bar{\mathbf R}(\tau_{j-1})\bar{\mathbf a}_j$ 来更新速度/位置（即把区间内姿态\emph{冻结}在起点）；$\mathrm{SE}_2(3)$ 精确版则插入 $\mathbf J(\Delta\tau_j\bar{\boldsymbol\omega}_j)$（速度）与 $\mathbf N(\Delta\tau_j\bar{\boldsymbol\omega}_j)$（位置）两个修正因子，把“区间内姿态也在随 $\bar{\boldsymbol\omega}_j$ 转动”这件事\emph{精确}积进去。当区间内不转（$\bar{\boldsymbol\omega}_j\to\mathbf 0$）时 $\mathbf J,\mathbf N\to\mathbf I$，两式逐字相同。换言之，\cref{eq:imu-euler-meas} 那条“速度/位置更新假设区间内姿态恒定”的近似（\cref{sec:imu-kinematics} 的有效域讨论），其\emph{精确}补偿恰是 $\mathbf J$ 与 $\mathbf N$。这正是慢率 IMU、剧烈旋转时 $\mathrm{SE}_2(3)$ 预积分更准的根源：它不靠提高采样率，而靠对区间内旋转作\emph{闭式}积分。
\end{insight}

\paragraph{时间机器与精确乘积形式。}
重力项使 \cref{eq:imu-se23-meanprop} 不是纯 $\mathrm{SE}_2(3)$ 乘法。Brossard 的妙招是引入一族\textbf{时间机器}（time machines）辅助矩阵：$5\times5$ 的
\begin{equation}
  \boldsymbol\Delta(\tau)=\begin{bmatrix}\mathbf I&\mathbf 0&\mathbf 0\\\mathbf 0^\top&1&0\\\mathbf 0^\top&\tau&1\end{bmatrix}\in\mathbb T,
  \qquad
  \boldsymbol\Delta(\tau)^{-1}=\boldsymbol\Delta(-\tau),
  \qquad
  \boldsymbol\Delta(\tau_1)\boldsymbol\Delta(\tau_2)=\boldsymbol\Delta(\tau_1+\tau_2),
  \label{eq:imu-timemachine}
\end{equation}
它们关于矩阵乘法构成一个一维交换群 $\mathbb T$；其 $9\times9$ 伴随版记 $\mathcal D(\boldsymbol\Delta)$。$\mathbb T$ 与 $\mathrm{SE}_2(3)$ 之间有一条关键搬移恒等式 $\mathrm{Exp}\!\big((\mathcal D(\boldsymbol\Delta)\boldsymbol\xi)^\wedge\big)=\boldsymbol\Delta^{-1}\mathrm{Exp}(\boldsymbol\xi^\wedge)\boldsymbol\Delta$。借它，单步均值传播 \cref{eq:imu-se23-meanprop} 可\emph{精确}写成三个 $\mathrm{SE}_2(3)$ 元素的乘积\cite{brossard2021associating,barfoot2024state}：
\begin{equation}
  \mathbf T_j=\mathbf T_{g,j}\,\boldsymbol\Delta_j^{-1}\mathbf T_{j-1}\boldsymbol\Delta_j\,\boldsymbol\Xi_j,
  \qquad
  \boldsymbol\Xi_j=\mathrm{Exp}\!\left(\begin{bmatrix}\tfrac12\Delta\tau_j^2\,\mathbf J_j^{-1}\mathbf N_j\,\bar{\mathbf a}_j\\[1pt]\Delta\tau_j\,\bar{\mathbf a}_j\\[1pt]\Delta\tau_j\,\bar{\boldsymbol\omega}_j\end{bmatrix}^{\!\wedge}\right),
  \label{eq:imu-se23-product}
\end{equation}
其中 $\mathbf T_{g,j}$ 是只含重力的 $\mathrm{SE}_2(3)$ 元素、$\boldsymbol\Delta_j=\boldsymbol\Delta(\Delta\tau_j)$、$\mathbf J_j=\mathbf J(\Delta\tau_j\bar{\boldsymbol\omega}_j)$、$\mathbf N_j=\mathbf N(\Delta\tau_j\bar{\boldsymbol\omega}_j)$。\textbf{增量 $\boldsymbol\Xi_j$ 只依赖 IMU 读数与偏置}，不含起点位姿/速度/重力——这就是 $\mathrm{SE}_2(3)$ 版的“预积分量”。把 \cref{eq:imu-se23-product} 沿多步连乘，再用搬移恒等式把所有 $\mathbf T_{g}$ 与 $\boldsymbol\Delta$ 滑到一起，可把整段窗口内的全部重力元素\emph{合并成一个}，大幅减少矩阵乘法数——这正是“time machines”名字的来历：把不同时刻的项搬到同一时刻。
\begin{equation}
  \boldsymbol\Xi_j=\begin{bmatrix}\mathrm{Exp}(\Delta\tau_j\bar{\boldsymbol\omega}_j)&\tfrac12\Delta\tau_j^2\,\mathbf N_j\bar{\mathbf a}_j&\Delta\tau_j\,\mathbf J_j\bar{\mathbf a}_j\\\mathbf 0^\top&1&0\\\mathbf 0^\top&0&1\end{bmatrix}\in\mathrm{SE}_2(3).
  \label{eq:imu-Xi}
\end{equation}

\begin{insight}[殊途同归之二：$\boldsymbol\Xi_j$ 的三行 $=$ Forster 的 $\Delta\mathbf R,\Delta\mathbf v,\Delta\mathbf p$ 装进一个群元素]\label{ins:imu-se23-fusion2}
逐行读 \cref{eq:imu-Xi}：左上角 $\mathrm{Exp}(\Delta\tau_j\bar{\boldsymbol\omega}_j)$ 就是单步旋转增量（Forster 的 $\Delta\tilde{\mathbf R}$ 因子）；速度列 $\Delta\tau_j\mathbf J_j\bar{\mathbf a}_j$、位置列 $\tfrac12\Delta\tau_j^2\mathbf N_j\bar{\mathbf a}_j$ 就是单步的速度、位置增量（Forster 的 $\Delta\tilde{\mathbf v},\Delta\tilde{\mathbf p}$ 贡献，只是多了 $\mathbf J_j,\mathbf N_j$ 的区间内旋转修正，见 \cref{ins:imu-se23-fusion1}）。\textbf{Forster 把这三件事当成三个分开的对象}——$\Delta\mathbf R$ 活在 $\SO(3)$、$\Delta\mathbf v,\Delta\mathbf p$ 活在 $\mathbb R^3$，各自递推、各自传协方差；\textbf{$\mathrm{SE}_2(3)$ 把它们当成一个群元素 $\boldsymbol\Xi_j$}，一次群乘法同时推进三者。这就是“汇聚百家”的几何含义：分量视角与整体视角携带\emph{同样}的信息，区别只是“三个零件”还是“一台机器”。VINS-Mono 的四元数三元组 $(\boldsymbol\alpha,\boldsymbol\beta,\boldsymbol\gamma)$（\cref{subsec:vins-quat}）是同一三元组的又一种坐标写法。
\end{insight}

\paragraph{协方差传播。}
扰动模型（右扰动误差 $\delta\mathbf x=[\delta\boldsymbol\rho;\delta\boldsymbol\nu;\delta\boldsymbol\phi;\delta\mathbf b^g;\delta\mathbf b^a]\in\mathbb R^{15}$）是一个 LTV 系统 $\dot{\delta\mathbf x}=\mathbf A\,\delta\mathbf x+\mathbf L\,\mathbf w$，其中（区间内 IMU/偏置常值时为 LTI）
\begin{equation}
  \mathbf A=\begin{bmatrix}
  -\bar{\boldsymbol\omega}^\wedge&\mathbf I&\mathbf 0&\mathbf 0&\mathbf 0\\
  \mathbf 0&-\bar{\boldsymbol\omega}^\wedge&-\bar{\mathbf a}^\wedge&\mathbf 0&-\mathbf I\\
  \mathbf 0&\mathbf 0&-\bar{\boldsymbol\omega}^\wedge&-\mathbf I&\mathbf 0\\
  \mathbf 0&\mathbf 0&\mathbf 0&\mathbf 0&\mathbf 0\\
  \mathbf 0&\mathbf 0&\mathbf 0&\mathbf 0&\mathbf 0\end{bmatrix},
  \label{eq:imu-se23-Amat}
\end{equation}
$\mathbf L$ 把白噪声 $\mathbf w=[\boldsymbol\eta^g;\boldsymbol\eta^a;\boldsymbol\eta^{bg};\boldsymbol\eta^{ba}]$ 注入对应行（旋转行 $-\boldsymbol\eta^g$、速度行 $-\boldsymbol\eta^a$、两偏置行各 $+\mathbf I$）。状态转移 $\boldsymbol\Phi_j=\exp(\mathbf A_j\Delta\tau_j)$，离散协方差递推
\begin{equation}
  \boldsymbol\Sigma_j=\boldsymbol\Phi_j\,\boldsymbol\Sigma_{j-1}\,\boldsymbol\Phi_j^\top+\boldsymbol\Sigma_{w,j},
  \qquad
  \boldsymbol\Sigma_{w,j}=\int_{\tau_{j-1}}^{\tau_j}\boldsymbol\Phi(\tau_j,t)\,\mathbf L\mathbf Q_C\mathbf L^\top\boldsymbol\Phi(\tau_j,t)^\top dt,
  \label{eq:imu-se23-cov}
\end{equation}
$\mathbf Q_C$ 为白噪声功率谱密度阵。把 $\boldsymbol\Phi(\tau_j,t)\approx\mathbf I+\mathbf A_j(\tau_j-t)+\cdots$ 代入逐项积分，得 $\boldsymbol\Sigma_{w,j}$ 到 $O(\Delta\tau_j^3)$ 的级数（保留到三阶以保正定）：
\[
  \boldsymbol\Sigma_{w,j}\approx\Delta\tau_j\,\mathbf L\mathbf Q_C\mathbf L^\top
  +\tfrac12\Delta\tau_j^2\big(\mathbf A_j\mathbf L\mathbf Q_C\mathbf L^\top+\mathbf L\mathbf Q_C\mathbf L^\top\mathbf A_j^\top\big)
  +\tfrac16\Delta\tau_j^3\big(2\mathbf A_j\mathbf L\mathbf Q_C\mathbf L^\top\mathbf A_j^\top+\mathbf A_j^2\mathbf L\mathbf Q_C\mathbf L^\top+\mathbf L\mathbf Q_C\mathbf L^\top(\mathbf A_j^\top)^2\big).
\]
窗口内多步 IMU 把 \cref{eq:imu-se23-cov} 逐步复合，得整段 $15\times15$ 协方差 $\boldsymbol\Sigma_J$。形式上这与 Forster 分量版的协方差递推 \cref{eq:cov-recursion}（$\boldsymbol\Sigma\!\leftarrow\!\mathbf A\boldsymbol\Sigma\mathbf A^\top\!+\!\mathbf B\boldsymbol\Sigma_\eta\mathbf B^\top$）完全同构——区别仅在于 $\mathrm{SE}_2(3)$ 版把旋转/速度/位置/偏置的全部互相关装进\emph{一个} $15$ 维高斯，其 $\mathbf A$ 的耦合（如 $-\bar{\mathbf a}^\wedge$ 把姿态误差喂进速度误差）由群结构自动给出，不必手工拼接。

\paragraph{批量 MAP 预积分因子。}
批量估计只在窗口两端 $\tau_0,\tau_J$ 保留状态。定义 $\mathrm{SE}_2(3)$ 上的预积分误差（旋转/速度/位置合成 $9$ 维 $\mathbf e_T$，偏置 $6$ 维 $\mathbf e_b$）\cite{barfoot2024state}：
\begin{equation}
  \mathbf e_T=\mathrm{Log}\!\Big(\mathbf T_J^{-1}\,\mathbf T_{g,J:0}\,\boldsymbol\Delta_{J:0}^{-1}\,\mathbf T_0\textstyle\prod_{j=1}^J\boldsymbol\Delta_j\boldsymbol\Xi_j\Big)^\vee,
  \qquad
  \mathbf e_b=\begin{bmatrix}\mathbf b^g_0-\mathbf b^g_J\\\mathbf b^a_0-\mathbf b^a_J\end{bmatrix},
  \label{eq:imu-se23-error}
\end{equation}
其中所有重力元素已用时间机器合并成 $\mathbf T_{g,J:0}$、所有 $\boldsymbol\Delta$ 合并成 $\boldsymbol\Delta_{J:0}$，$\mathrm{Log}(\cdot)^\vee$ 是 $\mathrm{SE}_2(3)$ 的对数。直观说，$\mathbf e_T$ 就是“窗口末端真实状态”与“由起点经 IMU 预积分推出的预测状态”之差，在李群意义下度量。对两端做右扰动 $\mathbf T_k=\mathbf T_{\mathrm{op},k}\mathrm{Exp}(\boldsymbol\epsilon_k^\wedge)$，并对起点偏置做 $\mathbf b_0=\mathbf b_{\mathrm{op},0}+\boldsymbol\beta_0$，线性化得
\begin{equation}
  \mathbf e\approx\mathbf e_{\mathrm{op}}+\mathbf F_0\,\boldsymbol\varepsilon_0-\mathbf E_J\,\boldsymbol\varepsilon_J,
  \quad
  \mathbf F_0=\begin{bmatrix}\boldsymbol{\mathcal J}(\mathbf e_{\mathrm{op},T})^{-1}\mathbf V_0&\boldsymbol{\mathcal J}(\mathbf e_{\mathrm{op},T})^{-1}\mathbf V_0\mathbf B_0\\\mathbf 0&\mathbf I\end{bmatrix},
  \quad
  \mathbf E_J=\begin{bmatrix}\boldsymbol{\mathcal J}(\mathbf e_{\mathrm{op},T})^{-1}&\mathbf 0\\\mathbf 0&\mathbf I\end{bmatrix},
  \label{eq:imu-se23-linerr}
\end{equation}
$\boldsymbol\varepsilon_k=[\boldsymbol\epsilon_k;\boldsymbol\beta_k]$，$\boldsymbol{\mathcal J}(\cdot)$ 是 $\mathrm{SE}_2(3)$ 的（左）雅可比（\cref{sec:se23}），$\mathbf V_0$ 是起点扰动经一串伴随搬到误差处的常量。偏置只通过 $\boldsymbol\Xi_j$ 进入，其敏感度由
\begin{equation}
  \boldsymbol\Xi_j\approx\boldsymbol\Xi_{\mathrm{op},j}\,\mathrm{Exp}\!\big((\boldsymbol{\mathcal J}(-\boldsymbol\xi_{\mathrm{op},j})\mathbf B_j\boldsymbol\beta_0)^\wedge\big),
  \qquad
  \mathbf B_0=\sum_{j=1}^J\Big(\textstyle\prod_{\ell=1}^j\mathcal D_\ell^{-1}\mathrm{Ad}(\boldsymbol\Xi_{\mathrm{op},\ell})\Big)\boldsymbol{\mathcal J}(-\boldsymbol\xi_{\mathrm{op},j})\mathbf B_j
  \label{eq:imu-se23-biasjac}
\end{equation}
累加得到（$\mathbf B_j$ 含对 $\mathbf J_j^{-1}\mathbf N_j\bar{\mathbf a}_j$ 关于偏置的展开，可用 $\mathbf J(\phi)^{-1}\mathbf N(\phi)=\mathbf I-\tfrac16\phi^\wedge+\tfrac1{360}\phi^{\wedge3}-\cdots$ 截断）。把 $\tfrac12(\mathbf e_{\mathrm{op}}-\mathbf F^{-1}\delta\mathbf x)^\top\boldsymbol\Sigma_J^{-1}(\cdot)$ 作为运动先验项加入批量 MAP \cref{eq:imu-map}，即得 $\mathrm{SE}_2(3)$ 预积分因子。

\begin{insight}[殊途同归之三：一个 $\mathrm{SE}_2(3)$ 残差 $=$ Forster 的 $[\mathbf r_{\Delta\mathbf R};\mathbf r_{\Delta\mathbf v};\mathbf r_{\Delta\mathbf p}]$]\label{ins:imu-se23-fusion3}
Forster 的 $9$ 维残差 \cref{eq:imu-residual} 把旋转残差用 $\mathrm{Log}$ 度量、速度/位置残差用向量减法度量——分别活在 $\SO(3)$ 与 $\mathbb R^3\times\mathbb R^3$。$\mathrm{SE}_2(3)$ 的 $\mathbf e_T$（\cref{eq:imu-se23-error}）把三者合成\emph{一个}群对数残差，用单一 $\mathrm{Log}_{\mathrm{SE}_2(3)}$ 度量整体差异。二者携带同样信息，但 $\mathrm{SE}_2(3)$ 残差的协方差权重 $\boldsymbol\Sigma_J$ 天然含旋转-速度-位置的全部互相关（Forster 版同样有，只是写成 $9\times9$ 分块）。偏置雅可比 $\mathbf B_0$（\cref{eq:imu-se23-biasjac}）正对应 Forster 的 $\partial\Delta\bar{\mathbf R}/\partial\mathbf b^g,\partial\Delta\bar{\mathbf v}/\partial\mathbf b,\ldots$（\cref{eq:bias-jacobians}）：两者都是“一次预计算、偏置小变时 $O(1)$ 线性修正”（\cref{eq:bias-correction} 与 \cref{ins:bias-firstorder} 的同一思想）。\textbf{结论}：Forster 分量预积分与 $\mathrm{SE}_2(3)$ 整体预积分不是两套理论，而是\emph{同一套预积分在两种坐标下的写法}——分量式贴近手算与 GTSAM/VINS 实现，群式贴近一致性分析与 InEKF。理解了一个，另一个只是“拆开”或“打包”。
\end{insight}

\begin{table}[H]\centering\small
\caption{Forster 分量预积分 vs.\ $\mathrm{SE}_2(3)$ 整体预积分（同一信息的两种坐标）}
\label{tab:imu-se23-vs-forster}
\begin{tabular}{lll}
\toprule
方面 & Forster 分量视角（\cref{sec:imu-preint}）& $\mathrm{SE}_2(3)$ 整体视角（本小节）\\
\midrule
状态载体 & $\Delta\mathbf R\in\SO(3)$，$\Delta\mathbf v,\Delta\mathbf p\in\mathbb R^3$ & 一个 $\boldsymbol\Xi_j\in\mathrm{SE}_2(3)$ \\
区间内旋转 & 冻结在起点（$\mathbf N=\mathbf J=\mathbf I$）& 闭式补偿 $\mathbf J_j,\mathbf N_j$（\cref{eq:imu-Nmatrix}）\\
增量定义 & 三式 \cref{eq:imu-rel-increment} & 一群元素 \cref{eq:imu-Xi}\\
残差 & $[\mathbf r_{\Delta\mathbf R};\mathbf r_{\Delta\mathbf v};\mathbf r_{\Delta\mathbf p}]$ \cref{eq:imu-residual} & 单一 $\mathbf e_T=\mathrm{Log}(\cdots)^\vee$ \cref{eq:imu-se23-error}\\
协方差 & $9\times9$ 分块递推 \cref{eq:cov-recursion} & $15\times15$ 群上递推 \cref{eq:imu-se23-cov}\\
偏置修正 & $\partial\Delta/\partial\mathbf b$ 一阶 \cref{eq:bias-jacobians} & $\mathbf B_0$ 累加 \cref{eq:imu-se23-biasjac}\\
最擅长 & 手算、GTSAM/VINS 实现 & 一致性分析、InEKF、地球自转 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{左右扰动只差一个符号。}
本小节用右扰动 $\mathbf T_{iv}=\mathbf T_{\mathrm{op},iv}\mathrm{Exp}(\boldsymbol\epsilon_r^\wedge)$，而全书别处（如 \cref{ch:eskf} 点云跟踪）惯用左扰动 $\mathbf T_{vi}=\mathrm{Exp}(\boldsymbol\epsilon_\ell^\wedge)\mathbf T_{\mathrm{op},vi}$。因 $(\mathbf T_{\mathrm{op},iv}\mathrm{Exp}(\boldsymbol\epsilon^\wedge))^{-1}=\mathrm{Exp}(-\boldsymbol\epsilon^\wedge)\mathbf T_{\mathrm{op},vi}$，两种扰动都落在“车体一侧”，故 $\boldsymbol\epsilon_r=-\boldsymbol\epsilon_\ell$、对应观测雅可比 $\mathbf G_r=-\mathbf G_\ell$\cite{barfoot2024state}。\textbf{推论}：换边只是统一改个符号——之所以在惯导/$\mathrm{SE}_2(3)$ 处选右扰动，正因 IMU 运动模型作用在 $\mathbf T_{iv}$（车到世界）上，右扰动让群仿射结构与本书右扰动主线同时成立（\cref{sec:se23} 记号对照）。

\paragraph{地球自转与导航级应用。}
群上传播不确定度的好处之一，是能干净纳入标准预积分忽略的\emph{地球自转}项（科氏力、离心力，对照 \cref{subsec:gravity} 末的讨论）：把地球自转角速度并入 $\boldsymbol\Omega$ 的标称部分即可，$\mathrm{SE}_2(3)$ 的群结构使之与重力项一样被时间机器干净处理。一个海上导航数值实例显示，用导航级 IMU 配线速度传感器、在 $\mathrm{SE}_2(3)$ 上做扩展位姿预积分，行驶 $1.8\,\mathrm{km}$、历时一小时后平移误差仅约 $5\,\mathrm m$\cite{carlone2026handbook}。配合不变滤波（InEKF），群仿射动力学给出\emph{与轨迹无关}的误差动力学，一致性优于标准 EKF——其滤波推导与“群仿射 $\Rightarrow$ 误差自治”的论证见 \cref{paper:inekf}。

\begin{insight}[何时该上 $\mathrm{SE}_2(3)$ 预积分]\label{ins:imu-se23-when}
对 $100$--$200\,\mathrm{Hz}$ 常规 MEMS、$0.1$--$1\,\mathrm s$ 关键帧间隔、近地小范围，Forster 分量预积分（\cref{sec:imu-preint}）足够，$\mathbf N,\mathbf J$ 修正被噪声淹没、不必上群。当 IMU 为\emph{导航级}、窗口\emph{长}、运动\emph{剧烈}（区间内旋转大）、需\emph{严谨一致性}（接 InEKF）或要纳入\emph{地球自转}时，$\mathrm{SE}_2(3)$ 预积分的整体视角才显出价值。两者是同一预积分的两副面孔（\cref{tab:imu-se23-vs-forster}），按需取用，而非彼此替代。
\end{insight}

\begin{practice}
\begin{enumerate}
  \item 由 \cref{eq:imu-Nmatrix} 的级数证明 $\mathbf N(\mathbf 0)=\mathbf I$，并验证 $\bar{\boldsymbol\omega}_j\to\mathbf 0$ 时 \cref{eq:imu-se23-meanprop} 退回欧拉更新 \cref{eq:imu-euler-meas}（即 \cref{ins:imu-se23-fusion1}）。
  \item 直接做 $5\times5$ 分块乘法验证 \cref{eq:imu-timemachine} 的三条群性质（封闭/逆/交换），并说明为何 $\mathbb T$ 是交换群而 $\mathrm{SE}_2(3)$ 不是。
  \item 把 \cref{eq:imu-Xi} 的三行与 Forster 单步增量（\cref{eq:imu-rel-increment} 的被加项）逐项对应，指出多出来的 $\mathbf J_j,\mathbf N_j$ 各修正了什么。
  \item （综合）论证 $\mathrm{SE}_2(3)$ 残差 $\mathbf e_T$（\cref{eq:imu-se23-error}）与 Forster 残差 \cref{eq:imu-residual} 在小残差下一阶等价（提示：对群对数做一阶展开，分量读出）。
\end{enumerate}
\end{practice}

\subsection{学习增强的惯性里程计}
\label{subsec:advanced-misc}

\paragraph{动机：视觉缺位时纯惯性必发散。}
...
\begin{table}[H]\centering\small
\caption{本章知识点总表}
\begin{tabularx}{\linewidth}{p{0.04\linewidth}LLcc}
\toprule
\# & 知识点 & 核心要点 & 难度 & 脉络层 \\
\midrule
...
\texttt{...::deltaRij()/\allowbreak deltaVij()/\allowbreak deltaPij()} & 取 $\Delta\tilde{\mathbf R},\Delta\tilde{\mathbf v},\Delta\tilde{\mathbf p}$ \\
...
\texttt{Rot3::Expmap/\allowbreak Logmap} & $\mathrm{Exp}/\mathrm{Log}$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{版本信息.}\;本章代码示例对照 GTSAM 4.x 的 \texttt{ImuFactor}/\allowbreak\texttt{PreintegratedImuMeasurements} 与 VINS-Mono 的 \texttt{IntegrationBase};\,符号约定见本章「记号与约定」note。
