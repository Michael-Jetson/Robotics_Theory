...
\textbf{错误}：第 1 步“按宽高比取整定初始节点数”写成 \texttt{nIni = round((maxX-minX)/\allowbreak (maxY-minY))}，当该金字塔层的图像\emph{高远大于宽}（宽高比 $<0.5$）时取整得 $0$。
\textbf{现象}：随后用初始节点宽 \texttt{hX=(maxX-minX)/\allowbreak nIni} 时除以 $0$，越界或崩溃；这是 ORB-SLAM2 源码注释里都标注的已知边界 bug。
...
下面用 OpenCV 提取并匹配 ORB，注意匹配筛选准则与经验阈值\footnote{本段代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak orb\_cv.\allowbreak cpp}\cite{gaoxiang2019slam14}，下同。}。
...
为看清描述子内部，下面手写一版 $256$ 位描述子（$8\times$ 32 位 \texttt{uint32}）：用质心法的 $\sin/\cos$ 避开 \texttt{arctan}，用 SSE 指令 \texttt{\_mm\_popcnt\_u32} 一条指令数汉明距离\footnote{代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak orb\_self.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
\textbf{正确}：二进制描述子用 \texttt{BruteForce-Hamming}/\allowbreak\texttt{NORM\_HAMMING}；浮点描述子用 \texttt{NORM\_L2}。自检：打印 \texttt{Min/\allowbreak Max dist}，二进制描述子的距离应是\emph{整数}且 $\le$ 位数。
\end{pitfall}
...
\subsection{生产级匹配：四类搜索策略与方向一致性检验}
\label{sec:vo-match-strategies}

\paragraph{动机：真实系统几乎不裸用暴力匹配。}
\cref{sec:vo-match} 的暴力匹配 $+$ 比率筛选是理解匹配的起点，但生产级视觉 SLAM 几乎不直接拿它跑“全图 $\times$ 全图”：单目初始化要提取平时跟踪 $2$ 倍（ORB-SLAM2）乃至 $5$ 倍（ORB-SLAM3）的特征点，暴力匹配的计算量随之飙升；更糟的是“毫无目的”地两两比对会产生大量误匹配——\emph{吃力不讨好}\cite{murartal2017orbslam2}。破解之道是\emph{利用一切可用先验}（相邻帧靠近、已有位姿、共视同场景）把搜索范围从“全图 $\times$ 全图”压到一个小邻域。按手头先验不同，ORB-SLAM2 给出四类搜索策略，下面逐一对照。

\paragraph{(1) 区域搜索匹配（\texttt{SearchForInitialization}）。}
\emph{先验：两帧接近，但无位姿、无地图点}\cite{murartal2017orbslam2}。用于\emph{单目初始化}：此时既无地图点、也无相机运动估计，只知参与初始化的两帧在时间与空间上都很接近。于是对参考帧中坐标 $(x_1,y_1)$ 的特征点，认为它在当前帧的对应点应落在以 $(x_1,y_1)$ 为圆心、半径 $r$ 的邻域内（源码用边长 $2r$ 的正方形，按圆形更合理）。因初始化不知运动幅度，$r$ 取得较大（代码取 $100$ 像素，$\pi r^2\approx 3.1\times 10^4$ 像素）；逐像素比对代价过高，故复用 \cref{sec:vo-orb-quadtree} 提特征时建好的网格 \texttt{mGrid}（默认 $64\times 48$ 像素一格）做 \texttt{GetFeaturesInArea} 区域检索，把“按像素”降为“按网格”。

\paragraph{(2) 词袋匹配（\texttt{SearchByBoW}）。}
\emph{先验：同场景共视，但无相对位姿}\cite{galvez2012bags}。两关键帧看到同一场景、但视角差大且没有可用的相对位姿（如\emph{刚初始化后跟踪参考关键帧}、\emph{闭环计算 Sim(3) 前的关键帧间匹配}、\emph{局部建图中两关键帧三角化新点}~\texttt{SearchForTriangulation}）。词袋（Bag of Words，BoW）把图像里的特征点当作无序“单词”，只关心某单词出没与频次、不关心其像素位置与先后顺序，从而高效索引（\cref{sec:loop-bow} 详述）。匹配时只在词汇树\emph{同一节点}下的特征点之间比对描述子——相当于预先做了一次粗区域划分，既提速又压低误匹配率（DBoW2 在 $26\,292$ 帧上 false positive 为 $0$）。代价是需\emph{离线训练字典树}、启动时\emph{加载字典}（约一百多兆字节）、每帧\emph{在线生成} \texttt{FeatureVector}/\allowbreak\texttt{BowVector}。

\paragraph{(3) 投影匹配（\texttt{SearchByProjection}）。}
\emph{先验：已有（哪怕粗略的）位姿}\cite{murartal2017orbslam2}。这是 ORB-SLAM2 中\emph{用得最多}的匹配方式。一旦有了位姿估计，就把三维地图点经位姿变换到当前相机系、再用针孔模型（\cref{sec:vo-pnp-ba}）投影到像平面，只在投影点的小邻域圆内找候选；位姿未估出时可先用一个初始猜测投影（不准也无妨，后续会继续优化）。按地图点来源不同有多个重载：\emph{恒速模型跟踪}（投上一普通帧的地图点）、\emph{局部地图跟踪}（投所有局部地图点）、\emph{重定位}（投候选关键帧的地图点）、\emph{闭环}（投闭环帧及其共视帧的地图点）。

\paragraph{(4) Sim(3) 相互投影匹配（\texttt{SearchBySim3}）。}
\emph{先验：已估出相似变换 Sim(3)}\cite{murartal2017orbslam2}。专用于\emph{闭环}。闭环候选帧与当前帧间隔久、无先验，最初只能靠词袋匹配（策略 (2)），而词袋会\emph{漏匹配}；成功闭环又需尽量多的匹配以稳健求解。于是在用 (2) 得到初匹配并解出相似变换 $\mathbf{S}_{21}\in\mathrm{Sim}(3)$（含尺度因子 $s$，矩阵形式与求解见 \cref{sec:loop-sim3}）后，再做\emph{双向}投影：把 $\mathrm{KF}_1$ 的地图点用 $\mathbf{S}_{21}$ 投到 $\mathrm{KF}_2$、把 $\mathrm{KF}_2$ 的地图点用 $\mathbf{S}_{12}$ 投到 $\mathrm{KF}_1$，分别在投影点邻域取描述子最近的候选；只有\emph{两次互投影都成立}的点对才作为新增的可靠匹配。它在已匹配之外专挖新匹配，为闭环校正补足约束。

\begin{table}[H]\centering\small
\caption{四类特征匹配策略的适用场景对照（ORB-SLAM2）}
\label{tab:vo-match-strategies}
\begin{tabular}{p{0.17\textwidth}p{0.19\textwidth}p{0.19\textwidth}p{0.31\textwidth}}
\toprule
策略（函数） & 所需先验 & 搜索范围限定 & 典型使用场景 \\
\midrule
区域搜索 \texttt{SearchForInitialization} & 两帧接近，无位姿、无地图点 & 原图固定半径圆 $+$ 网格 \texttt{mGrid} & 单目初始化 \\
词袋匹配 \texttt{SearchByBoW} & 同场景共视，无相对位姿 & 词汇树同一节点 & 初始化后跟参考帧、闭环 Sim(3) 前、三角化新点 \\
投影匹配 \texttt{SearchByProjection} & 已有（粗略）位姿 & 地图点投影点的邻域圆 & 恒速跟踪、局部地图跟踪、重定位、闭环投影 \\
Sim(3) 互投影 \texttt{SearchBySim3} & 已估出 Sim(3)（含尺度） & 双向 Sim(3) 投影点邻域 & 闭环：挖掘更多匹配以稳健校正 \\
\bottomrule
\end{tabular}
\end{table}

\begin{note}[四类策略共享同一套“候选 $\to$ 筛选”后处理]
四类策略的差别只在\emph{如何缩小搜索范围}；一旦取到候选，筛选准则是一致的三条：\textbf{条件 1}~最优匹配的描述子距离与次优之比小于设定比率（Lowe 比率测试，剔“模棱两可”）；\textbf{条件 2}~最优描述子距离小于阈值（剔“整体不像”）；\textbf{条件 3}~通过\emph{方向一致性检验}（下文，剔“与主导旋转不符”）。前两条是\emph{描述子级}判据，第三条是\emph{几何统计级}判据，三者叠加才得到干净匹配。
\end{note}

\paragraph{投影匹配的金字塔层级：近大远小。}
投影匹配还有一处工程细节\cite{murartal2017orbslam2}：恒速模型跟踪时，要按相机\emph{沿光轴前进或后退}来选候选点所在的金字塔层级，否则尺度不匹配会降低召回。记当前帧、上一帧的“世界 $\to$ 相机”外参为 $\mathbf{T}_{cw},\mathbf{T}_{lw}$（此处用相机外参约定，与本书主用的 $\mathbf{T}_{wc}$ 相反），则当前帧相对上一帧的位姿与平移为
\begin{equation}\label{eq:vo-match-tlc}
  \mathbf{T}_{lc}=\mathbf{T}_{lw}\mathbf{T}_{cw}^{-1},\qquad
  \mathbf{t}_{lc}=-\mathbf{R}_{lw}\mathbf{R}_{cw}^{\top}\mathbf{t}_{cw}+\mathbf{t}_{lw}.
\end{equation}
用平移 $\mathbf{t}_{lc}$ 的\emph{光轴（$z$）分量}与相机\emph{基线}比较即可判前进/后退。依“近大远小”：相机\emph{前进}时，上一帧 \texttt{level=0} 的特征在当前帧需到\emph{更高层} \texttt{level=1} 去匹配；\emph{后退}则反之。该判定只对\emph{双目/RGB-D}（有绝对尺度、已知基线）成立；\emph{单目}无绝对尺度，故不判前后，搜索层级限定在当前金字塔层 $\pm 1$ 范围内。

\paragraph{方向一致性检验：用旋转直方图剔误匹配。}
经条件 1、2 的描述子级筛选后\cite{murartal2015orbslam}，仍会残留误匹配（局部相似、重复纹理所致）。ORB-SLAM 在 RANSAC 之外再加一道\emph{廉价的统计闸}——方向一致性检验：它不看几何重投影，只统计所有匹配对的\emph{主方向差}，用一个直方图把“随大流的内点”和“离群的误匹配”分开。

\begin{proposition}[方向一致性 $\Rightarrow$ 旋转直方图]\label{prop:vo-rothist}
设两帧之间为整体刚体运动。对任一\emph{正确}匹配 $(\mathbf{x}_1,\mathbf{x}_2)$，其 Oriented FAST 主方向（\cref{eq:vo-orientation}）之差 $\Delta\theta=\theta_1-\theta_2$ 近似等于由相机绕光轴旋转分量决定的同一常值 $\Delta\theta^\star$；而\emph{误}匹配的 $\Delta\theta$ 在 $[0^\circ,360^\circ)$ 上近乎均匀散布。故将所有匹配的 $\Delta\theta$ 投入 $L=30$ 个等宽 bin 的直方图后，正确匹配聚成尖峰、占据计数最多的少数几个 bin，误匹配则淹没在其余 bin 的低计数背景里。保留落入计数最高的\emph{前三个} bin 的匹配、剔除其余，即以 $O(N)$ 的代价滤去大量误匹配。
\end{proposition}

\begin{insight}[军训口令“向左转”：为何取前三个 bin]
把每个特征匹配想成军训队列里的一个人，主方向差 $\Delta\theta$ 就是他听令后的转身角度。教官喊“向左转”，绝大多数人都左转约 $90^\circ$——幅度略有出入，于是聚在直方图里相邻的两三个 bin（这正是取“前三大 bin”而非“最大那一个”的来由）。可总有极个别分不清左右的人右转（$\Delta\theta\approx-90^\circ$）或干脆不转（$\Delta\theta\approx 0^\circ$）；这种人占比极低，对应 bin 的计数很小，落在前三大 bin 之外，恰是要剔除的误匹配。直方图把“整体一致的转身”与“个别离群”一眼分开。
\end{insight}

\begin{algo}[\texttt{SearchForInitialization}：区域搜索 $+$ 方向一致性检验]\label{alg:vo-rothist}
输入：参考帧 $F_1$、当前帧 $F_2$ 的特征点与描述子，搜索窗 \texttt{windowSize}（初始化取 $100$ 像素）。
\begin{enumerate}
  \item \textbf{构建旋转直方图}：开 $L=\texttt{HISTO\_LENGTH}=30$ 个桶，比例因子 $\mathrm{factor}=L/360$\footnote{ORB-SLAM2 原始源码此处误写 $\mathrm{factor}=1/L$，应为 $L/360$ 才能把 $[0^\circ,360^\circ)$ 线性映射到桶号 $[0,L)$。}。
  \item \textbf{区域取候选}：对 $F_1$ 每个特征点，用网格法 \texttt{GetFeaturesInArea} 在 $F_2$ 中以其坐标为圆心、\texttt{windowSize} 为半径取候选（\cref{sec:vo-orb-quadtree} 的 \texttt{mGrid} 加速）。
  \item \textbf{最优/次优}：在候选中找描述子汉明距离最小（最优 \texttt{bestDist}）与次小（次优 \texttt{bestDist2}）。
  \item \textbf{描述子级筛选}：要求 $\texttt{bestDist}\le\texttt{TH\_LOW}$（阈值，条件 2）且 $\texttt{bestDist}<\texttt{mfNNratio}\cdot\texttt{bestDist2}$（最优显著优于次优，Lowe 比率，条件 1）；双向建立匹配并剔除重复占用。
  \item \textbf{累计主方向差}：$\Delta\theta=\theta_1-\theta_2$（单位为度），若 $\Delta\theta<0$ 则加 $360$；桶号 $\mathrm{bin}=\mathrm{round}(\Delta\theta\cdot\mathrm{factor})$，若 $\mathrm{bin}=L$ 则环绕回 $0$；把该匹配记入 \texttt{rotHist[bin]}。
  \item \textbf{直方图剔误}：\texttt{ComputeThreeMaxima} 取计数最多的三个 bin $\{i_1,i_2,i_3\}$；凡 $\Delta\theta$ 不落入这三个 bin 的匹配判为误匹配并删除（条件 3）。
  \item \textbf{输出}：保存通过全部筛选的匹配。
\end{enumerate}
\end{algo}

\begin{codebox}[C++]{方向一致性检验核心:旋转直方图统计与“前三大 bin”剔除}
// Step 5: 把每个通过描述子筛选的匹配, 按主方向差累计进旋转直方图
const float factor = HISTO_LENGTH / 360.0f;  // 注: 原始源码误写 1.0f/HISTO_LENGTH
if (mbCheckOrientation) {
    // 主方向差, 单位是“度”而非弧度
    float rot = F1.mvKeysUn[i1].angle - F2.mvKeysUn[bestIdx2].angle;
    if (rot < 0.0) rot += 360.0f;
    int bin = round(rot * factor);           // 落入第几个 bin
    if (bin == HISTO_LENGTH) bin = 0;        // 角度环绕: 360 与 0 同桶
    rotHist[bin].push_back(i1);              // 记下该匹配在 F1 的索引
}

// Step 6: 只保留落在“数量最多前三个 bin”的匹配, 其余判为误匹配剔除
int ind1, ind2, ind3;
ComputeThreeMaxima(rotHist, HISTO_LENGTH, ind1, ind2, ind3);
for (int i = 0; i < HISTO_LENGTH; i++) {
    if (i == ind1 || i == ind2 || i == ind3) continue; // 主导旋转: 内点, 保留
    for (size_t j = 0; j < rotHist[i].size(); j++) {
        vnMatches12[rotHist[i][j]] = -1;     // 离群 bin: 删除该匹配
        nmatches--;
    }
}
\end{codebox}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.0]
    % 坐标轴
    \draw[->,thick] (-0.2,0) -- (12.5,0) node[right]{\footnotesize 主方向差 $\Delta\theta$（$L=30$ 个 bin）};
    \draw[->,thick] (0,-0.1) -- (0,3.8) node[above]{\footnotesize 落入该 bin 的匹配对数};
    % 离群 bin（灰）
    \foreach \x/\h in {0.5/0.4, 1.3/0.6, 2.1/0.3, 3.0/0.5, 9.0/0.4, 9.8/0.6, 10.6/0.3, 11.4/0.5}
      \fill[gray!40] (\x,0) rectangle ++(0.55,\h);
    % 主导旋转: 计数最多的前三个 bin（主色）
    \foreach \x/\h in {5.0/2.5, 5.8/3.2, 6.6/2.1}
      \fill[main!75] (\x,0) rectangle ++(0.55,\h);
    % 框住前三大 bin
    \draw[main!60!black, thick, dashed] (4.92,-0.05) rectangle (7.27,3.45);
    \node[main!60!black, font=\footnotesize] at (6.1,3.75) {前三大 bin：主导旋转（内点，保留）};
    % 离群标注
    \node[gray!65!black, font=\footnotesize] at (1.9,1.5) {离群 bin（误匹配，剔除）};
    \draw[->,gray!60] (1.9,1.3) -- (1.5,0.7);
    \draw[->,gray!60] (9.6,1.3) -- (9.9,0.7);
  \end{tikzpicture}
  \caption{方向一致性检验的旋转直方图（重画自 ORB-SLAM 的方向一致性检验原理\cite{murartal2015orbslam}）。两帧整体刚性运动使正确匹配的主方向差聚成一峰、落在计数最多的前三个 bin；散在其余 bin 的少数匹配为误匹配，予以剔除。}
  \label{fig:vo-rothist}
\end{figure}

\begin{pitfall}[方向一致性检验的适用边界]
\textbf{它不是万能去外点器。}方向一致性检验假设两帧间是\emph{整体刚体运动}：场景静止、相机自身运动主导。若画面被\emph{大量独立运动物体}占据，或相机\emph{几乎只有平移、绕光轴旋转分量近零}，则 $\Delta\theta$ 不再聚成尖峰，前三大 bin 失去“主导”含义，可能误删正确匹配或放过误匹配。\\
\textbf{三个工程陷阱}：(1)~$\theta$ 的单位是\emph{度}不是弧度，混用会把 $\mathrm{factor}$ 算错；(2)~bin 必须\emph{环绕}（$\mathrm{bin}=L\to 0$），否则 $360^\circ$ 附近的匹配会丢桶；(3)~ORB-SLAM2 源码 $\mathrm{factor}$ 的笔误（见 \cref{alg:vo-rothist} 脚注）会把所有 $\Delta\theta$ 挤进前几个 bin，自检时应打印直方图确认确有明显单峰。\\
\textbf{定位}：它是一道 $O(N)$ 的\emph{廉价统计闸}，剔掉与主导旋转明显不符的少数离群点；几何级外点仍要靠对极约束/单应一致性的 RANSAC（\cref{sec:vo-epipolar}）兜底。
\end{pitfall}

\begin{practice}
\begin{enumerate}
  \item 四类匹配策略“所需先验”各不相同（\cref{tab:vo-match-strategies}）。判断下列场景该选哪一类并说明理由：(a) 单目 SLAM 刚启动、连续两帧；(b) 跟踪丢失后在全局关键帧库里找回位姿；(c) 恒速模型预测出粗略位姿后跟踪上一帧；(d) 闭环已解出 Sim(3)、要在两关键帧间补更多匹配。
  \item 为什么方向一致性检验取\emph{前三个}最大 bin，而不是只取最大的一个？请从 \cref{prop:vo-rothist} 与军训类比两个角度回答。
  \item （编程）把 \cref{alg:vo-rothist} 的 $\mathrm{factor}$ 故意改回错误的 $1/L$，在一对实拍图上画出旋转直方图，观察前三大 bin 是否仍对应真实主导旋转，并解释错误现象。
  \item 投影匹配为何要“根据相机前进/后退选金字塔层级”？若相机明显前进却仍在同层搜索，匹配召回会如何变化？（提示：近大远小，\cref{eq:vo-match-tlc}）
\end{enumerate}
\end{practice}
...
下面用 OpenCV 算 $\mathbf{F}/\mathbf{E}/\mathbf{H}$ 并从 $\mathbf{E}$ 恢复 $\mathbf{R},\mathbf{t}$\footnote{代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak pose\_estimation\_2d2d.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
下面调 \texttt{cv::triangulatePoints} 做两视图线性三角化\footnote{代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak triangulation.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
下面手写一版高斯牛顿 PnP（更新行并列标注左乘与本书右乘版）\footnote{代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak pose\_estimation\_3d2d.\allowbreak cpp}\cite{gaoxiang2019slam14}，原实现用左乘更新。}。
...
也可用 g2o 建图：位姿为顶点、每个 3D 点的投影为一元边\footnote{代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak pose\_estimation\_3d2d.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
OpenCV 无带匹配的 ICP，下面自实现 SVD 闭式与非线性两版\footnote{代码改写自 \texttt{slambook2/\allowbreak ch7/\allowbreak pose\_estimation\_3d3d.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
下面手写单层 LK 光流，并行地对每个特征点在 patch 内装配 $\mathbf{H},\mathbf{b}$\footnote{代码改写自 \texttt{slambook2/\allowbreak ch8/\allowbreak optical\_flow.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
把上面的单层光流 \texttt{calc} 包成多层 driver：先用 \texttt{cv::resize} 构金字塔，再从顶层（最粗）向底层（最精）逐层跟踪，关键点坐标随层缩放，上一层结果作下一层初值\footnote{代码改写自 \texttt{slambook2/\allowbreak ch8/\allowbreak optical\_flow.\allowbreak cpp} 的 \texttt{OpticalFlowMultiLevel}\cite{gaoxiang2019slam14}。}。
...
下面随机选 $2000$ 点（不用任何特征提取），用视差/深度反投影，再以多层直接法估位姿\footnote{代码改写自 \texttt{slambook2/\allowbreak ch8/\allowbreak direct\_method.\allowbreak cpp}\cite{gaoxiang2019slam14}。}。
...
\end{codebox}
单层迭代：\texttt{dx=H.\allowbreak ldlt().\allowbreak solve(b)}；更新 \texttt{T21 = SE3::exp(dx)*T21;}（左乘；本书右扰动应改 \texttt{T21 = T21*SE3::exp(dx);} 并把 \texttt{J\_uxi} 换成右扰动块 $[\mathbf{R},-\mathbf{R}\mathbf{P}^\wedge]$）。把它包成迭代函数 \texttt{DirectPoseEstimationSingleLayer}（构造 \texttt{JacobianAccumulator}、\texttt{parallel\_for\_} 累加、解增量、更新 \texttt{T21}），再在多层 driver 里由粗到精调用。数值例：$2000$ 点每层 $1$\textasciitilde$2$ ms、四层约 $8$ ms，比光流（十几 ms）更快；随机选点也能正确追踪。
\end{practice}

\begin{practice}[多层直接法 driver（内参随层缩放）]
直接法的雅可比 \cref{eq:vo-direct-merge} \emph{带内参} $f_x,f_y,c_x,c_y$，故金字塔缩图时\emph{内参也必须同倍缩放}（这是它与光流 driver 的关键差别）。下面的多层 driver 把内参作全局量在进每层前按 \texttt{scales[level]} 改写，参考像素同乘该尺度，而\emph{深度是物理量、绝不缩放}\footnote{代码改写自 \texttt{slambook2/\allowbreak ch8/\allowbreak direct\_method.\allowbreak cpp} 的 \texttt{DirectPoseEstimationMultiLayer}\cite{gaoxiang2019slam14}。}。
...
\texttt{H.\allowbreak ldlt().\allowbreak solve(b)} & 解对称正规方程 $\mathbf{H}\delta=\mathbf{b}$（Cholesky） \\
