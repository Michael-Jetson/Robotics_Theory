...
\paragraph{光学绝对编码器与 Gray 码：分辨率从何而来。}
旋转编码器按工作原理（光学/磁式）、读数方式（绝对/增量）、输出（模拟/数字）分类；足式机器人最常用\emph{光学数字}编码器\cite{carlone2026handbook}。\textbf{绝对式}的原理是：一束红外光（如 LED）照向一片随转轴转动的码盘，码盘沿径向分成若干同心扇区、每区或透光或遮光；光的另一侧径向排布一列光敏元件（如光敏电阻），把“某扇区此刻有无光”读成一个二进制位。$n$ 个同心环就并出一个 $n$ 位二进制字，对应 $2^n$ 个可分辨的角度区间，故\textbf{角分辨率}
\[
  \Delta\theta=\frac{360^\circ}{2^n}.
\]
这些二进制字\emph{不}按普通二进制排列，而按 \textbf{Gray 码}排列——相邻自然数对应的码字\emph{只差一位}（\cref{tab:leg-graycode}）。这样当光敏列扫过扇区边界时至多一位翻转，避免了普通二进制里多位同时切换（如 $011\to100$ 三位齐变）因各位翻转不同步而造成的瞬态\emph{误读}。

\begin{table}[H]\centering\small
\caption{$3$ 位 Gray 码与标准二进制对照。Gray 码相邻行只差一位（含末行 $100$ 绕回首行 $000$ 亦只差一位），而标准二进制可能多位齐变（如 $3\!\to\!4$：$011\to100$ 三位全变）。绝对编码器用 Gray 码正为消除跨扇区时的多位瞬态误读。}
\label{tab:leg-graycode}
\begin{tabular}{ccc}
\toprule
十进制 & 标准二进制 & Gray 码 \\
\midrule
...
\begin{example}[$8$ 位编码器的角分辨率]\label{ex:leg-encoder}
取一片 $8$ 位光学绝对编码器（$8$ 个同心环、$8$ 个光敏元件）。可分辨的角度区间数为 $2^8=256$，故每圈被均匀量化成 $256$ 格，角分辨率
\[
  \Delta\theta=\frac{360^\circ}{2^8}=\frac{360^\circ}{256}\approx1.41^\circ.
\]
即该器件区分不了小于约 $1.41^\circ$ 的转角——这正是 \cref{eq:leg-encoder-noise} 中编码器量化噪声 $\boldsymbol{\eta}_q$ 的物理来源、也是 \cref{subsec:leg-encoder-noise} 协方差 $\boldsymbol{\Sigma}_q$ 的根。要更细只需加同心环：每多 $1$ 位分辨率折半（$12$ 位 $\to360^\circ/4096\approx0.088^\circ$），代价是码盘刻蚀工艺与成本上升。这条“位数 $\leftrightarrow$ 分辨率”的指数关系，是选编码器时的第一权衡。
\end{example}

\noindent \textbf{增量式}编码器则只测相对开机初始构型的角度变化：上电置零，码盘上等距径向开槽，单个光敏元件 $A$ 在码盘匀速转动时输出一列方波；第二个光敏元件 $B$ 与 $A$ 相差 $90^\circ$ 相位，二者合成的 $2$ 位字 $AB$ 在四种取值间跳变，由跳变的先后（如 $A$ 的上升沿先于 $B$ 的下降沿、$AB$ 由 $11$ 变到 $10$）判转向并据此增减计数。它无需 Gray 码盘、构造更简单成本更低，历史上常用“低分辨率绝对编码器先定初值 + 高分辨率增量编码器再累加”的组合；但随绝对式分辨率提升、成本下降，增量式正被快速取代。
...
\subsection{为何只阈值化 \texorpdfstring{$f_z$}{f_z}}
\label{subsec:leg-fz}
...
不同机器人的形态、设计、集成约束未必允许在脚上加力/接触传感器。此时可借\textbf{浮基动力学}，从关节力矩 $\boldsymbol{\tau}$ 反演地面反力（GRF）。本节先\emph{自包含地}写出浮基铰接体的完整动力学方程——它与 \cref{sec:ctrl-legged} 足式控制用的是\emph{同一套}方程（又一处“一体两面”，\cref{ins:leg-control-duality}），但估计侧不再把它外包给控制章，而是就地写清、再从中解出逐腿 GRF。

\paragraph{浮基铰接体动力学：两条耦合方程合一。}
足式机器人是一个\textbf{浮基铰接体系统}（floating-base articulated body）：一个不受驱动的 $6$ 自由度浮动基座（机身），通过 $N$ 个主动关节连着 $N$ 个刚体连杆（例中 $N{=}12$）。它的动力学由两条\emph{耦合}方程描述——一条管浮基（$6$ DoF，欠驱动）、一条管 $N$ 个主动 DoF，二者皆可用递推 Newton--Euler 算法\cite{featherstone2008rigid} 高效求得。把两条方程并成矩阵形式：
\begin{equation}
  \mathbf{M}(\mathbf{q})\begin{bmatrix}\dot{\mathbf{v}}\\\dot{\boldsymbol{\omega}}\\\ddot{\mathbf{q}}\end{bmatrix}+\mathbf{h}(\mathbf{q},\dot{\mathbf{q}})=\begin{bmatrix}\mathbf{J}_b^\top\\\mathbf{J}_q^\top\end{bmatrix}\mathbf{f}+\begin{bmatrix}\mathbf{0}_6\\\boldsymbol{\tau}\end{bmatrix}.
  \label{eq:leg-floating-dyn}
\end{equation}
逐项看：$\mathbf{M}(\mathbf{q})$ 是\textbf{质量（惯性）阵}，乘在加速度堆叠 $[\dot{\mathbf{v}};\dot{\boldsymbol{\omega}};\ddot{\mathbf{q}}]$ 上——其中 $\dot{\mathbf{v}},\dot{\boldsymbol{\omega}}\in\mathbb{R}^3$ 是浮基的线、角加速度、$\ddot{\mathbf{q}}\in\mathbb{R}^{12}$ 是主动关节加速度；$\mathbf{h}(\mathbf{q},\dot{\mathbf{q}})\in\mathbb{R}^{18}$ 是\textbf{偏置项}，收纳 Coriolis、离心与重力效应。等号右端：末项 $[\mathbf{0}_6;\boldsymbol{\tau}]$ 是广义驱动力矩——浮基不受驱动故上 $6$ 维恒为零，下 $12$ 维为主动关节力矩 $\boldsymbol{\tau}\in\mathbb{R}^{12}$；倒数第二项 $[\mathbf{J}_b^\top;\mathbf{J}_q^\top]\mathbf{f}$ 把脚处接触力/力矩 $\mathbf{f}$ 经雅可比转置映成广义力——\emph{这一项对腿式里程计最关键，因为待估的接触力就藏在里面}。

\paragraph{接触雅可比与接触力的堆叠（维度随构型而变）。}
设有 $c$ 条腿触地、每腿 $d$ 个主动关节（点足四足 $d{=}3$、平足人形 $d{=}6$）。则 $\mathbf{J}_b\in\mathbb{R}^{dc\times6}$ 是把\textbf{机身旋量}映到各触地脚速度的\emph{机身雅可比}（依正运动学与机身在 $\mathcal{F}^w$ 中的绝对朝向而定），而 $\mathbf{J}_q\in\mathbb{R}^{dc\times12}$ 是各腿雅可比 \cref{eq:leg-jacobian} 的\textbf{堆叠}，其排布随机器人类型与触地腿数而变。例如双腿站立的人形把两腿完整的 $6$ 维雅可比叠起来；而四足因脚可绕接触点转动、只约束位置，$\mathbf{J}_q$ 只取各腿的\emph{线}雅可比 $\mathbf{J}_{i,v}$（如站在第 $1,3,4$ 腿上的四足）：
\begin{equation}
  \mathbf{J}_q^{\text{hum}}=\begin{bmatrix}\mathbf{J}_1(\mathbf{q})\\\mathbf{J}_2(\mathbf{q})\end{bmatrix},\qquad
  \mathbf{J}_q^{\text{quad}}=\begin{bmatrix}\mathbf{J}_{1,v}(\mathbf{q})\\\mathbf{J}_{3,v}(\mathbf{q})\\\mathbf{J}_{4,v}(\mathbf{q})\end{bmatrix}.
  \label{eq:leg-Jq-stack}
\end{equation}
相应地，接触力堆叠 $\mathbf{f}\in\mathbb{R}^{dc}$ 收齐各触地脚的力/力矩：四足四脚着地时是四个\emph{线}力的堆叠，人形双脚着地时则每脚兼含作用在接触点三轴上的力\emph{与}力矩：
\begin{equation}
  \mathbf{f}^{\text{quad}}=\begin{bmatrix}\mathbf{f}_1\\\mathbf{f}_2\\\mathbf{f}_3\\\mathbf{f}_4\end{bmatrix},\qquad
  \mathbf{f}^{\text{hum}}=\begin{bmatrix}\mathbf{f}_1\\\boldsymbol{\tau}_1\\\mathbf{f}_2\\\boldsymbol{\tau}_2\end{bmatrix}.
  \label{eq:leg-f-stack}
\end{equation}
这条“点足只堆线分量、平足兼堆力与力矩”的差别，正是 \cref{ins:leg-two-measurements} 中点足/平足分野在动力学侧的回声。

\begin{derivation}[从浮基动力学解出逐腿 GRF]
把 \cref{eq:leg-floating-dyn} 按上 $6$ 行（浮基）、下 $12$ 行（关节）拆成两块。浮基那 $6$ 行因不受驱动（右端上块为 $\mathbf{0}_6$），是基座自身的运动方程；\emph{关节那 $12$ 行}才把关节力矩、偏置与接触力联系起来，正是反演 GRF 的入口：
\[
  \mathbf{M}_q\begin{bmatrix}\dot{\mathbf{v}}\\\dot{\boldsymbol{\omega}}\\\ddot{\mathbf{q}}\end{bmatrix}+\mathbf{h}_q=\mathbf{J}_q^\top\mathbf{f}+\boldsymbol{\tau},
\]
其中 $\mathbf{M}_q,\mathbf{h}_q$ 是质量阵、偏置项的下 $12$ 行。利用 $\mathbf{J}_q$ 的\textbf{块状结构}——每条腿的雅可比只占自己那 $d$ 列、互不耦合（\cref{eq:leg-jac-block}）——可\emph{逐腿}抽出第 $i$ 腿对应的 $d$ 行。对四足（$d{=}3$），把该腿关节行的惯性项归并、并保留其中与机身线加速度耦合的主导块 $\mathbf{F}^\top\dot{\mathbf{v}}$（$\mathbf{F}$ 为质量阵的一个 $3\times3$ 块），第 $i$ 腿的关节行约去其余腿后只剩
\[
  \bar{\mathbf{J}}_{i,v}^\top\,\mathbf{f}_i=\boldsymbol{\tau}_i-\mathbf{h}_{q,i}-\mathbf{F}^\top\dot{\mathbf{v}}.
\]
四足的非零块 $\bar{\mathbf{J}}_{i,v}$ 是\emph{方阵且可逆}（\cref{eq:leg-jac-block}），两边左乘 $(\bar{\mathbf{J}}_{i,v}^\top)^{-1}$ 即得逐腿 GRF：
\begin{equation}
...
浮基动力学 / GRF & Pinocchio \texttt{rnea}/\allowbreak\texttt{aba}（递推 Newton--Euler / ABA）；\cref{eq:leg-grf-from-torque} 反演 \\
李群 / 右扰动 & Sophus \texttt{SO3d/\allowbreak SE3d::exp/\allowbreak log}；接触/速度因子残差用 $\mathrm{Log}$ + 右扰动 \\
因子图 / 平滑 & GTSAM \texttt{ISAM2}/\allowbreak\texttt{FixedLagSmoother}、\texttt{ImuFactor}（预积分）、自定义 leg factor（\texttt{NoiseModelFactor}） \\
