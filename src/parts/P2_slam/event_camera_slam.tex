...
\paragraph{选型：当前器件谱系与演进趋势。}
“做事件 SLAM 该买/用哪款相机？”——进入这一新兴领域者常问此问，各款器件的参数也常以对照表横比\cite{gallego2022eventsurvey}。要点先摆明：事件传感器设计虽多，\emph{多数仍停在实验室原型}，真正商品化、足以支撑事件 SLAM 这类新老问题探索的只是少数，主要出自 SONY、Samsung、iniVation/SynSense、Prophesee、Omnivision 等厂商\cite{carlone2026handbook}。几条值得留意的演进趋势：
\begin{itemize}
  \item \textbf{像素间距（pixel pitch）持续缩小}：追随传统相机的“百万像素竞赛”与工业需求，像素间距从 DVS128 的 $40\,\mu\mathrm m$ 一路降到 $<5\,\mu\mathrm m$。但 DVS 像素要做调制、比较等\emph{比传统像素更多}的操作，需更多晶体管、更难在同等面积内塞下；为最大化\emph{填充因子}（像素受光面积占比）、缩小感光部分之间的间隙，普遍转向\emph{堆叠式工艺}（stacked technology）与\emph{背照式}（backside illumination）。
  \item \textbf{HD 机型砍掉灰度输出}：早期 DAVIS、ATIS 同时输出灰度图（对\emph{静止}相机的应用有用，但这并非 SLAM 的常态）；新一代 HD（百万像素）事件相机为给事件输出腾出面积、顺应百万像素竞赛，\emph{放弃了}并发的灰度输出。
  \item \textbf{彩色与惯性是可选项}：颜色对许多运动相关任务并非必需，仅少数机型加 RGB 滤光以分通道感知变化；另有部分相机在同一器件内集成 IMU——惯性是与视觉互补的本体感受传感器，直接支撑事件-惯性里程计（VIO）与 SLAM，较单传感器系统更鲁棒、更准。
\end{itemize}

\begin{insight}[高分辨率 $\ne$ 更适合 SLAM：HD 的实时瓶颈与 QVGA 偏好]
“空间分辨率越高越好”是事件相机选型最常见的误区。捕捉精细空间细节固然重要，但\emph{噪声}与\emph{带宽}在具体任务需求里同样举足轻重。SLAM 场景中事件相机常\emph{快速运动}、面对\emph{高纹理}，此时 HD（百万像素）相机每秒可吐出\emph{数亿}事件，足以\emph{饱和}相机的输出传输总线、压垮处理端——\emph{当前没有任何算法-硬件组合能实时处理}这种事件率（除非退而求其次做阵列式转换或下/降采样）\cite{carlone2026handbook}。正在研制的新型混合传感器（事件分辨率低于强度输出）与\emph{中央凹}（foveated）传感器，正试图按生物视觉之道压低带宽。因此在 SLAM 中，\emph{较低像素分辨率（如 QVGA）反更受青睐}：便于算法原型、也利于算力受限机器人上的实时运行；而\emph{视场（光学）}的选择，往往与像素数同等重要。
\end{insight}
...
\paragraph{模拟器：从 ESIM 到驾驶级与对抗式生成。}
学习与基准都吃高质量数据，而真实事件数据因传感器新、采集难而\emph{稀缺}，故\emph{模拟器}（把常规视频/渲染图转成合成事件）至关重要。这一脉的奠基是 \textbf{ESIM}\cite{rebecq2018esim}：早期做法仅对相邻两帧\emph{阈值化}差分得到类边缘图来“假装”事件，而 ESIM \emph{紧耦合}渲染引擎与事件生成器，能依视觉信号的动态\emph{自适应}地渲染中间帧，再按事件生成模型 \cref{eq:event-gen} 的噪声化阈值（阈值 $C\sim\mathcal N(C,\sigma_C^2)$、允许正负阈值 $C^+\!\ne\! C^-$）产生事件\cite{carlone2026handbook}。其后衍生出一族各有侧重的工具（\cref{tab:event-sims}）：\textbf{CARLA} 的事件相机模块把 ESIM 扩展到\emph{自动驾驶}的多样、丰富、复杂场景（取仿真器渲染图、逐像素算亮度变化，原理同 ESIM），服务于“从事件学单目深度”等任务；\textbf{Vid2E} 把任意常规视频经自适应插帧（Super-SloMo）再过 ESIM，化解“真实事件稀缺”，但只建模\emph{理想}事件相机；\textbf{V2E} 进一步按 DVS 电路建模\emph{真实噪声}，是首个含时间噪声、漏电事件（leak events）、有限且依光强的带宽的模拟器；\textbf{EventGAN} 改走\emph{对抗式}深度学习路线——用 U-Net 编解码网络从\emph{一对}时序图像生成\emph{每极性一张}的 3D 时空体素网格（而非一串离散事件），以判别器对抗损失加循环一致损失训练，体素网格正是喂给 ANN 的常用表示；\textbf{VISTA 2.0} 把 RGB、激光、事件多种传感器同纳一个仿真器、面向自动驾驶的\emph{感知-控制}策略学习与 sim-to-real（其事件模块仿 ESIM 自适应采样，双向光流由 ANN 估计）；\textbf{V2CE} 则专攻\emph{时间戳真实性}——前述工具多在\emph{离散}时刻吐事件，与真实的连续事件存在域偏移，V2CE 用两段式（3D U-Net 先出双极性体素，再反解出每个体素内事件的精确相对时间戳）逼近连续时间戳，并能在饱和光区与理想生成模型失效的边缘处仍生成事件\cite{carlone2026handbook,gallego2022eventsurvey}。

\begin{table}[H]\centering\small
\caption{事件相机模拟器一脉（按思路归纳，非穷举）}
\label{tab:event-sims}
\begin{tabular}{p{0.13\linewidth}p{0.52\linewidth}p{0.22\linewidth}}
\toprule
模拟器 & 核心思路 & 定位/特点 \\
\midrule
ESIM & 渲染引擎与事件生成紧耦合、按信号动态自适应渲染；$C\sim\mathcal N(C,\sigma_C^2)$、$C^+\!\ne\! C^-$ 噪声 & 奠基，多数后续工具的内核 \\
CARLA & 把 ESIM 扩到自动驾驶的多样复杂场景；取渲染图逐像素算亮度变化 & 驾驶级合成事件 \\
Vid2E & 任意视频经自适应插帧（Super-SloMo）$+$ ESIM；仅理想光照模型 & 解决真实事件稀缺 \\
V2E & 按 DVS 电路建模真实噪声：时间噪声、漏电事件、有限带宽 & 缩小 sim-to-real 间隙 \\
EventGAN & 对抗式 U-Net 从一对图像生成每极性 3D 体素；判别器 $+$ 循环一致损失 & 学习式、输出体素表示 \\
VISTA 2.0 & RGB/激光/事件多传感器一体；事件模块仿 ESIM，双向光流由 ANN 估 & 自动驾驶策略学习、sim-to-real \\
V2CE & 两段式（双极性体素 $\to$ 反解时间戳）逼近\emph{连续}时间戳 & 攻时间戳真实性、降域偏移 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{数据集：从 ECDS 到多平台多模态。}
数据集一脉以 \textbf{ECDS}\cite{mueggler2017ecds}（首个含同步事件 $+$ IMU $+$ 6-DoF 真值的数据集，位姿真值由动捕给出）为事实起点，此前的数据多限于纯旋转（3-DoF）或低自由度、真值靠 IMU 或编码器/轮速（易漂）。其后 MVSEC（首个跨平台含深度真值）、UZH-FPV（无人机竞速）、DSEC（驾驶级、Prophesee VGA 双目 $+$ 激光 $+$ RTK-GPS）、TUM-VIE（百万像素双目 $+$ 200\,Hz IMU）、EDS（分束器使 RGB 与事件\emph{共光轴}）、VECtor、EV-IMO（独立运动物体分割）、M3ED 等相继出现，覆盖手持/无人机/车/四足、单双目、室内外、低光/HDR 等场景\cite{gallego2022eventsurvey,carlone2026handbook}。

\paragraph{评估度量：定位与建图误差交织。}
理想上 SLAM 应\emph{分别}刻画定位与建图模块的质量；但实践中二者\emph{交织}——深度误差牵动位姿误差、反之亦然，且 6-DoF 定位真值远比稠密深度真值\emph{紧凑、易获取}，故\emph{深度误差常被并入轨迹误差}一并评估\cite{carlone2026handbook}。既然事件 SLAM 同样输出相机轨迹，它便\emph{继承}经典 SLAM 的评估协议（沿用 \cref{ch:vo}），核心度量见 \cref{tab:event-metrics}。

\begin{table}[H]\centering\small
\caption{事件 SLAM 评估度量（继承经典 SLAM，辅以速度/深度度量）}
\label{tab:event-metrics}
\begin{tabular}{p{0.22\linewidth}p{0.46\linewidth}p{0.20\linewidth}}
\toprule
度量 & 定义 / 计算 & 侧重 \\
\midrule
ATE（平移）& 对齐到固定世界系后，估计与真值\emph{位置}的欧氏距离（绝对轨迹误差）& 长期 / 全局精度 \\
ATE（旋转）& 估计与真值\emph{姿态}在 $\SO(3)$ 上的\emph{测地距离} & 长期 / 全局精度 \\
RPE & 相邻（邻近）时刻\emph{相对}位姿之差，分平移与旋转两部分 & 局部一致性 \\
尺度归一化 & 位置误差再除以\emph{均场景深度}或\emph{总行程}，使其对场景/轨迹尺度不变 & 跨尺度可比 \\
ARPE / AEE & 两\emph{平移向量}间的位置差（平均相对位姿误差 / 平均端点误差）& 平移估计 \\
ARRE & 两\emph{旋转矩阵}间的 $\SO(3)$ 测地距离（平均相对旋转误差）& 旋转估计 \\
线 / 角速度误差 & 平均线速度、角速度误差（位姿是速度对时间的积分）& 快速运动尤宜 \\
深度 RMSE-to-surface & 估计 3D 点到真值地图\emph{最近表面}的欧氏距离 RMSE & 建图（首选）\\
REL / completion & 相对深度误差 / 恢复点数（完成度）& 建图（补充）\\
\bottomrule
\end{tabular}
\end{table}

\noindent ATE 对齐到固定世界系、量\emph{长期/全局}漂移，RPE 量\emph{邻近}时刻的局部一致；二者平移部分取欧氏距离、旋转部分取 $\SO(3)$ 测地距离。\emph{速度误差}对事件相机尤有意义——它擅长估\emph{快速、剧烈}运动，而位姿正是线/角速度对时间的积分；社区已提供多种轨迹评估工具箱以利复现\cite{carlone2026handbook}。\emph{深度度量}仅在\emph{单独}评估建图、且数据集含深度真值时使用（含真值深度的事件数据集本就不多）：常用按若干深度截断求平均深度误差以跨尺度比较，首选\emph{估计点到真值表面最近距离}的 RMSE，辅以相对误差 REL 与恢复点数 completion。
