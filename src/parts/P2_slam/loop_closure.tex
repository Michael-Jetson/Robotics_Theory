...
\noindent 流程是：对 $10$ 张图提取 ORB 特征存进 \verb|vector|，再调 DBoW3 的字典生成接口\cite{gaoxiang2019slam14}。\Verb[breaklines]|DBoW3::Vocabulary| 的构造函数可指定树的分支数与深度，这里用默认的 \textbf{$k=10,\ L=5$}——一个小字典，最多容纳 $k^L=10^5=100{,}000$ 个单词（由 \cref{eq:loop-tree-capacity}）。图像特征用默认的每幅 $500$ 个特征点。最后把字典存成压缩文件。运行输出为：
...
\noindent 读出：分支数 $k=10$、深度 $L=5$、单词数 $4983$（未达 $10^5$ 上限，因为只用了 $10$ 幅图、特征不够多）。还有两项 \Verb[breaklines]|Weighting = tf-idf|（权重方案）和 \Verb[breaklines]|Scoring = L1-norm|（评分方式）——它们正是 \cref{sec:loop-score} 要讲的 TF-IDF 加权与 $L_1$ 相似度。
...
\noindent 程序先把每幅图 \verb|transform| 成稀疏的 BoW 向量（每个条目是 \Verb[breaklines]|<单词ID, 权重>|），再用 \verb|score| 按 \cref{eq:loop-dbow-score} 算相似度。数据库版本则用\textbf{倒排索引}：查询时只与“和当前图有共同单词”的历史图比较，而非遍历全库（\cref{sec:loop-pipeline} 详述）。以 image 0 为例的相似度输出：
...
\end{equation}
里程计边的 $\hat{\mathbf S}_{ij}$ 取 $s=1$（前端相邻关键帧间无显著尺度跳变），回环边的 $s$ 则\emph{自由}，用来在优化中把整段累积的尺度漂移\textbf{均摊}到链上。在相似变换群上做位姿图优化以消尺度漂移，最早由 Strasdat 等提出\cite{strasdat2012thesis}；ORB-SLAM 将其落地为 \textbf{$\mathrm{Sim}(3)$ 位姿图优化（本质图优化）}\cite{murartal2015orbslam}：$\SE(3)$ 版（\cref{eq:loop-pg-residual}）只能拉回位姿，$\mathrm{Sim}(3)$ 版连尺度一起拉回。
...
\paragraph{闭式解的全推导：去质心 $\to$ 平移 $\to$ 尺度 $\to$ 旋转。}
上面第 $2$ 步只用一句“$3$ 对点即可闭式解出 $\mathrm{Sim}(3)$”带过，背后其实是一段干净的最小二乘推导（Horn 的绝对定向\cite{horn1987closed}）。它的骨架与 \cref{ch:vo} 的 ICP 几乎逐字相同——去质心把平移剥离、再各自求旋转与尺度——\emph{唯一}多出来、也正是 $\mathrm{Sim}(3)$ 区别于 $\SE(3)$ 的那一处，就是\textbf{尺度因子的求解}。值得先点破一个朴素弯路：仅用 $3$ 个不共线点各自搭一个正交基 $\mathbf M_1,\mathbf M_2$、令 $\mathbf R=\mathbf M_2^\top\mathbf M_1$ 也能“算”出旋转，但它\emph{依赖选点顺序}、又\emph{用不上多于 $3$ 对}的点，故弃之不用，改走下面对任意 $n$ 都成立的最小二乘\cite{horn1987closed}。整段推导在旋转一步\emph{接回} ICP 的 SVD 解（\cref{thm:vo-icp-svd}），不重复造轮子。

\begin{derivation}[$\mathrm{Sim}(3)$ 闭式解：去质心剥平移、对称尺度、旋转复用 ICP]
设几何验证给出 $n\ge 3$ 对匹配的 $3$D 路标 $\{\mathbf p_i\}\leftrightarrow\{\mathbf q_i\}$（$\mathbf p_i$ 在候选帧 $K_m$、$\mathbf q_i$ 在当前帧 $K_a$，各为对方帧三角化出的地图点），按 \cref{eq:loop-sim3-action} 的作用要找 $(s,\mathbf R,\mathbf t)$ 使
\begin{equation}
  \mathbf q_i \approx s\,\mathbf R\,\mathbf p_i+\mathbf t,\qquad \mathbf R\in\SO(3),\ s>0 ,
  \label{eq:loop-sim3-lsq}
\end{equation}
在最小二乘意义下成立。理想无噪声时 $3$ 对点足以严格求解；有噪声时定义误差 $\mathbf e_i=\mathbf q_i-s\mathbf R\mathbf p_i-\mathbf t$，求 $\min_{s,\mathbf R,\mathbf t}\sum_{i=1}^n\lVert\mathbf e_i\rVert^2$，对 $n>3$ 也稳定\cite{horn1987closed}。

\textbf{(一) 去质心，剥离平移.}\;
记两组点的质心 $\bar{\mathbf p}=\tfrac1n\sum_i\mathbf p_i$、$\bar{\mathbf q}=\tfrac1n\sum_i\mathbf q_i$，去质心坐标 $\mathbf p_i'=\mathbf p_i-\bar{\mathbf p}$、$\mathbf q_i'=\mathbf q_i-\bar{\mathbf q}$，于是去质心后求和为零（后续反复要用）：
\begin{equation}
  \sum_i\mathbf p_i'=\mathbf 0,\qquad \sum_i\mathbf q_i'=\mathbf 0 .
  \label{eq:loop-sim3-decentroid}
\end{equation}
在 $\mathbf e_i$ 里加减 $s\mathbf R\bar{\mathbf p}$，并令常量 $\mathbf t_0=\bar{\mathbf q}-s\mathbf R\bar{\mathbf p}-\mathbf t$，把误差拆成“去质心项 $+$ 常量项”：
\begin{equation}
\begin{aligned}
  \sum_i\lVert\mathbf e_i\rVert^2
  &=\sum_i\big\lVert(\mathbf q_i'-s\mathbf R\mathbf p_i')+\mathbf t_0\big\rVert^2\\
  &=\sum_i\lVert\mathbf q_i'-s\mathbf R\mathbf p_i'\rVert^2
   +2\,\mathbf t_0^\top\!\Big(\underbrace{\textstyle\sum_i(\mathbf q_i'-s\mathbf R\mathbf p_i')}_{=\,\mathbf 0}\Big)
   +n\lVert\mathbf t_0\rVert^2 .
\end{aligned}
  \label{eq:loop-sim3-expand}
\end{equation}
中间交叉项因 \cref{eq:loop-sim3-decentroid} 而整体为零（$\sum_i\mathbf q_i'=\mathbf 0$ 且 $s\mathbf R\sum_i\mathbf p_i'=\mathbf 0$），故 $\sum_i\lVert\mathbf e_i\rVert^2=\sum_i\lVert\mathbf q_i'-s\mathbf R\mathbf p_i'\rVert^2+n\lVert\mathbf t_0\rVert^2$。两项皆非负且\emph{只有}后项含 $\mathbf t$，故最优平移使 $\mathbf t_0=\mathbf 0$：
\begin{equation}
  \boxed{\ \mathbf t^\ast=\bar{\mathbf q}-s\,\mathbf R\,\bar{\mathbf p}\ }
  \label{eq:loop-sim3-trans}
\end{equation}
——与 ICP 的平移公式（\cref{thm:vo-icp-svd}）形式完全一致，只是中间多了尺度 $s$。剩下的核心问题只含 $s,\mathbf R$：$\min_{s,\mathbf R}\sum_i\lVert\mathbf q_i'-s\mathbf R\mathbf p_i'\rVert^2$。

\textbf{(二) 尺度——朴素解及其不对称.}\;
展开核心目标，并用旋转保模长 $\lVert\mathbf R\mathbf p_i'\rVert=\lVert\mathbf p_i'\rVert$，引入三个标量 $S_P=\sum_i\lVert\mathbf p_i'\rVert^2$、$S_Q=\sum_i\lVert\mathbf q_i'\rVert^2$、$D=\sum_i\mathbf q_i'^\top\mathbf R\,\mathbf p_i'$，得到一个关于 $s$ 的一元二次式 $\sum_i\lVert\mathbf q_i'-s\mathbf R\mathbf p_i'\rVert^2=S_Q-2sD+s^2S_P$。把 $\mathbf R$ 暂视为已知，对 $s$ 求导置零 $-2D+2sS_P=0$，得\textbf{朴素（非对称）尺度}
\begin{equation}
  s=\frac{D}{S_P}=\frac{\sum_i\mathbf q_i'^\top\mathbf R\,\mathbf p_i'}{\sum_i\lVert\mathbf p_i'\rVert^2}
  \qquad(\text{ORB-SLAM2 采用此式\,\cite{murartal2015orbslam}}).
  \label{eq:loop-sim3-scale-asym}
\end{equation}
但它\textbf{不具对称性}：若把 $\{\mathbf p_i\}$ 与 $\{\mathbf q_i\}$ 对调（改求 $\mathbf p\approx s'\mathbf R^\top\mathbf q$），同理得 $s'=D/S_Q$（注意 $\sum_i\mathbf p_i'^\top\mathbf R^\top\mathbf q_i'=D$ 仍是同一标量）。本该有 $s'=1/s$，然而
\begin{equation}
  s'=\frac{D}{S_Q}\ \overset{?}{=}\ \frac1s=\frac{S_P}{D}
  \quad\Longleftrightarrow\quad D^2=S_PS_Q ,
  \label{eq:loop-sim3-asym}
\end{equation}
而 Cauchy--Schwarz 给出 $D\le\sqrt{S_PS_Q}$，仅在\emph{无噪声}时取等；有噪声时 $D^2<S_PS_Q$，故 $s'\ne 1/s$。也就是说“$P\!\to\!Q$ 的尺度”\emph{不等于}“$Q\!\to\!P$ 的尺度”的倒数——方向一换、答案就变，这说明 \cref{eq:loop-sim3-scale-asym} 解出的尺度并不稳定。

\textbf{(三) 尺度——对称解.}\;
为消除这种不对称，把尺度对称地劈到两侧（$\sqrt s$ 与 $1/\sqrt s$），构造对称目标\cite{horn1987closed}（它与原目标仅差一个正因子 $1/s$，最小点处的 $\mathbf R$ 不变）：
\begin{equation}
\begin{aligned}
  \sum_i\Big\lVert\tfrac{1}{\sqrt s}\,\mathbf q_i'-\sqrt s\,\mathbf R\mathbf p_i'\Big\rVert^2
  &=\tfrac1s\,S_Q-2D+s\,S_P\\
  &=\Big(\sqrt{s\,S_P}-\sqrt{S_Q/s}\Big)^2+2\big(\sqrt{S_PS_Q}-D\big) .
\end{aligned}
  \label{eq:loop-sim3-square}
\end{equation}
右端第一项\emph{只}含 $s$、第二项\emph{只}含 $\mathbf R$（经 $D$），二者\textbf{彻底解耦}。令第一个平方项为零，得\textbf{对称尺度}
\begin{equation}
  \boxed{\ s^\ast=\sqrt{\frac{S_Q}{S_P}}=\sqrt{\frac{\sum_i\lVert\mathbf q_i'\rVert^2}{\sum_i\lVert\mathbf p_i'\rVert^2}}\ }
  \label{eq:loop-sim3-scale-sym}
\end{equation}
它\emph{只}取决于两组点云相对各自质心的“铺开程度”之比，与 $\mathbf R$ 无关；对调 $P\leftrightarrow Q$ 立得 $1/s^\ast$，对称性一目了然。直观上，尺度因子就是两片点云“到各自均值中心的均方距离”之比的平方根。

\textbf{(四) 旋转——复用 ICP 的闭式解.}\;
\cref{eq:loop-sim3-square} 第二项 $2(\sqrt{S_PS_Q}-D)$ 与 $s$ 无关，最小化它等价于在 $\mathbf R\in\SO(3)$ 上\textbf{最大化}
\begin{equation}
  D=\sum_i\mathbf q_i'^\top\mathbf R\,\mathbf p_i'=\operatorname{tr}\!\big(\mathbf R\,\mathbf W^\top\big),\qquad
  \mathbf W=\sum_i\mathbf q_i'\,\mathbf p_i'^\top .
  \label{eq:loop-sim3-rot}
\end{equation}
这\emph{正是} \cref{ch:vo} ICP 去质心后那个 $\max_{\mathbf R}\operatorname{tr}(\mathbf R\mathbf W^\top)$ 旋转子问题（即 Wahba / 正交 Procrustes 问题），$\mathbf W$ 的结构与 \cref{thm:vo-icp-svd} 的互协方差阵\emph{一模一样}。其闭式全局最优早已在那里给出：对 $\mathbf W$ 作 SVD $\mathbf W=\mathbf U\boldsymbol\Sigma\mathbf V^\top$，则 $\mathbf R=\mathbf U\mathbf V^\top$（若 $\det(\mathbf U\mathbf V^\top)<0$ 取反射修正，全局最优性见 \cref{thm:vo-icp-opt}）\cite{arun1987leastsquares}。Horn 原文走的\textbf{单位四元数路线}是\emph{同一问题的等价路径}\cite{horn1987closed}：把 $D$ 写成二次型 $D=\mathbf u^\top\mathbf N\mathbf u$（$\mathbf u$ 为表示 $\mathbf R$ 的单位四元数，$\mathbf N$ 是由 $\mathbf W$ 各元素拼出的对称 $4\times4$ 矩阵），在约束 $\lVert\mathbf u\rVert=1$ 下由 Rayleigh 商，最大值在 $\mathbf N$ \emph{最大特征值}对应的特征向量处取得，该特征向量即所求旋转的四元数。矩阵-SVD 与四元数-特征分解两条路给出\emph{同一个} $\mathbf R$，仅参数化不同（前者免去四元数到矩阵的转换，后者免去 $\det$ 符号修正）——故本节不再重复推导，直接复用 \cref{thm:vo-icp-svd}。

\textbf{(五) 回代.}\;
求得 $\mathbf R$ 后，由 \cref{eq:loop-sim3-scale-sym}（推荐）或 \cref{eq:loop-sim3-scale-asym}（ORB-SLAM2 实现）算出 $s$，再由 \cref{eq:loop-sim3-trans} 算出 $\mathbf t$；三参数 $(s,\mathbf R,\mathbf t)$ 齐备，即拼成 \cref{eq:loop-sim3-mat} 的相对相似变换 $\hat{\mathbf S}_{am}$。整个过程一次算出、无需迭代，正是第 $2$ 步“闭式解”的全部内容；它再嵌进 RANSAC（每轮抽 $3$ 对、统计重投影内点）即得鲁棒初值。
\end{derivation}

\begin{insight}[尺度的“不对称”正是 \texorpdfstring{$\mathrm{Sim}(3)$}{Sim(3)} 区别于 \texorpdfstring{$\SE(3)$}{SE(3)} 的唯一新东西]\label{ins:loop-sim3-asym}
把上面五步与 \cref{ch:vo} 的 ICP 对照，会发现\textbf{去质心剥平移}（\cref{eq:loop-sim3-trans}）与\textbf{求旋转}（\cref{eq:loop-sim3-rot}）两步\emph{逐字照搬}了 $\SE(3)$ 的 ICP——$\mathrm{Sim}(3)$ 真正多出来、也是\emph{唯一}需要新推导的，只有\textbf{尺度因子}这一维。而恰是这一维带来一个 $\SE(3)$ 里不存在的陷阱：朴素尺度 $s=D/S_P$（\cref{eq:loop-sim3-scale-asym}）\textbf{方向相关}——“$P\!\to\!Q$”与“$Q\!\to\!P$”算出的尺度互不为倒数（\cref{eq:loop-sim3-asym}），其差距正是 Cauchy--Schwarz 不等式 $D\le\sqrt{S_PS_Q}$ 的“噪声间隙”。$\SE(3)$ 没有这个问题，因为它根本没有尺度可解（$s\equiv 1$）。Horn 的对称构造（\cref{eq:loop-sim3-square}）用一招“两侧各劈 $\sqrt s$”把尺度与旋转\emph{解耦}：尺度 $s^\ast=\sqrt{S_Q/S_P}$（\cref{eq:loop-sim3-scale-sym}）只看两片点云各自的铺开程度、与 $\mathbf R$ 无关且天然对称，旋转则退化为与 ICP 全等的子问题。\textbf{记住这条分野}：闭环求解里凡 $\SE(3)$ 与 $\mathrm{Sim}(3)$ 写法不同之处，差异\emph{必定}集中在尺度那一维，别处与刚体 ICP 无异。
\end{insight}
...
\begin{itemize}
  \item \textbf{“集卡式”共同区域检测}：\cref{sec:loop-stageC} 的时间一致性要求回环候选\emph{连续 $3$ 次}出现才确认，召回偏低。ORB-SLAM3 改为\emph{先做几何一致性、后做时间一致性}的“集卡式”验证：先用词袋取\emph{ $3$ 个}候选关键帧、为每个候选连同其共视邻域和地图点建一个\emph{局部窗口}，在窗口与当前帧间用 RANSAC 求对齐变换 $\mathbf T_{am}$（纯单目取 $\mathrm{Sim}(3)$，否则 $\SE(3)$）并引导匹配加密\cite{campos2021orbslam3}。其几何验证不再等连续帧，而是在活跃地图里另找\emph{ $2$ 个}与当前帧共视、且与局部窗口匹配点数超阈值的关键帧来佐证；验证\emph{持续到累计有 $3$ 个关键帧确认 $\mathbf T_{am}$}（或连续两个新关键帧验证失败即放弃），从而\textbf{在不牺牲准确率的前提下提高召回}（这与 \cref{ins:loop-precision} 的“宁缺毋滥”不矛盾：换的是\emph{更早确认}而非\emph{放松门槛}）。相对 ORB-SLAM2 的关键增益是这一步在共视\emph{局部窗口}内补充的\emph{中期}数据关联——把求 $\mathrm{Sim}(3)$ 的匹配从单帧扩展到整个共视窗，对应更充分。\footnote{此处“ $3$ 候选 $+$ $2$ 共视佐证 $+$ 累计 $3$ 帧确认”的数字依 ORB-SLAM3 原文“Place Recognition”一节六步流程（\cite{campos2021orbslam3} 步骤 1、5）核定。}
