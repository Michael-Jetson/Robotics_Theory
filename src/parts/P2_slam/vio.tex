...
\subsection{三视角合流：\texorpdfstring{$\mathrm{SE}_2(3)$}{SE2(3)} 整体视角与“群仿射统一”}
\label{subsec:vio-se23-unify}

\paragraph{动机：同一段预积分，本章已写了两副面孔。}
\cref{sec:vio-preint-delta} 给的是 Forster 的\emph{矩阵分量}写法（$\Delta\mathbf R\in\SO(3)$ 配 $\Delta\mathbf v,\Delta\mathbf p\in\mathbb R^3$，右雅可比），\cref{sec:vio-preint-vins} 给的是 VINS-Mono 的\emph{四元数}写法（$\boldsymbol\alpha,\boldsymbol\beta,\boldsymbol\gamma$）。两者在本节练习中已证一阶等价。还有第三副面孔——把姿态、速度、位置打包进\emph{一个}扩展位姿群 $\mathrm{SE}_2(3)$ 元素（\cref{sec:se23} 权威定义、\cref{def:se23}），整段预积分写成群上的一次乘法。三副面孔携带\emph{同样}的信息，差别只在“用什么坐标记录”。本小节点明这层统一，完整的 $\mathrm{SE}_2(3)$ 群上推导（$\mathbf N$ 矩阵均值传播、时间机器、协方差、批量 MAP 因子、左右扰动）已在 IMU 章 \cref{subsec:imu-se23-preint} 展开，此处不重复，只做\emph{合流}。

\paragraph{第三视角：把 $\Delta\mathbf R,\Delta\mathbf v,\Delta\mathbf p$ 装进一个群元素。}
$\mathrm{SE}_2(3)$ 把 \cref{def:vio-delta} 的三个增量收成一个 $5\times5$ 群元素 $\boldsymbol\Xi$（\cref{eq:imu-Xi}），其左上是旋转增量、两并列列是速度/位置增量。单步均值传播由群乘法一次推进三者；与 Forster 欧拉式相比，群版多出 $\SO(3)$ 左雅可比 $\mathbf J$ 与新矩阵 $\mathbf N$（\cref{eq:imu-Nmatrix}）两个因子，\emph{精确}补偿“积分区间内姿态也在转”——当区间内不转时 $\mathbf J,\mathbf N\to\mathbf I$，群版逐字退回 Forster 欧拉式。关键之处：惯导运动学在 $\mathrm{SE}_2(3)$ 上是 Barrau--Bonnabel 的\textbf{群仿射}动力学（\cref{sec:se23}），其不变误差动力学\emph{自治}（系统矩阵不含状态）——这正是把预积分接到不变滤波 InEKF（\cref{paper:inekf}）、并解释 VIO 一致性（\cref{sec:vio-fej} 的 FEJ 同一主题）的群论根源。

\begin{insight}[群仿射统一：分量是“拆开”，群是“打包”]\label{ins:vio-se23-unify}
为什么三视角必然等价？因为它们都在离散化\emph{同一条}连续运动学 \cref{eq:vio-kinematics}。Forster 选 $\SO(3)\times\mathbb R^3\times\mathbb R^3$ 坐标、VINS 选\emph{四元数} $\times\mathbb R^3\times\mathbb R^3$ 坐标、Brossard/Barfoot 选 $\mathrm{SE}_2(3)$ 群坐标——\emph{流形是同一个，只是图卡（chart）不同}。分量视角贴近手算与 GTSAM/VINS 实现（雅可比逐块可查）；群视角贴近一致性分析（群仿射 $\Rightarrow$ 误差自治，InEKF 的收敛保证与轨迹无关）。“汇聚百家”不是把多种方法堆在一起，而是认出它们\emph{本就是一个东西}的不同写法：理解了 Forster 的 $\Delta\mathbf R,\Delta\mathbf v,\Delta\mathbf p$，$\mathrm{SE}_2(3)$ 只是把它们\emph{打包}；理解了 $\mathrm{SE}_2(3)$，分量式只是把它\emph{拆开}读分量。
\end{insight}

\begin{table}[H]\centering\small
\caption{IMU 预积分的三种等价坐标（同一信息、同一流形、不同图卡）}
\label{tab:vio-three-views}
\begin{tabular}{llll}
\toprule
 & Forster 矩阵分量 & VINS-Mono 四元数 & $\mathrm{SE}_2(3)$ 群 \\
\midrule
旋转载体 & $\Delta\mathbf R\in\SO(3)$ & $\boldsymbol\gamma$（单位四元数）& $\boldsymbol\Xi$ 左上块 \\
速度/位置 & $\Delta\mathbf v,\Delta\mathbf p\in\mathbb R^3$ & $\boldsymbol\beta,\boldsymbol\alpha\in\mathbb R^3$ & $\boldsymbol\Xi$ 两并列列 \\
本章出处 & \cref{eq:vio-preint-meas} & \cref{eq:vio-vins-preint} & \cref{eq:imu-Xi}（IMU 章）\\
残差 & $[\mathbf r_{\Delta\mathbf R};\mathbf r_{\Delta\mathbf v};\mathbf r_{\Delta\mathbf p}]$ & $2[\cdots]_{xyz}$ 等 & $\mathrm{Log}_{\mathrm{SE}_2(3)}(\cdots)^\vee$ \\
最擅长 & 手算/解析雅可比 & Eigen/工程实现 & 一致性/InEKF/地球自转 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{practice}
\begin{enumerate}
  \item 对照 \cref{tab:vio-three-views}：把 VINS 的 $(\boldsymbol\alpha,\boldsymbol\beta,\boldsymbol\gamma)$ 逐一对应到 Forster 的 $(\Delta\mathbf p,\Delta\mathbf v,\Delta\mathbf R)$ 与 $\mathrm{SE}_2(3)$ 的 $\boldsymbol\Xi$ 各块，指出三者只是坐标不同。
  \item （概念）解释“群仿射 $\Rightarrow$ 误差动力学自治”为何对 VIO 一致性有利，并把它与 \cref{sec:vio-fej} 的 FEJ 动机联系起来。
\end{enumerate}
\end{practice}
...
\end{equation}
$\hat{\bar{\mathcal{P}}}^{c_j}_l=\pi_c^{-1}([\hat u^{c_j}_l,\hat v^{c_j}_l]^\top)$ 是观测的单位射线，$\mathcal{P}^{c_j}_l$ 是把第 $i$ 帧逆深度 $\lambda_l$ 的特征经“$i$ 相机 $\to$ $i$ body $\to$ world $\to$ $j$ body $\to$ $j$ 相机”链变到第 $j$ 帧的预测射线，$\mathbf{b}_1,\mathbf{b}_2$ 张成切平面（视觉残差仅 $2$ 自由度）。其雅可比对位姿用右扰动按 \cref{ch:lie} 与 \cref{ch:vo} 的重投影雅可比链式求得（VINS 论文称“detailed 略”，本书按右扰动约定补：位姿块含 $-\mathbf{R}(\cdot)^\wedge$ 项，再乘投影到切平面的 $2\times3$ 矩阵）。\footnote{VINS-Mono 原文此处仅称“detailed 略”、未给闭式；这里的右扰动雅可比是本书按全章统一的右扰动主线补全的一步。}
...
\begin{itemize}
  ...
  \item \textbf{局部地图跟踪联合惯性}：第二阶段不再只优化位姿，而是用“上一帧/上一关键帧 $+$ 当前帧”的视觉与 IMU 信息\emph{联合优化}位姿、速度、偏置（\texttt{PoseInertialOptimizationLastFrame/\allowbreak LastKeyFrame}）；
...
\paragraph{量化收益。}
Atlas 对精度与鲁棒性的提升很显著：在 EuRoC 上，ORB-SLAM3 的单目惯性精度约为 VINS-Mono 的 $2.6$ 倍（单段运行），多段（多地图）运行更达 $3.2$ 倍\cite{campos2021orbslam3}。\cref{tab:vio-multimap} 给出量化对比：覆盖率（成功定位帧数占比）从单地图基线的 $10\%$--$15\%$ 提到 $70\%$--$90\%$，绝对轨迹误差（ATE）亦改善（区间结论与具体数值均出自 ORBSLAM-Atlas\cite{elvira2019atlas}）。

\begin{table}[H]\centering\small
\caption{多地图量化收益：EuRoC 上多地图 vs 单地图（数据自 ORBSLAM-Atlas\cite{elvira2019atlas} 表 I，纯视觉单目，$5$ 次中位数；该多地图机制即 ORB-SLAM3 之 Atlas 前身）}
\label{tab:vio-multimap}
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{3}{c}{ORBSLAM-Atlas 多地图} & \multicolumn{3}{c}{ORB-SLAM 单地图} \\
...
\begin{enumerate}
  \item \textbf{共视几何校验}：取候选 $K_m$（及当前帧 $K_a$）的最佳共视关键帧组（实现取 $10$ 个）依次做几何校验，累计成功次数。若成功次数 $\ge 3$，直接通过、跳过时序校验（\emph{所有关键帧已在地图中，无须等待}——这是召回率提升的关键）；若成功数 $\in(0,3)$，进入第 2 步;若为 $0$，验证失败。\footnote{共视组规模以源码 \texttt{DetectCommonRegionsFromBoW} 中 \texttt{nNumCovisibles}$=10$ 为准；通过门槛为累计共视/时序成功 $\ge 3$（\texttt{nNumCoincidences}$\ge 3$）。}
...
\begin{enumerate}
  ...
  \item 因点变化，更新关键帧连接关系；
  \item \textbf{熔接 BA}（Welding BA，因子图 \cref{fig:vio-welding}）：优化\emph{活跃图}熔接窗口 $W_a$ 内的关键帧位姿、以及 $W_a,W_m$ 所观测到的地图点，而\emph{固定} $W_m$ 关键帧位姿（实现上把 $W_a$ 作为可调关键帧、$W_m$ 作为固定关键帧传入局部 BA\footnote{以 \texttt{MergeLocal()} 中 \texttt{LocalBundleAdjustment(mpCurrentKF, $W_a$, $W_m$, \dots)} 的实参为准：$W_a$ 进可调集、$W_m$ 进固定集。原文叙述笼统地称“优化两图熔接窗口内的关键帧”，此处以源码实际固定 $W_m$ 为准。}）；纯视觉故因子图只有重投影项;
...
本章 \cref{subsec:vio-se23-unify} 已点明 $\mathrm{SE}_2(3)$ 整体视角与 Forster/VINS 分量视角的合流，其完整群上推导（$\mathbf N$ 矩阵均值传播、时间机器、协方差、批量 MAP 因子）在 IMU 章 \cref{subsec:imu-se23-preint}。在工程上它的额外价值有二：用高阶噪声传播改进不确定度建模、并干净纳入标准预积分忽略的地球自转 Coriolis 与离心力\cite{brossard2021associating}，配合不变滤波即得与轨迹无关的误差动力学（\cref{paper:inekf}）——一个海上导航实例可量化其威力：导航级 IMU 配线速度传感器、在 $\mathrm{SE}_2(3)$ 上做扩展位姿预积分，行驶 $1.8\,\mathrm{km}$、历时一小时后平移误差仅约 $5\,\mathrm m$\cite{carlone2026handbook}。连续时间状态表示（B-spline 基、GP 先验）也能容纳众多 IMU 测量而不增状态维度（\cref{sec:est-steam}）。
...
\begin{itemize}
  ...
  \item \textbf{优化主线系统}：Qin 等《VINS-Mono》\cite{qin2018vins}（arXiv:1708.03852）——单目滑窗紧耦合、在线外参、初始化、$4$-DOF 位姿图，本章优化主线与初始化的主源；代码 \texttt{HKUST-Aerial-Robotics/\allowbreak VINS-Mono}。
...
\begin{enumerate}
  ...
  \item \textbf{症状}：轨迹尺度系统性偏大/偏小。\textbf{可能原因}：预积分递推 \texttt{dp}/\allowbreak\texttt{dv} 顺序写反，或加计偏置未优化。\textbf{排查}：按 \cref{eq:vio-vins-discrete} 顺序自检；确认 $\mathbf{b}^a$ 在状态向量内被优化（\cref{sec:vio-bias} 陷阱）。
