...
\small
\begin{tabularx}{\linewidth}{@{}Llp{0.42\linewidth}@{}}
\toprule
组件 & 锚定版本 & 本章涉及内容 \\
\midrule
mjlab & 1.4.0（实测锚点；项目仍在快速迭代） & \Verb[breaklines]|ManagerBasedRlEnv|、九大 Manager、\verb|env.step()| 时序 \\
Isaac Lab & 2.x（教学锚点；3.0 处于迁移期） & \Verb[breaklines]|ManagerBasedRLEnv|、对应 Manager API \\
...
\begin{itemize}
  \item \textbf{mjlab}（Linux / macOS，纯 Python + Warp，最轻）：依赖 \verb|mujoco-warp| 与一块支持 CUDA 的 GPU；官方推荐用 \verb|uv| 管理虚拟环境，安装后以 \Verb[breaklines]|uv run train <TASK>| 启动\,\cite{mjlab2026}。
  \item \textbf{Isaac Lab}（Linux 为主，Windows 部分支持）：依赖 NVIDIA Isaac Sim（Omniverse 平台），体量大、对驱动版本敏感；训练入口形如 \Verb[breaklines]|python scripts/reinforcement_learning/rsl_rl/train.py --task <TASK>|\,\cite{isaaclab2024}。
  \item \textbf{UniLab}（异构：CPU 物理 + GPU 策略，Linux CUDA/ROCm/XPU 或 Apple Silicon 多套路径）：\textbf{不}依赖 Isaac Sim、也\textbf{不}依赖 GPU 仿真后端（这与前两者根本不同），物理跑 CPU（核越多越好）、策略学习需一块 GPU/MPS。官方推荐 \verb|uv| 工作流：\Verb[breaklines]|git clone .../UniLab && cd UniLab|，再 \Verb[breaklines]|uv sync --extra mujoco|（MuJoCoUni 后端）或 \Verb[breaklines]|uv sync --extra motrix|（MotrixSim 后端）；训练入口形如 \Verb[breaklines]|uv run train --algo ppo --task <TASK> --sim mujoco|。首跑会从 HuggingFace 拉取资产，国内网络需先 \texttt{export HF\_ENDPOINT=https:/\allowbreak /\allowbreak hf-mirror.\allowbreak com} 走镜像。
\end{itemize}

\begin{note}[本章不需要 GPU 也能学]
即便你暂时没有可跑训练的 GPU，本章的\emph{源码精读}部分（\verb|env.step()| 18 步时序、Manager 加载顺序、各 Manager 的 \verb|compute()| 逻辑）完全可以离线阅读。建议你 \Verb[breaklines]|git clone| 三套框架的源码（哪怕不安装），对照本章逐处跳转——\textbf{读源码本身就是本章最重要的练习}。
\end{note}
...
读完这段，你应当能立刻回答三个问题：(1) \verb|env.step()| 只在 rollout 阶段被调用，update 阶段不调用；(2) 一次训练里它被调用 $\text{max\_iterations}\times\text{num\_steps\_per\_env}$ 次；(3) 它的输出 \Verb[breaklines]|(obs, reward, terminated, truncated)| 正是 PPO 的全部「原料」。\textbf{本章其余部分，都是在放大} \verb|env.step()| \textbf{这一行内部的世界}。
...
\begin{enumerate}
  ...
  \item \textbf{翻译} mjlab 与 Isaac Lab 的 Manager API（如终止项全名 \Verb[breaklines]|TerminationTermCfg| 与 Isaac Lab 常用别名 \verb|DoneTerm| 的对应、obs 组名 \verb|actor| $\leftrightarrow$ \verb|policy|），在两个框架间无障碍切换；
...
\paragraph{前置桥接（两段重述前章）。}本章紧接 \cref{ch:rlmc-obsact}（观测与动作空间设计）。在那一章你已经知道：观测分四族（本体感知 / 外部感知 / 命令 / 特权），actor 只看部署可得信号、critic 可看特权信号；动作有关节位置 / 速度 / 力矩 / 微分逆运动学四种抽象层次。本章把视角从「观测/动作\emph{是什么}」抬高到「\emph{所有} MDP 组件如何被一个统一的 Manager 体系编排、并在 \verb|env.step()| 里按精确时序执行」——\Verb[breaklines]|ObservationManager| 与 \verb|ActionManager| 只是九大 Manager 中的两个。
...
\begin{itemize}
  \item \textbf{Rollout（数据收集）}：循环 \Verb[breaklines]|num_steps_per_env| 步，每步「策略前向 $\to$ \verb|env.step()| $\to$ 存 transition」。env 的\emph{所有} Manager 在每步 rollout 时被执行一次。
...
\Verb[breaklines]|num_steps_per_env| & 24 & 太短 GAE 不准，太长占显存 & $16\!\sim\!64$ & 与 \verb|num_envs|、\Verb[breaklines]|num_mini_batches| \\
\Verb[breaklines]|num_mini_batches| & 4 & 切分 rollout 数据 & 2$\sim$8 & 须整除 $\text{num\_envs}\!\times\!\text{steps}$ \\
\Verb[breaklines]|num_learning_epochs| & 5 & 太多过拟合旧数据 & 3$\sim$8 & 与 \verb|clip_param| \\
\verb|entropy_coef| & 0.01 & 大则鼓励探索 & $0\!\sim\!0.02$ & 与 \Verb[breaklines]|init_noise_std| \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{已据 mjlab 1.4.0 实测核实}（G1/Go1 velocity \texttt{rl\_cfg.py}）：网络配置类为 \texttt{RslRlModelCfg}（actor/critic 分开）、算法配置类为 \texttt{RslRlPpoAlgorithmCfg}、runner 配置为 \texttt{RslRlOnPolicyRunnerCfg}（mjlab 自带 runner 类 \texttt{MjlabOnPolicyRunner}）；上面超参（\texttt{hidden\_dims}/\allowbreak\texttt{activation}/\allowbreak\texttt{clip\_param}/\allowbreak\texttt{entropy\_coef}/\allowbreak\texttt{num\_learning\_epochs}/\allowbreak\texttt{num\_mini\_batches}/\allowbreak\texttt{lr}/\allowbreak\texttt{desired\_kl}/\allowbreak\texttt{num\_steps\_per\_env}）均与实测一致，且字段名确为 \texttt{num\_learning\_epochs}/\allowbreak\texttt{clip\_param}（不是 \texttt{num\_epochs}/\allowbreak\texttt{clip\_range}），仅 \texttt{max\_iterations} 按任务定（mjlab 1.4.0 的 Go1 实测为 \texttt{10\_000}）。actor/critic 配置拆分这一形态自 \texttt{rsl-rl-lib} 4.0.0 起引入（此前用统一的 \texttt{RslRlPpoActorCriticCfg}）。\pz{Isaac Lab 侧的确切配置类与 import 路径随其捆绑的 rsl-rl 版本而变，以你所装版本为准。}

\begin{insight}[\texttt{max\_iterations} 这种「实测锚点」务必自己复核——别把书里的数字当真理]\label{ins:rlmc-mgr-recheck}
本书把 \texttt{max\_iterations=10\_000} 标成「实测值」，但它\emph{每个版本、每个任务都会漂移}（早期 mjlab 文档里曾是别的数）。\textbf{与其背这个数，不如学会一行自查}——打开你这台机器上对应任务的 \texttt{rl\_cfg.py}（mjlab 在 \texttt{src/\allowbreak mjlab/\allowbreak tasks/\allowbreak velocity/\allowbreak config/\allowbreak go1/\allowbreak rl\_cfg.\allowbreak py}），直接读 \texttt{max\_iterations=}；或不开源码，用一行命令把当前生效值打出来：
...
\begin{itemize}
  \item \textbf{mjlab}：直接使用 RSL-RL 作为 PPO 后端，配置以函数工厂返回 \Verb[breaklines]|RslRlOnPolicyRunnerCfg| 实例\,\cite{mjlab2026,schwarke2025rslrl}。
  \item \textbf{Isaac Lab}：同样以 RSL-RL 为主力后端（也支持 SKRL、rl\_games 等），配置以 \verb|@configclass| 类组织\,\cite{isaaclab2024}。
  \item \textbf{UniLab}：PPO \textbf{同样复用 RSL-RL} 的 \Verb[breaklines]|OnPolicyRunner|，但用自定义的 \Verb[breaklines]|FinalObservationAwarePPO|（在 \texttt{algos/\allowbreak torch/\allowbreak rsl\_rl\_ppo.\allowbreak py} 的同名类，UniLab commit \texttt{99936e08}）以正确处理终止帧的 bootstrap 观测；配置不是 \verb|@configclass| 而是 typed dataclass \verb|PPOConfig|（在 \texttt{structured\_configs.\allowbreak py}），经 Hydra/OmegaConf 组合；CLI 入口 \Verb[breaklines]|train --algo ppo|，超参走 Hydra 覆盖（如 \Verb[breaklines]|algo.num_envs=4096 algo.max_iterations=300|）。除 PPO 外 UniLab 还把 \textbf{APPO（异步 PPO）/ SAC / TD3 / FlashSAC 列为一等公民}（off-policy 经共享内存 \verb|ReplayBuffer|），这是其异构 collector-learner 架构的直接产物。
\end{itemize}

\subsection{RSL-RL VecEnv Wrapper：terminated/truncated 的关键处理}
\label{sec:rlmc-mgr-wrapper}

\verb|env.step()| 返回的是 Manager-Based 环境的\emph{原始}输出：观测字典、奖励张量、\verb|terminated| 张量、\verb|truncated| 张量、extras 字典。RSL-RL 需要的是简化格式，\Verb[breaklines]|RslRlVecEnvWrapper| 负责适配。其中最关键的一步是对两种「回合结束」的区分处理（\cref{tab:rlmc-mgr-wrapper}）。
...
日志项 & 正常表现 & 异常信号 \\
\midrule
\Verb[breaklines]|Episode_Reward/<term>| & 至少 tracking 项在涨 & 全部为零 $\Rightarrow$ sensor 接线问题 \\
...
\begin{practice}[本节练习]
\begin{enumerate}
  \item \textbf{[动手]} 在你能访问的任一框架日志里，找出 \verb|num_envs|、\Verb[breaklines]|num_steps_per_env|、\Verb[breaklines]|num_mini_batches|，验证 $\text{num\_envs}\times\text{num\_steps\_per\_env}$ 能被 \Verb[breaklines]|num_mini_batches| 整除。\emph{验收}：写出三个数与商。
  \item \textbf{[推理]} 若 \verb|num_envs=4096|、\Verb[breaklines]|num_steps_per_env=24|、\Verb[breaklines]|num_mini_batches=4|，每个 mini-batch 多少条 transition？\emph{验收}：给出 $4096\times24/4=24576$ 并解释。
...
\cref{tab:rlmc-mgr-step18} 是 mjlab \Verb[breaklines]|ManagerBasedRlEnv.step()| 的执行序列。\rebuilt{这是据 mjlab 1.4.0 \texttt{manager\_based\_rl\_env.\allowbreak py} 整理的「18 步」教学划分；具体步数与拆分粒度随版本变，请以你机器上的源码为准——本表的价值在相对顺序，而非「恰好 18」这个数字。}
...
步 & 操作 & 代码入口（示意） & 说明 \\
\midrule
1 & 清空 extras 日志 & \Verb[breaklines]|extras["log"] = {}| & 每步重收日志 \\
2 & 处理 action & \Verb[breaklines]|action_manager.process_action| & raw $\to$ scaled/clipped \\
3--7 & \textbf{decimation 子循环} & \Verb[breaklines]|for _ in range(decimation)| & 物理子步 \\
3 & \quad 应用 action & \Verb[breaklines]|action_manager.apply_action| & 经 actuator 算力矩 \\
4 & \quad 写入仿真 & \Verb[breaklines]|scene.write_data_to_sim| & 控制信号 $\to$ \verb|MjData.ctrl| \\
5 & \quad 物理步进 & \verb|sim.step()| & MuJoCo Warp 推进一个 \verb|physics_dt| \\
6 & \quad 更新场景 & \verb|scene.update| & 同步 GPU buffer \\
7 & \quad 子步指标 & \Verb[breaklines]|metrics_manager.compute_substep| & 记子步级指标 \\
8 & episode 计数 & \Verb[breaklines]|episode_length_buf += 1| & 增回合步数 \\
9 & 计算 termination & \Verb[breaklines]|termination_manager.compute| & 判是否终止 \\
10 & 计算 reward & \Verb[breaklines]|reward_manager.compute(dt)| & 加权 reward \\
11 & 计算 step 指标 & \Verb[breaklines]|metrics_manager.compute| & 记 step 级指标 \\
12 & reset 终止的环境 & \Verb[breaklines]|_reset_idx(env_ids)| & auto-reset \\
12a & \quad curriculum 更新 & \Verb[breaklines]|curriculum_manager.compute| & 调难度 \\
12b & \quad reset 级 event & \Verb[breaklines]|event_manager.apply("reset")| & reset DR \\
13 & 刷新派生量 & \verb|sim.forward()| & 更新接触力等 \\
14 & 更新 command & \Verb[breaklines]|command_manager.compute(dt)| & 重采样/更新命令 \\
15 & step/interval event & \Verb[breaklines]|event_manager.apply("step/interval")| & 周期事件（如推力） \\
16 & 刷新传感器 & \verb|sim.sense()| & 更新 sensor data \\
17 & 计算 observation & \Verb[breaklines]|observation_manager.compute| & 算并返回 obs dict \\
18 & 记录 & \Verb[breaklines]|recorder_manager.record_post_step| & 录制 rollout \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{reward 在 \texttt{sim.\allowbreak forward()} 之前}（步 10 vs 13）：reward 函数读到的派生量（接触力、传感器）有\emph{一个 substep 的滞后}。这不是 bug，而是性能权衡——若 reward 前也调一次 \verb|sim.forward()|，每步多一次 forward，在数千环境下显著降吞吐。
...
\verb|sim.sense()| & 独立调用（步 16） & 部分集成进 \Verb[breaklines]|scene.update()| \\
\verb|step| 模式 event & step() 内处理 & step() 内只自动处理 \verb|interval|；\verb|startup|/\allowbreak\verb|reset| 在初始化/重置路径 \\
...
上段为高层 MDP 对齐的简化示意，其中的关键语义\textbf{已据源码核实}：Isaac Lab \emph{永远} auto-reset（无 \texttt{auto\_reset} 开关，那是 mjlab 才有的）、奖惩在 reset 前算、obs 在 reset 后取、返回 5 元组 \texttt{(obs, reward, terminated, time\_outs, extras)} 且 \texttt{terminated} 与 \texttt{time\_outs} 分开返回；属性名为 \texttt{time\_outs}（复数）/\texttt{reward\_buf}。\pz{唯精确\emph{行序}仍以你所用 Isaac Lab 版本源码为准；另注意 3.0 起 \texttt{data.*} 返回类型由 torch 张量改为 Warp 数组（见后文陷阱），相关运算需 \texttt{wp.\allowbreak to\_torch()}。}

\begin{insight}[高层对齐 $\ne$ 源码逐行一致]\label{ins:rlmc-mgr-align}
对比两框架的 step()，你会发现\emph{高层 MDP 阶段}相似，但传感器/派生量刷新机制不同（MuJoCo Warp 的 \verb|sim.forward()|/\allowbreak\verb|sim.sense()| vs PhysX 的集成式更新）。掌握一个框架后迁移到另一个相对快——但\textbf{不能}把两者的 step() 源码当成逐行可互换。这正是「学方法、而非背某版数字」的具体体现。
\end{insight}

\paragraph{UniLab：第三种「非 Manager-Based」的 env.step。}这里出现本章最重要的一处三框架\emph{结构性}差异：UniLab \textbf{不是} Manager-Based——它没有 \Verb[breaklines]|ObservationManager|/\allowbreak\verb|RewardManager|/\allowbreak\Verb[breaklines]|TerminationManager| 这些类，obs/reward/terminated \textbf{在一个方法 \texttt{update\_state} 里一处算完}。其 \verb|NpEnv.step()|（UniLab commit \texttt{99936e08}，\texttt{base/\allowbreak np\_env.\allowbreak py} 的 \verb|NpEnv.step|）时序如下，可与 \cref{tab:rlmc-mgr-step18} 的 18 步逐项对照：
...
逐项映射到 mjlab 的 18 步：UniLab 的 \emph{decimation 子循环} 被压进后端一次 \Verb[breaklines]|backend.step(ctrl, nsteps)| 调用（CPU 多线程批量步进 \verb|sim_substeps| 个物理子步）；mjlab 的 termination(步 9)+reward(步 10)+observation(步 17) 三步在 UniLab 里\textbf{合并为 \texttt{update\_state} 一处}；mjlab 独有的 \Verb[breaklines]|MetricsManager|（步 7/11）UniLab \textbf{无对应}（指标经 \verb|info["log"]| 直接累加）；mjlab 的 \verb|sim.sense()|（步 16）在 UniLab 里由后端 \Verb[breaklines]|get_sensor_data()| 按需读取，而非独立刷新阶段。事件（DR）的分发：UniLab 经 \Verb[breaklines]|DomainRandomizationManager| 在三个时机触发——\verb|init|（$\approx$startup）、\verb|reset|（per-episode）、\verb|interval|（步 2，自计时），\textbf{无} mjlab 的 \verb|step| 模式（每步无条件 DR）。
...
\paragraph{场景 A：reward 读到的接触力「滞后」一步。}症状：你写了惩罚足端冲击力的 reward，但策略仍猛烈着地；打印 reward 里读到的冲击力，比可视化峰值小得多。诊断：reward 在步 10 算，而 \verb|sim.forward()| 在步 13 才执行——reward 读到的是\emph{上一个 substep} 的接触力，可能错过了着地瞬间的峰值。解法：用 ContactSensor 的 \Verb[breaklines]|history_length| 记录 decimation 期间所有 substep 力，reward 取 history 的最大值；或在 \Verb[breaklines]|metrics_manager.compute_substep()| 里记每个 substep 的峰值。
...
\begin{practice}[本节练习]
\begin{enumerate}
  \item \textbf{[源码]} 打开你能访问框架的 \Verb[breaklines]|manager_based_rl_env.py|（或等价文件），定位 \verb|step()|，对照 \cref{tab:rlmc-mgr-step18} 确认每一步的代码位置与相对顺序。\emph{验收}：列出你的版本里 termination/reward/reset/observation 的实际先后。
...
\paragraph{隐式依赖：最危险的工程债。}在单体类里，\Verb[breaklines]|_compute_reward()| 可能依赖 \Verb[breaklines]|_compute_observations()| 算出的中间量（经 \Verb[breaklines]|self.projected_gravity| 传递）。这种\emph{隐式}依赖没被代码结构表达，只有读完整个两千行才能发现。若你改了 obs 计算顺序而没意识到 reward 依赖它，reward 会用到错误值——\textbf{却不报任何错}。
...
Manager-Based 把 MDP 的每个组件拆到独立 Manager，每个 term 是一个\textbf{纯函数}——接收 \verb|env|，直接从 \Verb[breaklines]|env.scene["robot"].data| 读最新数据，不依赖任何中间量。
...
\paragraph{「改一行 config」不是口号——一行 CLI 即可消融任一 reward 项（mjlab 实测）。}上表「对比 N 种 reward 配置 = N 次 CLI 覆盖」在 mjlab 是\emph{字面可执行}的：mjlab 用 tyro 暴露了\textbf{任意深度}的配置覆盖，于是「把某条 reward 关掉/调权重」无需改任何文件、无需复制 config，一行命令就够（已实测 \Verb[breaklines]|--env.rewards.<name>.weight| 这一层覆盖确实生效）：
...
\end{codebox}
\noindent 注意 CLI 里点分路径用\textbf{连字符}（\Verb[breaklines]|track-angular-velocity|）对应 Python 的下划线字段名（\Verb[breaklines]|track_angular_velocity|），这是 tyro 的命名约定；覆盖只作用于本次 run、不污染注册的原始 cfg（\cref{sec:rlmc-mgr-loadorder} 讲的 \verb|deepcopy| 机制）。\textbf{这就是 \cref{ins:rlmc-mgr-value} 「迭代速度」的实证}——在 legged\_gym 里这三次对比要复制三份文件、改三处硬编码，在这里是三行命令。
...
Manager 的\emph{创建}顺序很重要——后创建的可能依赖先创建的输出。mjlab 的 \Verb[breaklines]|load_managers()| 按 \cref{tab:rlmc-mgr-loadorder} 创建。
...
两张表别混为一谈。\cref{tab:rlmc-mgr-loadorder} 表达的是\emph{初始化依赖}（谁先创建、谁依赖谁的产物）；\cref{tab:rlmc-mgr-step18} 表达的是\emph{运行时 MDP 数据对齐}。例如加载顺序里 Observation 排第 4、Termination 第 5，但运行时 termination 在 reward 之后、observation 在最后——因为初始化关心「依赖就绪」，运行关心「数据对齐」。\textbf{反事实}：若 ObservationManager 在 CommandManager 之前加载，obs 里的 \Verb[breaklines]|generated_commands| 项会找不到 command manager（可能不报错、只返回空张量），策略遂学不会跟踪命令。
\end{insight}
...
\begin{itemize}
  \item \textbf{mjlab}：用自有的 \Verb[breaklines]|register_mjlab_task()|，把\emph{已实例化}的 cfg 对象存进私有 dict \verb|_REGISTRY|（\texttt{tasks/\allowbreak registry.\allowbreak py}），每次 \Verb[breaklines]|load_env_cfg()| 返回一个 \verb|deepcopy|（而非「调函数」），从而避免训练运行间共享状态——这也是 CLI 覆盖（如 \Verb[breaklines]|--env.rewards.X.weight 0.5|，经 tyro 解析）不污染原始配置的原因。任务发现靠 Python entry\_points（group \verb|mjlab.tasks|）而非 gym registry，故 \verb|list-envs| 读的是 \verb|_REGISTRY|\,\cite{mjlab2026}。
  \item \textbf{Isaac Lab}：用 \Verb[breaklines]|gymnasium.register()|，以 \Verb[breaklines]|env_cfg_entry_point| 指向配置类，CLI 用 argparse 预定义参数\,\cite{isaaclab2024}。
  \item \textbf{UniLab}：用\textbf{双装饰器}注册（\texttt{base/\allowbreak registry.\allowbreak py}，UniLab commit \texttt{99936e08}）——\Verb[breaklines]|@registry.envcfg("Go2JoystickFlat")| 注册\emph{配置类}，\Verb[breaklines]|@registry.env("Go2JoystickFlat", sim_backend="mujoco")| 注册\emph{env 类 + 后端}（同名可叠多个后端，如再加一行 \Verb[breaklines]|sim_backend="motrix"|）。运行时 \Verb[breaklines]|make(name, sim_backend, env_cfg_override, num_envs)| 建实例，CLI 覆盖经 Hydra 的 \Verb[breaklines]|apply_cfg_overrides|（\textbf{深合并}嵌套键，如 \Verb[breaklines]|env.scene.terrain.generator.num_rows=4| 只改这一项、保其余默认）——与 mjlab 的 tyro「任意深度覆盖」同思想。注意 UniLab 有\textbf{两套命名}：注册名是 CamelCase（\Verb[breaklines]|Go2JoystickFlat|，实测共 26 个任务），CLI 的 \verb|--task| 是小写 slug（\Verb[breaklines]|go2_joystick_flat|），二者经 owner YAML 的 \Verb[breaklines]|training.task_name| 字段绑定。
\end{itemize}
...
本节是全章的对比实战主体：每个 Manager 先立\emph{它管什么 MDP 组件}（算法主线），再走读其 \verb|compute()|/\allowbreak\verb|apply()| 核心逻辑，最后给出 mjlab / Isaac Lab / UniLab 三框架的 API 对照。先看总览（\cref{tab:rlmc-mgr-naming}），这是两框架间切换的「翻译表」。
...
\midrule
环境基类 & \Verb[breaklines]|ManagerBasedRlEnvCfg| & \Verb[breaklines]|ManagerBasedRLEnvCfg| & 注意 \verb|Rl| vs \verb|RL| \\
机器人配置 & \verb|EntityCfg| & \Verb[breaklines]|ArticulationCfg| & mjlab 更通用 \\
场景配置 & \verb|SceneCfg| & \Verb[breaklines]|InteractiveSceneCfg| & Isaac Lab 有 Interactive 前缀 \\
obs 组名 & \verb|"actor"| / \verb|"critic"| & \verb|"policy"|(默认) / \verb|"critic"| & 见 \cref{ins:rlmc-mgr-asym} \\
obs 项 & \Verb[breaklines]|ObservationTermCfg| & \Verb[breaklines]|ObservationTermCfg|（常 import 为 \verb|ObsTerm|） & 全名同；短别名仅 Isaac Lab 侧习惯 \\
...
termination 项 & \Verb[breaklines]|TerminationTermCfg| & \Verb[breaklines]|TerminationTermCfg|（常作 \verb|DoneTerm|） & 全名同；\textbf{短别名} \verb|TermTerm|\textbf{/}\verb|DoneTerm| \textbf{均不存在于库，是 import 重命名} \\
任务注册 & \Verb[breaklines]|register_mjlab_task()| & \Verb[breaklines]|gymnasium.register()| & 机制不同 \\
CLI 覆盖 & tyro（任意深度） & argparse（预定义） & mjlab 覆盖更灵活 \\
\bottomrule
\end{tabularx}
\end{table}

\begin{insight}[mjlab 不提供 \texttt{ObsTerm/\allowbreak RewTerm/\allowbreak DoneTerm} 短别名]\label{ins:rlmc-mgr-alias}
一个跨框架抄代码时最易踩的坑：\verb|ObsTerm|/\allowbreak\verb|RewTerm|/\allowbreak\verb|EventTerm|/\allowbreak\verb|DoneTerm|/\allowbreak\verb|ObsGroup|/\allowbreak\verb|CurrTerm| 这些短名是 \emph{Isaac Lab 用户在 import 处自行重命名}的习惯（\Verb[breaklines]|from ... import ObservationTermCfg as ObsTerm|），\textbf{并非库本身导出的符号}。mjlab 的 \Verb[breaklines]|from mjlab.managers import ...| 只导出全名（\Verb[breaklines]|ObservationTermCfg|/\allowbreak\verb|RewardTermCfg|/\allowbreak\verb|EventTermCfg|/\allowbreak\Verb[breaklines]|TerminationTermCfg|/\allowbreak\Verb[breaklines]|CurriculumTermCfg|/\allowbreak\Verb[breaklines]|ObservationGroupCfg| 等），\textbf{没有任何短别名}。因此本节后续 codebox 为对照紧凑用了 \verb|ObsTerm| 等写法，但你在 mjlab 里要么写全名、要么自己在文件顶 \verb|as| 重命名——直接 \Verb[breaklines]|from mjlab.managers import ObsTerm| 会 \verb|ImportError|。mjlab 唯一的 term 基类是 \Verb[breaklines]|ManagerTermBase|（\emph{不是} \verb|ObsTermBase|/\allowbreak\Verb[breaklines]|RewardTermBase|，后两者不存在）。
\end{insight}
...
termination & TerminationManager & \verb|update_state| 直接算 + 基类 \Verb[breaklines]|_compute_truncated| \\
event (DR) & EventManager & \Verb[breaklines]|DomainRandomizationManager|（init/reset/interval 三模式） \\
command & CommandManager & \verb|Commands|（commands.py） \\
curriculum & CurriculumManager & \Verb[breaklines]|PenaltyCurriculum| + \Verb[breaklines]|TerrainCurriculumCfg| \\
action & ActionManager & \verb|apply_action|（位置目标残差，后端 PD 增益） \\
数据源 & Entity / Articulation & \verb|NpEnvState| + 后端 getter（\Verb[breaklines]|get_local_linvel| 等） \\
\bottomrule
\end{tabular}
\end{table}

\begin{insight}[obs 组名是高发的「翻译陷阱」，且需分清两层]\label{ins:rlmc-mgr-asym}
这一点经联网核对后必须讲清，因为它最易踩坑。\textbf{第一层（环境侧 group 名）}：mjlab velocity 任务的两个观测组源码里就叫 \verb|actor| 与 \verb|critic|\,\cite{mjlab2026}；而 Isaac Lab 的 locomotion \emph{velocity 基类}默认\textbf{只}定义一个 \verb|policy| 组，\verb|critic|（非对称特权组）只在启用非对称演员-评论家的具体机器人配置里才追加——所以「默认就有 policy 和 critic 两组」对 locomotion \emph{不准确}：\verb|policy| 才是唯一保证的默认\,\cite{isaaclab2024}。\textbf{第二层（RSL-RL runner 侧）}：runner 的 \verb|obs_groups| 是 \Verb[breaklines]|dict[str,list[str]]|，其\emph{键}是算法角色 \verb|"actor"|/\allowbreak\verb|"critic"|，\emph{值}是环境 group 名的列表\,\cite{schwarke2025rslrl,isaaclab2024}。两层别混：环境侧叫什么、与 runner 侧映射到哪个角色，是两件事。
\end{insight}
...
\paragraph{配置项耦合（关键）。}\Verb[breaklines]|enable_corruption| 是组级开关：actor 组设 \verb|True|（加噪以逼近真机传感器），critic 组设 \verb|False|（特权信息不该被噪声污染）。这与非对称演员-评论家的部署逻辑一致——actor 须可部署、critic 只在训练中输出价值（数学依据见 \cref{ch:rlmc-obsact} 与理论部 GAE 推导）。

\paragraph{三框架对比。}
\begin{itemize}
  \item \textbf{mjlab}：obs 项以 \Verb[breaklines]|terms={"name": cfg}| 字典组织，组名 \verb|actor|/\allowbreak\verb|critic|\,\cite{mjlab2026}。
  \item \textbf{Isaac Lab}：用 \Verb[breaklines]|ObservationsCfg| 的嵌套 \verb|@configclass|，每个 group 是继承 \Verb[breaklines]|ObservationGroupCfg|（别名 \verb|ObsGroup|）的内部类；基类默认只有 \verb|policy| 组\,\cite{isaaclab2024}。
  \item \textbf{UniLab}：\textbf{无 ObservationManager}——env 实现一个属性 \Verb[breaklines]|obs_groups_spec -> dict[str,int]|（组名$\to$维度，UniLab commit \texttt{99936e08}，如 Go2 joystick 返回 \Verb[breaklines]|{"obs": 49, "critic": 52}|），并在 \verb|update_state| 调用的 \verb|_compute_obs| 里\emph{手动}拼各组张量。env 侧组名\textbf{固定为 \texttt{obs} 与 \texttt{critic}}：\Verb[breaklines]|base/observations.py| 的 \Verb[breaklines]|split_obs_dict| 把 \verb|obs["obs"]| 喂 actor、\verb|obs["critic"]| 喂 critic（无 critic 组则复用 obs）。两层映射与 mjlab 同思想（Hydra \Verb[breaklines]|algo.obs_groups={actor:[actor],critic:[critic]}| 把 RL runner 角色路由到 env 组），但 env 侧固定只有 \texttt{obs}/\allowbreak\texttt{critic} 两组、且加噪/clip 是 \verb|_compute_obs| 里手写而非 term 级配置。
\end{itemize}
...
\paragraph{velocity 任务的典型奖励族（算法视角）。}腿足速度跟踪的奖励大体分四类：\textbf{跟踪}（正激励，如 \Verb[breaklines]|track_lin_vel_xy_exp|、\Verb[breaklines]|track_ang_vel_z_exp|，用指数核 $\exp(-e^2/\sigma^2)$）、\textbf{正则}（负惩罚，如 \Verb[breaklines]|action_rate_l2|、关节加速度）、\textbf{风格}（如躯干水平 \Verb[breaklines]|flat_orientation_l2|）、\textbf{接触}（如足端打滑 \verb|foot_slip|）。\cref{tab:rlmc-mgr-rewexample} 给出权重符号约定。\textbf{命名提醒（两库已逐字核实）}：本节用的函数名（\texttt{track\_lin\_vel\_xy\_exp}/\allowbreak\texttt{track\_ang\_vel\_z\_exp}/\allowbreak\texttt{foot\_slip}）取自 Isaac Lab 的 \texttt{mdp} 库，便于与理论部呼应；mjlab 的同功能函数\emph{命名不同}——其 \texttt{tasks/\allowbreak velocity/\allowbreak mdp} 里跟踪奖励叫 \texttt{track\_linear\_velocity}/\allowbreak\texttt{track\_angular\_velocity}、足端打滑叫 \texttt{feet\_slip}（而 \texttt{action\_rate\_l2}/\allowbreak\texttt{flat\_orientation\_l2} 等通用项两库同名）。故落地时务必按「功能」而非「照搬名字」查你所用框架的 \texttt{mdp} 模块。
...
类别 & 示例项 & 权重符号 \\
\midrule
跟踪 & \Verb[breaklines]|track_lin_vel_xy_exp| & $+$（如 1.5） \\
跟踪 & \Verb[breaklines]|track_ang_vel_z_exp| & $+$（如 0.75） \\
正则 & \Verb[breaklines]|action_rate_l2| & $-$（如 $-0.01$） \\
风格 & \Verb[breaklines]|flat_orientation_l2| & $-$ \\
接触 & \verb|foot_slip| & $-$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{三框架对比。}reward 项配置类全名 \verb|RewardTermCfg| 在 mjlab 与 Isaac Lab 中一致（Isaac Lab 教程常 import 重命名为 \verb|RewTerm|，mjlab 无此短别名、须写全名，见 \cref{ins:rlmc-mgr-alias}）；dt 缩放逻辑两者一致。\textbf{UniLab 在这里范式截然不同}：它\textbf{没有 RewardTermCfg 类}，reward 是一个 Hydra \emph{标量字典} \Verb[breaklines]|reward.scales={tracking_lin_vel: 1.0, lin_vel_z: -5.0, ...}| 加 env 内一张\emph{派发表} \Verb[breaklines]|_reward_fns: dict[str, Callable]|（UniLab commit \texttt{99936e08}，共享函数在 \texttt{envs/\allowbreak locomotion/\allowbreak common/\allowbreak rewards.\allowbreak py}，约 40 个）。每步 \Verb[breaklines]|run_reward_dispatch()| 算 $r=\sum_{\text{name}}\text{scales[name]}\cdot\text{fns[name]}(\text{ctx})$，跳过 \verb|scale==0| 的项，\textbf{返回 \texttt{reward * ctrl\_dt}}——即 UniLab \emph{也}做 dt 缩放（等价 mjlab/Isaac 的 \verb|scale_by_dt|），但乘的是 \verb|ctrl_dt|（控制周期本身），而非 \Verb[breaklines]|physics_dt*decimation|。下面把同一条 tracking 奖励在三框架并排：
...
def tracking_lin_vel(ctx):                       # common/rewards.py，返回 [num_envs]
    ...
    return np.exp(-e / ctx.tracking_sigma)       # 指数核，sigma 经 reward.tracking_sigma 配
...
\paragraph{算法主线。}ActionManager 把策略输出的 raw action 变成物理引擎可执行的控制信号，分两阶段：\Verb[breaklines]|process_action()|（步 2，一次性 scaling/clipping/offset）与 \Verb[breaklines]|apply_action()|（步 3，每个 substep 经 actuator 算力矩）。\cref{ch:rlmc-obsact} 已讲四种动作抽象（关节位置/速度/力矩/微分逆运动学）与缩放原理，这里只看 Manager 的两阶段编排。
...
\paragraph{velocity 任务默认用关节位置动作。}\Verb[breaklines]|JointPositionAction| 配 \verb|scale=0.25|、\Verb[breaklines]|use_default_offset=True|，意味着策略输出 $a\in[-1,1]$ 时实际关节目标是 $\text{default\_pos}+a\times0.25$（rad）。\verb|scale=0.25| 限制探索范围在默认姿态 $\pm 0.25$ rad 内，对训练稳定性至关重要：若 scale 太大（如 1.0），训练初期随机策略可能产生极端关节位置，导致物理不稳定（NaN）。

\paragraph{三框架对比。}动作类名（如 \Verb[breaklines]|JointPositionActionCfg|）在 mjlab 与 Isaac Lab 中语义对应、命名近似；actuator 模型实现不同（mjlab 经 MuJoCo builtin/explicit，Isaac Lab 经 PhysX/显式 PD）。\textbf{UniLab 无 \texttt{JointPositionActionCfg} 类}：动作固定是\emph{关节位置目标对默认姿态的残差}，在 \verb|apply_action| 里算 \Verb[breaklines]|ctrl = act * action_scale + default_angles|（UniLab commit \texttt{99936e08}，\texttt{envs/\allowbreak locomotion/\allowbreak common/\allowbreak base.\allowbreak py}；\verb|action_scale| 默认 0.25、\Verb[breaklines]|default_angles| 取自 keyframe \verb|home| 的 qpos 尾段，与 mjlab velocity 任务的 \Verb[breaklines]|scale=0.25, use_default_offset=True| 数值意图一致）。两阶段（process/apply）由 \verb|NpEnv| 模板隐式承担（\cref{ins:rlmc-mgr-unilab} 的时序里 \verb|apply_action| 后即进 \Verb[breaklines]|backend.step(ctrl, nsteps)|）。actuator 模型 = \textbf{后端位置执行器 + PD 增益}（Ideal PD 层级）：\verb|kp/kd| 经 \Verb[breaklines]|create_backend(..., position_actuator_gains={"kp": Kp, "kd": Kd})| 传入（\Verb[breaklines]|PdControlConfig| 默认 \Verb[breaklines]|Kp=35.0, Kd=0.5|），力矩/速度饱和由 MJCF/Motrix 模型自身的 actuator 定义（\verb|ctrlrange|/\allowbreak\verb|forcerange|/\allowbreak\verb|gear|）承担，\textbf{不}在 Python 配 \verb|DCMotorCfg| 等价类。UniLab 也无 learned-actuator 网络等价物（其执行器停在 Ideal PD 层级），sim2real 的执行器误差主要靠 kp/kd 域随机化覆盖（见 \cref{sec:rlmc-mgr-event} 的 UniLab DR）。
...
\paragraph{flat 与 rough 的 termination 差异（一个高发坑）。}见 \cref{tab:rlmc-mgr-flatrough}。在 rough 地形上若保留 \verb|fell_over|（姿态角检查），上坡时的合理前倾会被误判为摔倒，导致频繁 reset、训练效率极低；正确做法是改用 \Verb[breaklines]|illegal_contact|（检测特定 body 是否触地）。
...
\Verb[breaklines]|illegal_contact| & 无 & 有 & rough 用接触传感器检测非法触地 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{三框架对比。}\textbf{这是命名陷阱最典型处}：两库的终止项配置类全名都是 \Verb[breaklines]|TerminationTermCfg|，但 \emph{短别名习惯不同}——Isaac Lab 教程常 \Verb[breaklines]|import ... as DoneTerm|，而 mjlab \textbf{不提供任何短别名}（须写全名 \Verb[breaklines]|TerminationTermCfg| 或自行 \verb|as| 重命名）。换言之 \verb|DoneTerm| 与本节为紧凑所用的 \verb|TermTerm| \textbf{都不是库导出符号}（见 \cref{ins:rlmc-mgr-alias}）——直接 \Verb[breaklines]|from mjlab.managers import DoneTerm|（或 \verb|TermTerm|）会 \verb|ImportError|；跨框架抄代码忘了改 import 即报错。\textbf{UniLab 把这个陷阱直接消除了——因为它\emph{没有}终止项类}：\verb|terminated| 在 \verb|update_state| 里直接算（UniLab commit \texttt{99936e08}，如 Go2 用 \Verb[breaklines]|gravity[:, 2] <= 0.5| 判摔倒），超时 \verb|truncated| 由基类 \Verb[breaklines]|_compute_truncated| 按 \Verb[breaklines]|max_episode_steps| 自动算，二者经 \verb@done = terminated | truncated@ 合并。值得注意的是 \cref{ins:rlmc-mgr-term} 的 terminated/truncated 区分在 UniLab 里被\textbf{固化成一个显式契约类} \Verb[breaklines]|TransitionBootstrapContract|（\texttt{base/\allowbreak final\_observation.\allowbreak py}，区分 \Verb[breaklines]|timeout_terminal_mask| 与失败终止，并用 \Verb[breaklines]|terminal_next_obs| 给终止帧的 \verb|next_obs| 打补丁，保证 bootstrap 用真终止观测而非 autoreset 后的新 obs）——这与 mjlab/Isaac 的 \verb|time_out| 标记同语义，但 UniLab 做成了数据结构而非 term 上的一个 bool 字段。
...
\verb|startup| 的随机化在整个训练只执行一次——每个并行 world 的质量/摩擦在训练开始时确定、之后不变。若想\emph{每次 reset} 都重随机化质量，要把 mode 改成 \verb|reset|。mjlab 的实现机制（已据源码核实）：被随机化的 DR 函数上挂装饰器 \Verb[breaklines]|@requires_model_fields("body_mass", recompute=RecomputeLevel.set_const)|（\texttt{managers/\allowbreak event\_manager.\allowbreak py}），env 构造时 \Verb[breaklines]|sim.expand_model_fields(event_manager.domain_randomization_fields)|（\texttt{manager\_based\_rl\_env.\allowbreak py}）为这些字段一次性分配 per-world 内存——而 \Verb[breaklines]|expand_model_fields| 用 \verb|setattr| 换了 GPU 数组地址，故其末尾\textbf{必须 \texttt{create\_graph()} 重捕 CUDA Graph}（否则旧 graph 静默读旧数组、忽略新 per-world 值）。这是「改 reset 期 DR 字段集」带来的一次性图重建代价。所以\textbf{绝不要}在 \verb|step| 模式 event 里做会触发图重建的操作——每步重建图是性能灾难。（重算等级用枚举 \Verb[breaklines]|RecomputeLevel|，非字符串；用户不手动置 \Verb[breaklines]|_graph_captured|，捕获由 \Verb[breaklines]|wp.is_mempool_enabled| 门控自动进行。）
\end{insight}

\paragraph{三框架对比。}event 项配置类全名 \verb|EventTermCfg|（Isaac Lab 常作 \verb|EventTerm|，mjlab 无短别名）在 mjlab 与 Isaac Lab 中一致；但两库\emph{支持的模式集合}并不相同——mjlab 有四种模式（含 \verb|step|），\textbf{Isaac Lab 基础 \texttt{EventManager} 不含 \texttt{step} 模式}（其模式为 \verb|startup|/\allowbreak\verb|reset|/\allowbreak\verb|interval|，另有 USD 级 \verb|prestartup|），且 step() 内对模式的\emph{自动调度}也不同（\cref{tab:rlmc-mgr-stepdiff}：Isaac Lab step 内只自动跑 \verb|interval|）。\textbf{UniLab 无 EventManager / EventTerm}——域随机化由专门的 \Verb[breaklines]|DomainRandomizationManager| 承担（UniLab commit \texttt{99936e08}，\texttt{dr/\allowbreak manager.\allowbreak py}），模式集合是\textbf{三种}：\verb|init|（$\approx$mjlab/Isaac 的 \verb|startup|，\Verb[breaklines]|apply_init_randomization|，整训一次）、\verb|reset|（per-episode，\Verb[breaklines]|build_reset_plan| 经后端 \Verb[breaklines]|set_state(randomization=)| 写入）、\verb|interval|（步 2 自计时，\Verb[breaklines]|apply_interval_randomization_if_due(step)|）。\textbf{UniLab 无 \texttt{step} 模式}（每步无条件扰动）——逐步腐蚀（如观测噪声）在 UniLab 里由 \verb|_compute_obs| 内的 \verb|_obs_noise| 手写承担，而非一个 step-mode event。一个 UniLab 特有的工程点：每个后端经 \Verb[breaklines]|get_dr_capabilities()| 声明它支持哪些 reset 项（MuJoCo 后端静态 11 项、Motrix 后端动态门控基线 4 项），\Verb[breaklines]|filter_reset_payload| 自动剔除后端不支持的项（打 warning 跳过、不报错）——这是「同一份 DR 配置在两个后端上优雅降级」的设计，是 mjlab/Isaac 单后端时不存在的关切。

% -------------------------------------------
\subsection{CommandManager 与 CurriculumManager}
\label{sec:rlmc-mgr-cmd}

\paragraph{CommandManager（算法主线）。}它生成任务条件变量（如速度命令 $(v_x,v_y,\omega_z)$），并经 \Verb[breaklines]|generated_commands| obs term 进入 actor 观测——策略必须\emph{看到}命令才能做条件性行为。关键属性：\Verb[breaklines]|resampling_time_range|（每 $N$ 秒重采样一次命令）、\Verb[breaklines]|rel_standing_envs|（一部分环境给零命令，逼策略学会「站立」而非乱动）。
...
\paragraph{三框架对比。}command/curriculum 概念两框架一致，配置类名近似。\textbf{UniLab 同样无 CommandManager / CurriculumManager 类}，但两个职责都有专门承担者（UniLab commit \texttt{99936e08}）：命令由 \verb|Commands|（\texttt{envs/\allowbreak locomotion/\allowbreak common/\allowbreak commands.\allowbreak py}，速度命令采样 + heading 反馈）生成，经 \Verb[breaklines]|info["commands"]| 进入 \verb|_compute_obs| 拼进 actor 观测（与「策略必须看到命令」的逻辑一致）；课程则\textbf{拆成两个独立机制}——\Verb[breaklines]|PenaltyCurriculum|（\texttt{base/\allowbreak curriculum.\allowbreak py}，按滑窗 episode 长度自适应缩放\emph{负权重} reward，识别 \verb|scales| 里 \verb|<0| 的项在 \Verb[breaklines]|[min_scale, max_scale]| 间调，原地改 \Verb[breaklines]|cfg.reward_config.scales|）与 \Verb[breaklines]|TerrainCurriculumCfg|/\allowbreak\Verb[breaklines]|TerrainSpawnManager|（地形等级）。注意 UniLab 的 \Verb[breaklines]|PenaltyCurriculum| 是「按表现\emph{放松/收紧惩罚}」，与 mjlab/Isaac 课程常做的「放宽命令范围/升地形难度」是\emph{互补}的两类课程信号。
...
\rebuilt{上面的 \texttt{uv run play/\allowbreak train} 与 \texttt{--agent zero} 等 CLI 为 mjlab 约定；Isaac Lab 对应 \texttt{python scripts/\allowbreak .../\allowbreak train.\allowbreak py --task ...}；UniLab 对应 \texttt{uv run train --algo ppo --task <slug> --sim mujoco}（env 数/迭代数走 Hydra 覆盖，如 \texttt{algo.\allowbreak num\_envs=64 algo.\allowbreak max\_iterations=5}），但 UniLab 新增 obs/reward 是改 \texttt{update\_state}/\allowbreak\texttt{\_reward\_fns} 代码而非纯配置，故其「三步流程」第 1--2 步与 mjlab/Isaac 的「写函数+config 引用」不同。}

\begin{note}[\texttt{uv run train/\allowbreak play} 还是直接 \texttt{train/play}？看你怎么装的]
本章多处写 \Verb[breaklines]|uv run train| / \Verb[breaklines]|uv run play|，那是「克隆源码 + \texttt{uv} 管虚拟环境」的开发者形态——\Verb[breaklines]|uv run| 的作用只是「在该项目虚拟环境里执行」。但 mjlab 把 \verb|train|/\allowbreak\verb|play| 注册成了\textbf{控制台脚本（console\_scripts 入口）}：只要装进了当前已激活的环境（\Verb[breaklines]|pip install| 或在 \verb|uv| 环境内），就可\textbf{直接敲 \texttt{mjlab train ...} / \texttt{mjlab play ...}}（或视安装方式直接 \verb|train|/\allowbreak\verb|play|），\textbf{不必前缀 \texttt{uv run}}。本服务器即直接用入口脚本跑通。所以你照抄命令若提示 \Verb[breaklines]|uv: command not found| 或不在 uv 项目里，把 \Verb[breaklines]|uv run | 去掉再试——二者只是「在哪个环境执行」的差别，命令本身与所有 \verb|--env.*|/\allowbreak\verb|--agent.*| 覆盖项完全一致。\pz{究竟是 \texttt{mjlab train} 还是裸 \texttt{train} 取决于打包配置，以你那版 \texttt{--help} 实际可用的为准。}
\end{note}
...
\begin{enumerate}
  ...
  \item \textbf{[动手]} 实现 \Verb[breaklines]|maintain_base_height| reward，训练约 100 iteration，观察 WandB 的 \Verb[breaklines]|Episode_Reward/maintain_height| 曲线。\emph{验收}：曲线非零且随训练变化。
...
属性 & 形状 & 含义 & 坐标系 \\
\midrule
\Verb[breaklines]|root_link_pos_w| & $[N,3]$ & 基座位置 & 世界 \\
\Verb[breaklines]|root_link_quat_w| & $[N,4]$ & 基座四元数 (w,x,y,z) & 世界 \\
\Verb[breaklines]|root_link_lin_vel_b| & $[N,3]$ & 基座线速度 & 机体 \\
\Verb[breaklines]|root_link_ang_vel_b| & $[N,3]$ & 基座角速度 & 机体 \\
\verb|joint_pos| & $[N,J]$ & 关节位置 & 关节空间 \\
\verb|joint_vel| & $[N,J]$ & 关节速度 & 关节空间 \\
\Verb[breaklines]|default_joint_pos| & $[N,J]$ & 默认关节位置 & 关节空间 \\
\Verb[breaklines]|body_link_pos_w| & $[N,B,3]$ & 所有 body 的\emph{连杆原点}位置 & 世界 \\
\Verb[breaklines]|body_com_pos_w| & $[N,B,3]$ & 所有 body 的\emph{质心}位置 & 世界 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{\texttt{\_link\_} 与 \texttt{\_com\_}：mjlab 1.4.0 把每个位姿/速度都拆成了两套。}注意上表没有笼统的 \verb|body_pos_w|——mjlab 1.4.0 把 body 与 root 的位置/姿态/速度都\textbf{显式拆成「连杆系（\texttt{\_link\_}）」与「质心系（\texttt{\_com\_}）」两套属性}（\Verb[breaklines]|body_link_pos_w|/\allowbreak\Verb[breaklines]|body_com_pos_w|、\Verb[breaklines]|root_link_pos_w|/\allowbreak\Verb[breaklines]|root_com_pos_w| 等）。原因是 MuJoCo 里一个 body 的\emph{连杆坐标原点}（你在 MJCF 里定义关节、几何挂载点的那个点）与它的\emph{质心}（动力学积分点）一般不重合；动力学（动量、力）关心 \texttt{\_com\_}，而运动学约束、足端落点、可视化关心 \texttt{\_link\_}。这比 Isaac Lab 的单一 \verb|body_pos_w| 更细——\textbf{随手写 \texttt{body\_pos\_w} 会 \texttt{AttributeError}}，得先想清你要的是连杆原点还是质心。足端高度这类几何量用 \Verb[breaklines]|body_link_pos_w|。

\paragraph{机体系 vs 世界系。}locomotion 策略通常用\emph{机体系}速度（\Verb[breaklines]|root_link_lin_vel_b|）——策略应学「向前走」而非「向北走」。obs term \verb|base_lin_vel| 默认返回的就是机体系速度。

\begin{insight}[别照抄属性名——学会一行列出\emph{你这版}的全部字段]\label{ins:rlmc-mgr-introspect}
本章这张 \cref{tab:rlmc-mgr-entitydata} 是全章\textbf{最易随版本漂移}的表（\verb|body_pos_w| 在某些旧文档里有、mjlab 1.4.0 已拆成 \texttt{\_link\_}/\allowbreak\texttt{\_com\_} 两套就是活例）。与其把这张表当字典背，不如记住：\textbf{任何一版的真实属性，都能用一行 Python 当场列出}，不必猜、不必翻文档：
...
\cref{tab:rlmc-mgr-entityapi} 是 mjlab Entity 与 Isaac Lab Articulation 的对照表。\pz{具体属性名仍以各框架你所装版本为准；另需注意 Isaac Lab 3.0 起的两处官方 Breaking Change：\texttt{data.*} 返回类型由 \texttt{torch.\allowbreak Tensor} 改为 Warp 数组、四元数由 wxyz 改为 xyzw（见下方两个陷阱）。}
...
\midrule
基座位置 & \Verb[breaklines]|data.root_link_pos_w| & \Verb[breaklines]|data.root_pos_w| \\
基座四元数 & \Verb[breaklines]|data.root_link_quat_w| & \Verb[breaklines]|data.root_quat_w| \\
基座线速度(世界) & \Verb[breaklines]|data.root_link_lin_vel_w| & \Verb[breaklines]|data.root_vel_w[:, :3]| \\
基座线速度(机体) & \Verb[breaklines]|data.root_link_lin_vel_b| & 需手动转换 \\
关节位置 & \Verb[breaklines]|data.joint_pos| & \Verb[breaklines]|data.joint_pos| \\
默认关节角 & \Verb[breaklines]|data.default_joint_pos| & \Verb[breaklines]|data.default_joint_pos| \\
设置关节位置目标 & \Verb[breaklines]|set_joint_position_target(...)| & \Verb[breaklines]|set_joint_position_target(...)| \\
重置基座位姿 & \Verb[breaklines]|write_root_link_pose_to_sim| & \Verb[breaklines]|write_root_pose_to_sim| \\
\bottomrule
\end{tabular}
\end{table}

\begin{insight}[mjlab 直接给机体系速度，是面向 locomotion 的贴心优化]\label{ins:rlmc-mgr-bodyframe}
mjlab 的 Entity 直接暴露机体系速度 \Verb[breaklines]|root_link_lin_vel_b|，而 Isaac Lab 的 Articulation 只给世界系速度，需在 obs term 里用基座四元数手动转到机体系。对 locomotion 这是个小但高频的便利——因为几乎每个本体感知观测都要机体系量。\textbf{UniLab 数据源}：UniLab 没有 Entity/Articulation 这种「资产对象」，状态来自 \verb|NpEnvState|（\Verb[breaklines]|obs/reward/terminated/truncated/info/final_observation| 的不可变快照，UniLab commit \texttt{99936e08}）加一组\emph{后端 getter}——\Verb[breaklines]|get_local_linvel()|、\verb|get_gyro()|、\verb|get_dof_pos()|/\allowbreak\verb|get_dof_vel()|、\Verb[breaklines]|get_base_pos()|、\Verb[breaklines]|get_sensor_data(name)|。关键对照：UniLab \textbf{也}直接给机体系速度（后端 \verb|SimBackend| 的 \Verb[breaklines]|get_body_lin_vel_b()| / 便利方法 \Verb[breaklines]|get_local_linvel()| 走机体系传感器），与 mjlab 的便利一致、优于 Isaac Lab 2.x 需手动转换——三框架在「机体系速度可得性」上是 mjlab 与 UniLab 同档、Isaac Lab 需自己转。
\end{insight}
...
\textbf{错误描述}：在 3.0 上像 2.x 那样直接对 \verb|data.*| 做 torch 运算。\textbf{现象后果}：tensor 类型错误。\textbf{根本原因}：Isaac Lab 3.0 的数据管线返回类型有变——据其 v3.0.0-beta 发布说明，所有 \texttt{.data.*} 属性改为返回 \texttt{wp.array}（NVIDIA Warp 数组）而非 \texttt{torch.\allowbreak Tensor}（内部状态缓冲改用 \texttt{wp.vec3f}/\allowbreak\texttt{wp.quatf} 等结构化 Warp 类型）。\textbf{正确做法}：在 3.0 上需要 torch 运算时，先用 \texttt{wp.\allowbreak to\_torch()} 把 \texttt{data.*} 转成张量再算（2.x 仍直接是 torch 张量，不需此步）。
\end{pitfall}

\begin{practice}[本节练习]
\begin{enumerate}
  \item \textbf{[动手]} 分别在 mjlab 与 Isaac Lab 中获取机器人基座高度，写出含「如何从 env 取 robot 对象」的完整片段。\emph{验收}：两段代码 + 指出 \Verb[breaklines]|root_link_pos_w| vs \verb|root_pos_w| 的差异。
  \item \textbf{[动手]} 写一个 obs term，返回四个足端的世界系高度（$[\text{num\_envs},4]$）。\emph{验收}：说明如何由 \Verb[breaklines]|body_link_pos_w| 与 body 名索引取到四个足端的 $z$ 分量（提示：\Verb[breaklines]|entity.body_names| 给名字、\Verb[breaklines]|body_link_pos_w| 取连杆原点；想清为何用 \verb|_link_| 而非 \verb|_com_|）。
\end{enumerate}
\end{practice}
...
Sensor 是 Manager 的「数据源扩展」——它提供 Entity 不直接暴露的物理信息（接触力、地形高度、射线检测）。\textbf{一个 sensor 可服务多个 Manager 的多个 term}：例如足端接触传感器同时供 critic obs 的 \verb|foot_contact|、reward 的 \verb|air_time|/\allowbreak\verb|foot_slip|/\allowbreak\verb|soft_landing| 等多项。
...
actor obs 缺 command & 策略走但不跟命令 & 漏 \Verb[breaklines]|generated_commands| & 打印 obs 维度 \\
...
盯哪一行 & 健康样子 & 不对劲说明 \\
\midrule
\Verb[breaklines]|Steps per second| & 稳定的正数（本机 Go1\,@64env $\approx$2000） & 0 或持续掉 $\Rightarrow$ 物理/渲染卡住 \\
\Verb[breaklines]|Value function loss| & 有限、随迭代下降 & \verb|nan|/暴涨 $\Rightarrow$ obs 含 NaN 或 lr 太大 \\
\Verb[breaklines]|Episode_Reward/<term>| & 至少 tracking 项非零 & 某项恒 0 $\Rightarrow$ 该 term 接线失效 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{第 3 步：读出结论并收敛。}哪个 \Verb[breaklines]|Episode_Reward/<term>| 恒为 \verb|0.0000|，失效的就是\emph{那一项}——\textbf{不必逐行看代码}。上例 \verb|feet_slip| 恒零，按 \cref{ins:rlmc-mgr-wiring} 的逻辑：它依赖足端接触传感器，最可能是 sensor 的 geom 正则匹配到了空集（如把 \verb|FL_foot| 误写成小写）。于是排查从「整个奖励系统」一步收敛到「这一个 sensor 的 geom 名」，再去查该 sensor 的 regex 是否命中（\cref{tab:rlmc-mgr-diag} 步 4--5）。\textbf{这就是「打印各 term」相对「逐行排查」的真实威力}——它把 \cref{tab:rlmc-mgr-speedup} 里「定位 reward 为零」那一行的「打印各 term」从一句口号变成了你刚敲过的命令。

\begin{note}[三框架的 reward 日志前缀不同，跨框架查曲线别找错命名空间]
同一个「分项 reward」在三框架的日志键\textbf{前缀不一样}，跨框架排错时极易在 TensorBoard/WandB 里找不到曲线：mjlab 与 Isaac Lab（走 RSL-RL）都是 \Verb[breaklines]|Episode_Reward/<term>|（如 \Verb[breaklines]|Episode_Reward/feet_slip|）；而\textbf{UniLab 实测是 \texttt{reward/\allowbreak <term>}}（如 \Verb[breaklines]|reward/tracking_lin_vel|，无 \texttt{Episode\_} 前缀）。所以本节的「找 \Verb[breaklines]|Episode_Reward/<term>=0|」在 UniLab 上要改成找 \Verb[breaklines]|reward/<term>=0|。记住这条前缀差异，省得你对着 UniLab 日志搜 \Verb[breaklines]|Episode_Reward| 一无所获、误以为「没打分项」。
\end{note}

\subsection{三框架对比：诊断入口}

\begin{itemize}
  \item \textbf{mjlab}：\Verb[breaklines]|uv run play <TASK> --agent zero|（零动作看纯物理）、\Verb[breaklines]|--agent.max-iterations 2| 小训查日志；mjlab 另有针对腿足训练「NaN 爆炸」的 NaN Guard 工具（\texttt{NanGuard}/\allowbreak\texttt{NanGuardCfg}，经 \texttt{sim.\allowbreak nan\_guard.\allowbreak watch(data)} 挂载），可在检测到 NaN 时暂停、记录首次出现的环境与物理变量并支持回放\,\cite{mjlab2026}；命令行用 \texttt{--enable-nan-guard True} 开启（已据 mjlab 1.4.0 实测确认，回放命令名仍以你所装版本 \texttt{--help} 为准）。
  \item \textbf{Isaac Lab}：经脚本参数（如 \verb|--num_envs|）控制小规模调试；默认 NaN 行为不如 mjlab 友好（NaN 常直接 crash），需自行加 NaN 检测。
  \item \textbf{UniLab}：可视化用 \textbf{Viser}（web 3D，\Verb[breaklines]|--render-mode interactive| 浏览器远程看，跨境/无 X 都行）+ 直接读 \verb|NpEnvState|；NaN 处理是其工程亮点——\verb|NpEnv| 内建 \verb|NanGuard|（UniLab commit \texttt{99936e08}，\texttt{utils/\allowbreak nan\_guard.\allowbreak py}）：\verb|check_ctrl| 查动作 NaN、\verb|check| 查 obs/reward NaN、\verb|capture| 存物理快照（环缓存最近 \verb|buffer_size| 帧），命中即 \verb|dump| 写 \Verb[breaklines]|nan_dump_*.npz|（含状态环缓冲 + 复制模型文件），事后 \Verb[breaklines]|unilab-viz-nan <dump>| 用 \verb|mujoco.viewer| 离线回放出事前那几帧（默认 \verb|enabled=true|，且 \verb|step()| 末尾还有兜底 \Verb[breaklines]|np.nan_to_num(reward)|）——这与 mjlab 的 NaN Guard 是\emph{同级}的成熟工具，二者都\textbf{优于} Isaac Lab（NaN 常直接 crash）。另有 \verb|--profile| 产 chrome-trace 时间片，可视化 collector/learner/H2D 重叠（异构架构特供的诊断视角）。
\end{itemize}

\begin{practice}[本节练习]
\begin{enumerate}
  \item \textbf{[动手]} 给一个能跑的 velocity 任务，故意把某接触传感器的 geom regex 拼错，训练 2 iteration，观察哪些 \Verb[breaklines]|Episode_Reward/<term>| 变为零。\emph{验收}：列出受影响的 term，并说明为何不报错。
...
\Verb[breaklines]|enable_corruption| & 是否给 obs 加噪（actor True, critic False） & \cref{sec:rlmc-mgr-obs} \\
...
\begin{itemize}
  \item \textbf{mjlab（arXiv:2601.22074，``mjlab: A Lightweight Framework for GPU-Accelerated Robot Learning''）}\,\cite{mjlab2026}——教 MuJoCo Warp 后端如何复刻 Isaac Lab 的 Manager-Based API，并暴露原生 \verb|MjModel|/\allowbreak\verb|MjData|；其源码用 \verb|WarpBridge| 把 warp 数组零拷贝暴露成 torch 张量（\verb|TorchArray|）。对照本章的 env.step 时序与 Manager 设计阅读。
  ...
  \item \textbf{UniLab（arXiv:2605.30313）}\,\cite{unilab2026}——教「GPU 仿真主导」之外的第三条路：异构 CPU 物理 + GPU 策略、经共享内存 IPC（\Verb[breaklines]|SharedWeightSync|/\allowbreak\verb|ReplayBuffer|）解耦 collector 与 learner。对照本章看「同一条 MDP 数据流为何\emph{不必}拆成 Manager」（\cref{ins:rlmc-mgr-unilab}），以及 APPO/SAC 等异步 off-policy 为何天然契合这套架构。
...
策略不跟命令 & actor obs 缺 command 项 & 查 obs 是否含 \Verb[breaklines]|generated_commands| & \cref{sec:rlmc-mgr-cmd} \\
...
rough 频繁 reset & 保留了 \verb|fell_over| & 改用 \Verb[breaklines]|illegal_contact| & \cref{sec:rlmc-mgr-term} \\
