...
RSL-RL 在 4.0.0 与 5.0.0 各做了一次破坏性 API 变更：4.0.0 把统一的 \verb|policy| 配置拆成独立的 \verb|actor| 与 \verb|critic| 配置，并要求 runner 配置中出现 \verb|obs_groups| 键；5.0.0 进一步把模型分布抽成独立配置（核心库里是带 \verb|class_name| 的 \Verb[breaklines]|distribution_cfg| dict，字段 \verb|init_std| 取代旧 \Verb[breaklines]|init_noise_std|；Isaac Lab 再封装成嵌套类 \Verb[breaklines]|RslRlMLPModelCfg.GaussianDistributionCfg|）。\textbf{现象后果}：用 5.x 的库去加载 3.x/4.x 训练的旧 checkpoint 会因键名不匹配（如 \Verb[breaklines]|actor.0.weight| 与 \verb|mlp.0.weight|）而报 \verb|KeyError|；或 \Verb[breaklines]|OnPolicyRunner| 初始化时因缺 \verb|obs_groups| 而挂起。\textbf{根本原因}：模型结构命名随版本演进。\textbf{正确做法}：在教材/项目里\textbf{固定 RSL-RL 版本}（如 \Verb[breaklines]|rsl-rl-lib>=5.2.0,<6.0.0| 起步，本章实测环境为 \verb|5.4.0|；同一 \verb|5.x| 小版本间 API 行为一致，但\emph{源码文件归属可能迁移}，见 \cref{sec:rlmc-train-dataflow} GAE 走读处的导航提示），并优先用框架提供的 runner 子类（如 mjlab 的扩展 runner）而非直接继承基类——它通常内置了旧 checkpoint 的键名迁移逻辑。以上版本节点经 RSL-RL 官方 release 记录核对 \cite{schwarke2025rslrl}。
\end{pitfall}

\paragraph{分操作系统步骤（最小要点）。}三个框架在依赖隔离上的取向不同：mjlab 用 \verb|uv| 管理虚拟环境、对 Linux/macOS 较友好；Isaac Lab 绑定 Isaac Sim，强依赖 NVIDIA 驱动与特定 CUDA，推荐 Ubuntu 22.04；UniLab 也用 \verb|uv|，但走\textbf{异构}路线——物理仿真在 CPU、策略学习在 GPU，因此\emph{不绑定} Isaac Sim、也\emph{不依赖} GPU 物理后端（任何能跑 MuJoCo/Motrix 的机器都行），安装即 \Verb[breaklines]|uv sync --extra mujoco|（或 \Verb[breaklines]|--extra motrix|），GPU 仅需 CUDA/MPS/ROCm/XPU 之一供学习器使用。本章的 PPO 概念与诊断方法\emph{不依赖}具体 OS，故以下正文不再分 OS 重复。
...
\textbf{(1) 无显示器渲染（headless）：MuJoCo 默认要 GL 上下文。}现象：\verb|train|/\allowbreak\verb|play| 起手即报 \verb|GLFWError|/\allowbreak\Verb[breaklines]|Failed to create GL context|/\allowbreak\Verb[breaklines]|cannot open display|。为什么：MuJoCo 默认走需要窗口系统的 GLFW，而 SSH 服务器没有 X。做法：导出 \Verb[breaklines]|export MUJOCO_GL=egl|（用无窗口的 EGL 离屏渲染；纯软件回退用 \verb|osmesa|）；纯训练（不回放）可完全不渲染。自查：\Verb[breaklines]|echo $MUJOCO_GL| 是否为 \verb|egl|；EGL 是否可用看 \Verb[breaklines]|ls /usr/lib/x86_64-linux-gnu/libEGL*|。

\textbf{(2) \texttt{uv run} 每次想联网同步依赖。}现象：每条 \Verb[breaklines]|uv run ...| 卡在解析/下载，或离线机直接失败。为什么：\Verb[breaklines]|uv run| 默认会先\emph{同步} lockfile。做法：环境已装好时加 \verb|--no-sync| 跳过（\Verb[breaklines]|uv run --no-sync train ...|），或先一次性 \Verb[breaklines]|uv sync| 装全、之后都 \verb|--no-sync|。自查：\Verb[breaklines]|uv run --no-sync python -c "import mjlab"| 能否秒进。

\textbf{(3) WandB 默认开，无 key 会卡住。}现象：训练起手停在 \Verb[breaklines]|wandb: Logging into wandb.ai|（mjlab \Verb[breaklines]|--agent.logger| 默认就是 \verb|wandb|）。为什么：默认日志后端是 WandB，没登录会等交互输入 API key。做法：冒烟/离线一律 \Verb[breaklines]|export WANDB_MODE=disabled|（或 \verb|offline|），mjlab 也可显式 \Verb[breaklines]|--agent.logger tensorboard|；UniLab 同理离线设此环境变量。自查：起训第一屏不应再出现 \Verb[breaklines]|wandb: (1) Create| 的登录提问。

\textbf{(4) GPU 可用性先验证再训。}先 \Verb[breaklines]|python -c "import torch; print(torch.cuda.is_available())"| 必须 \verb|True|；驱动 CUDA 比 \Verb[breaklines]|torch.version.cuda| 新可向后兼容（无需精确等号）。把这条放最前，能把"训练起不来"快速二分成"环境/驱动问题"还是"任务/配置问题"。

记法：\textbf{这四条按"渲染$\to$包$\to$日志$\to$算力"排，正好是从零起训最常见的失败顺序}；把它们写进你自己的 \verb|activate| 脚本一次设好，后面所有命令就不再被环境噪声干扰。
\end{pitfall}

\begin{insight}[UniLab 与"GPU-sim 主导"正交：物理留 CPU，跨边界搬到 GPU 学习]\label{ins:rlmc-unilab-heterogeneous-train}
...
\begin{note}
实测 mjlab 1.4.0 的 \verb|list-envs| 共列出 \textbf{12 个任务}（含 Cartpole、Lift-Cube、Tracking 等），其中 4 个是 Unitree 速度任务：\Verb[breaklines]|Mjlab-Velocity-{Flat,Rough}-Unitree-{G1,Go1}|（即 \textbf{G1 人形与 Go1 四足都有}；\textbf{无 Go2}——若沿用其他框架的 Go2 命名会找不到任务）。本章正文以 Go1 为例讲数值，与上面命令一致。\verb|list-envs| 是已安装的 console script，可直接 \verb|list-envs| 调用（\Verb[breaklines]|uv run| 仅在未把入口装进当前环境时才需要）。
\end{note}
...
\begin{note}
UniLab 的两条命令\textbf{关键差异}（与 mjlab/Isaac 对照记牢）：(1) 算法、任务、后端用 \verb|--algo|/\allowbreak\verb|--task|/\allowbreak\verb|--sim| \emph{flag}（这四个键含 \Verb[breaklines]|training.sim_backend| 是 RESERVED，\textbf{不能}走 Hydra 透传，否则 \verb|SystemExit|）；(2) 而 \verb|num_envs|、\Verb[breaklines]|max_iterations| 等\emph{走 Hydra 覆盖}（\Verb[breaklines]|algo.num_envs=...|），与 mjlab 的 \Verb[breaklines]|--env.scene.num-envs| flag 风格相反。\verb|--task| 用小写 slug（\Verb[breaklines]|go2_joystick_flat|），对应注册名是 CamelCase 的 \Verb[breaklines]|Go2JoystickFlat|。
\end{note}

\begin{note}[steps/s 怎么算正常：先有估算心法，再看实测锚点（别拿冒烟数当稳态数）]
新手跑完一条小命令最容易问"我这 steps/s 是不是太低、是不是装坏了"。先记一条\textbf{估算心法}：稳态 \verb|steps/s| $\approx$ \verb|num_envs| $\times$ 控制频率 $\times$ 并行效率。它\emph{强依赖并行规模}——大并行 $+$ JIT 热身后才达峰值，\textbf{小 env 冷启（JIT/图未热、并行不足）见到几十到几百 sps 是正常的良性现象，不是故障}。本章正文反复用的 $4096$ 并行 $+$ \Verb[breaklines]|RTX 4090| 上 UniLab 约 $8900$ env-steps/s，是\emph{大并行热稳态}数字；下表是另一台机的小批冒烟实测锚点，用来校准"小 env 本就慢"这个直觉：
\begin{table}[H]
\centering
...
\begin{enumerate}
  ...
  \item PPO 的 batch size 由哪些量决定？\verb|num_envs|、\Verb[breaklines]|num_steps_per_env|、\Verb[breaklines]|num_mini_batches| 三者是什么关系？（答错回看本章 \cref{sec:rlmc-train-dataflow}）
...
\begin{enumerate}
  ...
  \item \textbf{计算}给定 \verb|num_envs| / \Verb[breaklines]|num_steps_per_env| / \Verb[breaklines]|num_mini_batches| / \Verb[breaklines]|num_learning_epochs| 下的 batch size、mini-batch size 与每迭代更新次数。
...
\textbf{第一段：Rollout Collection（采集）。}runner 对每个 env 收集 \Verb[breaklines]|num_steps_per_env| 步。若 \verb|num_envs=4096|、\Verb[breaklines]|num_steps_per_env=24|，一次采集得到 $4096\times24=98304$ 个 transition。每步内，runner 调用 wrapper 的 \verb|step(actions)|，拿到 obs、reward、done、extras，写入 rollout storage。

\textbf{第二段：PPO Learning（优化）。}计算回报（折扣累积 reward）、优势（GAE 估计）、策略损失（clipped surrogate）、价值损失（critic 预测误差）与熵奖励。设 \Verb[breaklines]|num_mini_batches=4|，每个 mini-batch 约 $24576$ 个 transition；设 \Verb[breaklines]|num_learning_epochs=5|，则这批数据被重复优化 5 轮。
...
PPO 的 clip 常被误解为"让更新更保守"。更准确的说法是：clip 是在\textbf{允许同一批数据做多轮 epoch 更新}（\Verb[breaklines]|num_learning_epochs|>1）的同时、维持 on-policy 近似有效的手段。若 \Verb[breaklines]|num_learning_epochs=1| 且 \Verb[breaklines]|num_mini_batches=1|，clip 几乎不起作用——因为只更新一次，新旧策略还没拉开。clip 的全部价值，在于让你用一批数据多榨几轮而不崩。这一洞察直接解释了 \cref{sec:rlmc-train-hyperparam} 里"为何 epochs 调太大反而危险"。
\end{insight}
...
逐行看那一行 \Verb[breaklines]|next_is_not_terminal += self.extras["time_outs"][step]|（关键行）：它确保了"时间到的正常截断"（truncation，奖励/终止章里 \verb|time_out=True|）\textbf{不会}清零 value。\textbf{先讲动机}：locomotion 的 velocity 任务通常是无限时域的，$20\,$s 到点只是回合\emph{被截断}，不是机器人\emph{失败}。
...
上面的 \Verb[breaklines]|compute_returns| 是\emph{同义简化}，用来讲清\emph{逻辑}；它在真实 RSL-RL 里落在哪个文件\textbf{随小版本迁移}。本章实测的 \Verb[breaklines]|rsl-rl-lib 5.4.0| 里，\Verb[breaklines]|compute_returns| 已\textbf{不在} \Verb[breaklines]|storage/rollout_storage.py|（该文件此版只剩 \Verb[breaklines]|add_transition|/\allowbreak\verb|generator|/\allowbreak\Verb[breaklines]|mini_batch_generator|），而是移到了 \Verb[breaklines]|algorithms/ppo.py|（签名变为 \Verb[breaklines]|compute_returns(self, obs)|）与 \Verb[breaklines]|runners/on_policy_runner.py|；timeout 自举的\emph{实现}也从"改 terminal mask"改写成等价的"在记录 transition 时 \Verb[breaklines]|rewards += gamma * values * time_outs|"（数学等价、效果相同）。\textbf{所以正确姿势不是记死文件名，而是在你自己装的库里反查}——这是本章反复要教的"举一反三"：
...
\textbf{运行验证看什么}：rollout storage 对 Go1 velocity 约 $105\,$MB，远小于模型与物理仿真显存。因此若遇到 OOM，优先把 \verb|num_envs| 减半（直接减半物理仿真显存），而不是动 \Verb[breaklines]|num_steps_per_env|（它对 GAE 质量更敏感）。注意 \Verb[breaklines]|mini_batch_generator| 对 MLP \emph{随机打乱}所有 transition——这对 RNN 不成立（见 \cref{sec:rlmc-train-network}）。
...
\Verb[breaklines]|num_steps_per_env| & 同上（典型 $24$） & 调大：rollout 更长、GAE 更准；调小：更新更频但偏差大 & $\ge16$；与 $\gamma,\lambda$ 共同决定优势质量 \\
\Verb[breaklines]|num_mini_batches| & 同上（典型 $4$） & 调大：mini-batch 更小、更新更多、梯度更噪 & $4$--$8$；与 \verb|num_envs| 共定 mini-batch size \\
\Verb[breaklines]|num_learning_epochs| & 同上（典型 $5$） & 调大：更充分用数据，但偏离 on-policy、KL 增大 & $3$--$8$（$\le10$）；与 \verb|clip_param| 强耦合（见 \cref{ins:rlmc-clip-reuse}） \\
\hline
\end{tabular}
\end{table}
...
\hline
runner 配置类 & \Verb[breaklines]|RslRlOnPolicyRunnerCfg| & \Verb[breaklines]|RslRlOnPolicyRunnerCfg| & \verb|PPOConfig|（typed dataclass） \\
rollout/PPO 后端 & RSL-RL 5.x & RSL-RL（共用） & RSL-RL \Verb[breaklines]|OnPolicyRunner| $+$ \Verb[breaklines]|FinalObservationAwarePPO| \\
\verb|num_envs| 入口 & \Verb[breaklines]|--env.scene.num-envs| & \verb|--num_envs| & \verb|algo.num_envs|（Hydra 覆盖） \\
\Verb[breaklines]|num_steps_per_env| & runner cfg & runner cfg & \Verb[breaklines]|algo.num_steps_per_env|（默认 $24$） \\
\hline
\end{tabularx}
\end{table}

mjlab 与 Isaac Lab 共用 RSL-RL，因此 \Verb[breaklines]|num_steps_per_env|、\Verb[breaklines]|num_mini_batches|、\Verb[breaklines]|num_learning_epochs| 三项在两框架\emph{语义完全一致}，仅 CLI 入口与默认任务不同。\textbf{UniLab 的 PPO 同样复用 RSL-RL}（\verb|PPOConfig| 在 \Verb[breaklines]|structured_configs.py|，UniLab；其 \Verb[breaklines]|policy.actor/critic_hidden_dims=[512,256,128]|、\Verb[breaklines]|num_steps_per_env=24| 与上面两框架\emph{逐字对齐}），故\cref{eq:rlmc-batch,eq:rlmc-minibatch,eq:rlmc-updates} 三个公式在三框架\emph{完全通用}——区别只在 CLI 入口（UniLab 走 Hydra \Verb[breaklines]|algo.num_envs=...|）。但有一个\textbf{本质不同}：UniLab 的 $98304$ 个 transition 是 CPU collector 写进\emph{共享内存} ring buffer、再由 GPU learner 整 slot 搬过去的（\cref{ins:rlmc-unilab-heterogeneous-train}），而非像 mjlab/Isaac 那样本就驻留 GPU。UniLab 还提供异步变体 APPO（\verb|APPOConfig|，\verb|num_envs=2048|），collector 不等 learner、被覆盖的旧 rollout 用 V-trace（\Verb[breaklines]|vtrace_clip_rho/c=1.0|）做 off-policy 校正——这是"on-policy 用完即弃"约束在异步架构下的放松（详见 \cref{sec:rlmc-train-ppo-sac}）。

\subsection{常见陷阱}

\begin{pitfall}[\texttt{num\_steps\_per\_env} 过小导致优势估计噪声化]
\textbf{错误描述}：把 \Verb[breaklines]|num_steps_per_env| 设成 $4$（只有 4 步 rollout）。\textbf{现象后果}：GAE 只能用 4 步实际 reward 加 critic 自举来估计优势，严重依赖 critic 准确性；训练初期 critic 很差，优势几乎是噪声，策略迟迟不收敛。\textbf{根本原因}：rollout 太短时 \cref{eq:rlmc-gae} 的有效求和项太少。\textbf{正确做法}：至少 $16$--$32$ 步。
\end{pitfall}
...
\begin{enumerate}
  \item \textbf{[计算]} \verb|num_envs=2048|、\Verb[breaklines]|num_steps_per_env=32|、\Verb[breaklines]|num_mini_batches=8|、\Verb[breaklines]|num_learning_epochs=5|。用 \cref{eq:rlmc-batch,eq:rlmc-minibatch,eq:rlmc-updates} 算出 batch size、mini-batch size、每 iteration 总更新次数。\emph{验收}：得到 $65536$、$8192$、$40$。
  \item \textbf{[分析]} 把 \Verb[breaklines]|num_learning_epochs| 从 5 改成 20 会出现什么问题？PPO 的 clip 能否完全防住？\emph{验收}：能说出"数据被过度复用、偏离 on-policy、KL 在后期 epoch 显著增大；clip 只能缓解不能根治（见 \cref{ins:rlmc-clip-reuse}）"。
\end{enumerate}
\end{practice}
...
PPO 训练器\emph{不直接}与 env 交互——中间隔着一个 wrapper 层（在 RSL-RL 生态中通称 \Verb[breaklines]|RslRlVecEnvWrapper|）。它是 env 与 runner 之间\textbf{唯一的桥}，负责把 env 的输出翻译成 runner 期望的格式。这一层看似只是"格式转换"，却是整条管线最容易出隐蔽 bug 的地方——因为它同时承担了一项\emph{数学}职责（timeout 自举信号），搞错不会报错，只会让 critic 学到错误的 label。

\subsection{wrapper 的四个核心职责}

\textbf{职责一：观测路由。}env 返回形如 \Verb[breaklines]|{"actor": tensor, "critic": tensor}|（mjlab）或 \Verb[breaklines]|{"policy": tensor, "critic": tensor}|（Isaac Lab）的观测字典。wrapper 依 runner 配置里的 \verb|obs_groups| 映射，把这些 group 组装成 runner 期望的张量结构。\verb|obs_groups| 的来源正是 \cref{ch:rlmc-obsact} 设计的 actor/critic 分组。
...
\textbf{职责四：timeout 自举信号。}对无限时域任务（\Verb[breaklines]|is_finite_horizon=False|），wrapper 把"被截断但未真实终止"的信息放进 \Verb[breaklines]|extras["time_outs"]|。GAE（\cref{sec:rlmc-train-dataflow} 的代码）看到 \Verb[breaklines]|time_outs=True| 就\emph{不}清零 value，而是用 critic 估计自举。这正是奖励/终止章 truncation-vs-termination 的工程落地。
...
mjlab 继承 RSL-RL 的 \Verb[breaklines]|OnPolicyRunner| 做了三类务实扩展，每一类都对应一个真实痛点：
...
\textbf{(3) 迁移旧 checkpoint 键名。}RSL-RL 4.x$\to$5.x 的 API 变化导致键名变化（如 \Verb[breaklines]|actor.0.weight| 与 \verb|mlp.0.weight|）；mjlab runner 自动检测并迁移。
...
wrapper 类 & \Verb[breaklines]|RslRlVecEnvWrapper| & \Verb[breaklines]|RslRlVecEnvWrapper| & 无（\verb|NpEnv| $+$ \Verb[breaklines]|split_obs_dict|） \\
\verb|obs_groups| 形如 & \Verb[breaklines]|{"actor":("actor",),...}|（tuple 值） & \Verb[breaklines]|{"actor":["policy"],...}|（list 值） & Hydra \Verb[breaklines]|{actor:[actor],critic:[critic]}| \\
curriculum 恢复 & extras 存 \Verb[breaklines]|common_step_counter| & resume 仅恢复网络权重，\Verb[breaklines]|common_step_counter| 不持久化（curriculum 进度重置） & \Verb[breaklines]|PenaltyCurriculum|（episode 长度驱动） \\
...
\begin{note}
Isaac Lab 自 RSL-RL $\ge4.0.0$ 起，\Verb[breaklines]|OnPolicyRunner| 初始化\textbf{必须}提供 \verb|obs_groups|（典型形如 \Verb[breaklines]|{"actor":["policy"], "critic":["policy"]}|），否则会卡死/报错——这是官方 issue 中的高频坑，已联网核对 \cite{schwarke2025rslrl}。两框架命名差一个词（\verb|actor| vs \verb|policy|），迁移配置时极易写错。
\end{note}

\textbf{UniLab 的接口边界形态不同}：它\emph{没有} \Verb[breaklines]|RslRlVecEnvWrapper| 这种"翻译层"——因为它不是 Manager-Based 架构，env 的四职责直接落在 \verb|NpEnv.step()| 模板与契约类里（\verb|np_env.py|，UniLab）。逐职责对照如下。\textbf{职责一（观测路由）}：\verb|NpEnv| 的 \Verb[breaklines]|obs_groups_spec| 直接声明 \Verb[breaklines]|{"obs": <actor 维>, "critic": <critic 维>}|（如 go2 joystick 的 \Verb[breaklines]|{"obs":49,"critic":52}|，多出的 $3$ 维即特权 base linvel）；\Verb[breaklines]|base/observations.py| 的 \Verb[breaklines]|split_obs_dict| 把 \verb|obs["obs"]| 喂 actor、\verb|obs["critic"]| 喂 critic（无 critic 则复用 obs）；env 内固定用 \verb|obs|/\allowbreak\verb|critic| 两个组名，而给 RL runner 的路由键经 Hydra \Verb[breaklines]|algo.obs_groups={actor:[actor],critic:[critic]}| 对齐——\emph{两层组名}（env 侧 \verb|obs|/\allowbreak\verb|critic| 与 runner 侧 \verb|actor|/\allowbreak\verb|critic|）是 UniLab 特有的、最易写混的地方。\textbf{职责三、四（done 合并 \& timeout 自举）}：UniLab 把这件事做成了\emph{显式数据契约} \Verb[breaklines]|TransitionBootstrapContract|（\Verb[breaklines]|base/final_observation.py|，UniLab），其中 \verb@done = terminated | truncated@，并用 \Verb[breaklines]|timeout_terminal_mask| 与失败终止区分（等价 mjlab/Isaac 的 \verb|time_outs| 标记，但 UniLab 升格为带字段的契约对象）；终止帧的 \verb|next_obs| 还会被 \Verb[breaklines]|_patch_terminal_next_observations| 打补丁成真终止观测，保证 autoreset 后的 bootstrap 用的是出事那一帧而非新回合首帧。换言之，\cref{sec:rlmc-train-dataflow} 反事实推理里"删掉 timeout 自举的灾难"，三框架都靠各自机制规避，只是 UniLab 把它做成了\emph{编译期可检查的契约}而非一个布尔标记。

\begin{note}[把 UniLab"最易写混的两层组名"做成一段自检，而非只停在告诫]
上面说 UniLab 有\emph{两层}组名（env 侧固定 \verb|obs|/\allowbreak\verb|critic|，runner 侧 \verb|actor|/\allowbreak\verb|critic|，靠 Hydra \Verb[breaklines]|algo.obs_groups| 对齐），是最容易写错的地方。与其只记"小心"，不如训练前\textbf{把两层映射打出来当场对账}——和 \cref{sec:rlmc-train-wrapper} 练习里 mjlab 侧打印 obs 维度对称：
...
\textbf{错误描述}：写成 \Verb[breaklines]|{"actor":("actor",), "critic":("actor",)}|——critic 与 actor 看到\emph{相同}观测。\textbf{现象后果}：\cref{ch:rlmc-obsact} 精心设计的特权信息（privileged）完全浪费，价值估计变差、但不报错。\textbf{根本原因}：把两个 key 都指到了同一 group。\textbf{正确做法}：训练启动时打印 actor 与 critic 的 obs 维度，若相同就极可能配错（除非你确实不用特权信息）。
\end{pitfall}
...
\begin{enumerate}
  \item \textbf{[源码]} 在你用的框架里找到 \Verb[breaklines]|RslRlVecEnvWrapper.step()|，确认 \Verb[breaklines]|extras["time_outs"]| 的设置逻辑。若 \Verb[breaklines]|is_finite_horizon=True|，\verb|time_outs| 还会被设置吗？\emph{验收}：能指出"有限时域任务不设 time\_outs，因为到点即真实终止"。
...
\Verb[breaklines]|num_learning_epochs| & 5 & 5 & 5 & 数据复用次数 \\
\Verb[breaklines]|num_mini_batches| & 4 & 4 & 4 & mini-batch 数 \\
...
\Verb[breaklines]|value_loss_coef| & 1.0 & 1.0 & 1.0 & 价值损失权重 \\
\Verb[breaklines]|num_steps_per_env| & 24 & 24 & 24 & rollout 长度 \\
\hline
\end{tabular}
\end{table}
...
\begin{note}
两框架的\textbf{关键 API 差异（已实测核对，勿混淆）}：分离 actor/critic 是\emph{两框架共有}的（都基于 RSL-RL 5.x），但\emph{模型配置类的名字不同}——mjlab 用自带的 \verb|RslRlModelCfg|（分布是\textbf{普通 dict}：\Verb[breaklines]|{"class_name":"GaussianDistribution","init_std":1.0,...}|，实测 mjlab 1.4.0 + rsl-rl-lib 5.4.0）；Isaac Lab 用 \Verb[breaklines]|isaaclab_rl.rsl_rl| 提供的\textbf{带类型的} \Verb[breaklines]|RslRlMLPModelCfg|（其分布是\emph{嵌套类} \Verb[breaklines]|RslRlMLPModelCfg.GaussianDistributionCfg(init_std=1.0)|）。\textbf{注意}：\Verb[breaklines]|RslRlMLPModelCfg|/\allowbreak\Verb[breaklines]|GaussianDistributionCfg| 是 \emph{Isaac Lab} 的封装，\textbf{不是} mjlab 的、也\textbf{不是}裸 \Verb[breaklines]|rsl_rl.modules| 的导出（实测裸库 \Verb[breaklines]|from rsl_rl.modules import RslRlMLPModelCfg| 会 \verb|ImportError|）。Isaac Lab 另\emph{同时}保留旧版\emph{统一}的 \Verb[breaklines]|RslRlPpoActorCriticCfg|（带 \Verb[breaklines]|init_noise_std|，实测仍未弃用），故上面 Isaac Lab 代码用统一式也能跑。actor/critic 分离始于 RSL-RL 4.0.0，分布 cfg（dict / 嵌套类）始于 5.0.0（\Verb[breaklines]|init_noise_std| 被 \verb|init_std| 取代）——经官方 release 与文档核对 \cite{schwarke2025rslrl}。
\end{note}
...
\begin{note}
\textbf{三框架 PPO 配置对照的结论}：超参数的\emph{数值}在 mjlab/Isaac/UniLab 三者一致（都源自 legged\_gym \cite{rudin2022legged} 经 RSL-RL 沉淀）；差异只在\emph{配置范式}——mjlab 用\emph{分离}的 \verb|RslRlModelCfg|$\times2$（分布是 dict），Isaac Lab 用\emph{分离}的 \Verb[breaklines]|RslRlMLPModelCfg|$\times2$（分布是嵌套类）\emph{或}旧版\emph{统一}的 \Verb[breaklines]|RslRlPpoActorCriticCfg|，UniLab 是\emph{统一}的 \Verb[breaklines]|policy=dict(actor_hidden_dims, critic_hidden_dims)| 且整体走 Hydra 覆盖。三者的算法核（clip/GAE/adaptive KL）是\textbf{同一份 RSL-RL 代码}，UniLab 仅外包一层 \Verb[breaklines]|FinalObservationAwarePPO| 来正确处理 autoreset 后的终止观测自举（\cref{sec:rlmc-train-wrapper} 的 \Verb[breaklines]|TransitionBootstrapContract|）。这印证了 \cref{ins:rlmc-params-allequal}：那张"几乎全等"的参数表，到了 UniLab 这第三个框架\emph{依旧}成立。
\end{note}
...
\Verb[breaklines]|num_learning_epochs| & 更充分用数据 & 更接近单次更新 & 后期 epoch 的 KL \\
\Verb[breaklines]|num_mini_batches| & 更小 mini-batch、更多更新 & 更大 mini-batch、更稳 & 梯度噪声 \\
\Verb[breaklines]|num_steps_per_env| & rollout 更长、GAE 更准 & 更新更频 & 优势质量 \\
...
reward 不上升 & reward 设计（奖励章） & \Verb[breaklines]|num_steps_per_env| 太短？ \\
...
\begin{enumerate}
  ...
  \item \textbf{[实验]} 把 \Verb[breaklines]|num_learning_epochs| 从 5 调到 20，跑 $\sim200$ iter 观察 KL。\emph{验收}：记录到后期 epoch 的 KL 明显增大，并能用 \cref{ins:rlmc-clip-reuse} 解释。
\end{enumerate}
\end{practice}
...
adaptive KL 调度是 RSL-RL 算法内的逻辑，mjlab 与 Isaac Lab 因共用 RSL-RL 而\emph{完全一致}（同一份 \verb|update_lr|），仅 \verb|desired_kl| 默认值随任务可能不同。\textbf{UniLab 的 PPO 也内置同款调度}——它的 \Verb[breaklines]|PPOConfig.algorithm| 默认 \Verb[breaklines]|schedule="adaptive", desired_kl=0.01|（\Verb[breaklines]|structured_configs.py|，UniLab），且因复用 RSL-RL 的 \Verb[breaklines]|OnPolicyRunner|，走的就是\emph{同一份} \verb|update_lr| 逻辑（\cref{ins:rlmc-acc} 的 ACC 类比对三框架等同成立）。一个 UniLab 特有的提醒：它的异步变体 APPO 不靠 adaptive KL 单独稳住——collector 与 learner 解耦后，被覆盖的旧 rollout 还要叠加 V-trace（\Verb[breaklines]|vtrace_clip_rho/c|）来约束 off-policy 偏移，相当于在 KL 调度之外多了一道"重要性比裁剪"的安全网。

\subsection{常见陷阱}

\begin{pitfall}[手动设 \texttt{schedule="fixed"} 却没仔细选 LR]
\textbf{错误描述}：图省事关掉自适应、随手填个 LR。\textbf{现象后果}：fixed 模式要你自己平衡初期与后期，比 adaptive 难得多——初期易爆 KL，后期易停滞。\textbf{根本原因}：丢掉了 \cref{ins:rlmc-acc} 的自动平衡。\textbf{正确做法}：除非有特殊理由，始终 \Verb[breaklines]|schedule="adaptive"|。
\end{pitfall}
...
\begin{enumerate}
  \item \textbf{[分析]} 若 \Verb[breaklines]|desired_kl=0.001|（而非 $0.01$），调度会怎样？策略更新速度如何变？\emph{验收}：目标 KL 更小 $\to$ 更频繁触发降 LR $\to$ 策略更新更慢更保守。
  \item \textbf{[实验]} 跑 $\sim200$ iter，在 TensorBoard 记录 \Verb[breaklines]|Loss/learning_rate| 与 \verb|Loss/mean_kl| 的对应关系，标注 LR 调整发生时对应的 KL 值。\emph{验收}：能指出每次 LR 下降都伴随 KL 超过 $2\times$\verb|desired_kl|。
\end{enumerate}
\end{practice}
...
标准 actor/critic 是三层 MLP \Verb[breaklines]|[512, 256, 128]| 加 ELU 激活。它不是唯一正确，却经过最广泛验证。
...
\textbf{层数}：两层通常不够（四足的非线性需至少三层）；四层通常多余（增参不增表达力）。任务明显更复杂时（如人形全身控制、obs 维度 $>100$）可加到 \Verb[breaklines]|[1024, 512, 256, 128]|。\textbf{宽度}：递减是常见模式，物理直觉是"逐级压缩信息"，类似信号处理的多级滤波；等宽（\verb|[256,256,256]|）差异通常很小。\textbf{激活}：ELU 在负区有非零梯度（$\mathrm{ELU}(x)=\alpha(e^x-1),\ x<0$），比 ReLU 的"死神经元"少。

\textbf{\texttt{init\_std} 的物理含义}：高斯分布的 \verb|init_std=1.0|（mjlab 写在 \Verb[breaklines]|distribution_cfg| dict 里、Isaac Lab 写在 \Verb[breaklines]|RslRlMLPModelCfg.GaussianDistributionCfg| 里）设策略输出高斯的初始标准差。$=1.0$ 意味着训练开始时 raw action 的约 $68\%$（$1\sigma$）落在 $[-1,1]$——合理的探索幅度。太小（如 $0.01$）几乎不探索、易卡初始行为；太大（如 $10$）初始噪声过猛、机器人剧烈抖动甚至 NaN。训练中 action std 会从 \Verb[breaklines]|Loss/mean_noise_std| 看到逐渐下降，反映"探索$\to$利用"的过渡。（旧版统一 API \Verb[breaklines]|RslRlPpoActorCriticCfg| 里同一字段叫 \Verb[breaklines]|init_noise_std|，自 RSL-RL 5.0 起由 \verb|init_std| 取代。）
...
对绝大多数 locomotion 任务，\textbf{推荐用 History-MLP}（\cref{ch:rlmc-obsact} 的 \Verb[breaklines]|history_length| 配置，展平最近 $3$--$5$ 帧）而非 RNN：固定窗口在实践中足以提供时间信息，且\emph{无需部署端维护状态}。只有确需长期记忆（如导航中的路径记忆）才上 RNN。本质区别在部署：MLP 推理是纯函数（obs$\to$action），RNN 推理\emph{有状态}（obs $+$ hidden $\to$ action $+$ new\_hidden）。
\end{insight}

若必须用 RNN，注意两条工程细节：(1) \textbf{rollout 顺序}——MLP 的 mini-batch 可随机打乱所有 transition，RNN \emph{不能}打乱，必须保持每个 env 内的时间顺序（RSL-RL 在 RNN 模式按 env 切 mini-batch）；(2) \textbf{回合边界}——\verb|done=True| 时 hidden state 要 reset，训练时 storage 自动清零，部署时需在检测到回合重启（如摔倒）时手动 \Verb[breaklines]|reset_hidden()|。
...
\begin{center}
\small\textbf{从观测特性到网络类型}\\[3pt]
\begin{forest} dtree
[{观测包含图像吗？}
  [{是 → CNN encoder + MLP\\（部署需 camera pipeline\\+ 含 CNN 的 ONNX）}]
  [{否 → 观测满足马尔可夫性吗？}
    [{是 → MLP [512,256,128]\\（默认）:velocity 跟踪 + 接触力}]
    [{否 → 有 history\_length 配置吗？}
      [{是 → 展平 history 的 MLP（首选），\\history=3$\sim$5 常够用}]
...
模型配置类随 RSL-RL 版本演进：mjlab/Isaac Lab 同处 RSL-RL 5.x 但\emph{封装类名不同}（mjlab \verb|RslRlModelCfg| / Isaac Lab \Verb[breaklines]|RslRlMLPModelCfg|，见 \cref{sec:rlmc-train-hyperparam} 的实测对照），旧版 Isaac Lab 可能仍是统一 \Verb[breaklines]|RslRlPpoActorCriticCfg|。两框架的 \Verb[breaklines]|RslRl*ModelCfg| 都已带 RNN/CNN 旋钮：mjlab 经 \verb|RslRlModelCfg| 的 \verb|rnn_type|（\verb|"lstm"/"gru"|）、\verb|cnn_cfg| 与 \verb|class_name|（\Verb[breaklines]|"RNNModel"/"CNNModel"|）切换（实测字段）；Isaac Lab 经 \Verb[breaklines]|RslRlMLPModelCfg| 的同名旋钮与 \Verb[breaklines]|RslRlCNNModelCfg|。两框架共用的 RSL-RL ONNX 导出器对\textbf{循环策略}的 hidden-state I/O 是完备的：LSTM 暴露 \verb|h_in|/\allowbreak\verb|c_in|\,$\to$\,\verb|h_out|/\allowbreak\verb|c_out|、GRU 暴露 \verb|h_in|\,$\to$\,\verb|h_out|（导出器按 \verb|rnn_type| 自动决定是否注册 cell-state buffer）。\pz{待核实：非 LSTM/GRU 的循环变体与 CNN/视觉策略的 ONNX 导出完备度仍随版本（Isaac Lab 曾有"导出非 LSTM RNN"的 open issue），以本地版本为准。}\textbf{UniLab 的 PPO 网络}经 \Verb[breaklines]|PPOConfig.policy| 配（\Verb[breaklines]|actor_hidden_dims=[512,256,128], critic_hidden_dims=[512,256,128], activation="elu"|），默认 MLP 与三框架一致；高斯探索 std 由 RSL-RL 模型管理。值得注意的是 UniLab 的\emph{off-policy} 算法另有不同模型旋钮：SAC 默认 \Verb[breaklines]|use_layer_norm=True| 且用\textbf{分布式 critic}（\verb|num_atoms=101|），FlashSAC 用 \Verb[breaklines]|actor_num_blocks=2| 的块状 actor——这些是 PPO/RSL-RL 模型族没有的（PPO 用普通 MLP $+$ 标量价值头）。至于 RNN/CNN：UniLab 当前 $26$ 个注册任务\textbf{全是本体感知 MLP}（无视觉/相机任务），其 RNN/历史接口与 mjlab/Isaac 一样可经 history 展平实现，但没有官方 CNN 视觉策略样例可直接照搬（视觉运动控制在 UniLab 当前任务集中欠缺，见本章无需展开）。

\subsection{常见陷阱}

\begin{pitfall}[把网络改大来"修复"训练]
\textbf{错误描述}：\verb|[512,256,128]| 不收敛就换 \Verb[breaklines]|[1024,512,256,128]|。\textbf{现象后果}：通常仍不收敛，还更慢。\textbf{根本原因}：根因几乎不在网络容量，而在 reward/obs（见调参优先级）。\textbf{正确做法}：先回查环境与奖励，网络放最后。
\end{pitfall}
...
\begin{enumerate}
  ...
  \item \textbf{[实验]} 只把 critic 的 \Verb[breaklines]|obs_normalization| 改 \verb|True|（actor 保持 \verb|False|），跑 $\sim200$ iter 比较 value loss。\emph{验收}：能确认这不影响部署用的 actor（actor 输入仍是原始尺度）。
\end{enumerate}
\end{practice}
...
归一化位于"观测进网络之前"。开启 \Verb[breaklines]|obs_normalization=True| 后，模型内部维护观测的滑动均值与方差，对输入做标准化。它能平衡量纲差异巨大的观测，但会给部署引入"归一化器同步"的负担。
...
\textbf{收敛特性}：训练初期 \verb|count| 小，均值/方差不准，前几百 iter 的归一化输出可能不稳定——这也是有些项目初期关闭归一化（或用预收集统计量）的原因。（RSL-RL 内部归一化类名实测确为 \Verb[breaklines]|EmpiricalNormalization|，在 \Verb[breaklines]|rsl_rl.modules| 与 \Verb[breaklines]|rsl_rl.models.mlp_model| 均可见，rsl-rl-lib 5.4.0。）
...
若 \Verb[breaklines]|obs_normalization=True| 且作用在 \textbf{actor} 上，归一化层必须随 actor 一起进 ONNX，否则部署端拿到的是"未归一化输入$\to$归一化网络"的错配。
...
归一化由 RSL-RL 模型实现，mjlab/Isaac Lab 共用同一机制（\Verb[breaklines]|obs_normalization| 字段）。导出时是否把归一化层写入 ONNX，两框架行为一致（共用 RSL-RL 导出）。\textbf{UniLab 的 PPO 走同一条 RSL-RL 归一化路径}：开 \Verb[breaklines]|empirical_normalization=true| 时归一化层随策略写进 ONNX（与 mjlab 一致，部署端\emph{不}再归一化）。但 UniLab 的异构架构给归一化加了一道独有难题——\textbf{归一化统计量在 CPU collector 与 GPU learner 两个进程里}，必须同步：它为此专门做了 \Verb[breaklines]|SharedObsNormStats|（\Verb[breaklines]|ipc/shared_obs_stats.py|，UniLab），用一个 \verb|Queue| 把 \Verb[breaklines]|(mean, std)| 在进程间传递。这正是 \cref{ins:rlmc-unilab-heterogeneous-train} "凡是跨进程跨设备的量都要显式同步"的又一处体现：mjlab/Isaac 的归一化器只在单进程 GPU 上、无须同步，UniLab 则把它做成了一个 IPC 对象。off-policy（SAC）默认 \Verb[breaklines]|obs_normalization=True| 且叠加 \Verb[breaklines]|use_layer_norm=True|。

\subsection{常见陷阱}

\begin{pitfall}[双重归一化]
\textbf{错误描述}：部署端自己做了一层归一化，而 ONNX 内部也含归一化器。\textbf{现象后果}：输入被归一化两次、行为错乱，但不报错。\textbf{根本原因}：没分清归一化在 ONNX 内还是外。\textbf{正确做法}：用相同输入对比 PyTorch 与 ONNX 输出，并用上面的 \Verb[breaklines]|check_onnx_normalizer| 确认归一化的归属。
\end{pitfall}
...
\begin{enumerate}
  \item \textbf{[实验]} 对一个 \Verb[breaklines]|obs_normalization=True| 的 actor 导出 ONNX，运行 \Verb[breaklines]|check_onnx_normalizer|。\emph{验收}：确认图中存在 Sub/Div；若不存在则定位为导出 bug。
...
这一节是"操作手册"层：给出三框架的端到端命令（含 UniLab 的异构训练 $+$ 多 GPU 命令），以及 \verb|train.py| 内部从 CLI 到 \Verb[breaklines]|runner.learn()| 的执行流。
...
\textbf{运行验证看什么}：恢复后应核对三处——iteration 计数对得上、\Verb[breaklines]|common_step_counter| 非零（否则 curriculum 回退）、LR 恢复到之前的值（adaptive 调度的状态延续）。
...
\begin{note}
\textbf{多 GPU 的线性缩放规则}：batch size 翻倍时 LR 原则上也应翻倍。但在 PPO 里 adaptive KL 调度会自动调 LR，\emph{不}需要手动翻倍——让调度自己适应即可。加速效率非线性 $2\times$（实测约 $1.6$--$1.8\times$/2 GPU），原因是 all-reduce 通信开销与"更大 mini-batch 后梯度方差更小"。Isaac Lab 用 \Verb[breaklines]|torchrun --nproc_per_node=N| 启动等价多 GPU 训练。
\end{note}
...
\textbf{(1) 共享内存预算守卫（独有失败面）。}GPU-sim 框架的并行度受\emph{显存}限制；UniLab 的并行度还受 \verb|/dev/shm| 限制——大 replay buffer / ring buffer 直接占共享内存。它在分配前用 \Verb[breaklines]|estimate_offpolicy_bytes| / \Verb[breaklines]|estimate_appo_bytes|（\Verb[breaklines]|ipc/memory_budget.py|，UniLab）估字节数，超 \verb|/dev/shm| 的 $80\%$ 直接抛 \verb|MemoryError| 并提示"减小 \verb|algo.num_envs| 或 \Verb[breaklines]|algo.replay_buffer_n|，或加大 \verb|/dev/shm|"（\Verb[breaklines]|UNILAB_SKIP_MEMORY_CHECK=1| 可关）。这是 \cref{sec:rlmc-train-dataflow} 里"OOM 优先减半 \verb|num_envs|"那条经验在 UniLab 的对应物，只是瓶颈从 GPU 显存换成了 CPU 共享内存。

\textbf{(2) NaN 守卫 $+$ 离线复现。}本章 \cref{sec:rlmc-train-diag} 给的 \Verb[breaklines]|diagnose_nan_chain| 是手搓 \verb|torch.isnan| 逐步检测；UniLab（与 mjlab 一样）把它做成了\emph{内建工具链}：\verb|NpEnv| 内嵌 \verb|NanGuard|（\Verb[breaklines]|utils/nan_guard.py|，UniLab），\verb|step()| 里 \verb|check_ctrl| 查动作 NaN、\verb|check| 查 obs/reward NaN，并\emph{环缓存最近若干帧物理快照}；命中即 \verb|dump| 成 \verb|.npz|（含快照 $+$ 复制的 \verb|model_file|），事后 \Verb[breaklines]|unilab-viz-nan <dump>| 用 \verb|mujoco.viewer| \textbf{回放出事前那几帧}来复现。默认开（\Verb[breaklines]|training.nan_guard.enabled=true|）。这把"loss 变 NaN 但难定位"从在线打印升级为可\emph{离线重演}的事故现场。
...
\textbf{(3) 流水线 profiling。}由于 collector/learner/H2D 三段在不同进程/设备并发，UniLab 提供 chrome-trace 风格的时间片记录（\Verb[breaklines]|training.trace_enabled=true|，off-policy 路径；\verb|TraceRecorder| 在 \Verb[breaklines]|logging/trace_event.py|，UniLab），产 \Verb[breaklines]|perfetto_*.json| 可视化 \verb|weight_sync/*|、\verb|replay/*|、\Verb[breaklines]|replay_pipeline/*| 的重叠——回答"训练慢是 GPU 算得慢，还是 CPU 喂不上/H2D 拖后腿"。这是 \cref{ins:rlmc-unilab-heterogeneous-train} 那条"多了一道显式数据搬运"必然带来的诊断需求：mjlab/Isaac 无此边界、故无此 profiling 维度。

\begin{insight}[异构架构把"训练健康"从一维扩成两维]\label{ins:rlmc-unilab-two-axes}
在 mjlab/Isaac 上，训练健康基本是\emph{一维}的——看 \verb|Loss/*| 与回放即可（数据在 GPU、无流水线）。在 UniLab 上它是\emph{两维}的：除了"策略学得好不好"（\verb|Loss/*|、KL、entropy），还要看"流水线堵不堵"（\Verb[breaklines]|Weight Sync| / \Verb[breaklines]|H2D Copy| / 共享内存预算 / collector 是否静默死亡）。后者是 GPU-sim 范式根本不存在的失败面。理解这一点，才算真正读懂了"异构 CPU-sim$\leftrightarrow$GPU-policy"这条第三范式的工程代价与收益。
\end{insight}

\subsection{常见陷阱}

\begin{pitfall}[恢复训练时忘了恢复 curriculum 状态]
\textbf{错误描述}：自定义 runner 存 checkpoint 时漏掉 \Verb[breaklines]|common_step_counter|。\textbf{现象后果}：恢复后 step-driven curriculum 从 stage 0 重来，难度骤降、策略"退化"。\textbf{根本原因}：curriculum 进度未持久化。\textbf{正确做法}：用框架提供的 runner 子类（已处理），或手动保存该计数（见上面 extras）。
\end{pitfall}
...
\begin{enumerate}
  \item \textbf{[实验]} 在你的框架里完整跑通 list$\to$train(flat,300 iter)$\to$tensorboard$\to$play$\to$resume。\emph{验收}：记录 wall-clock、最终 tracking reward、steps/s；恢复后 iteration 与 \Verb[breaklines]|common_step_counter| 正确。
  \item \textbf{[编程]} 写脚本：训练后从最新 checkpoint 导出 ONNX 并跑一致性验证（复用 \cref{sec:rlmc-train-onnx} 的 \Verb[breaklines]|export_and_verify|）。\emph{验收}：打印 max diff $<10^{-5}$。
\end{enumerate}
\end{practice}
...
新手跑完 Quick Start 第一条命令，最该先回答的不是"reward 多高"，而是"它\emph{起来}了吗"。RSL-RL 每迭代会在\textbf{终端}打印一段摘要（与上面 TensorBoard 的 \verb|Loss/*| 是\emph{同一批量}、但终端是\emph{人读字段名}，二者字面不同——别拿 TensorBoard 的 \verb|mean_kl| 去终端找）。本章实测 mjlab \Verb[breaklines]|train Go1 ... --agent.max-iterations 2| 第一屏的关键行与判读：
...
\end{codebox}
\textbf{"起来了"的最小判据}：上述字段\emph{齐全}、数值\emph{每迭代在变}、且\textbf{无 \texttt{NaN}}——即代表环境/网络/数据流贯通，可以放心拉大 \verb|num_envs| 和迭代数。\textbf{两点关于 KL 的提醒}（最容易扑空）：(1) mjlab 的\textbf{终端每迭代摘要里\emph{不打印} \texttt{kl}/\allowbreak\texttt{entropy} 数值}，要看 KL 须开 TensorBoard/WandB 读 \verb|Loss/mean_kl|；自适应调度仍在内部按 \verb|desired_kl| 调 LR（\cref{sec:rlmc-train-adaptkl}），只是不在终端露出。(2) 框架间\textbf{终端字段名不一定字面相同}（如 UniLab 实测首屏 \Verb[breaklines]|Mean action std=0.50|——它的默认探索 std 比 mjlab/Isaac 的 \verb|1.0| 小、起步更偏利用），所以\textbf{认\emph{语义}而非记\emph{字面}}：采集时间/学习时间/各 loss/reward/std 各对应什么，比记住某个框架的拼写更重要。
\end{note}
...
TensorBoard 字段名由 RSL-RL 写入，mjlab/Isaac Lab \emph{一致}（\verb|Loss/*|、\Verb[breaklines]|Episode_Reward/*| 等）。环境侧指标（\verb|Metrics/*|、\verb|Curriculum/*|）取决于各框架 manager 的实现，命名可能略有差异。\textbf{UniLab 的 PPO 日志因复用 RSL-RL，} \verb|Loss/*| \textbf{等核心字段同名}；reward 分项由其派发表写成 \verb|reward/<name>|（如 \Verb[breaklines]|reward/tracking_lin_vel|，每若干步聚合一次，实测出现）。UniLab 独有的是\textbf{异构架构的运行仪表盘}：APPO/off-policy 训练时控制台会打印 \Verb[breaklines]|Weight Sync|、\Verb[breaklines]|H2D Copy|、\Verb[breaklines]|Rollouts Read|、\Verb[breaklines]|Staging Pool|、\Verb[breaklines]|Vtrace/Rho Clip Fraction| 等面板（实测），分别对应 \cref{ins:rlmc-unilab-heterogeneous-train} 里的权重同步、CPU$\to$GPU 拷贝、ring buffer 读、双缓冲暂存与 V-trace 裁剪比例——这些是"数据本就在 GPU"的 mjlab/Isaac \emph{没有}的诊断维度。换言之，UniLab 比两框架多一类要监控的健康信号：不仅看策略学得好不好（\verb|Loss/*|），还要看\emph{流水线有没有堵}（collector 跟不上 learner？H2D 拖慢？）。
...
\begin{enumerate}
  \item \textbf{[诊断]} 训练 500 iter 后 total reward 高（$38.0$），但回放里机器人\emph{跳跃}前进而非正常走；\Verb[breaklines]|Episode_Reward/track_linear_velocity|$=0.85$（高），\Verb[breaklines]|undesired_contact|$=-0.01$（低）。问题在哪、该调什么？\emph{验收}：识别为"跳跃也能拿高 tracking 而几乎不触发接触惩罚"$\to$ 加大步态/接触相关惩罚或 action\_rate（奖励章），属 reward 设计而非 PPO 参数。
...
mjlab/Isaac Lab 的一等公民都是 PPO（RSL-RL）；接 SAC 通常要外接 SB3 或 RL Games 并自写 \cref{tab:rlmc-ppo-sac} 所述适配层。\textbf{UniLab 在这一点上与两框架根本不同}：on-policy 与 off-policy 算法\emph{并列}都是一等公民，经 \verb|--algo| 一键切换——\verb|ppo|/\allowbreak\verb|appo|（异步 PPO）/\verb|sac|/\allowbreak\verb|td3|/\allowbreak\verb|flashsac|（外加 macOS 的 \verb|mlx_ppo| 与脚本级 HORA、HIM-PPO）。这正是 \cref{ins:rlmc-onpolicy-jit} "JIT vs 仓储"权衡在 UniLab 里被\emph{架构层面}支持的体现：它的共享内存 IPC 对 on-policy 用 \Verb[breaklines]|RolloutRingBuffer|、对 off-policy 用 \verb|ReplayBuffer|$+$\Verb[breaklines]|ReplayPipeline|，两套缓冲都是原生设施，所以"PPO 不够就换 SAC"在 UniLab 是改一个 flag、而非外接一个库。\cref{tab:rlmc-ppo-sac} 里 SAC 的"replay 采样瓶颈"在 UniLab 被 pinned-host 双缓冲 H2D 流水线缓解（\cref{ins:rlmc-unilab-heterogeneous-train}），其 SAC 默认 \Verb[breaklines]|num_envs=4096, replay_buffer_n=512, updates_per_step=4, gamma=0.97|（注意 SAC 的 $\gamma$ 比 PPO 的 $0.99$ 略小）。对应到本节的选型逻辑：在 UniLab 上，"有限并行$\to$SAC、大并行$\to$PPO"的判据依旧成立，只是切换成本被压到了最低。
...
\textbf{部署端必须复现的四件事}：(1) 按正确顺序拼接观测（\cref{ch:rlmc-obsact} 的部署边界）——顺序错不报错但行为全错；(2) raw action 乘 scale 加 offset（\Verb[breaklines]|raw*scale + default_joint_pos|）；(3) 对 processed action 做关节限位 clip；(4) 按正确控制频率调用（policy dt $=$ physics\_dt $\times$ decimation；训练 $50\,$Hz 则部署也 $50\,$Hz）。
...
mjlab 与 Isaac Lab \emph{共用} RSL-RL 的 ONNX 导出，因此导出行为与"归一化是否进图"一致；差异主要在部署侧的 SDK 与 joint ordering（见 \cref{sec:rlmc-train-deploy}）。\textbf{UniLab 同样导出可部署 ONNX，并内置 \texttt{onnxruntime} 自验}（各算法都导 \verb|policy.onnx|：PPO 经 \Verb[breaklines]|export_policy_to_onnx|、APPO 用 \Verb[breaklines]|_DeterministicAPPOActor| 包住 actor 后 \Verb[breaklines]|torch.onnx.export| 再起 \Verb[breaklines]|InferenceSession| 比对 max/mean diff 并打印 "ONNX export verified OK"、off-policy 经 \Verb[breaklines]|actor.as_export_module()|），归一化层在 \Verb[breaklines]|empirical_normalization=true| 时随策略写进图——这些与 \cref{sec:rlmc-train-onnx} 的 \Verb[breaklines]|export_and_verify| 思路完全一致。\textbf{一个必须记住的时机差异}：与 mjlab"训练后自动导"不同，UniLab 的 PPO 在 \emph{play/eval 阶段}才导（故 \Verb[breaklines]|training.no_play=true| 的冒烟跑\textbf{不会}产 ONNX，实测证实），off-policy 则由 \Verb[breaklines]|training.export_onnx|（默认开）在训练流程内导。反过来，UniLab 的\emph{部署工具链比 mjlab 更完整}：\Verb[breaklines]|scripts/deploy/| 下有 \Verb[breaklines]|export_deploy_config.py|（导 \Verb[breaklines]|deploy_config.yaml|，显式记录 obs 装配顺序/\verb|action_scale|/\allowbreak\Verb[breaklines]|default_angles|/joint ordering）、\Verb[breaklines]|sim_prototype.py|（用 ONNX 策略在 MuJoCo 逐段校验"部署侧 obs 装配 $==$ 训练侧"——这正是 \cref{tab:rlmc-onnx-boundary} 强调的"观测拼接顺序在图外、必须部署端复现"的自动化关卡）、以及动作跟踪用的 \Verb[breaklines]|export_motion_bin.py|。也就是说，本章反复强调的"ONNX 外四件事"（拼接顺序、scale/offset、关节限位、控制频率），UniLab 把其中最易错的"拼接顺序"做成了可机器校验的 \Verb[breaklines]|deploy_config.yaml| $+$ \verb|sim_prototype| 闭环。

\subsection{常见陷阱}

\begin{pitfall}[导出时网络处于 training mode]
\textbf{错误描述}：忘了 \verb|model.eval()| 就 \Verb[breaklines]|torch.onnx.export|。\textbf{现象后果}：含 BatchNorm/Dropout 的网络在 train/eval 行为不同，ONNX 与 PyTorch 输出不一致。\textbf{根本原因}：模式影响这些层。\textbf{正确做法}：导出前必 \verb|eval()|；框架的导出函数通常已处理，自定义导出要自己加。
\end{pitfall}
...
以 Unitree 的两个参考仓库为例（\Verb[breaklines]|unitree_rl_lab| 走 Isaac Lab，\Verb[breaklines]|unitree_rl_mjlab| 走 mjlab），二者部署管线几乎完全一致：ONNX 推理 $+$ Unitree SDK2 $+$ FSM（有限状态机）。差异主要在\emph{训练阶段}的 env 配置与 reward 调优——因为物理引擎不同（PhysX vs MuJoCo Warp），foot\_slip、contact 相关项的最优权重、以及 action scale 可能需要分别微调；terrain 的离散化精度也不同。

\begin{pitfall}[joint ordering mismatch：跨框架部署头号陷阱]
\textbf{错误描述}：从一个框架训练的 ONNX 直接拿到另一个框架/真机部署，未核对关节顺序。\textbf{现象后果}：训练时 \verb|action[0]| 对应 FL\_hip，部署时若对应 FR\_hip，则\emph{所有}关节目标错位，机器人乱动——但程序不报错。\textbf{根本原因}：MJCF 与 USD 的关节解析顺序可能不同。\textbf{正确做法}：打印两侧 \Verb[breaklines]|action[i] -> joint_name| 映射逐一比对；Go1 标准 $12$ 关节顺序为 FL/FR/RL/RR 各 (hip, thigh, calf)。
\end{pitfall}
...
\begin{note}
\textbf{部署侧的版本兼容}：Unitree SDK2 的 Python 接口 \Verb[breaklines]|unitree_sdk2_python| 在 \verb|setup.py| 里把 CycloneDDS \textbf{钉死为 \texttt{cyclonedds==0.10.2}}（C++ SDK 同样建议用 \Verb[breaklines]|releases/0.10.x| 分支自编译）；系统若装了更新版（如 $0.11.x$）会导致 SDK 发现不到机器人——用 \Verb[breaklines]|pip show cyclonedds| 检查、必要时降级到 $0.10.2$，并在编译时设好 \Verb[breaklines]|CYCLONEDDS_HOME|。
\end{note}
...
\textbf{路线 A（mjlab 训练主链）}：\Verb[breaklines]|scripts/train.py|（CLI 入口）$\to$ 框架 runner（三处扩展）$\to$ wrapper（四职责）$\to$ 配置类 $\to$ RSL-RL 的 \Verb[breaklines]|on_policy_runner.py| 的 \verb|learn()|。\textbf{路线 B（Isaac Lab 对等）}：\Verb[breaklines]|scripts/reinforcement_learning/rsl_rl/train.py| $\to$ \Verb[breaklines]|isaaclab_rl/rsl_rl/| 的 wrapper/runner $\to$ 重点对比 \verb|obs_groups| 命名差异（\verb|policy| vs \verb|actor|）。\textbf{路线 C（RSL-RL 内部）}：\Verb[breaklines]|algorithms/ppo.py|（clipped objective、GAE \Verb[breaklines]|compute_returns|、adaptive KL；\textbf{实测 5.4.0 里 \texttt{compute\_returns} 与 timeout 自举就在此处}，不在 \Verb[breaklines]|rollout_storage.py|）$\to$ \Verb[breaklines]|runners/on_policy_runner.py|（\verb|learn()| 主循环）$\to$ \verb|modules/|（MLP、高斯分布、\Verb[breaklines]|EmpiricalNormalization|）$\to$ \Verb[breaklines]|storage/rollout_storage.py|（此版只剩存储与 mini-batch 切分）。\textbf{别记死文件名}：版本一变就漂，按 \cref{sec:rlmc-train-dataflow} 的 \texttt{inspect+grep} 法在你的环境实测定位（这正是本次发现该迁移的方法）。
...
\textbf{错误描述}：把 Isaac Lab 训的 ONNX 直接放 mjlab 部署（或反之）。\textbf{现象后果}：关节错位、或 PD 实现差异导致同一 action scale 产生不同幅度运动。\textbf{根本原因}：URDF/MJCF 解析顺序与 PD 实现细节差异。\textbf{正确做法}：先核对 \Verb[breaklines]|action[i]->joint_name| 完全一致，再用 random agent 检查 processed action 幅度。
\end{pitfall}
...
\begin{enumerate}
  \item \textbf{[编程]} 写 \Verb[breaklines]|verify_joint_ordering| 的完整版，对比环境真实关节顺序与 \Verb[breaklines]|GO1_JOINT_ORDER|，逐一报告不一致项。\emph{验收}：能打印每个 \verb|action[i]| 的 joint 名并标出错位。
...
loss 变 NaN & obs/reward NaN 传播 & 在 \Verb[breaklines]|wrapper.step()| 后检测 NaN（用诊断脚本） & \cref{sec:rlmc-train-diag} \\
...
恢复后 curriculum 回退 & checkpoint 缺 \Verb[breaklines]|common_step_counter| & 用框架 runner 子类或手动存该计数 & \cref{sec:rlmc-train-flow} \\
...
组件 & 版本/状态 & 备注 \\
\hline
RSL-RL & 5.x（实测 \Verb[breaklines]|rsl-rl-lib 5.4.0|） & actor/critic 分离始于 4.0.0；分布配置（\Verb[breaklines]|distribution_cfg|，\verb|init_std|）始于 5.0.0；\Verb[breaklines]|compute_returns| 等在 5.x 内随小版本在 \Verb[breaklines]|rollout_storage.py| 与 \verb|ppo.py|/\allowbreak\Verb[breaklines]|on_policy_runner.py| 间迁移，定位用 \cref{sec:rlmc-train-dataflow} 的 \texttt{inspect+grep} 法 \cite{schwarke2025rslrl} \\
...
PyTorch & $\ge 2.x$ & ONNX 导出 \Verb[breaklines]|opset_version=11| 起步 \\
\hline
\end{tabularx}
\end{table}
