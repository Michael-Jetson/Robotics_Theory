...
\begin{itemize}
  \item 观测/动作接口、非对称 actor-critic、噪声注入 \Verb[breaklines]|enable_corruption|——见 \cref{ch:rlmc-obsact}；
...
本章的\emph{框架配置类}（\Verb[breaklines]|TennisStrikeObsCfg| / \Verb[breaklines]|TennisStrikeActionsCfg| / \Verb[breaklines]|TennisDREventCfg| / \Verb[breaklines]|TennisStrikeTerminationsCfg| 等，字段名/默认值均经 gpufree mjlab 1.4.0 实跑 introspection 核对）是\textbf{照真实 API 写的}，把抓取物换成球、补齐 MJCF 即可挂上框架。但本章的\emph{算法/管理器骨架类}（\Verb[breaklines]|StrikeEventBuffer| / \Verb[breaklines]|PerceptionNoiseManager| / \Verb[breaklines]|LagrangianPPOForTennis| / \Verb[breaklines]|TennisFrequencySeparation| / \Verb[breaklines]|TennisCurriculumManager|）是\textbf{伪代码骨架}：它们引用 \Verb[breaklines]|env.perception| / \Verb[breaklines]|env.event_buffer| / \Verb[breaklines]|env.contact_sensor| / \Verb[breaklines]|env.physics_predictor| 这些\emph{本章约定的虚构属性}（真实工程里须由你在 \cref{ch:rlmc-tennisenv} 的感知管线与自建 env 上接出来），直接 \verb|copy-paste| 实例化会 \Verb[breaklines]|AttributeError|。读这些块时请抓\emph{结构与意图}（数据流、为什么这么算），落地时把虚构属性替换成你 env 的真实接口——\cref{sec:rlmc-strike-env} 的「动手第一步：先从 stock task 改造」正是把这些骨架接到真实 env 的可操作路径。
\end{note}
...
Isaac Lab & 2.x（主线） & Linux / Windows & 训练 $+$ 对照 & \Verb[breaklines]|DifferentialInverseKinematicsActionCfg|、PhysX 接触 \\
MuJoCo Warp & 随 mjlab 1.4.0 & Linux & GPU 物理后端 & 2000\,Hz 高频接触仿真 \\
RSL-RL & 随框架 & 跨平台 & PPO 实现 & 训练循环、\verb|time_outs| value bootstrap \\
UniLab & commit 99936e08（MuJoCoUni 3.8.0 / MotrixSim 0.8.2） & Linux CUDA/ROCm/XPU、macOS & 训练 $+$ 对照 & CPU 物理 $+$ GPU 策略；关节位置目标动作、\Verb[breaklines]|DomainRandConfig| DR \\
\bottomrule
\end{tabularx}
\end{center}
...
# (1) 先看 mjlab 注册了哪些任务（确认安装无误;本章击球任务不在其中是正常的）
mjlab list-envs
#   预期:列出 12 个任务，含 Mjlab-Velocity-Flat-Unitree-G1 / Mjlab-Lift-Cube-Yam 等

# (2) 极短训练 2 迭代，确认 PPO 真能起、无 NaN（WANDB_MODE=disabled 免登录 wandb）
...
# (3) Lift-Cube-Yam 是「机械臂 manip + IK」最接近网球击球的现成起点（见下）
...
# (4) 不开 GPU 也能查 API 名是否存在（不存在会 ImportError，能立刻发现版本漂移）
...
\noindent{}\textbf{为什么先跑 stock task？}（教方法而非背命令）三件事一次性确认：装的版本对不对（\verb|list-envs| 起得来）、GPU/PPO 链路通不通（2 迭代 loss 正常）、本章引用的 API 名在\emph{你这台}机器上还在不在（\Verb[breaklines]|python -c import| 自检）。这三步是后面所有自定义 task 的地基——地基塌了，再漂亮的 strike 代码也跑不起来。命令 (4) 同时是本章「不背 API、自己查」方法论的落地：任何一个本章用到的类，都可以用 \Verb[breaklines]|__dataclass_fields__| 当场列出真实字段，对照书里的写法。

\begin{insight}[从 stock task 改造，而非从空白文件凭空写 strike task]\label{ins:rlmc-strike-retrofit}
工程上搭新任务的真实路径不是「照着书里的代码框从空文件写 \texttt{TennisStrikeEnvCfg}」，而是\textbf{复制一个最接近的现成任务再改}。mjlab 1.4.0 里 \Verb[breaklines]|Mjlab-Lift-Cube-Yam| 就是离网球击球最近的起点——它已经是「机械臂 $+$ 末端操作 $+$ 接触」的完整可跑 EnvCfg，自带 ActionManager / RewardManager（含 \Verb[breaklines]|action_rate_l2|）/ 终止配置。改造路线：(1) 找到它的 \verb|EnvCfg| 源文件，复制成 \Verb[breaklines]|TennisStrikeEnvCfg|；(2) 把抓取物换成球、把 lift reward 换成本章的 staged reward（\cref{sec:rlmc-strike-reward}）；(3) 把动作从默认关节动作换成本章的 \Verb[breaklines]|DifferentialIKActionCfg|（\cref{sec:rlmc-strike-mdp}）；(4) 升频率到 2000\,Hz（\cref{sec:rlmc-strike-freq}）。从能跑的东西出发逐项改，每改一步都能 \Verb[breaklines]|train --max-iterations 2| 验证没改坏——这比从空白写完再 debug 一堆 \verb|ImportError|/\allowbreak\Verb[breaklines]|AttributeError| 高效得多。
\end{insight}
...
\textbf{错误描述}：沿用 \cref{sec:rlmc-tennis-manager} 的发球验证 task（\Verb[breaklines]|actions: dict = {}|）直接挂 PPO 训练击球。\\
...
\textbf{配置详解。}下面给出 mjlab 风格的观测配置。关键设计：球相关量来自感知管线（不是真值），并对位置量开 \Verb[breaklines]|enable_corruption| 以便训练时注入噪声（\cref{sec:rlmc-strike-noise}）；critic 额外看真值与未来轨迹。
...
\textbf{代码走读。}在 mjlab 1.4.0 中，观测项由 \Verb[breaklines]|ObservationManager| 按注册顺序拼接成张量（\cref{ch:rlmc-manager}）。\Verb[breaklines]|enable_corruption| 是\textbf{组级总闸}（在 \Verb[breaklines]|ObservationGroupCfg| 的 \verb|__post_init__| 里设）：开启时组内配了 \verb|noise=| 的项才注入噪声，评估时把它整组置 \verb|False| 即看干净——这让「训练有噪声、评估看干净」成为一行配置即可切换的事。\Verb[breaklines]|CriticCfg(ActorCfg)| 用类继承表达「critic 是 actor 的超集」，是非对称 AC 最省事的写法；critic 组把 corruption 关掉以保证看真值。

\textbf{两个被忽略的内置字段（实测可用，省掉手写代码）。}\Verb[breaklines]|ObservationGroupCfg| 实测还带 \Verb[breaklines]|history_length| / \Verb[breaklines]|flatten_history_dim| 与 \verb|nan_policy| / \Verb[breaklines]|nan_check_per_term| 两组字段，本任务正好用得上：（1）上面的 \verb|last_action| 是手搭的单步动作历史；若要让策略「看到」\Verb[breaklines]|time_to_impact| 等时序量的最近 $N$ 帧（高速接触任务很需要），直接在组上设 \Verb[breaklines]|history_length=4| 即可得到 N 步堆叠观测，比逐个加 \verb|last_*| term 通用得多。（2）本章后面多处担心 IK 在奇异处发散吐 NaN（\cref{eq:rlmc-strike-dls} 附近的陷阱、\cref{sec:rlmc-strike-freq} 的高频接触），\verb|nan_policy|（组级）与 \Verb[breaklines]|nan_check_per_term| 是内置的 NaN 守卫——可在 obs 侧把偶发 NaN 兜住（按策略置零/复用上一帧），不至于「一个 env 炸了整批 rollout 作废」，比单纯调大 \verb|lambda| 更稳。这两项都不必自己实现。
...
class TennisStrikeActionsCfg:
    ...
    racket_ik = DifferentialIKActionCfg(
        ...
        damping=0.05,                          # DLS 阻尼 lambda（扁平字段，非 lambda_val;默认 0.05）
...
\Verb[breaklines]|DifferentialIKActionCfg| 把 task-space 的 delta pose 转成 joint-space 的关节角增量，核心是 damped least squares（DLS，阻尼最小二乘）伪逆：
\begin{equation}
...
\midrule
\Verb[breaklines]|use_relative_mode| & action 为 delta 还是绝对 & \verb|True| & delta 以零为中心，适合 RL 探索 \\
\Verb[breaklines]|delta_pos_scale| & 位置 delta 缩放 & 0.05--0.1 m/step & 50\,Hz $\to$ 0.05\,m/step $\to$ 最大 2.5\,m/s \\
\Verb[breaklines]|delta_ori_scale| & 旋转 delta 缩放 & 0.1--0.2 rad/step & 过大则球拍翻转；与 \verb|damping| 耦合 \\
...
\noindent{}上表是 mjlab \Verb[breaklines]|DifferentialIKActionCfg| 的扁平字段（DLS 阻尼字段名为 \verb|damping|）；Isaac Lab 的等价量改放进 \Verb[breaklines]|DifferentialIKControllerCfg(ik_method="dls", ik_params={"lambda_val":...})|（\cref{sec:rlmc-strike-mdp} 的三框架对比），数学上同为式~\eqref{eq:rlmc-strike-dls} 的 $\lambda$。
...
\textbf{UniLab 对照（诚实记录：无 task-space IK action 类）。}UniLab 不提供与 \Verb[breaklines]|DifferentialIKActionCfg| 等价的 task-space differential IK action term——它的动作\emph{固定}是关节位置目标残差：在 \verb|np_env.py| 的 \Verb[breaklines]|LocomotionBaseEnv.apply_action()| 中（UniLab，commit 99936e08），\Verb[breaklines]|ctrl = exec_actions * action_scale + default_angles|（位置式 PD 目标，\verb|action_scale| 默认 0.25，\Verb[breaklines]|default_angles| 取自 keyframe \verb|home| 的 qpos），由后端位置执行器按 \Verb[breaklines]|{kp, kd}| 算力矩。因此本节的 9D「base velocity $+$ 球拍 task-space 增量」在 UniLab 里没有现成 action 类对应。
...
\textbf{现象后果}：手腕到位了，但球拍面因长度和法向偏移（\cref{sec:rlmc-tennis-robot} 的 \Verb[breaklines]|euler="0 -15 0"|）并没对准球，contact rate 长期偏低。\\
\textbf{根本原因}：球拍有长度和面法向偏移，wrist 到位不代表 face 到位。\\
\textbf{正确做法}：在 MJCF 里定义 \Verb[breaklines]|racket_face_center| site，IK target 与 approach reward 都指向该 site。
\end{pitfall}
...
\textbf{正确做法}：从 \Verb[breaklines]|lambda_val=0.1| 起步，观察 IK 残差与关节速度峰值后再逐步下调。
\end{pitfall}

\begin{pitfall}[沿用 \texttt{delta\_pos\_scale}/\allowbreak\texttt{delta\_ori\_scale} 的默认值 1.0]
\textbf{错误描述}：实例化 \Verb[breaklines]|DifferentialIKActionCfg| 时只填了 \verb|frame_name|/\allowbreak\verb|damping| 等，没显式给 \Verb[breaklines]|delta_pos_scale|/\allowbreak\Verb[breaklines]|delta_ori_scale|，以为框架会自动缩放。\\
...
\textbf{正确做法}：始终显式设 \Verb[breaklines]|delta_pos_scale=0.05|（50\,Hz $\to$ 最大 2.5\,m/s）、\Verb[breaklines]|delta_ori_scale=0.1|，再按关节速度峰值微调；\verb|damping|/\allowbreak\verb|max_dq| 默认 0.05/0.5 通常可直接用。
\end{pitfall}

\begin{practice}[本节动手任务]
\begin{enumerate}
  \item \textbf{[设计]} 设计 9D 动作 \Verb[breaklines]|[base_vx, base_vy, base_wz, dx, dy, dz, drx, dry, drz]|，说明每维物理含义与建议 scale。若控制频率 50\,Hz，每维 scale 设多少才能达到合理的最大速度/角速度？验收标准：给出位置维（目标 $\le 2.5$\,m/s）与旋转维（目标不翻拍）的具体数值。
...
\textbf{代码走读。}\verb|t_ready| 是理想提前量——策略应提前 \verb|t_ready| 到位开始挥拍。注意 \Verb[breaklines]|clamp_min(0.0)|：若球已飞过（\verb|tti<0|），不 clamp 会让 $\exp$ 在 $-\text{tti}/\tau$ 上爆炸成巨大正奖励，反而鼓励「打空」。这是高速接触任务里极易忽略的数值陷阱。
...
乘法结构确保层级优先：若 \verb|approach=0|（球拍离击球点很远），无论 timing 多好、是否碰到球，总 reward 都是零——强制策略先解决\emph{接近}。接近了（\verb|approach>0|），timing 才开始有意义；接近且时间对了，contact 才可能发生；contact 发生后，outcome 才能评估。这与物块抓取 \Verb[breaklines]|reaching * (1 + bringing)| 的模式（\cref{ch:rlmc-manip}）完全一致——必须先够到 cube，才有机会搬起来。
\end{insight}
...
\begin{center}
\small\textbf{奖励不收敛的系统化诊断流程}\\[3pt]
\begin{forest} dtree
[{Total reward 升但\\contact rate = 0？}
  [{是 → 策略在刷\\approach shaping\\修复:减小 approach sigma\\或增大 contact 权重}]
  [{否 → contact rate 升但\\over\_net rate = 0？}
    [{是 → 碰到球但\\力量/角度不对\\修复:加 orientation reward\\或 racket velocity reward}]
    [{否 → over\_net rate 升但\\landing rate = 0？}
...
\paragraph{一次真实调参 walkthrough：把 \texorpdfstring{$\sigma$}{sigma} 从 0.5 调到 0.3。}诊断流程图给的是\emph{方向}（「减小 sigma」），但「减到多少、看哪条曲线判断够了没」要靠一个可操作的闭环。以「\Verb[breaklines]|Reward/approach| 一路涨、\Verb[breaklines]|Metric/contact_rate| 卡在 0」这个最常见症状为例，演示怎么从曲线反推参数——\emph{每一步都是「改一个数 → 短训 → 读一条曲线 → 决定下一步」}：
\begin{enumerate}
  \item \textbf{先确认是刷 shaping，不是别的}：在 WandB 同时看 \Verb[breaklines]|Reward/approach| 与 \Verb[breaklines]|Metric/contact_rate|（\cref{sec:rlmc-strike-reward} 的分项日志）。若 approach 已逼近其上限 1.0、contact\_rate 仍 $\approx 0$ 持续 $>500$ 迭代，基本确诊「球拍稳定停在击球点附近但从不挥过去」。
  \item \textbf{量出 approach 的「饱和半径」}：approach $=\exp(-d^2/\sigma^2)$，$\sigma=0.5$ 时 $d=0.5$\,m 处 reward 还有 $e^{-1}\approx0.37$、$d=0.25$\,m 处高达 $0.78$——意味着球拍离击球点 25\,cm 就拿到近八成奖励，\emph{再靠近边际收益太小}，策略没动力挥进最后那几厘米。这就是 sigma 偏大的定量症结。
  \item \textbf{改一个数、短训、读曲线}：把 \verb|sigma| 从 0.5 改到 0.3（$d=0.25$\,m 处 reward 降到 $e^{-(0.25/0.3)^2}\approx0.50$，逼近击球点的梯度明显变陡），\Verb[breaklines]|train --agent.max-iterations 300| 跑一小段，只盯 contact\_rate 这一条：若它从 0 起步出现\emph{任何}非零上升（哪怕到 5\%），方向就对了，继续训满；若仍 0，再降到 0.2 或同时把 \verb|contact| 权重从 5 提到 8。
  \item \textbf{设刹车，防过冲}：sigma 不能无限小——\verb|sigma| 小于约 0.1\,m 时 approach 退化成「几乎只有贴脸才给分」的准稀疏奖励，远处又没梯度了（回到 \cref{sec:rlmc-strike-reward} 一开头反对的稀疏困境）。经验区间 $\sigma\in[0.2,0.4]$\,m，与 arm workspace 尺度同量级。判停信号：contact\_rate 稳定爬过当前 curriculum 阶段的通过阈值（如 Stage 1 的 $50\%$）即停手，不要为「曲线更好看」继续压 sigma。
\end{enumerate}
...
\textbf{三框架对比：奖励接线。}mjlab 用 \Verb[breaklines]|RewardTermCfg(func=..., weight=...)| 注册（\cref{ch:rlmc-reward}）；Isaac Lab 的 \verb|RewardManager| 与 \verb|RewTerm| 在 API 命名上略有差异但语义一致（每项 \verb|func| 返回 \verb|[N]| 张量，按 weight 加权求和）。乘法组合结构在两框架里都需要写成\emph{一个}组合 reward term（而非多个独立 term 相加），因为 Manager 默认是\emph{加法}聚合。

\textbf{UniLab 对照（标量字典 $+$ 派发表，乘法组合天然落在一个函数里）。}UniLab 不用 \verb|RewardTermCfg| 类树，而是 \verb|reward.scales| 标量字典 $+$ env 内 \verb|_reward_fns| 派发表：在 \Verb[breaklines]|envs/locomotion/common/rewards.py| 的 \Verb[breaklines]|run_reward_dispatch()|（UniLab）里，奖励按 $\sum_k \texttt{scales}[k]\cdot\texttt{fns}[k](\texttt{ctx})$ \emph{加法}聚合，并最终乘 \verb|ctrl_dt|（等价 mjlab/Isaac 的 scale-by-dt）。因为聚合是加法，本章的乘法组合（\cref{ins:rlmc-strike-multiplicative}）必须封进\emph{一个} reward 函数、在 \verb|_reward_fns| 里注册成单项，再给它 scale 1.0——这与「mjlab 写成一个组合 term」殊途同归。
...
\textbf{UniLab 对照（无成功率门控 CurriculumManager，须用外部 loop）。}UniLab \emph{不是} Manager-Based，没有 \Verb[breaklines]|CurriculumManager| 类（\cref{sec:rlmc-strike-essence} 已述其架构差异）。它内置的课程只有两类：\Verb[breaklines]|base/curriculum.py| 的 \Verb[breaklines]|PenaltyCurriculum|（UniLab，按 episode 长度自适应缩放\emph{负权重}reward，原地改 \Verb[breaklines]|cfg.reward_config.scales| 里 $<0$ 的项）与 \verb|terrains/| 的 \Verb[breaklines]|TerrainCurriculumCfg|（地形等级）——\textbf{都不是按成功率门控的难度递进}。因此本节的八阶段成功率门控在 UniLab 里只能走\emph{外部管理器}路线（恰是本章 \cref{sec:rlmc-strike-integration} 训练循环采用的那种）：在训练循环里读 metrics、达标则改发球参数。
...
下面的 \Verb[breaklines]|PerceptionNoiseManager| 用一个手写的 \Verb[breaklines]|DelayedObservationBuffer| 做延迟，是\emph{为讲清延迟机制}与覆盖三框架最小公分母而写的通用骨架。但对 mjlab 1.4.0，实测 \Verb[breaklines]|ObservationTermCfg| 原生带 \verb|delay_min_lag| / \verb|delay_max_lag| / \verb|delay_per_env| / \Verb[breaklines]|delay_hold_prob| 等字段（\Verb[breaklines]|observation_manager| 已实装、引用 30 余处），\textbf{凡走 \texttt{ObservationTermCfg} 的 obs（如球位置估计项）只需一行配置即可得到 per-env 随机延迟，不必自维护 buffer}——还顺手消解了「延迟 buffer 在 reset 时未清空致鬼球」那个陷阱（框架内部按 env 管理延迟缓存并随 reset 清理）。手写 buffer 只在「延迟的不是某个 obs term、而是整条感知管线输出」时才需要。配置示意：
...
\textbf{三框架对比（延迟支持差异，已实测）。}三框架的\emph{噪声}都是声明式或近似声明式，但\emph{延迟}支持不同：\textbf{mjlab 1.4.0 原生支持 per-term 观测延迟}（\Verb[breaklines]|ObservationTermCfg| 的 \verb|delay_min_lag|/\allowbreak\verb|delay_max_lag|/\allowbreak\verb|delay_per_env|/\allowbreak\Verb[breaklines]|delay_hold_prob|，见 \cref{note:rlmc-strike-mjlab-delay}），无需手写 buffer；Isaac Lab 的 \Verb[breaklines]|ObservationTermCfg| 有 \verb|noise| 字段（命名一致、\Verb[breaklines]|enable_corruption| 同为\emph{组级}开关）但\emph{无}内置延迟字段，延迟需自定义 buffer；UniLab 也需自定义 buffer。即\textbf{只有 mjlab 把观测延迟做成了配置项}——这是把 mjlab 选作主线的一个具体便利（\cref{sec:rlmc-strike-mjlab-mainline}）。

\textbf{UniLab 对照（噪声手写进 \texttt{\_compute\_obs}，只加 actor 组；延迟须自定义 buffer）。}UniLab 没有声明式的 \Verb[breaklines]|ObservationTermCfg(noise=...)|——观测在 env 的 \Verb[breaklines]|_compute_obs()| 里手工拼接，噪声经 \Verb[breaklines]|_obs_noise(data, scale)| 函数加（UniLab），且\emph{只对 actor 组（\texttt{obs}）加噪、critic 组不加}（与本章「噪声进 obs、奖励/critic 用真值」的原则\cref{ins:rlmc-strike-noise-reward}天然一致）；噪声幅度 $=\texttt{level}\cdot\texttt{scale}\cdot U(-1,1)$，\verb|level<=0| 时关闭（等价 \Verb[breaklines]|enable_corruption=False|）。延迟须自定义 buffer（与 Isaac Lab 同；mjlab 则有内置字段，见上）。
...
\textbf{正确做法}：Stage 0--4 用真值或极低噪声，Stage 5--7 逐步加噪（上方 \Verb[breaklines]|configure_for_stage|）。
\end{pitfall}
...
\textbf{正确做法}：在 \verb|env.reset()| 里调用 \Verb[breaklines]|noise_manager.reset(env_ids)|。
\end{pitfall}

\begin{practice}[本节动手任务]
\begin{enumerate}
  \item \textbf{[设计]} 为每个 Curriculum Stage（0--7）设计噪声配置（位置 $\sigma$、速度 $\sigma$、最大延迟帧），并解释理由。验收标准：给出表格并说明为何 Stage 0--4 不加噪。
  \item \textbf{[编程]} 实现 \Verb[breaklines]|PerceptionNoiseManager| 并集成进训练循环。验收标准：Stage 0 时 obs 与真值一致，Stage 7 时 obs 有明显偏差。
\end{enumerate}
\end{practice}
...
# pd_gains），参数用 ranges + operation in {abs,add,scale} + shared_random（不是 Isaac 的
...
\textbf{代码走读。}在 mjlab 1.4.0 中，\verb|EventManager| 在 \verb|mode="reset"| 时于每个 episode 起点对指定 entity 字段重采样（\cref{ch:rlmc-dr}）。\verb|dr.body_mass| / \Verb[breaklines]|dr.geom_friction| / \verb|dr.pd_gains| 是模块化 DR 包 \Verb[breaklines]|mjlab/envs/mdp/dr/| 的内置函数（函数名精简、参数用 \verb|ranges|$+$\verb|operation|，区别于 Isaac Lab 的 \Verb[breaklines]|randomize_rigid_body_*|）；\Verb[breaklines]|randomize_ball_aerodynamics| 是本任务自定义事件（包住 \cref{sec:rlmc-tennis-aero} 的 $C_d$、$C_L$，mjlab 无内置气动 DR）。
...
\textbf{Isaac Lab 对照。}Isaac Lab 2.x 的 DR 配置类似，但部分函数有框架特有参数（如 \Verb[breaklines]|"operation": "abs"| 表示绝对替换而非缩放）：
...
\textbf{UniLab 对照（\texttt{DomainRandConfig} $+$ 后端能力门控，气动 DR 须自定义）。}UniLab 的 DR 不用 \verb|EventTermCfg|，而是任务的 \Verb[breaklines]|DomainRandConfig|（dataclass，乘性区间）$+$ \verb|dr/manager.py| 的 \Verb[breaklines]|DomainRandomizationManager|（UniLab）：\verb|reset| 时 \Verb[breaklines]|build_reset_plan| $\to$ 后端 \Verb[breaklines]|set_state(randomization=payload)|。质量/摩擦/PD 增益都是内置 reset 项（\Verb[breaklines]|randomize_base_mass| / \Verb[breaklines]|randomize_ground_friction| / \verb|randomize_kp| / \verb|randomize_kd|，乘性 \Verb[breaklines]|*_multiplier_range| 保物理一致，对应 mjlab 的 \Verb[breaklines]|operation="scale"|）。但本任务的\emph{气动 DR}（随机 $C_d$、$C_L$，对应本节 \verb|aero_dr|）UniLab 无内置事件，须像 mjlab 的 \Verb[breaklines]|randomize_ball_aerodynamics| 一样自定义。
...
UniLab 双后端（MuJoCo / Motrix）的 DR 能力\emph{不同}：每后端经 \Verb[breaklines]|get_dr_capabilities()| 声明支持的 reset 项（\verb|dr/types.py| 的 \Verb[breaklines]|DomainRandomizationCapabilities|，UniLab），MuJoCo 后端支持 11 项（含 \verb|body_inertia| / \verb|dof_armature|），Motrix 基线仅 4 项、按能力追加 \Verb[breaklines]|kp/kd/geom_friction/gravity|。\Verb[breaklines]|DomainRandomizationManager| 的 \Verb[breaklines]|filter_reset_payload| 会\emph{自动剔除}后端不支持的项（打 warning 跳过、不报错）——这是 GPU-sim 框架没有的失败面。对网球任务的含义：若用 Motrix 后端，球的惯量类 DR 可能被静默跳过，须查 warning 日志确认实际生效项。另注：UniLab \emph{无} mjlab 的 \Verb[breaklines]|pseudo_inertia|（Cholesky 重构的正定惯量 DR），惯量 DR 不保证扰动后物理合法。
\end{insight}

\textbf{跨框架一致性验证。}由于 MuJoCo 与 PhysX 接触模型不同（\cref{ch:rlmc-physics}），同样的 friction 值在两引擎中行为可能不同。建议在两框架各做一次 COR 标定（\cref{sec:rlmc-tennis-contact} 方法），确保 DR 范围在两引擎里都产生合理弹跳。\textbf{UniLab 在这点上有一个独到的一等公民设施}：\Verb[breaklines]|training/sim2sim.py| 的跨后端契约校验（UniLab）——把一份后端训出的策略拿到另一后端（MuJoCo$\leftrightarrow$Motrix）回放前，\Verb[breaklines]|resolve_sim2sim_config()| 会逐字段比对「决定策略行为的字段」，\Verb[breaklines]|training.sim2sim_strict=true|（默认）时不兼容直接抛 \Verb[breaklines]|CrossBackendIncompatibleError|，把「接触模型/DR 能力不同导致策略行为漂移」前置为编译期错误。这正是本段「两引擎行为可能不同」的工程化关卡，是 mjlab/Isaac Lab（单一引擎）没有的。
...
用 reward penalty（\Verb[breaklines]|r -= w * violation|）惩罚不安全动作有一个根本问题：若不安全动作偶尔带来高奖励（极端挥拍恰好碰到球），策略可能学到「冒险挥拍」，因为期望奖励仍为正。仿真里没问题，真机上一次极端动作就可能烧电机。安全约束需要的是\textbf{硬限制}——不是「做了扣分」而是「尽量不允许做」。
...
要分清两类终止语义：\verb|time_out=True| 表示\textbf{人为时间截断}（truncation），进入 \Verb[breaklines]|truncated/timeouts| 通道，RSL-RL 据此对最终状态做 value bootstrapping（$V(s_T)\neq 0$，因为真实 MDP 本会继续）；\Verb[breaklines]|time_out=False| 表示\textbf{真正的 MDP 终止}（terminated），$V(s_T)=0$。\Verb[breaklines]|ball landed after contact| 是单次击球任务的真实终止（球已落地、本回合结束、无后续回报），应设 \Verb[breaklines]|time_out=False|；只有 5\,s 上限这种人为截断才用 \verb|time_out=True|。把成功落地错标成 timeout 会混淆终止统计、\verb|is_terminated| 类奖励与价值目标——这正是 \cref{ch:rlmc-diag} 反复强调的「terminated/truncated 不能混」。
\end{insight}
...
\textbf{策略的「放弃」能力。}策略应能决定「这个球不打了」——当感知输出 \Verb[breaklines]|validity=False|（球不可达）时回到 ready pose 而非强行挥拍。真实系统中「不可达球」比「危险挥拍」更可接受。可加一个 give-up reward：\Verb[breaklines]|validity=False| 时，关节越接近 ready pose 给越高奖励。但其权重不能大于 approach reward，否则触发下方陷阱。
...
\textbf{UniLab 对照（Lagrangian 同样外挂；终止经 \texttt{TransitionBootstrapContract} 显式区分）。}UniLab 的 PPO 也复用 rsl-rl（\Verb[breaklines]|structured_configs.py| 的 \verb|PPOConfig| $+$ \Verb[breaklines]|algos/torch/rsl_rl_ppo.py| 的 \Verb[breaklines]|FinalObservationAwarePPO|，UniLab），所以本节的 \Verb[breaklines]|LagrangianPPOForTennis| 增广奖励同样可作外部 wrapper 挂上去、不动算法核心——这一条在三框架完全通用。差异在终止语义的\emph{表达方式}：UniLab 没有 \Verb[breaklines]|TerminationTermCfg| 类，\verb|terminated| 直接在 env 的 \Verb[breaklines]|update_state()| 里算（如摔倒 \Verb[breaklines]|gravity[:,2]<=0.5|），超时 \verb|truncated| 由基类 \Verb[breaklines]|_compute_truncated| 按 \Verb[breaklines]|max_episode_steps| 自动算；\verb@done = terminated | truncated@，而「自举该用哪个 next-obs、超时 vs 失败」被固化成 \Verb[breaklines]|base/final_observation.py| 的 \Verb[breaklines]|TransitionBootstrapContract|（UniLab）——它的 \Verb[breaklines]|timeout_terminal_mask| 与失败终止 mask 正是 mjlab/Isaac Lab \verb|time_out| 字段的显式契约版。所以本章「成功落地是 terminated、5\,s 上限是 truncated」（\cref{ins:rlmc-strike-termination}）在 UniLab 里落地为：成功落地进失败/真终止 mask（$V=0$），5\,s 上限进 \Verb[breaklines]|timeout_terminal_mask|（value bootstrap）。
...
\begin{practice}[本节动手任务]
\begin{enumerate}
  \item \textbf{[设计]} 为击球任务设计完整终止列表（$\ge 8$ 项），区分安全/任务/仿真三类，标注 \verb|time_out| 与优先级。验收标准：成功落地项设为 \Verb[breaklines]|time_out=False| 并说明理由。
  \item \textbf{[编程]} 实现 \Verb[breaklines]|LagrangianPPOForTennis| 并接入 PPO 循环，在 WandB 监控 $\lambda$ 轨迹。验收标准：能解释 $\lambda$ 持续上升意味着什么、如何调整。
...
策略不是在「击球那一刻」做决策，而是在球飞来的 0.3--0.5\,s 内规划整个击球动作。可用决策步数 $\approx$ 反应窗口 $\times$ 控制频率：0.3--0.5\,s $\times$ 50\,Hz $\approx$ 15--25 步。这就是为什么 actor 观测里必须含 \Verb[breaklines]|time_to_impact|（\cref{sec:rlmc-strike-mdp}）——没有它，策略无法判断「现在该挥到哪个阶段」，只能记住固定时刻挥拍而无法泛化。
\end{insight}
...
\textbf{Phase 1 $\to$ strike task 的频率升级（配置详解）。}\cref{ch:rlmc-tennisenv} 的 \Verb[breaklines]|timestep=0.002|（500\,Hz）对纯弹道足够，但对球拍-球接触不足。strike task 应升到 \Verb[breaklines]|timestep=0.0005|（2000\,Hz，对齐 LATENT）或 \verb|0.001|（1000\,Hz），对应 \verb|decimation=40| 或 \verb|20|（保持 50\,Hz 控制）。
...
\textbf{正确做法}：\Verb[breaklines]|timestep=0.0005| 配 \verb|decimation=40|，保持 50\,Hz 控制。
\end{pitfall}
...
\begin{itemize}
  \item \textbf{LATENT}（arXiv:2603.12686，\cite{zhang2026latent}）——从不完美人类动捕学人形网球；教 latent action space $+$ wrist correction 与最完整的网球 sim2real 消融。代码 \Verb[breaklines]|github.com/GalaxyGeneralRobotics/LATENT|。
...
\midrule
mjlab & 1.4.0 & \Verb[breaklines]|DifferentialIKActionCfg|、\verb|EventTermCfg|、\Verb[breaklines]|ObservationTermCfg(noise=...)| \\
Isaac Lab & 2.x & \Verb[breaklines]|DifferentialInverseKinematicsActionCfg|、\Verb[breaklines]|DifferentialIKControllerCfg| \\
MuJoCo Warp & 随 mjlab 1.4.0 & GPU 接触解算（2000\,Hz） \\
RSL-RL & 随框架 & \verb|PPOAgent|、\verb|time_outs| value bootstrap \\
UniLab & commit 99936e08 & \Verb[breaklines]|LocomotionBaseEnv.apply_action|（关节位置目标）、\Verb[breaklines]|run_reward_dispatch|、\Verb[breaklines]|DomainRandomizationManager|、\Verb[breaklines]|TransitionBootstrapContract| \\
\bottomrule
\end{tabularx}
\end{center}

\begin{finenote}
本章 API 名、版本号与论文数值均经实跑introspection（gpufree mjlab 1.4.0）+ 官方文档 + arXiv 正文核对：mjlab \texttt{DifferentialIKActionCfg} 的 DLS 参数是\emph{扁平字段}（\texttt{damping}/\allowbreak\texttt{delta\_pos\_scale}/\allowbreak\texttt{delta\_ori\_scale}/\allowbreak\texttt{max\_dq}，按 \texttt{actuator\_names}$+$\texttt{frame\_name} 选目标，不嵌套 \texttt{controller=}）；mjlab DR 在 \texttt{mjlab/\allowbreak envs/\allowbreak mdp/\allowbreak dr/\allowbreak } 包（\texttt{dr.\allowbreak body\_mass}/\allowbreak\texttt{geom\_friction}/\allowbreak\texttt{pd\_gains}，参数 \texttt{ranges}$+$\texttt{operation}），\emph{非} Isaac 的 \texttt{randomize\_rigid\_body\_*}；mjlab 观测 term 真名 \texttt{ObservationTermCfg}（\texttt{ObsTermCfg} 不存在），\texttt{enable\_corruption} 是\emph{组级}开关。Isaac Lab 2.x 侧 \texttt{DifferentialInverseKinematicsActionCfg}/\allowbreak\texttt{DifferentialIKControllerCfg}(\texttt{ik\_method="dls"},\texttt{ik\_params=\{"lambda\_val":...\}})、\texttt{randomize\_rigid\_body\_mass}(\texttt{operation="abs"}) 均为真实 API。论文侧 LATENT/HITTER/PACE/Phybot/KungfuBot/WoCoCo 的编号与归属、LATENT 真机消融 90.90\%/77.78\% 与去 DR/去噪声的降幅、HITTER 106 连续回合、PACE「物理 predictor 算 reward $+$ learned predictor 喂 obs」、KungfuBot bi-level 自适应容忍度（NeurIPS'25）、Phybot footwork$\to$swing$\to$task 三阶段均已逐条核实。无法从公开来源直接确认的细节（WoCoCo 部署 Butterworth 参数、把 KungfuBot 自适应机制套用到二值击球任务的可行性、HITTER 与 Phybot 的\emph{物理仿真频率}——两文均只明示 50\,Hz 控制频率）以原文/实测为准。UniLab 实现槽位已据实装框架（commit 99936e08，MuJoCoUni 3.8.0 / MotrixSim 0.8.2，源码 \texttt{src/unilab}）补齐：其与 mjlab/Isaac Lab 的关键差异是\emph{非 Manager-Based}（动作固定为关节位置目标残差、奖励用标量字典 $+$ \texttt{run\_reward\_dispatch} 派发表、终止经 \texttt{TransitionBootstrapContract}），故 task-space IK action、成功率门控 curriculum、气动 DR 在 UniLab 里均无现成对应类、须自实现，本章已逐处诚实标注而非编造等价 API。
\end{finenote}
