...
本章的 actuator 能力分散在两套仿真栈里：mjlab 通过 MuJoCo 的 \verb|<general>|/\verb|<dcmotor>| actuator 与 \verb|MjSpec| API 配置，Isaac Lab 通过 \verb|isaaclab.actuators| 下的一族 \verb|*Cfg| 数据类配置。系统要求与版本兼容性如下表——\textbf{actuator 的 API 在版本间漂移}：Isaac Lab 2.x 起把硬限制拆成 \verb|effort_limit| 与 \verb|effort_limit_sim| 两个字段（详见本章末「\nameref{sec:rlmc-act-versions}」），MuJoCo 较新版本才内置 \verb|<dcmotor>| actuator，照抄旧博客的写法到新版上可能行为不符甚至报错。
...
MuJoCo & 3.x（含 \verb|<dcmotor>|） & Linux / macOS & 训练需 NVIDIA & \verb|<position>|/\verb|<general>|/\verb|<dcmotor>|、\verb|mjcb_control| \\
Isaac Lab & 2.3.0（主线） & Linux / Windows & NVIDIA RTX & \verb|ImplicitActuatorCfg| … \verb|ActuatorNetMLPCfg| \\
...
UniLab & main（commit 99936e08） & Linux CUDA/ROCm/XPU、macOS & 物理在 CPU、策略需 GPU & \verb|PdControlConfig| PD 增益、\verb|kp/kd| 域随机化 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{配置} mjlab 与 Isaac Lab 中各级 actuator——从 MuJoCo \verb|<position>|/\verb|<general>| 到 \verb|ImplicitActuatorCfg|/\verb|DCMotorCfg|/\verb|ActuatorNetMLPCfg|，并给出每个参数的物理含义；
...
同一个「Ideal PD」，三框架放在不同的抽象层：\textbf{Isaac Lab} 把 actuator 做成一棵 Python \verb|*Cfg| 类树（\verb|ImplicitActuatorCfg|/\verb|IdealPDActuatorCfg|…），增益、力矩限、延迟都是数据类字段；\textbf{mjlab/MuJoCo} 把它写进 MJCF 的 \verb|<actuator>|（\verb|kp|/\verb|kv|/\verb|forcerange|），Python 侧的 \verb|BuiltinPdActuatorCfg| 只是「往 MJCF 注入这些字段」的薄包装；\textbf{UniLab} 走第三条路——Python 侧只有一个极简 \verb|PdControlConfig|（\verb|Kp/Kd/action_scale|），把增益交给「后端位置执行器」算力矩，而力矩-速度饱和、\verb|forcerange| 等\emph{物理约束统一落在模型文件}（MJCF/Motrix XML）里。\textbf{这条分野解释了后面几节的一个反复现象：在 Isaac Lab 里「升级 actuator」是换一个 Python 类，在 UniLab 里却是改模型文件 $+$ 调 \texttt{Kp/Kd}$+$靠 DR}——UniLab 把 actuator 复杂度从代码层挪到了资产层。
\end{insight}

\begin{pitfall}[MuJoCo 的 \texttt{kv} 与 \texttt{joint damping} 效果叠加]
\textbf{错误描述}：以为 \verb|<position kv="4"/>| 就是关节的全部阻尼，跨框架对齐时只搬这一个数。\textbf{现象后果}：若同时设了 \verb|<joint damping="0.5"/>|，真实总阻尼是 $4.5$；照搬 $4.0$ 到 Isaac Lab 的 \verb|damping| 上，两端阻尼不一致，sim2sim 就对不上。\textbf{根本原因}：MuJoCo 把「actuator 速度增益」与「关节本身阻尼」当成两个独立来源、效果相加。\textbf{正确做法}：Isaac Lab 的 \verb|damping| 应填 MuJoCo 的 \verb|kv| $+$ \verb|joint damping| 之和（此例 $4.5$）。这是跨框架对齐最常见的 bug 来源之一，呼应 \cref{ch:rlmc-physics} 的 sim2sim 关切。
\end{pitfall}
...
\textbf{错误描述}：以为给 \verb|<general>| 加上 \verb|dyntype="filter"| 就等价于 Isaac Lab 的 \verb|DCMotorCfg|。\textbf{现象后果}：高速运动时仿真力矩仍可达上限，与真机「高速时力矩跌落」对不上，照样跳不动。\textbf{根本原因}：\verb|dyntype="filter"| 只是\emph{一阶低通/带宽限制}（让力矩\emph{爬升}而非跳变），它\textbf{不会}按关节速度改变力矩上限；而式~\eqref{eq:rlmc-act-torquespeed} 的力矩-速度约束是\emph{随速度衰减的上限}，两者是不同的物理效应。\textbf{正确做法}：要真正的力矩-速度约束，用 MuJoCo \verb|<dcmotor>|，或在 \verb|mjcb_control| 里按式~\eqref{eq:rlmc-act-torquespeed} 显式 clip；\verb|<general>+filter| 只负责带宽这一项。
\end{pitfall}

\begin{insight}[\texttt{<general>} 是 MuJoCo 的「线性 actuator 瑞士军刀」]\label{ins:rlmc-act-general}
\verb|<general>| 通过组合三件套表达\emph{任何线性时不变} actuator：\verb|gaintype="fixed" gainprm="100"| 设 $k_p=100$；\verb|biastype="affine" biasprm="0 -100 -4"| 设 $\text{bias}=0+(-k_p)q+(-k_d)\dot q$，于是 $\tau=k_p q^* + 0 - k_p q - k_d\dot q = k_p(q^*-q)-k_d\dot q$；\verb|dyntype="filter" dynprm="0.02"| 再叠加 20~ms 一阶低通模拟带宽。这三个旋钮的组合实现了一个带宽限制的 PD 控制器，\textbf{不写一行 Python}。对\emph{非线性}模型（actuator network），MuJoCo 走 \verb|mjcb_control| 回调——但回调在 CPU 执行，牺牲 GPU 并行；Isaac Lab 的 \verb|ActuatorNetMLP| 则在 GPU 上跑，更适合大规模训练。\textbf{这条「MuJoCo 灵活/CPU 回调、Isaac Lab 分层/GPU 原生」的分野，是两框架 actuator 哲学的根本差异}。
\end{insight}
...
\midrule
\verb|saturation_effort| & 电机数据手册堵转力矩 & 决定零速时力矩上限 & 与 \verb|effort_limit| 通常相等；高速时实际上限按式~\eqref{eq:rlmc-act-torquespeed} 更低 \\
\verb|velocity_limit| & 数据手册空载转速 & 决定力矩衰减到 0 的速度 & 设太小 $\to$ 中速就没力矩；与 gear ratio 有关 \\
\verb|stiffness| ($k_p$) & 调参 / 辨识 & 跟踪刚度 & 与 \verb|damping| 共同决定二阶响应 $\omega_n,\zeta$ \\
\verb|damping| ($k_d$) & 临界阻尼估计 & 抑制振荡 & 含 MuJoCo \verb|joint damping| 叠加项 \\
\verb|effort_limit_sim| & 同 \verb|effort_limit| & PhysX 求解器硬限制 & 不设则可能突破 \verb|effort_limit|（见版本信息速查） \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{进阶（mjlab）：用反射惯量\emph{物理推导}增益，而非手调。}上面 \verb|datasheet_to_cfg| 用经验式估 $k_p/k_d$，工程上够快，但 mjlab 1.4.0 在 G1 上给出了更严谨的范式：先算电机经减速器折算到关节侧的\textbf{反射惯量}（reflected inertia），再令 PD 增益由「期望二阶响应」反推。mjlab 的 \verb|utils/actuator.py| 提供 \verb|reflected_inertia()| 与 \verb|reflected_inertia_from_two_stage_planetary()|（两级行星减速）两个工具，把电机转子惯量 $\times$ 减速比$^2$ 折算到关节（注意工具在 \verb|mjlab.utils.actuator|，actuator \emph{类}才在 \verb|mjlab.actuator|）。随后按 $\omega_n,\zeta$ 设增益（这正是临界阻尼式 $k_d=2\sqrt{k_p J}$ 的「指定 $\omega_n/\zeta$」版本）：
...
mjlab 1.4.0 的 \verb|actuator/| 不止一个 \verb|ActuatorCfg|，而是一族（已读 \verb|actuator/__init__.py| 导出核对）：基类 \verb|ActuatorCfg|（字段 \verb|armature/frictionloss/viscous_damping| 与延迟模型 \verb|delay_min_lag/max_lag/hold_prob|）派生出 \verb|BuiltinPositionActuatorCfg|、\verb|BuiltinPdActuatorCfg|（\verb|stiffness=kp/damping=kd|）、\verb|BuiltinMotorActuatorCfg|、\verb|BuiltinVelocityActuatorCfg|、\verb|BuiltinMuscleActuatorCfg|、\verb|DcMotorActuatorCfg|（直流电机）、\verb|IdealPdActuatorCfg|、\verb|LearnedMlpActuatorCfg|（\textbf{学习型 actuator 网络}，对应本章 Level 2）、\verb|XmlActuatorCfg|。\textbf{这棵族谱与 Isaac Lab 的继承树（\cref{ins:rlmc-act-tree}）一一呼应，但有两点 mjlab 独有}：(1) 反射惯量工具族（\verb|reflected_inertia*|），让增益可由电机$+$减速器参数\emph{物理推导}；(2) \verb|pseudo_inertia| 这类「物理合法的惯量随机化」（见 \cref{sec:rlmc-act-inertia-dr}）。\textbf{第三框架对照}：UniLab 没有这套 actuator 族谱——它只有 \verb|PdControlConfig|（\cref{ins:rlmc-act-unilab-philosophy}），\verb|LearnedMlpActuatorCfg| 这类学习型 actuator 在 UniLab 无对应（见 \cref{sec:rlmc-act-net-integrate} 的三框架对照）。

注意 mjlab 的一处命名陷阱：实体层执行器绑定用 \verb|target_names_expr|（关节名），但动作层 \verb|JointPositionActionCfg| 用 \verb|actuator_names|（执行器名），内部经 \verb|entity.find_joints_by_actuator_names()| 解析——两者别混（Isaac Lab 无此区分，动作直接落到 \verb|joint_names|）。
\end{insight}

\begin{pitfall}[把数据手册「峰值扭矩」直接当训练 forcerange]
\textbf{错误描述}：直接把手册峰值扭矩填进 \verb|forcerange|/\verb|effort_limit|。\textbf{现象后果}：训练中关节长期顶在力矩上限，真机持续输出该扭矩会过热降额，仿真却不会，导致真机「跑着跑着没劲」。\textbf{根本原因}：手册峰值是\emph{短时}峰值（常 $<1$~s），\emph{持续可用}扭矩通常只有峰值的 30\%--50\%。\textbf{正确做法}：训练 forcerange 应设为\emph{持续}扭矩而非峰值扭矩；若任务确需短时峰值，配合温度/占空比约束或留出余量。
\end{pitfall}
...
\begin{enumerate}
  ...
  \item \textbf{[跨章综合题]} 结合 \cref{ch:rlmc-physics}：若 DC Motor 显示关节高速时力矩不足，除增大 $k_p$ 外还能调哪些 actuator 参数？\emph{验收}：列出 \verb|velocity_limit|（决定衰减斜率）、\verb|saturation_effort|、gear ratio 至少两项。
\end{enumerate}
\end{practice}
...
训练时务必做\textbf{输入/输出归一化}（按训练集均值方差），并把 \verb|X_mean/X_std/Y_mean/Y_std| 一并存进 checkpoint——部署时要用同一组统计量反归一化，否则力矩量纲全错。
...
\noindent 上面的 \verb|mjcb_control| 回调在 \textbf{CPU} 执行（牺牲 GPU 并行，见 \cref{ins:rlmc-act-general}）。但 mjlab~1.4.0 其实\textbf{原生提供了一个跑在训练管线里的学习型 actuator} \verb|LearnedMlpActuatorCfg|（实跑核对其字段，对应本章 Level 2，无需自己写回调）——这正是 mjlab 与 Isaac Lab \verb|ActuatorNetMLPCfg| 对位的那一档：
...
Isaac Lab 把 actuator 组织成一棵继承树（本书已联网核对类名）：抽象基类 \verb|ActuatorBase| 派生出 \verb|ImplicitActuator|（PhysX 内部 PD）与 \verb|IdealPDActuator|（Python 显式 PD）；后者再派生 \verb|DCMotor|（$+$力矩-速度饱和），\verb|DCMotor| 又派生 \verb|ActuatorNetMLP| 与 \verb|ActuatorNetLSTM|（$+$MLP/LSTM 力矩预测）；\verb|IdealPDActuator| 还派生 \verb|DelayedPDActuator|（$+$通信延迟）与 \verb|RemotizedPDActuator|（$+$遥控杆非线性）。\textbf{这棵树让你能从 Implicit 起步训练、确认 baseline 后再切到 DCMotor 或 ActuatorNet 做最后微调}——逐级升级、可归因。对应的 \verb|*Cfg| 数据类一一对应（\verb|ImplicitActuatorCfg|/\verb|DCMotorCfg|/\verb|ActuatorNetMLPCfg|…）。
\end{insight}
...
\textbf{错误描述}：望文生义，以为 delta 是用 \verb|mse_loss(sim_step(s, a+delta), s_real)| 直接对仿真 step 反向传播训练的监督回归。\textbf{现象后果}：照此实现既要求仿真器可微，又对不上官方代码，复现失败。\textbf{根本原因}：ASAP 官方 delta 是 \textbf{open-loop RL（PPO）} 训练——reward 最小化 sim/real 状态误差并带 action-norm 惩罚，随后再微调 policy；统一入口靠配置切换，\textbf{没有}独立的 \verb|train_delta.py|，也\textbf{不用} mjlab（支持 IsaacGym/IsaacSim/Genesis）。\textbf{正确做法}：把 delta 训练理解为 RL 而非监督回归（本书已联网核对官方仓库 LeCAR-Lab/ASAP；与 \cref{paper:rlmc-dr-asap} 的勘误一致）。
\end{pitfall}

\begin{pitfall}[\texttt{max\_delta} 设太大导致微调不稳定]
\textbf{错误描述}：为追求修正幅度把 \verb|max_delta| 设得很大。\textbf{现象后果}：修正后动作超出策略训练时见过的动作范围，策略在「陌生」动作下行为不可预测，微调发散。\textbf{根本原因}：delta 把动作推出了策略的有效域；另外若 delta 与策略\emph{同时}训练，两网络互相「追逐」也会不稳。\textbf{正确做法}：从 \verb|max_delta=0.05| 起步，逐步增大到不超过 0.2；且严格按 ASAP 第 4 步\emph{冻结} delta 再微调策略。
\end{pitfall}
...
\textbf{错误描述}：照「属性=值」直觉写 \verb|jnt.damping = 0.05|（在 MuJoCo~3.8.x 实测会抛 \verb|TypeError|）。\textbf{现象后果}：脚本在写回这一步就崩，前面辨识白做。\textbf{根本原因}：\verb|MjsJoint| 的某些字段是\emph{定长数组}而非标量——\verb|damping| 是 \verb|[3,1]| 数组（为兼容多自由度关节），\verb|gainprm/biasprm/dynprm| 也都是数组；只有 \verb|frictionloss| 这类才是标量。直接整体赋一个 Python float，类型对不上。\textbf{正确做法}：\textbf{数组字段用索引赋值}（\verb|jnt.damping[0]=...|、\verb|act.gainprm[0]=...|、\verb|act.biasprm[1]=...|），\textbf{标量字段直接赋}（\verb|jnt.frictionloss=...|、\verb|act.dyntype=...|）。怎么自己查某字段是标量还是数组？跑一行 \verb|print(type(jnt.damping), getattr(jnt.damping,"shape",None))|——返回 \verb|ndarray (3,1)| 就是数组、得用 \verb|[i]=|；这就是排这类 \verb|TypeError| 的通用手段（先 \verb|print(type(...))| 再决定赋值方式），而非死记哪个字段是数组。
\end{pitfall}
...
\textbf{错误描述}：把扫频/摩擦实验得到的 $\tau_c$（单位 N$\cdot$m）直接填进 Isaac Lab/PhysX 的 \verb|friction| 字段。\textbf{现象后果}：摩擦量级与语义全错，关节阻力与真机对不上。\textbf{根本原因}：MuJoCo \verb|<joint frictionloss>| 是\emph{N$\cdot$m 干摩擦}，而 Isaac Lab/PhysX 的 \verb|friction| 是\emph{无量纲关节摩擦系数}——单位与语义都不同。\textbf{正确做法}：MuJoCo 一侧可直接填 \verb|frictionloss=tau_c|；Isaac Lab 一侧需单独标定 PhysX 摩擦，或把它当作待校准参数，切勿把 N$\cdot$m 数值直接搬过去。
\end{pitfall}
...
\textbf{mjlab} 1.4.0 把 pseudo-inertia 做成一等公民——\verb|envs/mdp/dr/body.py| 的 \verb|pseudo_inertia| 不直接扰动 $3\times3$ 惯量分量，而是在 $4\times4$ 伪惯量矩阵上扰动、再经 Cholesky 分解/重构（含平行轴定理把 COM 惯量搬回 body 原点）保证扰动后惯量\emph{仍正定、物理可实现}。\textbf{Isaac Lab} 无现成函数，需在质量随机化的 \verb|recompute_inertia| 基础上手动加 Cholesky 重构。\textbf{UniLab 无对应}：其 reset 期 DR 只到 \verb|body_inertia/body_ipos/body_iquat| 级（已读 \verb|dr/types.py| 与两后端 \verb|get_dr_capabilities()| 核对），\emph{不保证}扰动后惯量正定，无 Cholesky 重构这一步——在 UniLab 上做惯量 DR 须自行约束幅度或退而只随机化质量。\textbf{这与本章主线呼应：actuator 物理保真度（增益$+$反射惯量$+$惯量 DR）的「物理合法性」也分框架——mjlab 在这一项上最完备}。
\end{insight}
...
\begin{practice}[本节练习]
\begin{enumerate}
  \item \textbf{[动手题]} 有真机就对一个关节做阶跃响应；没有就用 MuJoCo \verb|<general>+dyntype="filter"| 当「真机」，跑阶跃并用 \verb|fit_step| 拟合。\emph{验收}：给出 $\omega_n,\zeta,k_p,k_d$，并与你设的 \verb|biasprm|/\verb|dynprm| 对照是否自洽。
...
\begin{codebox}{actuator 建模层级选型}
有真机吗?  否 -> Ideal PD + DR (纯仿真研究)
           是 -> 有力矩传感器/电流测量吗?
                 是 -> 任务需极限力矩? 是->Actuator Network  否->DC Motor + DR
                 否 -> 任务需极限力矩? 是->ASAP Delta(免力矩传感器)  否->Ideal PD + DR
最后: DR 在真机上够不够(真机性能 >= 80% 仿真)?  够->当前层级足够  不够->升一级
\end{codebox}
...
\begin{practice}[本节练习]
\begin{enumerate}
  \item \textbf{[决策题]} 你的四足在仿真里能 2~m/s 快跑，真机只能 1.2~m/s 且高速时「腿软」。按决策流程图，下一步该做什么？\emph{验收}：先用 \verb|quick_diagnosis| 看是否常饱和；若是，升 DC Motor 并检查 \verb|velocity_limit| 是否过小。
...
\verb|dyntype="filter"| 等于 DC Motor & filter 只是带宽限制（力矩爬升），不改力矩\emph{上限}；力矩-速度约束要 \verb|<dcmotor>| 或显式 clip \\
\verb|saturation_effort| 就是电机最大力矩 & 它是\emph{堵转}（零速）力矩；高速时按式~\eqref{eq:rlmc-act-torquespeed} 衰减 \\
...
\begin{itemize}
  ...
  \item MuJoCo 官方 XMLReference 的 actuator 章节 \cite{todorov2012mujoco}——\verb|<general>|/\verb|<dcmotor>| 的 gaintype/biastype/dyntype 完整参数；Isaac Lab actuators API 文档 \cite{mittal2025isaaclab}——\verb|ImplicitActuator|/\verb|DCMotor|/\verb|ActuatorNetMLP| 的字段与继承关系。
\end{itemize}
...
真机动作「拖沓」、响应慢 & 仿真无带宽限制 & 加 \verb|dyntype="filter"| $\to$ 扫频测带宽 $\to$ 设 \verb|dynprm| & \cref{sec:rlmc-act-dcmotor} \\
真机高速时正常仿真不行 & 力矩-速度约束未建模 & 查 \verb|velocity_limit| $\to$ 对比高速可用力矩 & \cref{sec:rlmc-act-dcmotor} \\
...
\verb|<general>| 力矩异常 & \verb|biasprm| 符号错 & 手验力矩公式 $\to$ 打印 \verb|actuator_force| 对比 & \cref{ins:rlmc-act-general} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \textbf{Isaac Lab 2.x 起：\texttt{effort\_limit} 与 \texttt{effort\_limit\_sim} 分离}——\verb|effort_limit| 是 actuator 模型的软限制（Python 层 clip），\verb|effort_limit_sim| 是 PhysX 求解器的硬限制。\textbf{只设 \texttt{effort\_limit} 而漏设 \texttt{effort\_limit\_sim} 时，PhysX 可能允许超过 \texttt{effort\_limit} 的力矩}，导致仿真与真机不一致——推荐两者设为相同值（本书已联网核对 Isaac Lab 文档与 issue 讨论）。
  ...
  \item \textbf{Isaac Lab actuator 类名}——\verb|ImplicitActuatorCfg| / \verb|IdealPDActuatorCfg| / \verb|DCMotorCfg| / \verb|DelayedPDActuatorCfg| / \verb|RemotizedPDActuatorCfg| / \verb|ActuatorNetMLPCfg| / \verb|ActuatorNetLSTMCfg|；网络权重字段统一为 \verb|network_file|；DC Motor 堵转力矩字段为 \verb|saturation_effort|（均已联网核对）。
  \item \textbf{mjlab actuator 族（1.4.0）}——\verb|actuator/__init__.py| 导出 \verb|BuiltinPositionActuatorCfg| / \verb|BuiltinPdActuatorCfg|（\verb|stiffness/damping|）/ \verb|BuiltinMotorActuatorCfg| / \verb|BuiltinVelocityActuatorCfg| / \verb|BuiltinMuscleActuatorCfg| / \verb|DcMotorActuatorCfg| / \verb|IdealPdActuatorCfg| / \verb|LearnedMlpActuatorCfg|（学习型）/ \verb|XmlActuatorCfg|；基类 \verb|ActuatorCfg| 带 \verb|armature/frictionloss/viscous_damping| 与延迟字段；另有反射惯量工具 \verb|reflected_inertia()| / \verb|reflected_inertia_from_two_stage_planetary()|。实体层绑定用 \verb|target_names_expr|，动作层用 \verb|actuator_names|（别混）。
  \item \textbf{UniLab actuator（main，commit 99936e08；mujoco-uni 3.8.0 / motrixsim-core 0.8.2）}——\textbf{无 actuator 类树}：Python 侧只有 \verb|PdControlConfig|（继承 \verb|ControlConfigBase|；\textbf{自有}字段仅 \verb|Kp| 默认 35.0、\verb|Kd| 默认 0.5，而 \verb|action_scale| 默认 0.25、\verb|simulate_action_latency| \textbf{默认 \texttt{False}} 是\emph{继承自基类}的——勿误记为 \verb|True| 或当成本类字段），增益经 \verb|create_backend| 的 \verb|position_actuator_gains={"kp","kd"}| 下沉到后端位置执行器（= Ideal PD）。\textbf{DC Motor 饱和 / forcerange 写在模型层}（MJCF \verb|<general>|/\verb|<dcmotor>| 或 Motrix XML），不在 Python 配类。\textbf{无学习型 actuator 网络等价物}（\verb|LearnedMlpActuatorCfg| / \verb|ActuatorNetMLPCfg| 无对应，需自实现或靠 DR 覆盖）。\textbf{无 \texttt{pseudo\_inertia} 物理合法惯量 DR}（reset 项只到 \verb|body_inertia/body_ipos/body_iquat| 级，不保证正定）。sim2real 主力是 \verb|kp/kd| 域随机化（\verb|DomainRandConfig.randomize_kp/kd| $+$ \verb|*_multiplier_range|，MuJoCo 后端原生支持、Motrix 经能力门控）。以上均据源码核对（路径 \verb|unilab/envs/locomotion/common/base.py|、\verb|dr/types.py|、两后端 \verb|get_dr_capabilities()|）。
\end{itemize}
