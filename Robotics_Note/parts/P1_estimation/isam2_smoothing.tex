...
%  记号归一：R∈SO(3)、T∈SE(3)（preamble 宏 \SO \SE）；se(3) 排序 ξ=[ρ;φ]；右扰动为主、retraction 写 ⊕（= \cref{ch:lie} 的 ⊞）。
...
$\mathrm{Exp}/\mathrm{Log}$、$\mathrm{Ad}$ 沿用 \cref{ch:lie}。本章源笔记为 AI 撰写、经多轮审仍可能有错，存疑处就地以 \pz{} 标注，并汇入章末复核清单。
\end{note}
...
考虑一个自动驾驶场景：$4500$ 帧序列、每帧一个 $6$-DoF 位姿，$N=2.7\times10^4$。在 COLAMD 排序下批量 Cholesky 约需 $O(N^{3/2})\approx 4.6\times10^6$ 次浮点运算，桌面 CPU（约 $10$\,GFLOPS）约 $0.5$\,ms/帧——看似可行。但一旦加入 $5000$ 个 $3$-DoF 路标，$N$ 跳到 $4.2\times10^4$，Hessian 从位姿图的带状结构变成 BA 的箭头（arrow-head）结构，即便先做 Schur 消元路标，仍需 $O(N_{\text{pose}}^2\,N_{\text{lm}})\approx 10^{11}$ 量级运算\pz{存疑：该 BA flops 量级是原稿的粗估，量纲与系数未严格推导，仅作数量级直觉}。此时若改用增量——每步只消元受影响的约 $50$ 个团——可省下 $3$--$4$ 个数量级。
...
\begin{tabular}{cllp{0.30\textwidth}}
\toprule
年份 & 工作 & 核心贡献 & 局限 \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{线性代数角度}：$\mathbf{R}$ 的每个超节点是一组共享非零行结构的列。回代 $\mathbf{R}\,\delta=\mathbf{d}$ 时，超节点\emph{内部}用稠密 Level-3 BLAS（如 \texttt{dtrsv}/\texttt{dgemm}），超节点\emph{之间}只传 separator 分量——这正是 wildfire 在矩阵层面的实现。
...
$\mathrm{Pose3}$ 的切向量同时含旋转与平移 $\delta\boldsymbol{\xi}=[\boldsymbol{\rho}^\top,\boldsymbol{\phi}^\top]^\top$\pz{存疑：原稿此处把切向量写成 $[\omega;\rho]$（旋转在前）；本书 $\se(3)$ 约定平移在前 $\boldsymbol{\xi}=[\boldsymbol{\rho};\boldsymbol{\phi}]$，已统一，物理结论不变}，“旋转 $0.1$\,rad”与“平移 $0.1$\,m”物理意义不同。工程上更合理的是按变量类型设阈值：
...
\noindent GTSAM 默认 \texttt{relinearizeThreshold}$=0.1$（标量，大致是“旋转 $0.1$\,rad 或平移 $0.1$\,m”），也支持 \texttt{FastMap<char,Vector>} 对 \texttt{X}/\texttt{V}/\texttt{B} 等符号分别给阈值向量；另有 \texttt{relinearizeSkip}$=10$（每 $10$ 次 \texttt{update()} 才触发一次 relin 检查，实时系统常改为 $1$）。实测 $\beta=0.1$ 对 $\chi^2$ 几乎无影响，而受影响矩阵元数可降到批量的 $5$--$15\%$（Kaess 2012 Fig.~9\cite{kaess2012isam2}）。
...
\begin{enumerate}
  ...
  \item 在 GTSAM 中分别设 \texttt{relinearizeThreshold}$=0.001,0.1,1.0$，预测 \texttt{ISAM2Result.variablesRelinearized} 的相对大小。
\end{enumerate}
\end{practice}
...
\begin{enumerate}
  ...
  \item \texttt{isam\_.update(factors, values, timestamps, constrainedKeys, ...)}；
  \item \texttt{isam\_.marginalizeLeaves(marginalizableKeys)}；
...
\end{itemize}
LIO-SAM 回环后约 $6$ 次空 update（常规 $2$ 次里 $1$ 次空，加回环块的 $5$ 次）的经验来自：闭合误差通常 $<5$\,m，按 \cref{eq:gn-quadratic}，多次二次收敛可把误差迅速降到远低于传感器精度（设 $C\approx 0.1$）\pz{存疑：原稿给出 $5\times(0.5)^{64}\approx 10^{-10}$\,m 的估计；二次收敛的“迭代次数指数化”仅在 $C\|\delta^{(0)}\|<1$ 时成立，且 $6$ 次叠加的常数处理偏粗，量级直觉可用、精确数值待核}。
...
GTSAM 默认不算非线性误差（代价高）。开发阶段应设 \texttt{params.evaluateNonlinearError = true}，观察 $\Delta F=F_{\text{after}}-F_{\text{before}}$。若单次 update 后误差明显上升，常见原因与对策见下表。
...
\begin{itemize}
  ...
  \item \textbf{iSAM2 每步}：纯探索（无回环、每步常数个新约束）时只有树顶常数个团需重消元，故每步 $O(1)$\cite{kaess2012isam2}；典型含回环的 SLAM 图受影响团数 $\approx$ 路径长度 $\approx$ separator 长度，按嵌套剖分约 $O(\sqrt N)$（2D）/ $O(N^{2/3})$（3D），最坏（大回环遍及全树）退化为批量 $O(N^{3/2})$。\pz{存疑：每步 $O(\sqrt N)$/$O(N^{2/3})$ 是由分隔符大小推导的工程估计；Kaess 2012 原文只给纯探索 $O(1)$ 与一般批量 $O(N^{3/2})$（引 Krauthausen 2006），未逐 2D/3D 给出每步 $\sqrt N$/$N^{2/3}$ 闭式}
...
\paragraph{API 与关键参数。}
头文件 \texttt{gtsam/nonlinear/ISAM2.h}、\texttt{ISAM2Params.h}、\texttt{ISAM2Result.h}、\texttt{ISAM2-impl.h}。\texttt{ISAM2Params} 关键字段见下表。
...
\texttt{evaluateNonlinearError} & \texttt{false} & 为 true 则 \texttt{errorBefore/After} 有值 \\
...
\noindent \texttt{update()} 完整签名（按位：新因子、新初值、待删因子下标、约束 key、不重线性化 key、额外重消元 key、强制重线性化）。返回 \texttt{ISAM2Result}，含 \texttt{variablesRelinearized}、\texttt{variablesReeliminated}、\texttt{factorsRecalculated}、\texttt{cliques}、\texttt{newFactorsIndices}（删因子时必需）、以及（开启时）\texttt{errorBefore/After}。
...
\paragraph{内部数据结构。}
\texttt{ISAM2} 主体是一棵 \texttt{BayesTree<GaussianBayesTreeClique>}，每个团（\texttt{GaussianBayesTreeClique}）含：一个 \texttt{GaussianConditional}（条件密度 $P(F_k\mid S_k)$，即 $\mathbf{R}$ 的行块 $[R_{FF}\,R_{FS}]$ 加 RHS $d_F$）、子团指针、以及 \texttt{cachedFactor}/\texttt{cachedSeparatorMarginal}（缓存边缘因子，用于 orphan 挂回）。其余成员：\texttt{theta\_}（当前线性化点 $\Theta$）、\texttt{delta\_}（当前增量 $\delta$）、\texttt{deltaNewton\_}/\texttt{RgProd\_}（Dogleg 专用）、\texttt{variableIndex\_}（因子$\to$变量反向索引）、\texttt{nonlinearFactors\_}（原始因子存档，供重线性化）、\texttt{linearFactors\_}（缓存线性因子）、\texttt{fixedVariables\_}（已 marginalize、不许 relin 的变量）。

\paragraph{案例：LIO-SAM 的每帧流程。}
\texttt{LIO-SAM/src/mapOptmization.cpp} 配置 \texttt{relinearizeThreshold}$=0.1$、\texttt{relinearizeSkip}$=1$（比默认激进 $10\times$），不设 \texttt{factorization}/Dogleg（即默认 GN）。每帧 \texttt{saveKeyFramesAndFactor()}：
...
下表汇总 iSAM2 工程中最常踩的坑（合并自源笔记 §C.23 与各分散陷阱）。
...
\begin{enumerate}
  ...
  \item \textbf{症状}：每帧耗时\emph{单调递增}。\textbf{原因}：Bayes 树根团膨胀 / factor slot 不复用（\cref{sec:isam-complexity}）。\textbf{对策}：打印 \texttt{ISAM2Result.cliques}；开 \texttt{findUnusedFactorSlots}；检查是否需 constrained ordering。
