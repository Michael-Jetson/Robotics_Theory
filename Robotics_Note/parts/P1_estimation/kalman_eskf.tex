...
归纳可证 KF 估计\emph{无偏}（$\mathbb{E}[\hat{\mathbf{x}}_k-\mathbf{x}_k]=\mathbf{0}$）且\emph{一致}（其报告的 $\hat{\mathbf{P}}_k$ 确为真实误差协方差，下节会把“一致”量化为 NEES）。即便去掉高斯假设，只要噪声零均值、不相关、方差已知，KF 仍是\textbf{最优线性无偏估计}（BLUE，高斯–马尔可夫定理），其协方差达到克拉默–拉奥下界（CRLB）\cite{barfoot2024state}。
...
%  来源是 AI 写作多轮审稿稿，存疑公式/论文归属就地打 \pz，并写入 claimsToVerify。
% ════════════════════════════════════════════════════════════════
...
\begin{itemize}
  \item \textbf{LM--IEKF}（Levenberg--Marquardt 化）：在近似 Hessian 上加阻尼 $\mathbf{A}_\lambda^{(j)}=\check{\mathbf{P}}_k^{-1}+\mathbf{H}^{(j)\top}\boldsymbol{\Sigma}_{\mathbf{v}}^{-1}\mathbf{H}^{(j)}+\lambda\mathbf{I}$，$\lambda$ 大时趋最速下降（小步稳）、$\lambda$ 小时趋 GN（大步快）；$\lambda$ 按代价升降自适应调整（接受步缩小 $\lambda$、拒绝步放大 $\lambda$）\pz{存疑：LM--IEKF 的具体 $\lambda$ 调整策略（Nielsen 系数 $\max(1/3,1-(2\rho-1)^3)$ 等）源里给到 Skoglund 等 FUSION 2015，公式细节与系数待核}。
