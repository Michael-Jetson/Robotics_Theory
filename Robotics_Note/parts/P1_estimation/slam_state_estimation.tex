...
要把这先验放进 MAP，须把它写成代价项（先验的负对数似然）。\cref{eq:steam-gp} 的 GP 给出相邻状态的条件分布，定义\emph{二元因子}误差（首时刻为锚定的一元因子）：
\begin{equation}
  \mathbf{e}_{v,k}=
  \begin{cases}
...
为用高斯牛顿，在工作点 $\mathbf{x}_{\mathrm{op}}$ 处线性化。位姿用右扰动 $\boldsymbol{\epsilon}_k$、速度用加性扰动 $\boldsymbol{\eta}_k$，堆为 $\boldsymbol{\varepsilon}_k=[\boldsymbol{\epsilon}_k;\boldsymbol{\eta}_k]\in\mathbb{R}^{12}$。利用 BCH 一阶近似 $\ln(\exp(\boldsymbol{\epsilon}_k^\wedge)\mathbf{T}_{\mathrm{op},k}\mathbf{T}_{\mathrm{op},k-1}^{-1}\exp(-\boldsymbol{\epsilon}_{k-1}^\wedge))^\vee\approx\ln(\mathbf{T}_{\mathrm{op},k}\mathbf{T}_{\mathrm{op},k-1}^{-1})^\vee+\boldsymbol{\mathcal{J}}_{r,\mathrm{op}}^{-1}(\boldsymbol{\epsilon}_k-\boldsymbol{\mathcal{T}}_{\mathrm{op},k,k-1}\boldsymbol{\epsilon}_{k-1})$ 与 $\boldsymbol{\mathcal{J}}_r^{-1}(\mathbf{x})\approx\mathbf{I}-\tfrac12\mathbf{x}^\curlywedge$，得
\begin{equation}
  ...
  \label{eq:steam-lin}
\end{equation}
其中（记 $\boldsymbol{\mathcal{T}}_{\mathrm{op},k,k-1}=\mathrm{Ad}(\mathbf{T}_{\mathrm{op},k}\mathbf{T}_{\mathrm{op},k-1}^{-1})$、$\boldsymbol{\mathcal{J}}_{\mathrm{op},k,k-1}=\boldsymbol{\mathcal{J}}_r(\ln(\mathbf{T}_{\mathrm{op},k}\mathbf{T}_{\mathrm{op},k-1}^{-1})^\vee)$，$k>0$）
\begin{equation}
...
\end{equation}
它可\emph{利用箭头结构高效抽取}——这正是\emph{选择性稀疏逆}（selected inverse / Takahashi 方程）的用武之地：只回收 $\hat{\mathbf{P}}=\boldsymbol{\Lambda}^{-1}$ 中\emph{需要的}块，绝不显式求整个稠密逆（\cref{sec:nlopt-cov} 的“按需回代特定块”即此，连续时间下因要反复查询而尤为关键）。拿到两端边缘后，把原本连接 $t_k,t_{k+1}$ 的单个先验因子\emph{替换}为经过 $\tau$ 的两段先验因子（\cref{eq:steam-FE} 取相应时间差），构造一个仅含 $\{t_k,\tau,t_{k+1}\}$ 三状态的 $36\times36$ 小逆协方差，再对 $t_k,t_{k+1}$ 做\emph{边缘化}（信息形式舒尔补，\cref{eq:info-marginal}），即得 $\hat{\mathbf{P}}(\tau,\tau)^{-1}$。单次协方差查询同样 $O(1)$。
