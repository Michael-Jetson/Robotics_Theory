...
$\boldsymbol{\Lambda},\ \boldsymbol{\eta}$ & 信息矩阵（$=\boldsymbol{\Sigma}^{-1}$）/ 信息向量 \\
...
$\mathbf{H}=\mathbf{J}^\top\mathbf{W}\mathbf{J}$ & 高斯–牛顿近似 Hessian（信息矩阵） \\
$\mathbf{W}=\boldsymbol{\Sigma}^{-1}$ & 权重（信息）矩阵 \\
\bottomrule
\end{tabular}
\end{center}
