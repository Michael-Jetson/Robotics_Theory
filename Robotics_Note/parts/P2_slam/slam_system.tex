...
上面 \texttt{triangulation} 的返回逻辑与注释/调用方语义\emph{相反}：当奇异值比 $<10^{-2}$（“解质量不好、视差不足”）时却 \verb|return true|，质量好时反而 \verb|return false|。而调用方写的是 \verb|if (triangulation(...) && pworld[2]>0)|，即“返回 true 才接受”。这是 \texttt{slambook2} 仓库里一个长期存在的 bug（多个 issue/fork 指出）。\textbf{正确逻辑应当是：质量好 \texttt{return true}、质量差 \texttt{return false}}（把两个 \texttt{return} 对调）。此函数仅见于配套源码、教材正文未印，书面因而不会暴露这个坑——这恰恰说明：\emph{读懂一个系统，光看书里印出的片段不够，必须去读完整源码并亲自跑}。
\end{pitfall}
...
\end{enumerate}
这里的“可见 / 可观测帧数”依赖一个常被忽略的量：每个地图点记录自己的\emph{可观测金字塔尺度区间与平均观测方向}，投影时只有\emph{尺度和视角都匹配}才算“可见”——这让“被找到率”这个比值有意义，而非把所有掠过视野的帧都算成“本该看见”。
...
重定位拿到“当前帧 2D 点 — 候选关键帧地图点 3D”的一批对应后，要解 PnP 求位姿。迭代 PnP（如 \cref{sec:sys-poseonly} 的 pose-only BA）需要初值、可能陷局部极小；而\textbf{EPnP}\cite{murartal2017orbslam2} 给出一个\emph{无须初值、复杂度随点数线性}的闭式解，恰好适合“刚丢失、手里没有任何位姿先验”的重定位场景。它的核心是一招坐标技巧（下文）。
\end{insight}
...
\end{equation}
把 $n\ge 4$ 个点对的方程堆成 $\mathbf{M}\mathbf{x}=\mathbf{0}$（$\mathbf{x}\in\mathbb{R}^{12}$ 装 $4$ 个控制点），对 $\mathbf{M}^\top\mathbf{M}$ 做 SVD、取零空间即得控制点的相机系坐标；再由控制点恢复 $\mathbf{R},\mathbf{t}$。\textbf{全程线性、无须初值、代价 $O(n)$}——这就是它适合重定位的根由（推导依程小六《视觉惯性 SLAM》§11.3.4，式 11-16$\sim$11-22）。
