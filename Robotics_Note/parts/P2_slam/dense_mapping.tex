...
GPOM/GPIS/Hilbert 都是\emph{无学习}的连续函数——核与特征是人定的。一个自然的下一步是\emph{让数据学这个函数}：用一个神经网络（MLP）去逼近 SDF 或占据场，DeepSDF\cite{park2019deepsdf} 一类工作即直接由 3D 坐标回归符号距离，得到连续、紧凑、拓扑灵活的隐式曲面（仍可用 Marching Cubes 提网格）。把它搬进 SLAM，便有了在线训练、边估位姿边建图的\emph{神经隐式 SDF}（如 iMAP\cite{sucar2021imap}）。但这类方法的训练信号来自一个更强的范式——\emph{可微渲染}：让 2D 像素的梯度回流去优化 3D 表示。这恰是下一节的主题，也是本章“经典稠密表示 $\to$ 可微渲染地图”的收束。
...
iMAP\cite{sucar2021imap} 是首作——纯 MLP 地图，靠 MLP 的\emph{平滑先验}天然\emph{补洞与压缩}。随后经典与神经表示融合：NICE-SLAM\cite{zhu2022niceslam} 首次用\emph{多分辨率体素特征格 $+$ 小 MLP}的混合表示，NeRF-SLAM\cite{rosinol2023nerfslam} 借 DROID-SLAM 的深度加速重建。这里有一组关键\emph{取舍}（对地图表示的选择影响最大）：
