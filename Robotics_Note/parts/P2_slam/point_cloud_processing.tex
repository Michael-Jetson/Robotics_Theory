...
\begin{insight}
平面拟合代码取 \texttt{matrixV().col(3)}（最小、可只算 \texttt{ThinV}），直线拟合取 \texttt{matrixV().col(0)}（最大、需 \texttt{FullV}）——“取首列还是末列”这一行之差，正是“最大方向 vs 最小方向”的全部分别。把它推广到 $N$ 维：拟合 $N\!-\!1$ 维超平面取最小奇异向量、拟合 $1$ 维直线取最大奇异向量；更一般地，丢掉小奇异值的方向即\emph{数据降维}（在不同语境又叫主成分分析、低秩近似、子空间的零空间/值域构造）。一把 SVD，串起本章法向估计、GICP 协方差、LOAM 特征判别与降维，全是“看奇异值谱”的同一件事。
\end{insight}
...
% ════════════════════════════════════════════════════════════════
\section{在 $\SE(3)$ 上跟踪一个已知点：流形估计最小范式}
...
\subsection{单帧位姿估计 = $\SE(3)$ 上的高斯--牛顿}
...
\subsection{递推情形：$\SE(3)$ 上的 EKF 五式}
