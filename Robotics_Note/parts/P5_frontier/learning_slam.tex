...
\end{equation}
它一次性算出“$I_1$ 每点与 $I_2$ 每点的视觉相似度”（实现上是一次 GPU 友好的矩阵乘）。再对其后两维反复平均池化，堆成多分辨率\emph{相关金字塔} $\{\mathbf{C}^{\mathrm{corr},0},\dots,\mathbf{C}^{\mathrm{corr},3}\}$，兼顾小位移的细节与大位移的概览。

\emph{（2）查找算子（lookup）。}\;给定\emph{当前}光流估计 $\mathbf{f}^{\mathrm{flow}}$，对 $I_1$ 中像素 $(u,v)$ 算出其在 $I_2$ 的估计对应 $(u',v')=(u,v)+\mathbf{f}^{\mathrm{flow}}(u,v)$，在其邻域里从相关金字塔各层\emph{采样}相关特征、拼接成一个向量——这是“当前流估计下，邻域的视觉证据长什么样”。
...
\subsection{DROID-SLAM：迭代更新算子 $+$ 展开式 BA}
...
\subsection{网络与训练：Siamese ViT $+$ 置信加权回归}
...
\paragraph{为何要匹配头。}
DUSt3R 也能从点图里找对应（在某不变特征空间里找互为最近邻的点），但因网络\emph{从未}专门为匹配训练、且回归天生带噪，这种对应\emph{不够精确}\cite{leroy2024mast3r}。MASt3R 因此给 DUSt3R \emph{添一个匹配头}：再输出两张稠密\emph{局部描述子}图 $\mathbf{D}^1,\mathbf{D}^2$（各像素一个单位范数描述子），用\emph{InfoNCE} 对比损失训练——鼓励每个描述子只与另一图中“同一三维点”的那个描述子匹配（本质是逐像素的交叉熵分类，比回归更追求\emph{高精度}匹配）。总损失为置信回归损失（\cref{eq:learn-dust3r-loss}）加匹配损失之和。

\paragraph{从两视先验到完整系统。}
有了快速、精确的两视重建与匹配，就能搭完整的 SfM/SLAM。本书指明几条代表路线、不逐一复述其工程管线\cite{leroy2024mast3r}：\emph{MASt3R-SfM} 把冻结编码器复用为快速图像检索（近线性复杂度），并借 MASt3R 对外点的鲁棒性\emph{彻底去掉 RANSAC}，分两步梯度下降（先三维匹配损失、后二维重投影损失）求解；\emph{MUSt3R} 改成对称架构 $+$ 工作记忆，让所有图的点图直接落在\emph{同一}坐标系、免去成对的全局后处理，可高帧率地兼做离线 SfM 与在线稠密 VO/SLAM；\emph{MASt3R-SLAM} 则把两视先验当作跟踪、建图、重定位的统一基座——前端对点图做局部滤波、后端做大规模全局优化（沿用本章 \cref{sec:learning-diffba} 的滤波/优化思想），得到一个\emph{免相机模型}、可处理时变内参的实时稠密单目 SLAM。可以看到，越往系统走，越是“网络两视先验”与“因子图全局优化”的再度联手。
