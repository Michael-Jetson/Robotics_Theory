...
\begin{itemize}
  ...
  \item \textbf{同时跟踪与重建}。要重建\emph{任意}运动物体（无类别先验），常作刚体假设，为每个物体\emph{与}背景各建一个稠密模型，按“检测 $\to$ 对背景跟相机 $\to$ 对当前帧跟物体 $\to$ 把测量融进各自模型”四步走。物体多用 \textbf{TSDF}（\cref{sec:dense-tsdf}）表示，因其善于融合带噪深度——这正是 \cref{sec:intro-dynamic} 预告的 \emph{Co-Fusion}\cite{runz2017cofusion}：为背景与每个独立运动物各维护一个 TSDF、分别跟踪与融合。
\end{itemize}
