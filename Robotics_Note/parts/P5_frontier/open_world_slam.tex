...
\begin{itemize}
  ...
  \item \textbf{学习层接母方程}（\cref{ch:learning} 的 \cref{tab:learn-spectrum}）：上上一章立的统一立场——“学习不推翻 MAP 母方程（\cref{eq:intro-nls}），只换其零件”。本章把这一立场推到\emph{状态表示}与\emph{语义层}：地图元素的几何 $\mathbf{p}_k$ 仍由前面各章的 SLAM 后端解出，本章只往上嵌一层\emph{特征}。几何与语义\emph{解耦}，正是开放词汇建图能站在经典 SLAM 肩上的前提。
...
开放词汇建图的全部魔力，来自一类被称为\textbf{基础模型}（foundation model）的机器学习模型——在\emph{足够庞大}的数据集上训练、因而能合理期望它泛化到广泛工况的模型\cite{paull2026openworld}。完整的深度学习发展史（AlexNet $\to$ ResNet $\to$ BERT/Transformer $\to$ 对比学习 $\to$ ViT $\to$ DINO $\to$ CLIP）有无数专著可循，本书不复述；本节只取读懂后文所必需的\emph{三块支点}：(1) 什么是特征、为什么余弦相似度能度量“像不像”；(2) CLIP 如何把\emph{图像与文本}对齐到同一空间（后文一切开放词汇查询的支点）；(3) 类无关分割 SAM（后文从图像抠出物体的公共底座）。其余概念（ViT、对比学习、LLM 的训练细节）点到为止，需要深究者请查阅深度学习专著\cite{goodfellow2016deeplearning}。
...
\begin{enumerate}
  ...
  \item \textbf{（选读）地图 API 与工具调用。}\;设计三个你认为最有用的符号化地图 API 函数（除正文给的 \texttt{exists/where\_is/distance} 外），各写出输入输出，并说明 LLM 在生成一个机器人计划时会\emph{如何交错}调用它们与机器人动作 API。再说明“地图 API”相比“把地图写成文本塞进提示”，在传递\emph{几何}信息上的优势。
