...
在计算机里对 $3\times3$ 矩阵做通用矩阵求逆既耗时又易引入数值误差，而旋转矩阵的逆\emph{恰好就是转置}\cite{unitree2023quadruped}。代码里凡需要 ${}^{\mathcal{B}}\mathbf{R}_{\mathcal{A}}$，一律写 \texttt{R.transpose()} 而非 \texttt{R.inverse()}。这是 \cref{eq:qk-R-orth} 在工程上的直接红线。
\end{pitfall}
...
\subsection{角速度张量：$\dot{\mathbf{R}}\mathbf{R}^\top=\boldsymbol{\omega}^\wedge$ 的严格推导}
...
\subsection{静力学对偶：$\boldsymbol{\tau}=\mathbf{J}^\top\mathbf{F}$}
...
\subsection{Moore--Penrose 伪逆 $=$ 最小范数解}
...
\subsection{机身姿态 $\leftrightarrow$ 足端位置}
