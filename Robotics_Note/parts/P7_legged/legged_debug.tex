...
源项目里以 \texttt{phase}/\texttt{P3-NNN} 标号的过程编号在此一律省略，只保留可迁移的结论。
\end{note}
...
\begin{description}
  ...
  \item[第二层 — MPC 自己的解可行吗] 查 MPC 的 \texttt{getPerformanceIndices()} 六字段：\texttt{dynamicsViolationSSE}、\texttt{equalityConstraintsSSE}、\texttt{inequalityConstraintsSSE}、\texttt{cost}、\texttt{merit}、\texttt{Lagrangian}。它回答“MPC 给出的这条最优轨迹，在它自己的模型里自洽吗”。配合保留“恶化前一拍”（\texttt{*\_pre\_failure}）的快照，可定位\emph{恶化起点}。
...
层 & 问题 & 客观证据 & 报警含义 \\
\midrule
① WBC QP & 此刻解出来了吗 & \texttt{lastSolveSucceeded()}/返回码 & 当前拍执行层不可行 \\
② MPC 解 & 这条解自洽吗 & \texttt{getPerformanceIndices()} 六字段（尤其 \texttt{inequalitySSE}）& 最优解本身违约/恶化 \\
③ 预测窗口 & 计划里就要摔吗 & \texttt{evaluatePolicy($t{+}\Delta$)} 看单调发散 & 规划本身把自己带翻 \\
\midrule
...
\subsection{文献交叉验证：把证伪结论独立坐实}
\label{subsec:ld-ol-lit}

源项目的一条纪律是：纯靠读源码/调实验得到的结论，要去\emph{独立}文献里找几何/理论下界来交叉验证，而不是只信自己的实验。over-launch 的结论恰好能被一组关于质心角动量率下界的结果坐实\cite{quad2025angmombound,cam2025flight}。

\begin{insight}[摩擦锥对质心角动量率有几何下界——“调权重修不了”是结构性的]\label{ins:ld-angmom-bound}
对 $N=2$ 支撑（对角小跑），摩擦锥对质心角动量率 $\|\dot{\boldsymbol H}_G\|/m$ 沿“两脚连线方向”存在一个\emph{几何下界}，且“与权重整定无关”（independent of weight tuning）\cite{quad2025angmombound}。这从\emph{外部}独立坐实了表中“十余轮调权重修不了”是\emph{结构性}的，而非没调对。配套还有两条可迁移结论：其一，存在临界水平加速度 $a_x^\star=\mu g/(1+2\mu\kappa)$（$\kappa$ 仅由足几何定），超过它摩擦锥连几何地板都够不到——\emph{更重的机器人阈值更低、更易超}；其二，$N\ge 3$（满秩接触）时该障碍\emph{消失}，实测 $\|\dot{\boldsymbol H}_G\|/m$ 比 $N=2$ 降约 $227\times$。\textbf{$\Rightarrow$ 真正的杠杆是“爬升时多脚同时接触（$N\ge 3$）”}，但它只在“没被弹离、还留着支撑”时才兑现——这解释了“切静走仍失败”：静走只动了调节层、没动\emph{触发层}的 base-z 抬升，触发层不治则脚先被弹离，$N\ge 3$ 兑不了现。质心动量/角动量的物理见 \cref{sec:srbd}。
\end{insight}

\begin{insight}[注入角动量参考是徒劳的——砍掉一整条分支]\label{ins:ld-angmom-futile}
同一组结果还给出一条关键\emph{负}结论：给 WBC 加“注入角动量/动量参考”是\emph{徒劳}（futile）——它只让解沿任务参考滑离摆式流形，\emph{并不能逃出}摩擦锥下界\cite{quad2025angmombound}。这从理论上\emph{砍掉}了“注入 $\dot{\boldsymbol H}_G$ 参考”这条曾被设想的 over-launch 修复分支。而 over-launch 本身在文献里有名——它是\emph{飞行相质心角动量（CAM）失稳}\cite{cam2025flight}：飞行相姿态守恒，姿态只能在支撑相改，飞起来就只能等落地（呼应 Raibert：flight 守恒、attitude 只 stance 可控）。\textbf{方法论}：一个负结论（“某条修法理论上不可能奏效”）和一个正结论同样宝贵——它让你\emph{不去}浪费时间在注定失败的方向上。
\end{insight}
...
分布式/容器化栈里，“我改了代码”与“运行的是改后代码”之间隔着许多静默失效点：\texttt{devel/lib/*.so} 常是\emph{符号链接}（看链接本身显示的是旧时间，要看链接\emph{目标}的 mtime）；编译“succeeded”不等于 \texttt{.so} 真更新（空编译几秒、零符号；真编译十几到几十秒）；改了类成员 = ABI 变 $\to$ 须重编全链。\textbf{规范}：\emph{内容核查胜过时间核查}——用 \texttt{strings .so | grep <环境标志>}、\texttt{nm -C .so | grep <符号>}、跨机 \texttt{md5sum} 一致性来确认“跑的真是最新”，而非信文件时间戳。
\end{insight}
...
$\dot{\boldsymbol H}_G$ 下界 & 摩擦锥几何下界与权重无关；注入角动量徒劳（\cref{ins:ld-angmom-bound,ins:ld-angmom-futile}）\\
...
\begin{enumerate}
  ...
  \item \textbf{查被置零的反馈/约束}：\texttt{kp=kd=0}/\texttt{enabled=false} 视为高优先嫌疑（\cref{ins:ld-zero-suspect}）。
...
11 & 角动量率下界 & 摩擦锥几何下界与权重无关；注入徒劳 & \(\bigstar\bigstar\bigstar\) \\
...
\section*{延伸阅读}
\begin{itemize}
  \item \textbf{角动量率几何下界与飞行相 CAM 失稳}：四足质心角动量率的摩擦锥几何下界、临界水平加速度 $a_x^\star=\mu g/(1+2\mu\kappa)$、$N\ge 3$ 降约 $227\times$、注入角动量参考徒劳\cite{quad2025angmombound}；over-launch 作为飞行相质心角动量失稳\cite{cam2025flight}。本章 \cref{sec:ld-overlaunch} 的证伪结论由它们独立坐实。质心动力学本身见 \cref{sec:srbd}、\cite{orin2013centroidal}。
