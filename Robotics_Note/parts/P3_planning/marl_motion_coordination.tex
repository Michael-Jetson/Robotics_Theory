...
%  记号归一：R∈SO(3)、T∈SE(3)（preamble 宏 \SO \SE \so \se）；MARL/多机自然记号保留。
...
2018 & Everett, Chen, How（GA3C-CADRL） & IROS & 多机社会化导航：LSTM 处理变长邻居集合 \rebuilt{原稿三处误：(1)“注意力”错——GA3C-CADRL 用 LSTM 而非 attention（attention 属更晚的 SARL, Chen 2019）；(2)“社会礼貌编入奖励”张冠李戴——该特性属前作 SA-CADRL（Chen, Everett, Liu, How, IROS 2017）, GA3C-CADRL 反而刻意移除手工社会范式、让合规行为隐式涌现；(3)期刊“IJRR”虚构——实为 IEEE Access 2021 (arXiv:1910.11689)。已删 IJRR、删 attention/社会礼貌断言} \\
2024 & Xiong 等（MQE） & IROS & 多四足 MARL 标准基准，GPU 大规模并行 \pz{存疑：论文/作者/会议(IROS 2024)/引擎(IsaacGym)属实，但原稿任务名 cooperative\_transport/formation/chase 系编造——repo 实际任务为 go1gate/go1seesaw/go1sheep/go1pushbox/go1football 等 12 项；已删伪造任务名} \\
2024 & An, Hutter 等 & arXiv & 排列不变网络：多机协作策略天然对队友排列不变，零样本可扩展（训练 $\le 4$、零样本测 $1$--$10$） \rebuilt{原稿“RSS 2024”错——该文(arXiv:2402.18345)仅 arXiv 预印本、不在 RSS 2024 录用名单（Hutter 另有一篇 RSS 2024 对抗攻击论文, 疑似混淆来源）；已改出处为 arXiv、并把“训 $N{=}2$、部署 $N{=}10$”更正为“训 $\le 4$、零样本 $1$--$10$”} \\
2025 & Pandit, Shrestha, Fern 等（decPLM） & arXiv & $N$ 台四足-臂\emph{零通信}协同搬运（pinch-lift-move），分层策略 $+$ constellation 奖励，仅靠接触力协调，真机迁移 \\
2025 & An, Hutter 等 & RA-L & 可扩展多机协作完成多目标任务（RL）\rebuilt{原稿标题“可扩展多足协调”系编造——RA-L 2025 续作真实标题为 “Scalable Multi-Robot Cooperation for Multi-Goal Tasks Using Reinforcement Learning” (RA-L 10(2), 2025)，主题是多目标协作/导航而非“多足协调”；已订正标题方向} \\
\bottomrule
\end{tabular}
\end{table}
...
把“多个决策者在共享环境里序贯交互”形式化的源头是 1994 年 Littman 的\emph{马尔可夫博弈}\cite{littman1994markov}（\cref{ch:plan-safemarl} 已讲），但它默认每个智能体能观测全局状态，对机器人不成立。更贴合机器人现实的是 Bernstein 等 2002 年形式化的 \emph{Dec-POMDP}\cite{bernstein2002decpomdp}——在 Markov Game 上加“部分可观测”，每个智能体只能通过自己的观测函数看到状态的一部分。Dec-POMDP 一般性极强但极难（最优求解 NEXP-完全，比 NP 还难）；好在我们不追求精确最优，而用 MARL（CTDE/MAPPO）在仿真里\emph{近似}地学一个好策略。到了机器人领域，Long/Fan/Pan 等的去中心化避障工作\cite{long2018collision}（ICRA 2018, arXiv:1709.10082）是关键一步：第一次令人信服地证明，\textbf{一个完全去中心化、只用本地观测、无需通信的避障策略，可端到端学出来并在真实多机器人上跑}，把 Dec-POMDP 从理论框架变成可落地的机器人方案。\pz{存疑（已校）：该 keyed 引用确为 ICRA 2018, 归属 Long/Fan/Pan 及“去中心化/无通信/激光端到端/真机”断言均属实；原 Tutorial 源稿曾误记“2017 RSS”（系与该组 2017 RA-L 模仿学习论文 arXiv:1609.06838 混淆 $+$ 虚构 RSS 会议），本章统一记为 ICRA 2018。另“邻居相对状态”略不精确：邻居经激光感知, 非显式相对状态向量}
...
这段代码把 Dec-POMDP 每个元素都对应到一个方法：\verb|_get_obs_i| 是观测函数 $O_i$，\verb|step| 里的动力学是转移 $P$，\verb|_compute_rewards| 是奖励 $r_i$，\verb|global_state| 是仅训练可用的全局状态 $s$。\textbf{请特别注意 \texttt{\_get\_obs\_i} 返回的 \texttt{neighbors} 是变长、无序的列表}——这一个细节，就是整个 \cref{sec:mc-perminv} 存在的理由：策略必须能吃下“长度不定、顺序无关”的邻居集合。

\begin{pitfall}[编程陷阱：执行时偷偷在 actor 里调用了 \texttt{global\_state()}]
\textbf{错误做法}：图省事给 actor 也喂全局状态（“反正环境里有这个方法”），训练曲线很漂亮、成功率很高。\textbf{现象}：仿真里完美，一上真机（或一旦没有中央通信）策略直接失效——真机上每个机器人拿不到 \verb|global_state()|，actor 输入分布和训练时完全不同，输出垃圾动作。\textbf{根本原因}：违反 CTDE 铁律——critic 可用全局状态，actor 绝不可以。CTDE 的全部价值就在“训练作弊、执行自立”，actor 用了全局就把“执行自立”丢了，退化成不可部署的联合方案。\textbf{正确做法}：用代码强制隔离——actor 的 \verb|forward| 只接收 $o_i$，critic 的 \verb|forward| 才接收 $s$，让“actor 用全局”成为类型层面的错误；可在部署模式下让 \verb|global_state()| 抛异常，任何 actor 路径触发立刻暴露。
\end{pitfall}
...
\begin{enumerate}
  ...
  \item （改写题）把 \verb|MultiRobotCoordEnv._get_obs_i| 改写为同时支持“障碍观测”：环境新增 \verb|self.obstacles|（一组 $(x,y,r)$），观测里加入“半径内最近 $K$ 个障碍的相对位置”。思考：障碍数也是变长的，你打算怎么处理这个变长集合？（为 \cref{sec:mc-perminv} 埋伏笔。）
...
\paragraph{反面：固定槽位拼接。}
最朴素、新手几乎必然第一次会写的办法：给每个队友分配固定槽位，按编号拼进观测向量（\verb|obs_i = concat([self, nbr_1, nbr_2, nbr_3])| 喂给普通 MLP）。它在双机、且数量永不变时勉强能用，但有三个层层递进的致命缺陷：
...
处理“变长邻居集合”，机器人 MARL 走过一条清晰的进化路线。\emph{早期（2017--2018，硬扛阶段）}：把邻居先按某种规则排序（如按距离从近到远），再用 LSTM/RNN 顺序“读”进去；Everett/Chen/How 的 GA3C-CADRL 用 LSTM 处理任意数量的邻居，就是代表\cite{everett2018cadrl}。% \rebuilt 待修：原稿此处把“用注意力对邻居加权求和”归给 GA3C-CADRL——错误，GA3C-CADRL 仅用 LSTM；attention-for-crowd-nav 属更晚的 SARL(Chen 2019) 等不同工作。已删去“注意力”分支，勿在此重建。
这能处理变长，但排序仍在往集合里强加顺序，且 RNN 的顺序处理引入了与物理无关的“先后”偏置。\rebuilt{见上：GA3C-CADRL 用 LSTM 非注意力；attention 聚合的归属另属他文}\emph{中期（Deep Sets，2017，理论根除）}：Zaheer 等给出漂亮的表示定理\cite{zaheer2017deepsets}——\textbf{任何对集合的排列不变函数，都可写成“对每个元素先做同一变换、再做排列不变聚合（如求和）、最后再做一个变换”的形式}，从根本上告诉我们处理集合不需排序、不需 RNN。\emph{近期（2024--2025，机器人多机协调主流）}：An、Hutter 等把排列不变网络确立为多机协作策略可扩展到任意 $N$ 的关键技术\cite{an2024perminv}——训练时至多 $4$ 个实体、零样本测到 $1$--$10$ 个，正是靠排列不变。\pz{存疑（已校）：an2024perminv(arXiv:2402.18345) 概念归属属实, 但原稿“训 $N{=}2$、部署 $N{=}10$”数值不准，实为训 $\le 4$、零样本 $1$--$10$；且该文为 arXiv 预印本非 RSS 2024}
...
\pz{存疑（陈述放宽，归属属实）：归属正确——Deep Sets, Zaheer 等, NIPS/NeurIPS 2017 (arXiv:1703.06114) 的 Theorem 2 即此 iff 分解。但两处不严谨：(1) 原文 iff 仅对\emph{求和} $\sum$ 证明；mean/max 在原文只作等变层/实践变体, 不在该不变性刻画定理内, 故把 $\bigoplus$ 写成“求和/平均/取最大”并列暗示三者皆满足 iff 过强；(2) 原文 iff 限\emph{可数域}, 连续域只对固定集合大小成立——Wagstaff 等 2019(arXiv:1901.09006)证连续域下潜维须 $\ge$ 最大集合基数否则有反例。教学保留此实用形式, 严格条件如上}
\end{theorem}
...
排列不变解决了“队友无序变长”，但还有一个同样重要、却容易被忽略的设计：\textbf{观测必须用本地坐标系（egocentric/相对坐标）表达，而非全局坐标。} 回看 \cref{sec:mc-decpomdp-code} 的 \verb|_get_obs_i|：队友用 \verb|self.pos[j] - self.pos[i]|（相对位置），到目标用 \verb|self.goal[i] - self.pos[i]|（相对向量）——全是“相对于我自己”的。这不是随手写的，而是和排列不变同等重要的可扩展性支柱：
...
\begin{itemize}
  ...
  \item \textbf{三件套合体}：\textbf{参数共享（所有机器人同一个 $\pi_\theta$）$+$ 排列不变（对队友集合不变）$+$ 本地坐标（相对表达）}，三者共同支撑“训练时少、部署时多，零样本可扩展”这个多机 MARL 最诱人的性质（本章下文以训 $N{=}2$、部署 $N{=}4/8$ 作教学算例）。缺任何一个，可扩展性都会打折\cite{an2024perminv}。\pz{存疑（已校）：此处 $N{=}2\to N{=}10$ 原作论文数值不符——an2024perminv 实为训练时至多 $4$ 个实体、零样本测 $1$--$10$；正文已改为不绑定具体数的表述, 教学算例数值（训 2/部署 4/8）见下文代码、属本书示例非论文复现}
\end{itemize}
...
\begin{practice}
\begin{enumerate}
  \item （实现题）把 \verb|PermInvariantPolicy| 接到 \cref{sec:mc-decpomdp-code} 的 \verb|MultiRobotCoordEnv| 上：写一个函数把 \verb|_get_obs_i| 返回的字典转换成 \verb|(self_obs, neighbor_obs, neighbor_mask)| 三个张量（处理变长邻居的 padding 和 mask）。构造一批假数据，验证关键性质：\textbf{对调任意两个邻居的顺序，网络输出必须不变}（\verb|torch.allclose|）。
  \item （调试挑战）下面聚合代码在邻居数恒定时正常、变化时性能下降，请定位并修复：\verb|feat=(self.neighbor_encoder(obs)*mask.unsqueeze(-1)); return feat.mean(dim=1)|。提示：问题出在 mean 的分母（应是真实邻居数）。
...
\begin{enumerate}
  ...
  \item （实现题）把 \verb|VelocityActionHead| 接到 \verb|PermInvariantPolicy| 后面（替换原直接输出 \verb|act_dim| 的 \verb|action_head|），构成完整的“排列不变观测 $\to$ 速度动作”策略。再加入动作平滑：修改采样逻辑使其接受上一步动作 \verb|prev_action|，并返回用于奖励的“动作变化量” $\|a_t-a_{t-1}\|^2$（供 \cref{sec:mc-reward} 的平滑惩罚使用）。
...
\begin{enumerate}
  ...
  \item （实现题）给 \verb|CentralizedCritic| 增加第四种 mode \verb|"state_plus_privileged"|：输入 $=$ 真值全局状态 $+$ 特权信息（每个 agent 真实目标 $g_j$、负载真实质量）。写出 \verb|forward|，说明哪些信息属于“actor 也有”、哪些属于“特权（仅 critic 有）”。回答：为什么把特权信息只给 critic 而不给 actor，能让训练既稳又保持 actor 可部署？
...
\begin{enumerate}
  ...
  \item （实现题）把 \verb|cbf_safety_filter| 接到 \cref{sec:mc-perminv-code}$+$\cref{sec:mc-action-code} 构成的完整策略后面，形成“排列不变观测 $\to$ 速度动作 $\to$ CBF 安全过滤 $\to$ 执行”的完整管线。再做一个实验设计（不需真跑，描述即可）：如何量化“安全层介入的频率”和“安全层对任务性能的影响”？提示：记录 $\|a^{\text{safe}}-a^{\mathrm{RL}}\|$ 的分布、对比有/无安全层的碰撞率和到达时间。
...
MQE（Multi-agent Quadruped Environment）是多四足 MARL 的标准基准，基于 IsaacGym 的 GPU 大规模并行仿真\cite{xiong2024mqe}，设计直接对应本章的建模：机器人是多 agent（$\leftarrow$ 对应 $N$），含一组协作/竞争任务（$\leftarrow$ 三大应用），观测是本体感知 $+$ 邻居相对位置/状态 $+$ 任务特征（$\leftarrow O_i$），动作是关节位置偏移或经底层控制器的速度层（$\leftarrow\mathcal{A}_i$），奖励是任务 reward $+$ 个体 alive reward $+$ 组惩罚（$\leftarrow$ 混合奖励）。关键工程特性：多 agent 协作/竞争任务、IsaacGym GPU 并行（万级 fps、$4096$ 环境 $\times N$ agent 同时模拟）、自定义地形生成（为 sim-to-real 域随机化铺路）。\pz{存疑（已删伪造细节）：MQE(arXiv:2403.16015, IROS 2024) 真实存在, 基于 IsaacGym/万级 fps 属实。但原稿两处编造已删：(1) 任务名 cooperative\_transport/formation/chase 不存在——实际 12 任务为 go1gate/go1seesaw/go1sheep/go1pushbox/go1football-defender（协作）与 go1tug/go1revolvingdoor/go1bridge/go1bridgewrestling/go1football-1vs1/2vs2（竞争）；(2)“支持 $2/4/8$ 灵活切换”不实——论文以 $2$ agent 为主、最多 $4$（Football 2v2）, 无 $8$-agent 配置（repo 约 141 star, 非原稿“200”）}
...
\begin{practice}
\begin{enumerate}
  \item （实现题，核心）补全 \verb|MAPPOTrainer.update| 的完整实现：(a) 用 critic 对 \verb|buf["state"]| 估值，调 \verb|compute_gae| 得到每个 agent 的优势；(b) 把所有 agent 的 \verb|(obs, act, old_logp, advantage, return)| 沿 agent 维度合并成大 batch；(c) 写出 PPO 裁剪损失（actor）$+$ critic MSE $+$ 熵正则，做若干 epoch 的 minibatch 更新。在 \verb|MultiRobotCoordEnv(n_robots=2)| 上跑通，画出成功率曲线。
...
\noindent 多机 sim-to-real 的成败，很大程度上取决于“额外”那几行（负载、负载形状、接触力、通信延迟、数量）随机化得够不够。\pz{说明（经验值, 非论文精确数）：\cref{tab:mc-dr} 各区间为示例性工程默认, 六类轴均为文献真实使用的随机化维度且数量级合理（质量 $\pm20\%$、电机延迟 $[5,30]$ms、负载、通信延迟 $[0,100]$ms、训练 $N\in[2,4]$ 等均有据），但具体数值不可由单一来源精确复现。一处宜注意：摩擦上界这里取 $1.0$, 而主流 legged\_gym/walk-these-ways 常用 $[0.5,1.25]$, 实践可参后者}
