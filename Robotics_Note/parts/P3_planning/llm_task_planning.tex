...
%  记号归一：R∈SO(3)、T∈SE(3)（preamble 宏 \SO \SE）；规划/LLM 自然记号保留。
...
\rebuilt{本章重渲染自一份 Tutorial 笔记（任务运动规划线“大模型任务规划”），源稿由 AI 撰写。已经一轮联网复核：论文归属、年份、会议绝大多数核实无误（详见 claims 台账）；\textbf{PlanBench 的 $\approx 3\%$ 经核实为伪数值（应为常识域约 $34\%$、全域约 $12\%$），已就地删除并 \texttt{\textbackslash rebuilt} 标注}；Guan 的“$40$ 余动作 / $48$ 任务”经核实\emph{属实}（与论文摘要一致）。其余存疑/待核处已就地 \texttt{\textbackslash pz}/\texttt{\textbackslash rebuilt} 标注。}
