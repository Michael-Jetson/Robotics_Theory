...
\begin{itemize}
  ...
  \item \textbf{分层混合一侧}：% Bjelonic 等 —— 作者归属错误：bjelonic2023mpcrl 的真实论文为 arXiv:2307.07752《Combining model-predictive control and predictive reinforcement learning for stable quadrupedal robot locomotion》，作者是 Kovalev, Shkromada, Ouerdane, Osinenko（Skoltech），并非 Bjelonic；Marko Bjelonic（ETH/ANYmal）仅出现在其参考文献中，系貌似可信的张冠李戴。
  \rebuilt{“Bjelonic 等”作者归属已注释，错因=幻觉作者；正确方向=改为 Kovalev 等并将 bib 标题/作者/eprint 校正为 2307.07752，建议键改名为 kovalev2023mpcrl，勿径直重建。}\emph{Combining MPC and Predictive RL for Quadruped Locomotion}\cite{bjelonic2023mpcrl}（2023，\cref{sec:mh-interface} “RL 出尾代价”接口的代表作）；RoCo\cite{mandi2024roco}（ICRA 2024，混合范式向“LLM 高层”延伸）。
