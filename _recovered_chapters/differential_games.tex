...
\node[blk] (game) {博弈 = 最优控制\\ +“对手也在最优”};
\node[grp, right=of game] (isaacs) {Isaacs / HJI\\ $\min_u\max_v$、鞍点};
\node[grp, right=of isaacs] (visc) {黏性解\\ 消失黏性、唯一性};
\node[blk, below=7mm of game] (reach) {可达性\\ BRS / BRT / reach--avoid};
\node[adv, right=of reach] (numer) {水平集数值\\ WENO5 + TVD-RK3};
\node[adv, right=of numer] (curse) {维度诅咒\\ 解耦·FaSTrack·DeepReach};
\node[blk, below=7mm of reach, fill=second!6, draw=second!60!black] (lq) {LQ 博弈\\ 耦合 Riccati};
\node[adv, right=of lq] (chase) {追逃经典\\ Chauffeur · Air3D};
\draw[ln] (game)--(isaacs); \draw[ln] (isaacs)--(visc);
\draw[ln] (isaacs.south) -- ++(0,-3.5mm) -| (reach.north);
\draw[ln] (reach)--(numer); \draw[ln] (numer)--(curse);
\draw[ln] (numer.south) -- (lq.north|-numer.south) -- (lq.north);
\draw[ln] (lq)--(chase);
\end{tikzpicture}
\end{center}
...
\texttt{hj.step(..., 0.0, ..., -T)} & 从终端沿时间负方向积分（TVD-RK 内部保证 CFL） \\
