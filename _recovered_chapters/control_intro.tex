...
学完本章，你应当能够：
\begin{enumerate}
  \item \textbf{说清}反馈如何抑制扰动与稳态误差，并独立\textbf{证明}积分作用消除稳态误差（不假设线性）；
  \item \textbf{陈述}并\textbf{运用} Lyapunov 稳定/渐近稳定/指数稳定三层定义、Lyapunov 直接法与 LaSalle 不变原理，判定一个平衡点的稳定性；
  \item \textbf{推导} PID 三项的作用、滤波微分、抗积分饱和与数字实现，并用 Routh--Hurwitz 判 PID 闭环稳定；
  \item \textbf{判别}线性系统的可控性/可观性（秩判据、Gramian、PBH），理解可镇定/可检测的松弛；
  \item \textbf{独立推导} LQR：经动态规划（离散）与 HJB（连续）两条路得到 Riccati 方程与最优增益 $\mathbf{K}=\mathbf{R}^{-1}\mathbf{B}^\top\mathbf{P}$，并解释“最优 $\ne$ 稳定”为何需要无限时域；
  \item \textbf{表述} MPC 的滚动时域问题、凝聚为 QP，并\textbf{证明}其闭环渐近稳定性（终端代价 + 终端集 = 闭环 Lyapunov 函数）；
  \item \textbf{连接}机器人动力学与控制：计算力矩（反馈线性化）、PD+重力补偿（无源性 + LaSalle）、操作空间控制、全身控制 QP 与足式凸 MPC 的分层架构。
\end{enumerate}
...
\paragraph{动机：为什么要反馈。}
机器人要把温度、速度、关节角、位姿稳定在某个期望值，并在扰动（坡道、负载、风）下维持。最朴素的想法是\emph{开环}：算好需要的输入 $\mathbf{u}(t)$ 直接施加。可一旦模型不准或有扰动，开环就失准且无从纠正。\textbf{反馈}（闭环）控制把测得的输出 $y$ 与参考 $r$ 比较，用误差 $e=r-y$ 实时修正输入。这一节先讲反馈的基本结构与它为何能消除稳态误差，再建立判断闭环是否“稳定”的数学工具——稳定性是后面 LQR、MPC、机器人控制一切收敛性论证的公共地基。
...
\paragraph{动机与几何直觉。}
要判稳定，最直接是求解 $\dot{\mathbf{x}}=f(\mathbf{x})$ 看轨迹——但非线性系统通常无闭式解。Lyapunov 的天才在于：\emph{不解方程}，只找一个“广义能量”函数 $V(\mathbf{x})$，若沿轨迹这个能量不增（甚至递减），系统就不会远离平衡。物理直觉：一个有阻尼的机械系统，总能量随时间耗散，故最终停在最低能量点（平衡）。
...
接上控制器，置 $k_i=k_d=0$，逐步增大 $k_p$ 至闭环出现\emph{等幅振荡}，记此临界增益 $k_u$（亦记 $K_c$）与振荡周期 $T_u$（亦记 $T_c$）。由 Nyquist 判据，此时 $L=k_uP$ 在 $\omega_u=2\pi/T_u$ 恰过临界点 $-1$，即捕获了对象相位滞后 $180^\circ$ 的点。
...
\rebuilt{若 $(\mathbf{A},\mathbf{B})$ 可控，则状态反馈 $\mathbf{u}=-\mathbf{K}\mathbf{x}$ 可把闭环 $\dot{\mathbf{x}}=(\mathbf{A}-\mathbf{B}\mathbf{K})\mathbf{x}$ 的极点\emph{任意}配置（极点配置定理；可控性是充要条件）。} 对偶地，可观系统可设计观测器（增益 $\mathbf{L}$）任意配置误差动态 $(\mathbf{A}-\mathbf{L}\mathbf{C})$ 的极点。二者结合成输出反馈控制器，且\emph{分离原理}保证估计器与控制器可独立设计——这是 LQG（\cref{sec:ctrl-lqe-dual}）的基础。极点配置要人工选极点，LQR 则给出 $\mathbf{K}$ 的\emph{最优}选择，是下一节的主题。

\begin{pitfall}[🔬证明陷阱：把“可控”当成“能任意快地驱动”]
可控性只保证能在\emph{有限}时间到达任意状态，\textbf{不}保证能任意快或用有限大的输入到达。极点配置理论上能把极点配到任意远（任意快），但实际受执行器饱和与模型有效频率上限约束——配得越快，所需控制量越大、对模型误差越敏感。正确理解：可控性是\emph{定性}的“能否”，速度/代价是\emph{定量}问题，由 LQR 的 $\mathbf{Q},\mathbf{R}$ 权衡（\cref{sec:ctrl-lqr}）。
\end{pitfall}
...
极点配置要人工选极点，但“极点放哪最好”往往不清楚。LQR 换个问法：给定一个\emph{二次代价}（同时罚状态偏差与控制能量），求使代价最小的反馈律。答案是一个优雅的线性反馈 $\mathbf{u}=-\mathbf{K}\mathbf{x}$，其中 $\mathbf{K}$ 由一个矩阵方程——\textbf{Riccati 方程}——给出。LQR 是现代最优控制的基石，也是 MPC 的无约束无限时域特例，其 Riccati 与 Kalman 滤波的 Riccati 对偶。本节给\emph{两条}完整推导（离散动态规划、连续 HJB），并用 Kalman 的反例揭示“最优 $\ne$ 稳定”这一深刻教训。
...
\# & 知识点 & 核心要点 & 难度 & 脉络层 \\
\midrule
1 & 反馈 + 稳定性 & 灵敏度折中、积分消差、Lyapunov/LaSalle & \(\star\star\) & 骨干 \\
2 & PID & 三项作用、滤波微分、抗饱和、Routh & \(\star\star\) & 骨干 \\
3 & 状态空间 & 可控/可观秩判据、Gramian、PBH、对偶 & \(\star\star\star\) & 骨干 \\
4 & LQR / Riccati & DP/HJB 两推导、CARE/DARE、Kalman 反例 & \(\star\star\star\) & 骨干 \\
5 & MPC & 滚动时域、凝聚 QP、终端配料保稳定 & \(\star\star\star\star\) & 次优 \\
6 & 机器人控制 & 计算力矩、PD+重力补偿、操作空间、WBC & \(\star\star\star\) & 次优 \\
7 & 足式控制 & 分层架构、单刚体凸 MPC、摩擦锥 & \(\star\star\star\star\) & 前沿 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{最优控制与 LQR}：Tedrake《Underactuated Robotics》\cite{tedrake_underactuated}（在线，含 HJB$\to$Riccati 与轨迹 LQR）；Bansal LQR 讲义\cite{bansal2017lqr}（离散 DP 完整归纳证明）。
  ...
  \item \textbf{机器人控制}：Spong, Hutchinson \& Vidyasagar《Robot Modeling and Control》\cite{spong2006robot}（计算力矩、PD+重力补偿、无源性证）；Khatib 1987\cite{khatib1987operational}（操作空间公式）；Takegaki \& Arimoto 1981\cite{takegaki1981feedback}（PD+重力补偿里程碑）。
...
本章主要证明（积分消差、Lyapunov 直接法/LaSalle、Lyapunov 方程、可控 Gramian 双向、LQR 的 DP/HJB 推导、MPC 单调性与主定理、计算力矩与 PD+重力补偿）均已在正文相应定理处给出。此处补全 Joseph 形式与紧凑形式的等价代数细节。
