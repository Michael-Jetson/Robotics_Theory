...
\begin{enumerate}
  ...
  \item \textbf{换算}连续↔离散噪声参数，并用 Allan 方差辨识 IMU 噪声系数；
...
\end{equation}
可对实采曲线做最小二乘拟合提系数（$\sqrt{2\ln2/\pi}\approx0.664$）\rebuilt{各项系数随源约定，使用前以本台 IMU 数据复核}。
...
\end{equation}
直观：$\mathbf R_w^b$ 最后一列是世界 z 轴在体系的方向，世界 z 轴与重力对齐，故从 $\mathbf g^b$ 测量算出 $\mathbf z_w^b$、正交补全其余两列。\textbf{高端 IMU} 的陀螺能测地球自转 $\boldsymbol\omega_{ie}$，可用 $\mathbf g^b$ 与 $\boldsymbol\omega_{ie}^b$ 解析对准（堆叠 $\{\mathbf g,\boldsymbol\omega_{ie},\mathbf g\times\boldsymbol\omega_{ie}\}$ 在两系下由同一旋转关联，求逆得 $\mathbf R$，再投影回 SO(3)）。
...
\paragraph{动机：欧拉积分有误差。}
标准预积分用欧拉法（矩形法则）把信号近似为分段常值，双重积分迅速累积误差；采样频率低时尤甚\cite{carlone2026handbook}。改进方向有二。

\textbf{连续加速度预积分.}\;用连续时间表示更好近似真实惯性信号并做解析积分，还允许\emph{异步查询}预积分测量（对事件相机等非硬件同步传感器尤有用）。先解耦旋转与平移：把“旋转修正后的加速度计测量” $\hat{\mathbf a}_k=\Delta\mathbf R_{ik}\tilde{\mathbf a}_k$ 以连续方式表示。分段线性近似对应\emph{常 jerk（加加速度）运动模型}（第一重积分即梯形法则），比欧拉法显著增益；无模型的高斯过程（GP）回归用 $\hat{\mathbf a}\sim\mathcal{GP}(0,k_a(t,t')\mathbf I)$（平方指数核），对 GP 施加线性算子解析推断积分，不依赖显式运动模型。假设局部加速度常值的连续模型在 EuRoC 数据集上 VIO 精度比标准预积分改善约 $5\%$\cite{carlone2026handbook}。

\textbf{连续旋转预积分.}\;旋转空间积分难，因 SO(3) 非欧、群运算不交换——求解运动学的乘积积分 $\mathbf R(t{+}\Delta t)=\mathbf R(t)\prod_{\tau}\mathrm{Exp}(\boldsymbol\omega_b^b(\tau))^{d\tau}$ \emph{无通用闭式}\cite{carlone2026handbook}。一种思路用旋转向量 $\mathbf r(t)$（$\mathbf R=\mathrm{Exp}(\mathbf r)$）作为线性空间，动力学 $\dot{\mathbf r}=\mathbf J_r(\mathbf r)^{-1}\boldsymbol\omega_b^b$，用 GP 建模 $\dot{\mathbf r}$ 并以虚拟观测（控制点）插值，精度比标准离散预积分至少改善一个数量级。

\paragraph{扩展位姿与本体感知里程计.}
近期趋势用\emph{扩展位姿流形}（含位置/速度/姿态的“扩展位姿”，本质 $\mathrm{SE}_2(3)$ / 不变滤波视角）与高阶噪声传播改进不确定度建模，并考虑地球自转的科氏与离心力\rebuilt{$\mathrm{SE}_2(3)$ 预积分细节属不变滤波专题，本章未展开}。足式/轮式机器人可用本体感知（足地接触、轮编码器）做辅助惯性里程计；无视觉时纯惯性发散，近期用学习方法（神经网络建模偏置、直接预测位移、可微积分模块）减小漂移，但泛化能力仍有限\cite{carlone2026handbook}。
...
连续↔离散噪声 & \cref{eq:noise-white-disc}, \cref{eq:noise-rw-disc}, \cref{eq:kalibr-conv} \\
...
\# & 知识点 & 核心要点 & 难度 & 脉络层 \\
\midrule
1 & IMU 测量/误差模型 & 比力、偏置、白噪声；尺度/失准/g-敏感 & \(\star\star\) & 骨干 \\
2 & 连续/离散运动学 & $\dot{\mathbf R},\dot{\mathbf v},\dot{\mathbf p}$；欧拉离散；$+\mathbf g$ 来由 & \(\star\star\) & 骨干 \\
3 & 相对增量与预积分 & 与初态/重力解耦；分离噪声 & \(\star\star\star\) & 骨干 \\
4 & 协方差迭代传播 & $\boldsymbol\Sigma\leftarrow\mathbf A\boldsymbol\Sigma\mathbf A^\top+\mathbf B\boldsymbol\Sigma_\eta\mathbf B^\top$ & \(\star\star\star\) & 骨干 \\
5 & 偏置一阶修正 & 免重积分；五个雅可比 & \(\star\star\star\) & 骨干 \\
6 & IMU 因子残差/雅可比 & $9$ 维残差；解析雅可比 & \(\star\star\star\) & 骨干 \\
7 & 连续/离散噪声 + Allan & $1/\Delta t$ vs $\Delta t$；五项辨识 & \(\star\star\) & 次优 \\
8 & 初始对准/可观测性 & 重力对齐；4 维不可观；退化 & \(\star\star\star\) & 次优 \\
9 & 因子图/VIO 接口 & 滑窗、边缘化、四元数对照 & \(\star\star\star\) & 次优 \\
10 & 连续/高阶预积分 & GP、扩展位姿、本体感知 & \(\star\star\star\star\) & 前沿 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Kalibr / Rehder 等}\cite{rehder2016kalibr}——相机-IMU 时空标定与 IMU 噪声模型的事实标准工具；连续↔离散噪声换算。
