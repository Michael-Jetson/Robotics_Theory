...
\node[soft, below left=9mm and 2mm of uni] (cilqr) {CILQR\\ log-barrier 软化};
\node[soft, right=of cilqr] (teb) {TEB\\ hinge 软化 + g2o};
\node[hard, below=8mm of cilqr] (obca) {OBCA\\ 对偶精确重构};
\node[hard, right=of obca] (minco) {MINCO\\ 光滑映射消去};
\node[hard, right=of minco] (cpc) {CPC\\ 互补约束};
\node[blk, below=8mm of minco, fill=main!6, draw=main!60!black] (sel) {方法选型\\ 离散定 homotopy + 连续精修};
\draw[ln] (uni)--(cilqr); \draw[ln] (uni)--(teb);
\draw[ln] (cilqr)--(obca); \draw[ln] (teb)--(minco);
\draw[ln] (uni.east) to[out=-20,in=90] (cpc.north);
\draw[ln] (obca)--(sel); \draw[ln] (minco)--(sel); \draw[ln] (cpc)--(sel);
\end{tikzpicture}
\end{center}
...
\subsection*{前置知识桥接}
本章是 \cref{ch:planning}（运动规划导论）的直接延续，建立在它的三块基础上，重叠处不再重复、一律 \cref 指过去：
\begin{itemize}
