...
%  对称/不变/等变 sec:lie-symmetry）。右扰动为主；SE_2(3) 排序遵循本书 def:se23 即 [ρ;ν;φ]，
%  源文献用 [φ;ν;ρ]，note 盒显式对照。教学层全要素（v5.0）。
% ════════════════════════════════════════════════════════════════
...
\textbf{$\mathrm{SE}_2(3)$ 分量排序}：本书遵循权威定义 \cref{def:se23}，李代数 $\boldsymbol{\xi}=[\boldsymbol{\rho};\boldsymbol{\nu};\boldsymbol{\phi}]$（位置、速度、\textbf{旋转在最后}），伴随分块\emph{上}三角；而 Barrau--Bonnabel/Hartley 等源文献用 $[\boldsymbol{\phi};\boldsymbol{\nu}_v;\boldsymbol{\nu}_p]$（\textbf{旋转在最前}），伴随分块\emph{下}三角。两者只是基的置换、同一线性映射；本章正文统一用本书序，凡引源文献矩阵处以 note 盒对照排序。
\end{note}
...
对 \cref{eq:imu-kin} 的右不变误差，按本书 $\boldsymbol{\xi}=[\delta\boldsymbol{\theta};\delta\mathbf{v};\delta\mathbf{p}]$（旋转—速度—位置，源文献序）算 \cref{eq:log-linear} 的系统矩阵\cite{barrau2017invariant}：
\begin{equation}
...
\begin{note}
\textbf{“精确”的边界（务必澄清）.}\;
log-linear 的精确性\emph{仅在无噪声传播段}成立，且受 $\log$ 映射收敛域限制——$\SO(3)$ 旋转误差接近 $\pi$ 时 $\log$ 多值，BCH 收敛要求 $\|\boldsymbol{\xi}\|$ 在单射半径内。工程滤波里的过程噪声作为一阶随机扰动 $\mathbf{B}_t\mathbf{n}_t$ 叠加进协方差传播（含噪时自治性退化为\emph{近似}，仍优于标准 EKF）。\textbf{切勿}把“无噪声精确线性”误读为“InEKF 全局收敛”或“后验最优”——后者需更强假设，TAC 2017 未声称。另：$\mathfrak{se}_2(3)$ 含旋转部分、整体\emph{不是}幂零李代数，\cref{eq:se23-A} 的 $\mathbf{A}^3=\mathbf{0}$ 是该\emph{特定}右不变误差系统矩阵的幂零性（额外的计算友好性），\emph{不是}李代数幂零；log-linear 精确性本身\emph{不需}幂零，仅需群仿射 $+$ \cref{eq:leibniz}。\pz{存疑：源文档对“log-linear 在 $SO(3)$ 接近 $\pi$ 处的拓扑限制”表述较细，需核对 TAC 2017 §II.3 原文措辞。}
\end{note}
...
\end{equation}
对单个 landmark $\boldsymbol{\ell}_i$（world 坐标，$\mathrm{SE}_2(3)$ 嵌入齐次 $[\boldsymbol{\ell}_i;0;1]$），在本书 $\boldsymbol{\xi}=[\delta\boldsymbol{\theta};\delta\mathbf{v};\delta\mathbf{p}]$ 排序下，观测雅可比
\begin{equation}
...
\paragraph{Cassie 双足实证。}Hartley 等\cite{hartley2020contact} 在 Cassie 上做 $100$ 次 Monte-Carlo 仿真（相同噪声统计与初始协方差，但初始姿态/速度随机化——姿态在 $\pm30^\circ$、速度在 $\pm1.0\,$m/s 内均匀采样）：在相同接触信息下，右不变 InEKF 对\emph{所有可观状态}的收敛\emph{显著快于}四元数 EKF，收敛盆地（吸引域）明显更大。\rebuilt{源笔记原称“$60$ 次、初始 yaw $\pm45^\circ$、位置 RMSE 降约 $30\%$、yaw RMSE 降 $>50\%$、NEES 始终在带内”——核对原文（Hartley 2020 IJRR / RSS 2018）：实为 $100$ 次、初始姿态随机 $\pm30^\circ$，且论文给出的是\emph{定性}的“可观状态收敛更快、吸引域更大”，未报这些具体百分比；又 IMU$+$接触下 yaw 本不可观，"yaw RMSE 降 $>50\%$" 作稳态指标亦不成立。已改为论文实际结论；具体数值若需引用请回查原文表格。}性能差异的根因（\cref{sec:log-linear,sec:consistency-inekf}）：传播 $\mathbf{A}$ 与接触观测 $\mathbf{H}$ 都不依赖估计，零空间自动一致——\emph{同样的}接触信息在四元数 EKF 中无法享有这些结构性优势。
...
\begin{itemize}
  ...
  \item \textbf{$\to$ \cref{ch:imu}（IMU）}：$\mathrm{SE}_2(3)$ 不变预积分\cite{brossard2021associating} 把本章群仿射结构用于协方差精确传播，是 Forster 预积分\cite{forster2017preintegration} 的范式升级。
...
\begin{table}[H]\centering\small
\begin{tabular}{p{0.26\linewidth}p{0.32\linewidth}p{0.36\linewidth}}
\toprule
症状 & 可能原因 & 排查步骤 \\
\midrule
