...
\noindent 输出（$K=4\times10^5$，$x_0=1$）为 \texttt{t=0.00: $-0.8455$ / $-0.8333$}、\texttt{t=0.50: $-1.3853$ / $-1.4286$}、\texttt{t=0.98: $-4.5407$ / $-4.5455$}：三个时刻蒙特卡洛估计都贴着闭式 Riccati 解，残差来自有限采样的方差，$K$ 调大还会继续缩小。它真正的意义在于：把终端代价换成任意（哪怕不连续的）代价，这段代码\emph{一字不改}照样跑，可 Riccati 在那时早已失效。
