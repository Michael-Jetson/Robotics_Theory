...
%    MC·线性化·sigma-point/UKF·SPKF/ISPKF/粒子滤波/滤波器谱系/批量MAP·GN/
%    Laplace协方差/ML偏差/滑窗滤波SWF/KF≡批量MAP）
...
\end{equation}
本章先把“前向”因子做成递归滤波器（\cref{sec:bayes-filter}），再在 \cref{sec:kf} 用后向修正补成平滑器。
...
\begin{equation}
  ...
  \label{eq:kf-info}\label{eq:kf-info-P}
\end{equation}
...
归纳可证 KF 估计\emph{无偏}（$\mathbb{E}[\hat{\mathbf{x}}_k-\mathbf{x}_k]=\mathbf{0}$）且\emph{一致}（其报告的 $\hat{\mathbf{P}}_k$ 确为真实误差协方差，下节会把“一致”量化为 NEES）。即便去掉高斯假设，只要噪声零均值、不相关、方差已知，KF 仍是\textbf{最优线性无偏估计}（BLUE，高斯–马尔可夫定理），其协方差达到克拉默–拉奥下界（CRLB）\cite{barfoot2024state}。时不变系统的稳态协方差满足离散代数 Riccati 方程，可预先迭代到收敛后用常值增益（\emph{稳态卡尔曼滤波}），是常见工程简化。
...
\]
后者对任意（哪怕次优的）$\mathbf{K}$ 都给出对称正定结果，工程实现强烈推荐。
\end{pitfall}
...
\begin{itemize}
  ...
  \end{equation}
\end{itemize}
线性观测时众数 $=$ 均值、二者一致；非线性时\textbf{众数 $\ne$ 均值，故 MAP 是有偏的}。用“均值误差” $\hat{e}_{\text{mean}}=\mathbb{E}_{XN}[\hat{x}]-\check{x}$（对先验抽真值、对噪声抽 $n$ 同时取期望）度量：取参数 $\check{x}=20$m、$\check{P}=9$m$^2$、$f=400$px、$b=0.1$m、$R=0.09$px$^2$，$10^6$ 次试验得 $\hat{e}_{\text{mean}}(\hat{x}_{\text{map}})\approx-33.0$cm（小但非零的系统偏差）、均方误差 $\hat{e}_{\text{sq}}\approx4.41$m$^2$\cite{barfoot2024state}。\textbf{方法论提醒}：评判一个非线性滤波器，先问它\emph{捕获完整后验的哪个方面}（均值？众数？），目标不同时谈“谁更准”不公平。
...
% ════════════════════════════════════════════════════════════════
\section{把高斯过非线性的三种办法与滤波器谱系 \texorpdfstring{$\star$}{(进阶)}}
...
弃线性化、用 sigma 点过非线性运动/观测模型，即 \emph{sigma 点卡尔曼滤波}（SPKF）/ 无迹卡尔曼滤波（UKF）\cite{barfoot2024state}。\emph{预测步}：把先验与过程噪声堆叠 $\boldsymbol{\mu}_z=[\hat{\mathbf{x}}_{k-1};\mathbf{0}]$、$\boldsymbol{\Sigma}_{zz}=\mathrm{diag}(\hat{\mathbf{P}}_{k-1},\boldsymbol{\Sigma}_{\mathbf{w}})$，转 sigma 点、拆为状态与噪声、各过运动模型 $\check{\mathbf{x}}_{k,i}=\mathbf{f}(\hat{\mathbf{x}}_{k-1,i},\mathbf{u}_k,\mathbf{w}_{k,i})$，重组 $\check{\mathbf{x}}_k=\sum\alpha_i\check{\mathbf{x}}_{k,i}$、$\check{\mathbf{P}}_k=\sum\alpha_i(\check{\mathbf{x}}_{k,i}-\check{\mathbf{x}}_k)(\cdot)^\top$。\emph{校正步}：堆叠预测态与观测噪声、转 sigma 点、各过观测模型，重组 $\boldsymbol{\mu}_{y,k},\boldsymbol{\Sigma}_{yy,k},\boldsymbol{\Sigma}_{xy,k}$，代入广义校正步 \cref{eq:ggf}。可证（把观测在预测态线性化代入重组式，块对角使交叉项为零）：$\boldsymbol{\Sigma}_{yy,k}\approx\mathbf{H}\check{\mathbf{P}}_k\mathbf{H}^\top+\boldsymbol{\Sigma}_{\mathbf{v}}'$、$\boldsymbol{\Sigma}_{xy,k}\approx\check{\mathbf{P}}_k\mathbf{H}^\top$，于是 $\mathbf{K}_k\approx\check{\mathbf{P}}_k\mathbf{H}^\top(\mathbf{H}\check{\mathbf{P}}_k\mathbf{H}^\top+\boldsymbol{\Sigma}_{\mathbf{v}}')^{-1}$——\textbf{退回 EKF 增益}。即 SPKF 在“轻非线性”极限与 EKF 一致，强非线性时更准。两优点：不需任何解析导数；只用基本线性代数（黑箱模型也能用）。
...
\begin{equation}
  ...
  \label{eq:iekf}
\end{equation}
首次取 $\mathbf{x}_{\text{op}}=\check{\mathbf{x}}_k$ 时 IEKF 退化为 EKF（\textbf{EKF $=$ IEKF 的第一次迭代}）；协方差只在均值收敛后算一次。\textbf{IEKF 收敛到后验的众数，即一个 MAP 估计}（与高斯牛顿同源）。把 sigma 点也迭代（ISPKF\cite{barfoot2024state}）则\textbf{趋近后验均值}：对前述立体相机例（夸大差异 $x_{\text{true}}=26$m）数值显示 $\hat{x}_{\text{iekf}}=\hat{x}_{\text{map}}=24.5694$、$\bar{x}=24.7770$、$\hat{x}_{\text{ispkf}}=24.7414$，即 IEKF 落在众数、ISPKF 接近均值。

\paragraph{粒子滤波：非高斯、非线性的样本表示。}
当噪声/后验明显非高斯，可用一组带权样本（粒子）表示置信度，扩展 \cref{eq:bayes-filter}\cite{barfoot2024state}。基本（SIR / bootstrap）版三步：(1) 从先验与过程噪声联合抽 $M$ 个样本；(2) 各过非线性运动模型得\emph{预测粒子} $\check{\mathbf{x}}_{k,m}=\mathbf{f}(\hat{\mathbf{x}}_{k-1,m},\mathbf{u}_k,\mathbf{w}_{k,m})$；(3) 按观测似然给权 $w_{k,m}=\eta\,p(\mathbf{z}_k\mid\check{\mathbf{x}}_{k,m})$，再按权\emph{重采样}（如 Madow 系统重采样：建累积权 bin、取 $\rho\sim\mathrm{Uniform}[0,1)$ 等步长扫描）。优点：处理非高斯 + 非线性，甚至不需 $\mathbf{f},\mathbf{h}$ 解析式，anytime。缺点：样本数随状态维\emph{指数}增长，高维易样本贫化；且加样本不能优于 CRLB。
...
\begin{tabular}{llll}
\toprule
滤波器 & 过非线性的方法 & 迭代 & 逼近完整后验的 \\
\midrule
贝叶斯滤波 & 精确（不可实现） & — & 完整后验 \\
EKF & 线性化 & 否 & 无明确关系 \\
IEKF & 线性化（重线性化） & 是 & \textbf{众数（MAP）} \\
UKF/SPKF & sigma 点 & 否 & 无明确关系 \\
迭代 SPKF（ISPKF） & sigma 点 & 是 & \textbf{均值} \\
粒子滤波 & 蒙特卡洛 & — & 任意（非高斯） \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{滑动窗口滤波（SWF）：递归与批量的折中 \texorpdfstring{$\star$}{(选读)}}\label{par:swf}
...
% ════════════════════════════════════════════════════════════════
\section{估计器的健康度：无偏、一致与偏置 \texorpdfstring{$\star$}{(进阶)}}
...
\textbf{预测}（每个高频惯性输入）：① 标称态按 \cref{eq:eskf-nominal-kin} 非线性积分；② 误差协方差 \cref{eq:eskf-predict}。\\
\textbf{校正}（每个低频外部观测）：③ 算 $\mathbf{H}=\mathbf{H}_{\mathbf{x}}\mathbf{X}_{\delta\mathbf{x}}$（\cref{eq:eskf-H,eq:eskf-Qdtheta}）与增益 $\mathbf{K}$；④ 由新息 $\mathbf{z}-\mathbf{h}(\mathbf{x})$ 解 $\widehat{\delta\mathbf{x}}$、更新 $\mathbf{P}$（\cref{eq:eskf-correct}，Joseph 形式）。\\
\textbf{注入 $+$ 复位}：⑤ $\mathbf{x}\leftarrow\mathbf{x}\oplus\widehat{\delta\mathbf{x}}$（\cref{eq:eskf-inject}）；⑥ $\widehat{\delta\mathbf{x}}\leftarrow\mathbf{0}$，$\mathbf{P}\leftarrow\mathbf{G}\mathbf{P}\mathbf{G}^\top$（\cref{eq:eskf-reset}）。
\end{algo}
...
\noindent 理论与实践比较都表明：\emph{同等计算量}下，非线性优化在精度与鲁棒性上通常优于滤波\cite{gaoxiang2019slam14,carlone2026handbook}。这正是上一章把 SLAM 钉成“因子图上的 MAP”、并由 \cref{ch:nlopt} 专门求解的原因。但这\emph{不}意味着滤波过时——
...
\paragraph{鲁棒核与 IRLS。}
把二范数 $\rho(u)=\tfrac12u^2$（$u_i=\sqrt{\mathbf{e}_i^\top\mathbf{W}_i^{-1}\mathbf{e}_i}$）换成增长更慢、仍光滑的 $\rho(u)$，即\emph{鲁棒核}\cite{gaoxiang2019slam14,barfoot2024state}：
\begin{equation}
  \rho_{\text{Huber}}(u)=\begin{cases}\tfrac12u^2,&|u|\le\delta\\ \delta(|u|-\tfrac12\delta),&|u|>\delta\end{cases},\quad
  \rho_{\text{Cauchy}}(u)=\tfrac12\ln(1+u^2),\quad
  \rho_{\text{GM}}(u)=\tfrac12\frac{u^2}{1+u^2}.
  \label{eq:robust-kernels}
\end{equation}
Huber 在 $|u|>\delta$ 后由二次转一次（限梯度上界、保光滑）；Cauchy、Geman–McClure（GM）增长更慢。对鲁棒代价 $J'=\sum_i\alpha_i\rho(u_i(\mathbf{x}))$ 求梯度并整理，可写回“加权最小二乘”的形式，只是协方差被\emph{膨胀}：
\begin{equation}
  \frac{\partial J'}{\partial\mathbf{x}}=\sum_i\mathbf{e}_i^\top\mathbf{Y}_i^{-1}\frac{\partial\mathbf{e}_i}{\partial\mathbf{x}},\qquad
  \mathbf{Y}_i^{-1}=\frac{\alpha_i}{u_i}\frac{\partial\rho}{\partial u_i}\mathbf{W}_i^{-1}.
  \label{eq:irls-weight}
\end{equation}
在上一迭代点 $\mathbf{x}_{\text{op}}$ 处冻结 $\mathbf{Y}_i(\mathbf{x}_{\text{op}})$、每步解一个普通加权最小二乘，即\emph{迭代重加权最小二乘}（IRLS）：收敛时梯度与原鲁棒问题相同、故同极小，只是路径不同。以 Cauchy 为例 $\mathbf{Y}_i(\mathbf{x}_{\text{op}})=\tfrac1{\alpha_i}(1+\mathbf{e}_i^\top\mathbf{W}_i^{-1}\mathbf{e}_i)\mathbf{W}_i$——正是“误差越大、信任越低”的协方差膨胀。

\begin{theorem}[鲁棒核 $=$ 协方差逆 Wishart 先验下的 MAP \texorpdfstring{$\star$}{(选读)}]\label{thm:robust-iw}
...
\begin{equation}
  ...
  \label{eq:iw-cov}
\end{equation}
代回消去 $\mathbf{M}$，目标函数化为 $J'(\mathbf{x})=\tfrac12\sum_i\alpha_i\ln(1+\mathbf{e}_i^\top\boldsymbol{\Psi}_i^{-1}\mathbf{e}_i)$——当 $\boldsymbol{\Psi}_i=\mathbf{W}_i$ 时\textbf{恰是加权 Cauchy 鲁棒代价}。
\end{theorem}
\begin{proof}[要点]
...
\begin{enumerate}
  ...
  \item （证明骨架）由 \cref{thm:robust-iw} 出发，验证 $\boldsymbol{\Psi}_i=\mathbf{W}_i$ 时消去 $\mathbf{M}$ 的目标 $\tfrac12\alpha_i\ln(1+\mathbf{e}_i^\top\mathbf{W}_i^{-1}\mathbf{e}_i)$ 的梯度，与 \cref{eq:irls-weight} 的 Cauchy $\mathbf{Y}_i$ 一致。答错回看 \cref{eq:irls-weight} 与 \cref{thm:robust-iw}。
...
$\hat{\mathbf{P}}=(\mathbf{I}-\mathbf{K}\mathbf{C})\check{\mathbf{P}}$ 总没问题 & 数值上易失对称正定，应用 Joseph 形式（\cref{sec:kf} 数值陷阱）\\
\bottomrule
\end{tabular}
\end{table}
...
$\boldsymbol{\Lambda}=\boldsymbol{\Sigma}^{-1}$ & 信息矩阵（信息形式 KF） & \cref{eq:kf-info-P} \\
...
信息形式 & 校正 $=$ 信息相加，多传感器免大逆 & \cref{eq:kf-info-P,eq:kf-info-x} \\
...
\# & 知识点 & 核心要点 & 难度 & 脉络层 \\
\midrule
1 & 贝叶斯滤波 & 预测–校正、精确但不可实现 & $\star\star\star$ & 骨干 \\
2 & 卡尔曼滤波 & 配方/条件化两路、增益折中、信息形式 & $\star\star\star$ & 骨干 \\
3 & KF $\equiv$ 批量 MAP & 与 \cref{ch:slam_est} 缝合；RTS 平滑 & $\star\star\star\star$ & 骨干 \\
4 & EKF / 广义高斯 & 线性化雅可比、三宗罪 & $\star\star\star$ & 骨干 \\
5 & 高斯过非线性谱系 & MC/线性化/sigma 点；IEKF/ISPKF/PF/SWF & $\star\star\star\star$ & 次优 \\
6 & 一致性与偏置 & NEES/NIS、过自信、状态增广可观测性 & $\star\star\star\star$ & 次优 \\
7 & ESKF / 右扰动 & 三态、误差动力学、注入/复位、左右扰动 & $\star\star\star\star$ & 骨干 \\
8 & 滤波 vs 优化 + 鲁棒 & 四局限、稀疏对偶、鲁棒核 = 逆 Wishart MAP & $\star\star\star$ & 骨干 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{本章与后续章节的关系。}
\begin{table}[ht]\centering\small
\begin{tabular}{lll}
\toprule
后续章节 & 与本章的关系 & 本章哪个知识点为其铺垫 \\
\midrule
\cref{ch:imu} IMU 模型 & 给 ESKF 填入具体运动学与噪声标定 & \cref{algo:eskf}、\cref{eq:eskf-Fx} \\
\cref{ch:vio} VIO & ESKF 前端 + 因子图后端、边缘化老帧 & \cref{sec:eskf}、SWF（\cref{par:swf}） \\
\cref{ch:nlopt} 非线性优化 & 批量 MAP 的 GN/LM 解、鲁棒核/IRLS & \cref{thm:kf-map}、\cref{eq:irls-weight} \\
...
\begin{derivation}[RTS 后向协方差（\cref{eq:rts-cov}）]
承 \cref{eq:rts}，平滑协方差后向递推为
\[
  \hat{\mathbf{P}}_{k-1}=\hat{\mathbf{P}}_{k-1,f}+\mathbf{G}_{k-1}\big(\hat{\mathbf{P}}_k-\check{\mathbf{P}}_{k,f}\big)\mathbf{G}_{k-1}^\top,\qquad
  \mathbf{G}_{k-1}=\hat{\mathbf{P}}_{k-1,f}\mathbf{A}_{k-1}^\top\check{\mathbf{P}}_{k,f}^{-1},
\]
由块三对角 Cholesky 的后向回代式经矩阵求逆引理化简而得\cite{barfoot2024state}。它给每步\emph{平滑}边缘协方差，恒不大于对应滤波协方差（未来数据只会增加信息）。\hfill$\square$
\end{derivation}
...
\begin{derivation}[ML 偏差的近似闭式 \texorpdfstring{$\star$}{(选读)}]
对纯测量（无先验）的极大似然 $J(\mathbf{x})=\tfrac12\sum_k(\mathbf{z}_k-\mathbf{h}_k(\mathbf{x}))^\top\boldsymbol{\Sigma}_{\mathbf{v},k}^{-1}(\cdot)$，设估计偏差 $\delta\mathbf{x}=\hat{\mathbf{x}}-\mathbf{x}$ 对噪声至多二次依赖，代入收敛最优条件并对噪声取期望，Barfoot\cite{barfoot2024state} 得系统偏差
\[
